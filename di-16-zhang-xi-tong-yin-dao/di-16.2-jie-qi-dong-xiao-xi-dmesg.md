...
# aesni(4) 驱动加载，提供 AES 各模式（CBC/CCM/GCM 等）以及 SHA-1/SHA-256 硬件加速。
# GELI 全盘加密、ZFS 原生加密、IPsec、OpenSSL 等都可利用此驱动实现线速加密。

# ===== ACPI 根设备 -----
acpi0: <ALASKA A M I>
# ACPI 根设备（插槽/主板的 ACPI 命名空间），OEM ID = ALASKA（AMI BIOS 通用默认 OEM ID）。
...
# bus_attach_children → ... → acpi_pcib_acpi_attach：遍历总线子设备的递归 attach 调用链，层层向上经 ACPI 层回到 PCI 桥顶的 ACPI attach 入口。
