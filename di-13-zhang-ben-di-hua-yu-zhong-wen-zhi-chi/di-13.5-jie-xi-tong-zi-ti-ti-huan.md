...
FreeBSD 默认字体对中文的显示效果有限。本节涵盖图形界面引入 Windows TrueType 字体的方法，以及控制台字体的替换配置。

## GUI 图形界面字体

首先提取 Windows `C:\Windows\Fonts` 目录下的所有 `.ttf` 和 `.ttc` 字体文件。macOS 字体需特殊处理，尽管其文件格式也为 `.ttf`。
...
上述命令为临时生效，若需永久生效，应加入 **/etc/rc.conf** 文件：
