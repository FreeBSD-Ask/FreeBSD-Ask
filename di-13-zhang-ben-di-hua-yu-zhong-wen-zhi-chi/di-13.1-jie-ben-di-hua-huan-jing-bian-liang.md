...
本地化环境变量（LANG、LC_*、MM_CHARSET）决定系统界面语言、字符编码与排序规则，本节梳理各场景下的配置方法。
...
1. 单纯的界面中文环境仅需设置 `LC_MESSAGES` 为 `"zh_CN.UTF-8"`（于 SDDM/Xfce 验证）
2. 较常见的做法是将 `LANG`、`LC_ALL`、`LANGUAGE` 三个环境变量都设为 `"zh_CN.UTF-8"`
3. 在纯英文环境下同时使用中文输入法，仅设置输入法相关变量，其他保持默认英文设置

将三个环境变量都设置为 `"zh_CN.UTF-8"` 的原因是不同软件实现读取本地化变量的优先级存在差异，此配置策略可避免因软件读取优先级差异导致的语言设置不一致。
