...
本地化环境变量（LANG、LC_*、MM_CHARSET）决定系统界面语言、字符编码与排序规则，生效路径取决于显示管理器（SDDM、LightDM、GDM）和 Shell 环境。本节梳理各场景下的配置方法。
...
将三个环境变量都设置为 `"zh_CN.UTF-8"` 的原因是不同软件实现读取本地化变量的优先级存在差异，此配置策略可确保最大程度的系统兼容性。
