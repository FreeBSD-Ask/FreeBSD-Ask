...
> **技巧**
>
> 经测试，在上述格式中 Audacious 默认构建仅支持 `.flac` 格式，目前不支持 `.m4a` 和 `.av3a` 等编码格式，须安装相应插件或另行配置。

![Audacious](../.gitbook/assets/audacious.png)

## VLC

VLC（VideoLAN Client，视频局域网客户端）播放器的安装方法可参见本书视频播放器相关章节。FreeBSD 中的 `ffmpeg` 多媒体框架（名称中的“FF”意为“Fast Forward”，“mpeg”指 MPEG 标准）默认构建配置未启用 libuavs3d（AVS3 解码）和 libdavs2（AVS2 解码）支持，本节不再展开重新编译的配置方法。

经过实际测试，VLC 播放器可正常播放 AC-4 编码的 m4a 格式音频：
...
命令行环境下建议安装 mpc（`pkg install mpc`），适合用于绑定桌面环境的全局快捷键。
