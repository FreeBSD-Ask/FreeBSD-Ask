...
usbhid 驱动最早出现于 FreeBSD 11.0。[FreeBSD 代码审查 D45658](https://reviews.freebsd.org/D45658) 将 usbhid、hidbus 和 hkbd 加入各架构的 GENERIC 内核配置，并将 hms 加入 arm 和 powerpc 架构的配置，使新 HID 栈驱动默认可用；而将 `hw.usb.usbhid.enable` 默认值由 0 改为 1 的变更由 [D45659](https://reviews.freebsd.org/D45659) 提出。usbhid 自 15.0 起成为系统默认启用的驱动。
