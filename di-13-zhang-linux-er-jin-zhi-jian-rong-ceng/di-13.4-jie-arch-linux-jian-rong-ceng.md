...
```sh
# echo "compat.linux.osrelease=7.0.11" >> /etc/sysctl.conf # 将 Linux 兼容内核版本写入 sysctl 配置文件，实现持久化
# sysctl compat.linux.osrelease=7.0.11 # 立即生效设置，无需重启
```
...
# tar --use-compress-program=unzstd -xpvf archlinux-bootstrap-x86_64.tar.zst --strip-components=1 -C /compat/arch --numeric-owner   # 解压压缩包到 /compat/arch：同时保持原有 GID 和 UID。若出现 tar 错误通常可忽略
```
