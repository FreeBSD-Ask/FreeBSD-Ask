...
echo "Setting USTC mirror"
        chroot "${TARGET_PATH}" /bin/bash -c "sed -i.bak \
            -e 's|http://archive.debian.org/debian/|https://mirrors.ustc.edu.cn/debian/|g' \
            -e 's|http://security.debian.org/debian-security/ |https://mirrors.ustc.edu.cn/ubuntu/|g' \
...
if ping -c 1 -W 3 223.5.5.5 > /dev/null 2>&1; then
    ...
    if [ "$INSTALL" = "1" ]; then
        ...
        # 确保 locale 安装成功后再生成，如果安装失败则退出
        chroot "${TARGET_PATH}" /bin/bash -c "$INSTALL_CMD nano && update-locale LC_ALL=zh_CN.UTF-8 LANG=zh_CN.UTF-8" || exit 1
    fi
