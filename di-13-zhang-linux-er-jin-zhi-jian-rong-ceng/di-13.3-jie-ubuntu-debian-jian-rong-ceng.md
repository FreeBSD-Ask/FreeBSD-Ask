...
check_integrity() {
    ...
    if [ ! -f "SHA256SUMS" ]; then
        echo "Error: SHA256SUMS file not found in the current directory." >&2
        return 1
    fi
    
    # 3. 动态提取：从 SHA256SUMS 中过滤出包含当前 ${FILE} 的那一行
    #    然后通过管道丢给 `sha256sum -c` 进行标准校验
    grep "${FILE}" SHA256SUMS | sha256sum -c >/dev/null 2>&1
    
    # 4. 返回 sha256sum 的执行状态（0 为成功，非 0 为失败）
    return $?
}

# 步骤 1：检查本地是否已有符合要求的文件（动态校验）
