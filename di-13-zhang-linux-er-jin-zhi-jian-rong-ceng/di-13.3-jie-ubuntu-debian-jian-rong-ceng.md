...
`7.0.11` 仅为示例，建议参考 [The Linux Kernel Archives](https://www.kernel.org/) 中公布的版本号设置。

```sh
# echo "compat.linux.osrelease=7.0.11" >> /etc/sysctl.conf   # 将 Linux 兼容层内核版本写入 sysctl 配置文件，使其永久生效
# sysctl compat.linux.osrelease=7.0.11                      # 立即设置 Linux 兼容层内核版本，便于当前会话继续使用
```
...
chroot 进入 Ubuntu 兼容环境，移除可能导致报错的软件：
