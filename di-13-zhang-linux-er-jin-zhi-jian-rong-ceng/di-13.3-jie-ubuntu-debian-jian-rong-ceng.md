...
echo "Setting USTC mirror"
		...
		echo "APT::Cache-Start 90000000;" >> ${TARGET_PATH}/etc/apt/apt.conf   # APT::Cache-Start 可设置 apt 默认缓存偏小，按提示增大
        ;;
    [Nn][Oo]|[Nn])
...
if [ "$DNS_CONFIGURED" = "1" ]; then
	...
	[ $INSTALL = 1 ] && { echo "Installing language-pack-zh-hans and nano"; chroot "${TARGET_PATH}" /bin/bash -c "${INSTALL_CMD} nano language-pack-zh-hans && update-locale LC_ALL=zh_CN.UTF-8 LANG=zh_CN.UTF-8"; }
    [ $UPGRADE = 1 ] && { echo "Upgrading system packages and remove snap"; chroot "${TARGET_PATH}" /bin/bash -c "${UPGRADE_CMD} && apt remove -y snap"; }
else
