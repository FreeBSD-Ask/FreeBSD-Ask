...
echo "Starting dbus service"   # 输出启动信息
...
if [ ! "$(sysrc -f /boot/loader.conf -qn nullfs_load)" = "YES" ]; then
        echo "nullfs_load should be set to YES. Continue? (Y|n)"
        read answer
        case $answer in
            [Nn][Oo]|[Nn])
...
echo "Mounting required file systems for Linux"   # 输出挂载提示
...
echo "compat.linux.osrelease needs to be changed for Gentoo Linux to upgrade the Linux kernel version. Continue? (Y|n)"
read answer
case $answer in
	[Nn][Oo]|[Nn])
		echo "Almost done, but this step is required"
		exit 4
		;;
	[Yy][Ee][Ss]|[Yy]|"")
...
echo "Complete!"   # 提示完成
echo "To enter: chroot ${rootdir} /bin/bash"   # 提示如何进入 Gentoo 环境
echo ""
...
echo "Writing MAKEOPTS and FEATURES to /compat/gentoo/etc/portage/make.conf -- using USTC mirrors for GENTOO_MIRRORS"
