...
if [ "$(sysrc -n dbus_enable)" != "YES" ]; then
    ...
    case $ANSWER in
        [Nn][Oo]|[Nn])
            echo "WARNING: You must start dbus with \"service dbus start\" after each FreeBSD reboot."
            echo "Are you sure you want to continue without enabling dbus? (y|N)"
            read ANSWER
            case $ANSWER in
                [Yy][Ee][Ss]|[Yy])
...
if [ $DNS_CONFIGURED = 1 ]; then
    [ $UPDATE = 1 ] && echo "Updating package cache" && chroot ${ROOT_DIR}/${DIST} /bin/bash -c "${UPDATE_CMD}"
    [ $UPGRADE = 1 ] && echo "Upgrading system packages" && chroot ${ROOT_DIR}/${DIST} /bin/bash -c "${UPGRADE_CMD}"
    [ $INSTALL = 1 ] && echo "Installing vim" && chroot ${ROOT_DIR}/${DIST} /bin/bash -c "${INSTALL_CMD} vim"
else
