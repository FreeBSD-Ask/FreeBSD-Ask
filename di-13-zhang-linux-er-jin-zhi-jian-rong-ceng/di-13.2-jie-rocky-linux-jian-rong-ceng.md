...
FreeBSD Ports 的 [emulators/linux-rl9](https://www.freshports.org/emulators/linux-rl9/)（其实际对应的基础包为 [emulators/linux_base-rl9](https://www.freshports.org/emulators/linux_base-rl9/)）基于 Rocky Linux 9.7 发行版构建。
...
check_integrity() {
    ...
    # 3. 动态提取：精准匹配 "(文件名)"，并提取最后一个字段（$NF）作为期望哈希值
    local expected_hash
...
# 更新、升级和安装软件
# 使用 ping 实时检测网络连通性，注意在 FreeBSD 中，-W 指定的单位是毫秒
