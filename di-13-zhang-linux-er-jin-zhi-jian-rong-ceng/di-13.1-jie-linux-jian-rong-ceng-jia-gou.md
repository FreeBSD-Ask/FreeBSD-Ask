...
`sysctl -n compat.linux.emul_path` 将输出当前 Linux 兼容层的路径，默认值上文已经提及“**/compat/linux/**”。当在 **/compat/linux/** 路径下构建兼容层时，可以实现自动挂载相关文件路径，刷新 ldd 依赖。自行构建时，需要将 sysctl 项 **compat.linux.emul_path** 指向自定义路径，从而避免过多的手动配置。
...
- FreeBSD Project. linux(4) -- Linux ABI support[EB/OL]. [2026-04-17]. <https://man.freebsd.org/cgi/man.cgi?query=linux&sektion=4&manpath=FreeBSD+16.0-current>. Linux 二进制兼容层手册页。
