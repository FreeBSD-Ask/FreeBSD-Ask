...
| `qt6-base` | Qt 6 基础模块 |
| `qt5-wayland` | Qt 5 Wayland 支持模块，能让 Qt 5 应用程序在 Wayland 环境中运行 |
| `qt6-wayland` | Qt 6 Wayland 支持模块，能让 Qt 6 应用程序在 Wayland 环境中运行 |
| `xdg-desktop-portal-hyprland` | Hyprland XDG Desktop Portal 实现，提供与其他应用程序的通信接口 |
| `hyprpicker` | 颜色选择器 |
| `swaybg` | 壁纸工具 |
| `mako` | 显示通知 |
| `nerd-fonts` | 图标字体，可用于在配置文件中插入图案，在 waybar 等处显示 |
| `slurp` | 屏幕选择工具，用于截屏 |
| `grim` | 屏幕截图工具 |
| `swaylock` | 锁屏工具 |
| `kitty` | 终端 |
...
先设置环境变量 `XDG_RUNTIME_DIR`，避免启动时出现错误。
...
如不使用显示管理器，但需开机自动启动 Hyprland（即在 D-Bus 会话中启动 Hyprland 窗口管理器），可将下行写入 **~/.profile** 文件：
...
# 将窗口移动到别的工作区，例如 mainmod+Shift+2（win 键 +2 是查看工作区 2）
...
waybar 配置文件位于 **~/.config/waybar**，包含 `config.jsonc` 和 `style.css` 文件。示例配置需安装 `noto-emoji` 方可正常显示图标，可根据需要修改。
...
swaylock 的配置文件为 **~/.config/swaylock/config**。
...
需确认是否已将普通用户添加到 `video` 组，是否已安装显卡驱动，以及是否已正确设置环境变量 `XDG_RUNTIME_DIR`，并确认上述设置已生效。
