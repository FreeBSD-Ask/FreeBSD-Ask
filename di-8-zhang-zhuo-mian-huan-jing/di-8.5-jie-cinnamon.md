...
| `slick-greeter` | LightDM 的登录界面美化插件，LightDM 需要至少一个 greeter 才能正常工作 |
...
1. 购买锡兰肉桂和桂皮，闻一闻或通过烹饪感受区别。
2. 选取 Cinnamon 的 procfs 挂载机制，分析 procfs 在 FreeBSD 是否真正拥有意义和作用。
3. Cinnamon 最初是 Linux Mint 为回应 GNOME 3 的激进设计变革而创建的桌面分支。分析桌面环境的分叉现象是否反映了用户界面设计领域缺乏客观评价标准的问题，并讨论 FreeBSD 作为次要平台在这一生态中的话语权。
