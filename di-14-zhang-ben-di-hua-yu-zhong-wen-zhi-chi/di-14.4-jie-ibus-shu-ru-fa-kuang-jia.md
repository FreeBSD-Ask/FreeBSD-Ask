...
在 Wayland 下，IBus 的环境变量配置与 X11 有所不同。GNOME 自 3.6 起已集成 IBus，在 GNOME Wayland 下无需手动设置 `GTK_IM_MODULE` 和 `QT_IM_MODULE`。对于其他桌面环境，`GTK_IM_MODULE` 和 `QT_IM_MODULE` 的设置策略与 Fcitx 5 类似（参见 14.3 节 Wayland 部分）。`XIM_PROGRAM` 和 `XIM_ARGS` 仅用于 X11/XWayland 环境，在纯 Wayland 会话中不需要。
