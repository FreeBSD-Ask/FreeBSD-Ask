...
若无需 CA 签署，也可创建自签名证书。这会生成两个新文件：私钥文件 cert.key 和证书文件 cert.crt。建议将两者存放于 **/etc/ssl/** 下，权限设为 `0700`（仅 `root` 可读），可用 `chmod` 设置。
...
该完整性检查可通过创建 **/etc/ssl/fipsmodule.cnf** 来配置，并在 OpenSSL 主配置文件 **/etc/ssl/openssl.cnf** 中引用。OpenSSL 提供了 `openssl-fipsinstall` 辅助此过程，用法如下：
