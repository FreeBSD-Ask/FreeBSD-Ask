...
> - 定期更换密码进一步加剧管理难度，而“密码管理工具”本身的信任问题也难回避。
...
密码是技术领域不得已的负担。既然必须使用，就应当足够复杂，并以强哈希机制加密后存入密码数据库。FreeBSD 的 `crypt()` 库支持 SHA-256、SHA-512、Blowfish 等多种算法。
...
如果要确认某位用户所用哈希算法，root 用户可查看 FreeBSD 密码数据库中对应哈希值。每个哈希值均以一个符号开头，表明加密该密码的哈希机制类型：
