...
FreeBSD 的强制访问控制（Mandatory Access Control，MAC）框架允许通过可加载安全策略模块对系统安全施加精细控制。MAC 可加载访问控制模块以执行安全策略。部分模块为系统子集提供保护，强化特定服务；其他模块则跨所有主体与客体提供全面的标签化安全。定义中的“强制”（Mandatory）一词，指控制由管理员和操作系统执行，与默认的自主访问控制（Discretionary Access Control，DAC）形成对比——DAC 中执行由用户自行决定。
