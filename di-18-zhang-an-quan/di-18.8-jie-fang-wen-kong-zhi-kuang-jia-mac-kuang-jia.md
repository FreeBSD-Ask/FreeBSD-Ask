...
MAC 配置不当可能造成无法访问系统、困扰用户或无法启动 Xorg。更重要的是，不应仅依赖 MAC 保护系统。MAC 框架仅增强现有安全策略，如果没有健全安全实践和定期安全检查，系统永远无法完全安全。
...
MAC 标签是可应用于系统中主体和客体的安全属性。设置标签时管理员需要理解其含义，以防止系统出现意外或非期望行为。客体可用属性取决于已加载策略模块，各模块以不同方式解释属性。
...
主体和客体对中的等级和区间用于构造**支配（dominance）**关系：主体支配客体、客体支配主体、互不支配或互相支配。“互相支配”见于两标签相等时。因 Biba 信息流特性，用户拥有一组可能对应项目区间权限的集合，客体亦拥有一组区间。用户可能需要借助 `su` 或 `setpmac` 缩减权限以访问非受限区间中的客体。

### 用户标签

用户需要配置标签，确保其文件和进程与系统安全策略正确交互。这在 **/etc/login.conf** 中以登录类配置。各使用标签的策略模块均实现用户类设置。
...
使用 MAC 的系统保证用户不能随意更改安全属性。所有用户实用程序、程序和脚本都需要在选定安全策略模块的访问规则约束内运行，MAC 访问规则控制权在系统管理员。
...
实施 MAC 类似实施防火墙，需要防止将自身完全锁出系统。应考虑恢复先前配置的能力，通过远程连接实施 MAC 时应格外小心。
