...
FreeBSD 原生内置了一个叫作 mtree 的基本 IDS。虽然每夜安全邮件会通报变更，但信息毕竟存储在本地，恶意用户仍有机会篡改这些记录以掩盖其操作。因此，应当另建一套独立的二进制签名，保存在只读、属主为 root 的目录中；置于可移动 USB 磁盘或远程服务器上更为理想。

建议在每次更新后运行 `freebsd-update IDS`。
