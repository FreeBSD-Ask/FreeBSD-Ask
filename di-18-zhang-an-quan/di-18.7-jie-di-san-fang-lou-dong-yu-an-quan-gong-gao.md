...
近年来，安全领域在漏洞评估处理方面取得了许多改进。随着第三方工具被安装和配置到当今几乎每个可用的操作系统中，系统入侵的威胁也在增加。

漏洞评估是安全的关键因素。虽然 FreeBSD 为基础系统发布公告，但为每个第三方工具都这样做超出了 FreeBSD 项目的能力范围。有一种方法可以缓解第三方漏洞，并警告管理员已知的安全问题。FreeBSD 的附加工具 pkg 专门为此目的提供了相应功能。

pkg 轮询着一个安全问题数据库。该数据库由 FreeBSD 安全团队和 ports 开发者更新和维护。periodic 配置文件，用于维护 pkg 审计数据库，并提供了一种编程化的方法保持其更新。

要随时审计作为  一部分的第三方工具，管理员可以选择更新数据库并查看已安装软件包的已知漏洞，执行以下命令：
...
通过将 Web 浏览器指向显示的 URL，管理员可获取更多关于漏洞的信息。
