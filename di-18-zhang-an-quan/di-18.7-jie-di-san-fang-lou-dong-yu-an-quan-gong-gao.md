...
实际上默认调用的是 <https://www.vuxml.org/freebsd/index.html> 中的数据。

第三方工具的广泛使用也增加了系统入侵的威胁。如需审计作为 Ports 一部分的第三方工具，管理员可以选择更新数据库并查看已安装软件包的已知漏洞，执行以下命令：
...
- 安全公告的名称始终以 `FreeBSD-SA-`（代表 FreeBSD 安全公告）开头，后跟两位数字格式的年份（`26:`, 2026 年），然后是该年份的公告序号（`08.` 表示第 8 份公告），再然后是受影响的应用程序或子系统名称（`rpcsec_gss`）。
- `Topic` 字段概括了该漏洞。
...
- `Module` 字段指的是组件位置。在此示例中，`rpcsec_gss` 模块受到影响；因此，此漏洞影响随操作系统安装的应用程序。
...
- `Correction Details` 字段显示每个受影响的 Git 分支以及包含更正代码的修订版号。
