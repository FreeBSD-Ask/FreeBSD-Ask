...
- [ ] 18.7.在 MAC Jail 中运行 Nagios（无意义，不写。请用其他案例代替）
- [ ] 第 11 章 打印（本节对中文无意义，不引入）
- [ ] 20.7.创建和使用软盘（谁还有这种东西？2024，日本政府决定全面淘汰软盘）
- [ ] 24.8.基于 FreeBSD 的 Xen™ 虚拟机（真的支持 Windows 11 吗？10 也行。Xen 真难用，而且删除了 PV 支持，本节似乎无意义，考虑删掉）
- [ ] 31.4.Sendmail（用 Postfix 等代替）
- [ ] 32.2.inetd 超级服务器（谁在用？）
- [ ] 32.4.网络信息系统（NIS）（无意义，用 SSSD-LADP 代替）
- [ ] 30.5.PPPoA（过时）
- [ ] 29.4.拨入服务（过时）
- [ ] 29.5.拨出服务（过时）
- [ ] 20.10.文件系统快照（UFS）（无意义）
...
- [ ] 第 7 章 网络
  ...
  - [ ] 7.4.无线网络（引入高级加密等）
- [ ] 第 9 章 多媒体
  - [ ] 9.2.设置声卡（命令行设置声音音量、音效、切换声卡、禁音、编译声卡驱动）
  - [ ] 9.5.视频会议
...
- [ ] 第 19 章 安全事件审计
  ...
  - [ ] 19.4.使用审计跟踪
- [ ] 第 20 章 存储
  - [ ] 20.6.创建和使用 DVD & 20.5.创建和使用 CD（命令行与 GUI 并行介绍）
  - [ ] 20.11.磁盘配额
  - [ ] 20.12.加密磁盘分区
  - [ ] 20.13.加密交换分区
  - [ ] 20.14.高可用性存储（HAST）     
- [ ] 第 21 章 GEOM: 模块化磁盘转换框架（考虑结合 FreeBSD 期刊）
  - [ ] 21.2.RAID0——条带
  - [ ] 21.3.RAID1——镜像
  ...
  - [ ] 21.6.GEOM Gate 网络设备
  - [ ] 21.7.为磁盘设备添加卷标
- [ ] 第 22 章 Z 文件系统（ZFS）（考虑结合 FreeBSD 期刊）
  - [ ] ZFS 设计哲学
  - [ ] 22.6.高级主题
  - [ ] 拆分 zfs、zpool（池）管理为独立章节
- [ ] 第 24 章 虚拟化
  - [ ] 24.6.使用 FreeBSD 上的 QEMU 虚拟化
- [ ] 第 26 章 FreeBSD 更新与升级
  - [ ] 26.7.多台机器的追踪
- [ ] 第 27 章 DTrace（考虑结合 FreeBSD 期刊）
  - [ ] 27.2.实现上的差异
  - [ ] 27.3.开启 DTrace 支持
...
- [ ] 第 31 章 电子邮件（？）
- [ ] 第 32 章 网络服务器
  - [ ] LDAP/SSSD（Port [security/sssd2](https://www.freshports.org/security/sssd2/)）
  - [ ] 32.6.动态主机设置协议（DHCP）
  - [ ] 32.7.域名系统（DNS）
