...
>
> 类似的方法可以构建 Debian、Arch 兼容层（`FATAL: kernel too old`：需要在 `/etc/sysctl.conf` 添加 `compat.linux.osrelease=6.0.0` 把 Linux 兼容层的内核版本改为 6.0.0 才可、）。Gentoo 兼容层则提示 bash so 文件错误，即使静态编译了 zsh。
>
