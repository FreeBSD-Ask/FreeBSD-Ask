...
> **注意**
>
> `portsnap` 自 FreeBSD 14.0 起已从基本系统中移除，FreeBSD 15.0 中已不再支持。从旧教程迁移的用户请使用下文介绍的 Git 方式获取 Ports。
...
```sh
# tar -zxvf ports.tar.gz -C /usr/ # 解压至 /usr/ 目录
# rm ports.tar.gz # 删除压缩包
```
...
完整的 OPTIONS 列表见 <https://cgit.freebsd.org/ports/tree/Mk/bsd.port.mk>。完整的 DEFAULT_VERSIONS 列表见 <https://cgit.freebsd.org/ports/tree/Mk/bsd.default-versions.mk>。
