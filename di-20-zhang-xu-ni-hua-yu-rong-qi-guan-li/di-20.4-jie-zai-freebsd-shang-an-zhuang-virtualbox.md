...
需要选中 Ports 的配置菜单中的实用选项 `GuestAdditions`，它将生成 **/usr/local/lib/virtualbox/additions/VBoxGuestAdditions.iso** 文件。
...
为使用户能够使用 VirtualBox 的 DVD/CD 功能，必须具有对 **/dev/xpt0**、**/dev/cdN** 和 **/dev/passN** 的访问权限。将以下内容添加到 **/etc/devfs.rules** 文件（如不存在请创建）：
