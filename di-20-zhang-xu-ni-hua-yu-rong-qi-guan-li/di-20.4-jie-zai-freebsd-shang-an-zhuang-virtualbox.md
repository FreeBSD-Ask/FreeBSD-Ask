...
VirtualBox 是甲骨文（Oracle）公司开发的跨平台第二类虚拟机管理程序（Type-2 hypervisor）。
...
闭源的 VirtualBox 扩展包（VirtualBox Extension Pack）尚不支持 FreeBSD，该扩展包提供 USB 2.0/3.0 支持、磁盘镜像加密、VRDP 远程桌面协议等附加功能。

## 安装 VirtualBox

VirtualBox 在 FreeBSD 上有多个版本可供选择。6.1 版本（legacy）和 7.0 版本已停止上游支持，7.1 版本已于 2026 年 4 月停止上游支持，7.2 版本为当前维护版本。各版本均提供无 X11 的 `-nox11` 变体（例如 **emulators/virtualbox-ose-nox11-71**），适用于无图形界面的服务器环境，仅可通过命令行工具 `VBoxManage` 操作。

本节安装 **emulators/virtualbox-ose-71**。

使用 pkg 安装的预编译内核模块可能与当前运行的内核版本不匹配，而通过 Ports 编译安装可以确保内核模块与当前系统内核完全匹配，因此推荐使用 Ports 安装：
...
```ini
devfs_system_ruleset="system" # 设置 DevFS 使用规则集 "system"
```
...
完成所有配置后，可以启动 VirtualBox 并创建虚拟机测试，如图所示：
...
- Oracle. Oracle VM VirtualBox Extension Pack[EB/OL]. docs.oracle.com, [2026-04-17]. <https://docs.oracle.com/en/virtualization/virtualbox/7.0/user/Introduction.html>. 扩展包提供 USB 2.0/3.0、磁盘镜像加密、VRDP 等功能。
...
1. 使用 VBoxManage 命令行工具创建虚拟机并启动，记录操作步骤。

2. 对比 VirtualBox 与 bhyve 在 FreeBSD 上的性能差异，使用基准测试工具进行测量。
