...
使用 pkg 安装的预编译内核模块可能与当前运行的内核版本不匹配，而通过 Ports 编译安装可以确保内核模块与当前系统内核完全匹配，推荐使用 Ports 安装：
...
因为其存在导致配置使用它的虚拟机无法启动的错误，从 VirtualBox 6.1.24 起，PULSEAUDIO 选项已被强制禁用。
...
完成所有配置后，启动 VirtualBox 并创建虚拟机测试，如图所示：
