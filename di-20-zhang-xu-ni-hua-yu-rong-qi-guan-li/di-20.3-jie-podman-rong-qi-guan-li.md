...
Podman 是由红帽（Red Hat）主导开发的开源容器运行时，是 Docker 的替代方案，命令行接口与 Docker 高度兼容。Podman 采用无守护进程（daemonless）架构，每个容器作为独立进程运行，无需始终运行后台守护进程，从而提高了系统安全性，降低了单点故障风险。

在 FreeBSD 平台上，Podman 基于原生的 Jail 机制实现容器隔离，可使用命令 `jls` 查看当前运行的容器。

## 安装 Podman

安装 sysutils/podman-suite 元包时，会同时安装 sysutils/buildah（用于构建容器镜像）、sysutils/podman（容器运行时）和 sysutils/skopeo（用于操作容器镜像）。
...
在 FreeBSD 上使用 Podman，需要配置文件系统挂载点。fdescfs 是 FreeBSD 提供的文件描述符文件系统，可将进程的文件描述符映射为文件系统节点，Podman 依赖该文件系统实现容器内部的文件描述符传递。
...
3. 使用 Podman 部署一个多容器应用（如 Web 服务 + 数据库），测试容器间通信。
