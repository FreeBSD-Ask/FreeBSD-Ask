...
antiX 不使用 systemd，而是使用 rc 进行初始化。此处 **/etc/init.d/rc 3** 指定 antiX Jail 在运行级别 3 启动。由于 antiX 不依赖 systemd，Debian Jail 中因 systemd 受限而导致的服务启动问题不会在此出现，可直接在 Jail 启动时运行服务（如 sshd）。
