...
FreeBSD 的 Linux Jail 是 FreeBSD 操作系统中的一项功能，能在 jail 中运行 Linux 二进制文件和应用程序。通过集成一层兼容层，将部分 Linux 系统调用和库转换为 FreeBSD 内核可执行的形式，从而实现此功能。Linux Jail 的目的是在无需单独部署 Linux 虚拟机或运行环境的情况下，使 FreeBSD 系统能运行 Linux 软件。

以下介绍在 FreeBSD Jail 中部署 Linux Jail 的一些初始化配置。
...
> **技巧**
>
> 上述示例中的 `192.168.5.1`、`em0` 为占位符，需要替换为实际的值。
...
即使不使用防火墙规则，也需要启用 pf 服务来实现 NAT 功能。请参照其他文章启用 pf 防火墙。
