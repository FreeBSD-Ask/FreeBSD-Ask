...
Fcitx 即“小企鹅输入法”，原始英文全称为“Free Chinese Input Toy for X”（X 窗口系统的自由中文输入玩具）。自 Fcitx 5 起，该缩写不再对应单一全称，官方提供了多种释义（如“Flexible Context-aware Input Tool with eXtension”等），以体现 Fcitx 的发展已超越中文输入范畴。
...
me:\
	:lang=zh_CN.UTF-8:\
	:setenv=LC_ALL=zh_CN.UTF-8,LC_COLLATE=zh_CN.UTF-8,LC_CTYPE=zh_CN.UTF-8,LC_MESSAGES=zh_CN.UTF-8,LC_MONETARY=zh_CN.UTF-8,LC_NUMERIC=zh_CN.UTF-8,LC_TIME=zh_CN.UTF-8,GTK_IM_MODULE=fcitx,QT_IM_MODULE=fcitx,XMODIFIERS="@im=fcitx":\
	:charset=UTF-8:
```
...
在 Wayland 下， `QT_IM_MODULE` 的设置取决于桌面环境：在 KDE Plasma 下应取消设置（Qt 通过 KWin 的 `text-input` 协议与 Fcitx 5 通信），在其他桌面环境下仍需设置为 `fcitx`。

在 Wayland 下，`GTK_IM_MODULE` 的设置需视情况而定。
...
运行在 XWayland 下的程序，输入法由环境变量 `XMODIFIERS='@im=fcitx'` 配置。示例 **~/.login_conf** 配置文件如下（以非 KDE 桌面环境为例）：

```ini
me:\
	:lang=zh_CN.UTF-8:\
	:setenv=LC_ALL=zh_CN.UTF-8,LC_COLLATE=zh_CN.UTF-8,LC_CTYPE=zh_CN.UTF-8,LC_MESSAGES=zh_CN.UTF-8,LC_MONETARY=zh_CN.UTF-8,LC_NUMERIC=zh_CN.UTF-8,LC_TIME=zh_CN.UTF-8,QT_IM_MODULE=fcitx,XMODIFIERS="@im=fcitx":\
	:charset=UTF-8:
```

> **注意**
>
> 若使用 KDE Plasma 桌面，应从上述 `setenv` 中移除 `QT_IM_MODULE=fcitx`。
