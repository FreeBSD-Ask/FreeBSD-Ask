...
> **注意**
>
> 若使用 KDE Plasma (Wayland) 桌面，应当跳过此步骤，并在系统设置——键盘——虚拟键盘中选择 Fcitx 5。
...
在 Wayland 桌面环境中，推荐同时设置 `QT_IM_MODULES="wayland;fcitx"` 和 `QT_IM_MODULE=fcitx`。这一配置在 X11 桌面上也不会导致问题。

|情形            |生效环境变量   |行为                          |
|----------------|---------------|------------------------------|
|Qt 6.7+, Wayland|`QT_IM_MODULES`|使用 Wayland `text-input` 协议|
|Qt 6.7+, X11    |`QT_IM_MODULES`|使用 Qt IM Module             |
|Qt 6.6-         |`QT_IM_MODULE` |使用 Qt IM Module             |
