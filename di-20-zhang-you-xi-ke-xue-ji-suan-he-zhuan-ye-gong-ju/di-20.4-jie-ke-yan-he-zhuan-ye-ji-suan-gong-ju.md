...
`dc` 是采用后缀表示法的开源计算器。在传统 Unix 系统中，`bc` 曾作为 `dc` 的预处理器实现——先将中缀表达式转换为后缀表达式，再交由 `dc` 执行计算。FreeBSD 自 13.3 起采用 Gavin D. Howard 开发的 bc/dc 实现，该实现将 `bc` 和 `dc` 集成在同一二进制文件中，`dc` 通过符号链接访问，不再沿用传统的预处理器架构。
