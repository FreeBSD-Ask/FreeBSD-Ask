...
网桥通过学习各网络接口上设备的 MAC 地址进行工作，仅在源和目标 MAC 地址位于不同网段时才转发流量。本质上，**现代以太网交换机即是一种多端口网桥**。可配置具有多个网络接口的 FreeBSD 系统作为网桥设备。
...
网桥可连接两个网络段，以便使用 bpf(4) 和 tcpdump(1) 在网桥接口上检查通过它们的所有以太网帧，或通过将所有帧的副本发送到另一个称为 span 端口的接口进行检查。
...
可通过多条链路将网络连接，并使用生成树协议（STP）阻止冗余路径。
