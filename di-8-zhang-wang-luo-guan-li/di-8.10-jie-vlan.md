...
> **注意**
>
> 在定义 VLAN 时，确保同时配置和启用了父网络接口。上面的示例中，最简单的配置为：
>
> ```sh
> # ifconfig em0 up
> ```
...
> **注意**
>
> 在 /etc/rc.conf 中定义 VLAN 时，请确保同时配置和启用了父网络接口。上面的示例中，最简单配置为：
> 
> ```sh
> ifconfig_em0="up"
> ```
...
为接口指定符号名称具有实用价值：当更换关联的硬件时，只需更新少数几个配置变量。例如，安全摄像头需要在 `em0` 上运行 VLAN 1，若日后将 `em0` 网卡替换为使用 ixgbe(4) 驱动程序的网卡，则无需将所有指向 `em0.1` 的引用更改为 `ixgbe0.1`。
