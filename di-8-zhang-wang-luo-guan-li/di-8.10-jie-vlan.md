...
> **注意**
>
> 在定义 VLAN 时，确保同时配置和启用父网络接口。上面的示例中，最简单的配置为：
>
> ```sh
> # ifconfig em0 up
> ```
...
为接口指定符号名称是很有用的，这样当更换关联的硬件时，只需要更新少数几个配置变量。例如，安全摄像头需要在 `em0` 上运行 VLAN 1。如果以后将 `em0` 卡替换为使用 [ixgbe(4)](https://man.freebsd.org/cgi/man.cgi?query=ixgbe&sektion=4&format=html) 驱动程序的网卡，那么就不需要再将所有指向 `em0.1` 的引用改为 `ixgbe0.1`。
