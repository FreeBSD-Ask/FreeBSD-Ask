...
```sh
# tmux ls
xxx: 1 windows (created Thu Jun 12 10:00:00 2026)
```

重新附着（恢复）已脱离的会话：

```sh
# tmux attach -t xxx	# 重新附着会话 xxx（名称或编号）
```

如果会话仍处于附着状态，需先脱离再重新附着：

```sh
# tmux detach -t xxx	# 将会话 xxx 从当前终端脱离，保持会话在后台运行
# tmux attach -t xxx	# 恢复
```

tmux 的所有快捷键均以**前缀键** `Ctrl+b` 触发，常用操作如下：
...
> **技巧**
>
> tmux 的用户级配置文件为 **~/.tmux.conf**，系统级配置文件为 **/usr/local/etc/tmux.conf**。可在配置文件中自定义快捷键、状态栏等内容，详见 tmux(1)。
