...
使用时可将 `sudo` 直接替换为 `doas`，二者在基本使用场景下功能等价。doas 支持 `nopass`（免密码）、`keepenv`/`setenv`（环境变量控制）、`args`（命令参数限制）和 `nolog`（不记录日志）等选项；此外还支持 `persist`（密码缓存），但该选项**仅在 OpenBSD 上可用，FreeBSD 不支持此选项**；doas 不具备 sudo 的插件架构和 LDAP 集成等高级特性。
...
sudo 的初始作者是 Bob Coggeshall 和 Cliff Spencer（原始版本，约 1980 年），主要重写和维护者是 Todd C. Miller。
