...
使用时可将 `sudo` 直接替换为 `doas`，二者在基本使用场景下功能等价。doas 支持 `nopass`（免密码）、`persist`（密码缓存）、`keepenv`/`setenv`（环境变量控制）、`args`（命令参数限制）和 `nolog`（不记录日志）等选项，但不具备 sudo 的插件架构和 LDAP 集成等高级特性。
