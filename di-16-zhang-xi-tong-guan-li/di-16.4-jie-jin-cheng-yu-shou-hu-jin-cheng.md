...
这些长期在后台运行的服务通常命名为 `xxxd`，例如 `sshd`、`ntpd`，其中的 `d` 表示守护进程（[daemon](https://www.freebsd.org/copyright/daemon/)），这是 UNIX 系统的通用命名约定。例如，BIND 是 Berkeley Internet Name Domain，但实际执行的程序名为 named。Apache Web 服务器程序是 httpd，数据同步 Rsync 进程是 rsyncd。但这仅是命名约定，并无规范，如 `cron` 和 `sendmail` 的进程都不带 `d`。
...
| 列名 | 说明 |
| ---- | ---- |
| PID | 进程 ID。PID 从 1 开始分配，递增至 99999 后回绕至起始值。若某个 PID 已被占用，则不会再次分配。 |
| TT | 程序运行所在的 tty。 |
| STAT | 程序的状态。 |
| TIME | 程序在 CPU 上运行的时间量，通常不是程序启动以来的经过时间，因为大多数程序在需要花费 CPU 时间之前会花费大量时间等待事件触发。 |
| COMMAND | 用于启动程序的命令。
...
选项 `ww` 可以使 ps(1) 显示每个进程的完整命令行，而非在超出屏幕宽度时截断。但是，因为内核中存储的命令行长度有限制，在某些情况下，使用带有 `ww` 选项的 `ps` 时，输出可能仍然被截断。对于内核线程，命令字段可能显示为 `[ ]` 包裹的名称。
...
可通过界面或配置文件持久化 htop 的配置。需注意：在设置菜单中按 F10 保存后，若使用 Ctrl + C 退出 htop，已保存的设置将被丢弃；必须使用 F10 或 q 正常退出才能确保配置写入 **~/.config/htop/htoprc**。

也可以手动编辑用户目录下的 **~/.config/htop/htoprc** 配置文件，添加以下内容以启用 CPU 频率和温度显示功能：
