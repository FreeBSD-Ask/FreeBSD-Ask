...
FreeBSD 还支持多个“虚拟控制台”，可以使用它们同时进行多个活动会话。使用 **ALT + F<数字>** 切换屏幕，其中 `F<数字>` 是对应希望查看的屏幕的功能键。在默认情况下，系统启用了 8 个虚拟控制台（ttyv0 至 ttyv7）；可以通过编辑 **/etc/ttys** 文件，将相关 vty 条目的“off”字段改为“on”来启用更多虚拟控制台（启用第 9 个及以上时还需手动添加新条目），默认内核最多支持 12 个虚拟终端（可通过内核选项 `VT_MAXWINDOWS` 增加至最多 16 个）。
