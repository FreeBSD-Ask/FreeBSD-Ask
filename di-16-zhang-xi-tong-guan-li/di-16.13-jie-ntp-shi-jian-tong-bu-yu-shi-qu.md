...
若需让 Cron 任务在特定时区下执行命令，可在 crontab 文件中设置 `TZ` 环境变量。`TZ` 变量会被传递给子进程，从而影响命令执行时的时区行为。

> **警告**
>
> FreeBSD 基本系统使用的 Vixie cron **不支持** 设置 `CRON_TZ` 变量来改变调度时区——Cron 调度始终基于系统本地时间进行。`TZ` 仅影响命令执行时的时区环境，不影响 Cron 判断“何时触发任务”。

在每天 08:00（系统本地时间）执行 `date` 命令并将输出追加到 **~/date.log** 文件：
