...
```sh
Setup a user? (enter a lower-case loginname, or 'no') [no] ykla # 输入你要的用户名，按回车键确认
```
...
这一步是选择要将系统安装在哪一块硬盘。按 `?` 列出识别的所有硬盘。请务必记住所有的盘符。然后输入需要安装的位置，若选择 `sd0`。输入 `sd0` 回车。
...
>
> 你认为这种设计的根本缺陷在哪里？请尝试改正并通过邮件列表向 OpenBSD 项目提交改进建议。
