...
若使用 `pkg_add` 软件包安装，在 `/etc/pulse/client.conf` 文件中将 `autospawn = yes` 改为 `autospawn = no`。阻止开机加载 pulseaudio。
...
2. 从源码编译 KDE Plasma 桌面，在构建前于 `/etc/make.conf` 文件中设置 `OPTIONS_UNSET+=PULSEAUDIO` 和 `OPTIONS_SET+=SNDIO`，验证音频输出效果，尝试回溯上游。
