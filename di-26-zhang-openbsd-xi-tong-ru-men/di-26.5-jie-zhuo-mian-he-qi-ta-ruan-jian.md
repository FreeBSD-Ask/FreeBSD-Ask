...
在完成基本系统配置与软件包管理设置后，本节介绍 OpenBSD 上常用桌面环境的安装与配置，以及一些实用工具的使用方法，为用户提供完整的桌面计算体验。
...
若使用 `pkg_add` 软件包安装，在 `/etc/pulse/client.conf` 中将 `autospawn = yes` 改为 `autospawn = no`。阻止开机加载 pulseaudio。
