...
除了树莓派等 ARM 架构设备外，市场上也有基于 x86 架构的开发板可供选择。本章介绍 Radxa X4 开发板在 FreeBSD 上的使用方法。

Radxa X4 是基于 x86 架构的开发板，搭载英特尔（Intel）N100 处理器。本节以 16GB 内存和 128GB eMMC 版本为基础进行介绍。
...
显卡驱动是图形显示的基础，正确安装显卡驱动可以确保系统正常显示并获得良好的图形性能。本节介绍如何安装显卡驱动。使用 pkg 安装：
...
安装显卡驱动后，需要进行相应配置，确保系统启动时自动加载驱动模块。在 `/etc/rc.conf` 中添加 `i915kms` 内核模块，以便开机时自动加载：
...
无线网络连接是现代设备的基本功能，正确安装和配置无线网卡驱动可以提供便捷的网络接入方式。本节介绍如何安装无线网卡驱动。使用 pkg 安装：
...
完成无线网卡驱动安装后，还需要进行一些配置以优化无线连接性能。目前不支持 Wi-Fi 5/6 标准。
...
系统状态监控是运维工作的重要组成部分，通过实时监控可以及时发现系统异常和性能瓶颈。本节介绍如何查看系统实时状态。该命令每秒刷新一次虚拟内存及系统状态信息：
...
CPU 温度监控对于确保设备稳定运行和延长硬件寿命至关重要。过高的温度可能导致系统不稳定或硬件损坏。

### 读取 CPU 温度

本节介绍如何读取 CPU 温度。动态加载 `coretemp` 内核模块，用于监控 CPU 温度：
...
为了确保每次系统启动后都能正常监控 CPU 温度，需要将温度监控模块的加载配置持久化。为了便于 htop 等软件读取 CPU 温度，编辑 `/boot/loader.conf` 文件，添加以下内容：
...
除了 FreeBSD 系统外，部分用户可能需要在 Windows Server 系统上使用 Radxa X4 开发板。本附录介绍在 Windows Server 2025 上安装英特尔 i226-V 网卡驱动的方法。i226-V 在 Windows Server 下可使用 I226-LM 驱动（I226-LM 与 I226-V 功能相同），无需修改任何文件或系统配置：

- 首先下载并解压驱动 [适用于 Windows Server 2025* 的英特尔 ® 网络适配器驱动程序](https://www.intel.cn/content/www/cn/zh/download/838943/intel-network-adapter-driver-for-windows-server-2025.html) [备份](https://web.archive.org/web/20260115142513/https://www.intel.cn/content/www/cn/zh/download/838943/intel-network-adapter-driver-for-windows-server-2025.html)
