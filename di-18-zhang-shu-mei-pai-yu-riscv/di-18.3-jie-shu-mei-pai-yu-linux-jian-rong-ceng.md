# 18.3 树莓派 Linux 兼容层配置

Linux 兼容层（Linux Binary Compatibility Layer）是 FreeBSD 操作系统的一项关键特性，它通过系统调用接口转换机制，允许在 FreeBSD 环境中原生运行 Linux 二进制程序，从而极大地扩展了系统的可用软件生态。本章详细介绍如何在树莓派平台上配置和使用 Linux 兼容层。

## Arch Linux 兼容层部署

### Arch Linux 发行版概述

Arch Linux 是一款以轻量级、滚动更新、高度可定制化为特点的 Linux 发行版，其 ARM 架构移植版本在树莓派等嵌入式计算设备上具有良好的性能表现和社区支持。本节介绍如何在树莓派 FreeBSD 系统上部署 Arch Linux 兼容层。以下是完整的自动化部署脚本内容：
...
- [Raspberry Pi 4 に Arch Linux をインストール](https://tonooo71.github.io/posts/archlinux-on-raspberrypi4) [备份](https://web.archive.org/web/20260115143302/https://tonooo71.github.io/posts/archlinux-on-raspberrypi4/)
