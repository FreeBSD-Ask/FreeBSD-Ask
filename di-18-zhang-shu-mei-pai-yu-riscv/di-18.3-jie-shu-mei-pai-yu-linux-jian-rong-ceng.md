...
Linux 兼容层是 FreeBSD 系统的重要特性，它允许在 FreeBSD 上运行 Linux 应用程序，极大地扩展了系统的软件生态。本章介绍如何在树莓派上配置和使用 Linux 兼容层。

## Arch Linux 兼容层

Arch Linux 是一款轻量级、灵活的 Linux 发行版，其 ARM 版本在树莓派等嵌入式设备上表现出色。本节介绍如何在树莓派 FreeBSD 上安装 Arch Linux（Arch Linux）兼容层。脚本内容如下：
