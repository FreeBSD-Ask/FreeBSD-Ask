...
>
>- `U1`（`1` 环绕着圆形“U”符号）:`U1` 最低写入速度为 10MB/秒；U3 则为 30MB/秒。**没有 U2**。**只有老产品、低速设备会标 U1 或 U3**——`U3` 新产品一般标为 **V30**。不用找 V60，有但是非常贵，V90 的存储卡我没有见过。速率标准换算：***C10 = U1 = V10*** 这 3 个东西其实是一回事，但是同时标着。
>
