...
配置好兼容层后，到 [STM32CubeMX 官网](https://www.st.com/en/development-tools/stm32cubemx.html) [备份](https://web.archive.org/web/20250301000000/https://www.st.com/en/development-tools/stm32cubemx.html)下载最新压缩包后进行解压。(写作文章时，下载无需注册登录，仅需点击以访客身份下载即可，填入信息，注意下载链接会被发送到邮件中，请确保邮件地址可以收信。)
...
定位到解压缩文件夹并运行其中的`SetupSTM32CubeMX-{VERSION}`安装脚本（{VERSION}对应你下载的STM32CubeMX版本），指定安装的目录即可。
...
>
> 虽然通过一定手段能在 FreeBSD 上成功编译 ESP-IDF，但稳定性不如官方平台，也可能会出现未知错误，
> 因此并不建议在生产环境中使用。
