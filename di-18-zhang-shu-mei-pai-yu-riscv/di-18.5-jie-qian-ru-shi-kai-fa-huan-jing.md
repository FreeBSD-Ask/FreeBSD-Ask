...
嵌入式开发是计算机科学与技术的重要分支，广泛应用于物联网、工业控制、消费电子等领域。本章介绍如何在 FreeBSD 系统上搭建常见的嵌入式开发环境。

## STM32

STM32 是意法半导体（STMicroelectronics，ST）从 2007 年开始推出的 32 位微控制器（Microcontroller Unit，MCU）系列，基于 ARM Cortex-M 内核，目前是嵌入式开发领域最主流的 MCU 之一。

### 安装 STM32CubeMX

STM32CubeMX 是意法半导体官方提供的图形化配置工具，能够快速生成 STM32 项目的初始化代码。本节介绍如何在 FreeBSD 平台上安装 STM32CubeMX。在 FreeBSD 平台进行 STM32 嵌入式开发，推荐使用 STM32CubeMX 来初始化项目，而 STM32CubeMX 官方只支持 Windows、Linux、macOS 平台，FreeBSD 上使用该软件需要用到 Linux 兼容层。
...
配置好兼容层后，到 [STM32CubeMX 官网](https://www.st.com/en/development-tools/stm32cubemx.html) [备份](https://web.archive.org/web/20250301000000/https://www.st.com/en/development-tools/stm32cubemx.html)下载最新压缩包后进行解压。写作本文时，下载无需注册登录，仅需点击以访客身份下载即可，填入信息，注意下载链接会被发送到邮件中，请确保邮件地址可以收信。
...
定位到解压缩文件夹并运行其中的 `SetupSTM32CubeMX-{版本号}` 安装脚本（`{版本号}` 对应您下载的 STM32CubeMX 版本），指定安装的目录即可。
...
除了 STM32 外，乐鑫科技的 ESP 系列芯片也是物联网开发领域的热门选择，具有出色的 Wi-Fi 和蓝牙功能。

[Espressif 乐鑫科技](https://espressif.com/) 的主要项目是 ESP-IDF，这是他们为 ESP8266、ESP32 等芯片提供的官方开发框架，以 Apache 2.0 许可开源。里面包含 Wi-Fi/Bluetooth 协议栈、FreeRTOS、各种外设驱动、组件库和示例代码。
...
> ```
>
> 但这是把 FreeBSD 平台映射到 Linux 的手段，是一种兼容性处理方式，并非官方的直接支持。
>
> 虽然通过一定手段能在 FreeBSD 上成功编译 ESP-IDF，但稳定性不如官方平台，也可能会出现未知错误，因此并不建议在生产环境中使用。

### 安装 esptool 和 xtensa-esp-elf

在开始使用 ESP-IDF 之前，需要先安装一些基础工具，包括烧录工具和编译工具链。本节介绍如何安装 esptool 和编译工具链。使用 pkg 安装：
...
- 把 `--chip` 改成您的芯片型号，如 `esp32s3`、`esp32c3`；
...
Arduino 是一款广受欢迎的开源硬件平台，以其简单易用的特点深受电子爱好者和教育工作者的喜爱。本节介绍 Arduino 开发平台的使用方法。[Arduino](https://www.arduino.cc/)（阿尔杜伊诺）是一个开源的电子原型平台，2005 年由意大利伊夫雷亚交互设计学院（Interaction Design Institute Ivrea, IDII）的学生和老师发起，最初目的是用于教学（交互设计硕士课程），让非专业人士也能轻松开展电子项目。2006 年，面对缺乏资金的困境，伊夫雷亚交互设计学院（2001-2006）停办，Arduino 的原型却被保留了下来。

与 FreeBSD 项目的愿景一致，[Arduino 愿景](https://www.arduino.cc/en/about/)是让 Arduino 可供任何人使用。

> **技巧**
>
> Arduino 是一千多年前一位意大利国王的名字（Arduin of Ivrea，意为“勇敢的朋友”）。而在这位国王的出生地，意大利北部一个风景如画的小镇伊夫雷亚（Ivrea），有家叫 di Re Arduino（属于国王 Arduino 的）的酒吧，Arduino 联合创始人经常光临这家酒吧，公司因此而得名。

本节介绍如何安装 Arduino。使用 pkg 安装：
...
- [Arduino，你的名字究竟该怎么读？](https://mp.weixin.qq.com/s/O4wfBF_WlHksmoWbMs7QeA)，Arduino 中国微信公众号载。运营主体为 Arduino 在中国的外企独资公司，法定代表人为创始人 Federico Musto 先生。
- [Arduin of Ivrea](https://www.britannica.com/biography/Arduin-of-Ivrea)，关于这位国王的有记载文献或许并不算多，大英百科全书中也只有寥寥数语。

## 故障排除与未竟事项

嵌入式开发环境可能会遇到各种问题，本节介绍一些常见问题的解决方法以及一些待撰写的主题，为后续内容预留位置。
