...
STM32 是 STMicroelectronics（意法半导体，简称 ST）推出的 32 位微控制器（MCU）系列，基于 ARM Cortex-M 内核，从 2007 年开始推出，现在是嵌入式开发里最主流的 MCU 之一。
...
在安装 STM32CubeMX 前，先安装 openjdk（版本号>=17.0.6）依赖：
...
# cd home && chmod 775 username 
```

配置好兼容层后，到 [STM32CubeMX 官网](https://www.st.com/en/development-tools/stm32cubemx.html) [备份](https://web.archive.org/web/20250301000000/https://www.st.com/en/development-tools/stm32cubemx.html)下载最新压缩包后进行解压。(写作文章时，下载无需注册登录，仅需点击以来宾用户下载即可，填入信息，注意下载链接会被发送到邮件中，请确保邮件地址可以收信。)
...
定位到解压缩文件夹并运行其中的`SetuoSTM32CubeMX-{VERSION}`安装脚本（{VERSION}对应你下载的STM32CubeMX版本），指定安装的目录即可。
...
[Desktop Entry]
Name=STM32CubeMX
Exec=/path/to/STM32Cube/STM32CubeMX %U      # 改为对应链接
Terminal=false
Type=Application
Icon=/path/to/STM32Cube/help/STM32CubeMX.png      # 改为对应链接
...
![](../.gitbook/assets/stm32cubemx.png)
...
```sh
# cd /usr/ports/devel/gcc-arm-embedded && make install clean      # 嵌入式ARM工具链包
# cd /usr/ports/devel/cmake && make install clean      # 项目构建系统核心
# cd /usr/ports/devel/ninja && make install clean      # 高效的构建工具
# cd /usr/ports/devel/openocd && make install clean      # 通用 TAG/SWD 调试和烧录工具
# cd /usr/ports/devel/stlink && make install clean      # ST官方ST-LINK调试器的用户空间工具集
```

### 编译与烧录

使用 STM32CubeMX 创建项目，在 `Project Manager` 的 `Project` 里的 `Toolchain/IDE` 栏选择 CMake，
在 `Code Generator` 里 选择 `copy all used libraries into project folder` 和 `Generate peripheral initialization as a pair of '.c/.h' files per peripheral`。
...
对于 fish，写入 ~/.config/fish/config.fish
...
Espressif 的主要项目是 ESP-IDF，这是他们为 ESP8266、ESP32 等芯片提供的官方开发框架，完全开源（Apache 2.0 许可）。里面包含 Wi-Fi/Bluetooth 协议栈、FreeRTOS、各种外设驱动、组件库和示例代码。

> **警告**
>
> ESP-IDF 官方不支持 FreeBSD，文档也仅列出 Windows/Linux/macOS 的支持。
...
>
> 虽然通过一定手段，能在 FreeBSD 上能成功编译 ESP-IDF，但稳定性不如官方平台，也可能会出现未知错误，
> 因此并不建议在生产环境中使用。
...
> **注意**
> 
> ESP-IDF 的版本可能会更新，请检查 [ESP-IDF 的 GitHub 仓库](https://github.com/espressif/esp-idf) [备份](https://web.archive.org/web/20260301000000/https://github.com/espressif/esp-idf) 以获取最新版。
...
$ cd esp-idf-v5.5.3
$ ./install.sh                               # 运行 ESP-IDF 安装脚本（让其生成 espidf.constraints.v5.5.txt 文件）
```
...
PyPI 和 Espressif 的 extra-index 里**根本没有 FreeBSD-amd64 的预编译 wheel**（官方只提供了 linux-amd64 / win-amd64 / macosx 等）。  
同时 `espidf.constraints.v5.5.txt` 文件里还强制写了 `--only-binary cryptography` 和 `--only-binary tree-sitter-c`，导致 pip 拒绝从源码编译。
...
- 把 `--chip` 改成你的芯片型号，如 esp32s3、esp32c3；端口根据 `dmesg | grep usb` 或 `usbconfig` 找，通常是 /dev/cuaU0 或 /dev/ttyU0
...
```sh
mkdir -p /path/to/new/project      # 创建一个文件夹存储示例项目
cd /path/to/new/project 
idf.py create-project hello_world      # 使用 ESP-IDF 创建一个 Espressif 项目，项目名称是 hello_world
idf.py set-target esp32          # 设定项目使用的是 esp32 的芯片，可以换成 esp32s3 等
...
Arduino（阿杜伊诺）是一个开源的电子原型平台，2005 年由意大利 Ivrea Interaction Design Institute 的学生和老师发起，目的是让非专业人士也能轻松做电子项目。
...
# cd /usr/ports/comms/uarduno &7 make install clean       # Arduino Uno 内核驱动模块
```
...
> **注意**
>
> FreeBSD ports 上的 Arduino IDE 只有 1.8.x 版本的，而最新的 Arduino IDE 已经到了 2.x 版本，
> 这是因为 1.8.x 是基于 Java 写的，易于移植，而 Arduino IDE 2.x（从 2.0 开始）基于 Theia（VS Code 衍生）和 Electron/Chromium 框架，
> 这在 FreeBSD 上移植更复杂，因为 Electron 需要 Linux-specific 依赖，且 FreeBSD 的 Chromium port 有兼容性挑战。
> 如果对此感兴趣，读者可尝试移植。

![](../.gitbook/assets/arduino18.png)
