...
定位到解压缩文件夹并运行其中的 `SetupSTM32CubeMX-{版本号}` 安装脚本（`{版本号}` 对应你下载的 STM32CubeMX 版本），指定安装目录即可。
...
更多子命令帮助：`esptool.py {子命令} --help`，例如 `esptool.py write_flash --help`
...
- Arduino 中国官方微信公众号. Arduino，你的名字究竟该怎么读？[EB/OL]. (2026-12-09)[2026-03-25]. <https://mp.weixin.qq.com/s/O4wfBF_WlHksmoWbMs7QeA>. 详细介绍 Arduino 名称的由来与正确发音。微信公众号运营主体为 Arduino 在中国的外企独资公司，法定代表人为创始人 Federico Musto 先生。
