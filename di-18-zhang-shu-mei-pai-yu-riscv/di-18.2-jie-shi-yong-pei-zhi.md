...
本节介绍树莓派 FreeBSD 的 SSH 登录方法。使用 XShell 登录树莓派，用户名和密码均为 `freebsd`。如需切换到 root 用户，请登录后执行 `su`，并输入 root 用户的密码。
...
接下来进行时间设置。树莓派 5 及之前的型号没有板载实时时钟（Real Time Clock，RTC）来保证时间的准确性，因此完全依赖网络时间协议（Network Time Protocol，NTP）服务进行时间校正。如果时间不准确，可能会影响多项服务的正常运行。
...
本节介绍如何设置主机名。在 `/etc/rc.conf` 文件中，可以修改主机名，例如 `hostname="generic"`，将 `generic` 替换为所需的主机名。


## CPU 频率

本节介绍如何调整 CPU 频率（范围为 600 MHz 到 1500 MHz）：
...
本节介绍如何查看 CPU 温度信息。列出所有系统内核参数，并筛选包含温度的信息：
...
本节介绍如何查看 CPU 频率信息。显示 CPU 核心 0 的可用频率等级列表：
