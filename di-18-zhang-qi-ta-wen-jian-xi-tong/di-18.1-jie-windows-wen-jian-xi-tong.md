...
FreeBSD 当前对 NTFS 和 exFAT 的读写均需借助用户态驱动（分别为 ntfs-3g 和 fusefs-exfat）；FAT32 则由内核原生支持读写。

## NTFS 文件系统

NTFS 是微软开发的文件系统，广泛应用于 Windows 操作系统。FreeBSD 通过 **filesystems/ntfs** 软件包提供对 NTFS 的完整读写支持，便于读写 NTFS 格式的存储设备。
...
将 NTFS 格式的设备，例如 USB 闪存盘插入计算机。此时可看到其设备名称，例如 `da0`（对应设备文件为 **/dev/da0p1**）。
...
使用 ntfs-3g 将 **/dev/da0p1** 挂载至 **/mnt**，设置读写权限，并为文件指定合适的属主和权限掩码：
...
- `-f`：表示快速格式化
- `-v`：表示显示详细输出信息
- `NTFS_USE_UBLIO=0`：UBLIO（User space Block I/O）是一个用户空间块 I/O 库，用于提升 FUSE 文件系统的性能。但在 FreeBSD 环境中，UBLIO 与 ntfs-3g 协同工作时可能导致文件系统损坏及数据丢失。若不设置该参数禁用 UBLIO，命令将挂起无响应。
...
FAT 文件系统结构简单、兼容性广泛。尽管在性能、可靠性和可扩展性方面不及现代文件系统，但凭借跨平台优势，仍是设备间数据交换的常用选择。
...
exFAT（扩展文件分配表）是专为闪存设备（如 USB 闪存盘和 SD 卡）优化的轻量级文件系统。它支持大容量文件，在多个平台上广泛使用，适用于外部存储设备。
...
需要自行安装 ports 中的 **exfat-utils**，其提供的 `mkexfatfs` 工具用于将磁盘格式化为 exFAT。
...
挂载 exFAT 卷需指定其 FreeBSD 分区名及一个已存在的挂载点。以下示例将 **/dev/ada1p1** 挂载到 **/mnt**：
...
- FreeBSD Project. ntfs-3g manpage[EB/OL]. [2026-03-25]. <https://man.freebsd.org/cgi/man.cgi?query=ntfs-3g&sektion=8&manpath=FreeBSD+16.0>. ntfs-3g 官方技术文档，说明各项参数配置与使用方法。
