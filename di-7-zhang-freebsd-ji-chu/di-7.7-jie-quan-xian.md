...
> **注意**
>
> chmod(1) 符号模式中的 `t`（sticky bit, 粘滞位）在 POSIX 标准中标记为 XSI 扩展选项，非 POSIX 基本要求。对“其他”权限（`o`）单独操作 `s`（设置 UID 或 GID）或 `t` 标志（粘滞位）时会忽略。设置粘滞位应使用 `+t` 或 `a+t`，而非 `o+t`。
...
> **警告**
>
> setuid 机制是一把双刃剑：如果 setuid 程序存在安全漏洞，攻击者便可能利用该程序以文件属主（通常是 root）的权限执行任意代码，因此，系统管理员应尽量减少 setuid 程序的数量，并定期审计系统中的 setuid 文件。`nosuid` 挂载选项会使内核忽略文件系统上可执行文件的 setuid/setgid 位，程序仍可运行但不会获得提权。
