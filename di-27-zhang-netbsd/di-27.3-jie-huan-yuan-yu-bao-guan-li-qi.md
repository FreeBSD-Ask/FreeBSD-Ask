...
以 NetBSD 10.1 为例，更换二进制源的方法如下。首先需要配置 pkgin 软件仓库：
...
由于二进制包的构建可能存在差异，NetBSD 用户经常需要在不同源之间切换：
...
pkgsrc 编译安装的方法与 FreeBSD 相似，以下是示例：
...
pkgsrc-wip（pkgsrc work-in-progress，pkgsrc 进行中）包含许多尚未完成的项目，但不少软件依赖这些项目。以下是使用方法：
