...
使用 pkgin 安装软件时，NetBSD 默认会检测 ABI（Application Binary Interface，应用程序二进制接口）版本。由于软件包更新可能不及时，这可能导致某些软件包无法安装，因此可以通过以下方式运行：
...
pkgsrc-wip（pkgsrc work-in-progress，pkgsrc 进行中）包含许多尚未完成的项目，但其中的许多软件依赖这些项目。
