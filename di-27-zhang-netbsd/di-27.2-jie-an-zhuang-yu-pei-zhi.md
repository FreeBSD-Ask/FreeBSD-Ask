...
选择使用 GPT 分区表。GPT 是现代 UEFI 启动方式的标准分区表格式，支持大于 2TB 的磁盘和更多分区。

![install NetBSD](../.gitbook/assets/nb7.png)

设置分区大小，按回车键。建议 EFI 分区设置为 200MB-500MB，根分区占用剩余空间。
...
配置项 `e`：安装 `pkgin` 包管理器，直接按回车键以继续。pkgin 是 NetBSD 的二进制包管理工具。
...
在安装过程中可能会遇到一些问题，以下是相关的观察和说明。通过对照实验观察，使用 IDE 硬盘接口或较少的 CPU 核心配置时未出现类似问题，而当 CPU 核心数量设定为 16 时可能触发兼容性问题。
