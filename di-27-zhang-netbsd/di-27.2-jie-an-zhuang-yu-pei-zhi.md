...
本章以 NetBSD 10.1 版本为例，系统介绍其在 x86_64 架构下的安装流程，包括系统配置及常见问题处理，为读者提供可复现的安装实践指南。

## 下载 NetBSD

NetBSD 官方提供了适用于不同硬件环境的安装镜像，用户可根据实际使用场景选择下载。以下是常用的下载链接：

- 面向虚拟机：[https://cdn.netbsd.org/pub/NetBSD/NetBSD-10.1/images/NetBSD-10.1-amd64.iso](https://cdn.netbsd.org/pub/NetBSD/NetBSD-10.1/images/NetBSD-10.1-amd64.iso) [备份](https://web.archive.org/web/20260117045946/https://cdn.netbsd.org/pub/NetBSD/images/10.1/NetBSD-10.1-amd64.iso)
- 面向物理机：[https://cdn.netbsd.org/pub/NetBSD/NetBSD-10.1/images/NetBSD-10.1-amd64-install.img.gz](https://cdn.netbsd.org/pub/NetBSD/NetBSD-10.1/images/NetBSD-10.1-amd64-install.img.gz) [备份](https://web.archive.org/web/20260117045946/https://cdn.netbsd.org/pub/NetBSD/images/10.1/NetBSD-10.1-amd64-install.img.gz)

## 安装图解

以下示例演示了在 VMware Workstation Pro 17 中使用 `NetBSD-10.1-amd64.iso` 进行的 UEFI、GPT 搭配 SATA 的安装过程，读者可参照以下步骤完成安装。

> **警告**
>
> 若在 VMware Workstation Pro 17 虚拟机中安装 `NetBSD-10.1-amd64.iso`，NVMe 不受支持，可能导致系统卡住、无法卸载 DVD 或无法重启等问题。建议 CPU 数量不宜过多，经测试 4 个 CPU 可正常使用。
...
确认写入分区信息到硬盘，选择 OK 后按回车键。

![install NetBSD](../.gitbook/assets/nb11.png)

选择默认的完整安装（Full installation），然后按回车键。
...
配置项 `f`：安装 `pkgsrc` 包管理器，直接按回车键开始安装。
...
配置项 `o`：添加普通用户。输入用户名后按回车键，再按回车键将用户加入 `wheel` 组。
...
在安装过程中可能会遇到一些问题，以下是相关的观察和说明。虽然影响安装的具体原因尚不明确，但根据观察，使用 IDE 硬盘或较少的 CPU 数量并未出现类似问题，而当指定 CPU 数量为 16 时可能会出现问题。
