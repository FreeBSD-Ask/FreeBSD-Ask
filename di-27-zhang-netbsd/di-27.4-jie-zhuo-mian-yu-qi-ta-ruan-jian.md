...
配置中文环境是使用中文桌面的前提。创建或编辑 `~/.xinitrc` 文件，应以将要登录的用户身份进行修改，可使用 `su` 切换到该用户。在该文件的最上方加入下列几行：
...
Fcitx 是一个流行的中文输入法框架。安装 Fcitx 5 输入法框架及其 Qt 和 GTK 支持模块，以及中文输入法插件：
...
Xfce 是一个轻量级的桌面环境，以下是安装步骤。首先需要安装 Xfce 桌面环境、FAM 文件监控服务、SLiM 登录管理器、中文字体以及 ee 文本编辑器：
...
MATE 是一个传统风格的桌面环境，以下是安装步骤。首先需要安装 MATE 桌面环境及其组件、SLiM 登录管理器、Marco 窗口管理器、FAM 文件监控服务、中文字体，以及 ee 文本编辑器：
...
> **警告**
>
> 在 VB、VM 下目前无法正常进入桌面（UEFI），启动后会黑屏，该问题已报告至 [NetBSD Problem Report #57554](https://gnats.netbsd.org/cgi-bin/query-pr-single.pl?number=57554) [备份](https://web.archive.org/web/20251207044854/https://gnats.netbsd.org/cgi-bin/query-pr-single.pl?number=57554)，如果读者知道原因欢迎告知。
...
NetBSD 默认的 Xorg 配置可能存在问题，导致鼠标无法正常使用。需要在退出 Xorg 后手动生成并修改配置文件，操作步骤如下：
