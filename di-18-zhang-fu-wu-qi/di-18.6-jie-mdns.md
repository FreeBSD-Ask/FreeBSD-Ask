...
零配置网络（[Zero-configuration networking](https://en.wikipedia.org/wiki/Zero-configuration_networking)，有时被称为 *Zeroconf*）是一组简化网络配置的技术，旨在使网络设备无需管理员配置即可实现自动地址分配、域名解析与服务发现。

该功能在嵌入式设备上较为实用。在某些场景下，用户无法通过屏幕或串口获取树莓派的 IP 地址，却需要以 SSH 方式连接到该设备。在树莓派 OS 上，Avahi 服务默认可用且支持多播 DNS。在这种环境下，用户可直接通过 `ping raspberrypi.local` 获取树莓派的 IP 地址，或者直接使用 SSH 连接到 `raspberrypi.local`。
...
Zeroconf 的常用实现是 [Avahi](https://avahi.org/)。**nss-mdns** 能够在所有使用 **nsswitch**（名称切换服务，The Name Service Switch，NSS）的工具中，通过 mDNS 协议查找 `*.local` 后缀的主机名。
...
为了启用 mDNS，需要在 `hosts: files dns` 行之后追加 `mdns` 和 `mdns6`。
...
对照 FreeBSD 主机的 IP 地址进行验证，表明配置正确。
...
1. 在 FreeBSD 上部署 Avahi 并配置 nss-mdns，实现局域网内通过 `主机名.local` 进行无密码 SSH 连接，验证 Windows、macOS 与 Linux 客户端对该 mDNS 主机名的解析一致性。
2. mDNS 使用 224.0.0.251（IPv4）与 ff02::fb（IPv6）多播地址在链路本地范围内广播查询，这一机制在不安全网络中存在服务冒充风险。分析 mDNS 的安全边界，比较 DNSSEC 与 mDNS 的信任模型差异，并设计一种在不依赖 PKI 的前提下增强 mDNS 真实性的方案。
3. 零配置网络的哲学前提是：网络管理者的存在本身是一种故障模式。分析这一前提所隐含的关于复杂性、专业知识与自动化关系的假设，并讨论其在企业网络环境中的适用边界。
