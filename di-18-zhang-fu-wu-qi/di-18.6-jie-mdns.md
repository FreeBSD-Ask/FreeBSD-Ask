...
该功能在嵌入式设备上非常实用，在某些场景下无法通过屏幕或串口获得树莓派的 IP 地址，但是又需要获得 SSH 连接树莓派进行使用。在树莓派 OS 上，Avahi 服务开箱即用，且支持多播 DNS。在这种环境下，可以直接通过 `ping raspberrypi.local` 来获得树莓派的 IP 地址，或者直接使用 SSH 连接到 `raspberrypi.local`。
...
ping 一下 FreeBSD 的主机名，即可显示出 FreeBSD 设备的 IP 地址。
