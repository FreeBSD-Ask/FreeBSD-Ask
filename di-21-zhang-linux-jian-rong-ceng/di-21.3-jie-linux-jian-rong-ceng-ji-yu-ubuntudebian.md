...
echo "begin to install ubuntu ..."                    # 输出开始安装提示
...
if ! /usr/bin/which -s debootstrap; then              # 判断 debootstrap 是否存在
        ...
        case $answer in                              # 根据用户输入进行分支判断
            [Nn][Oo]|[Nn])                            # 用户选择否
                echo "debootstap not installed"      # 输出提示信息（原文拼写保留）
                exit 3                               # 退出脚本并返回错误码 3
                ;;
            *)                                       # 其他情况，视为同意
...
echo "Now you can run '#chroot /compat/ubuntu/ /bin/bash' Into Ubuntu 22.04 LTS"   # 输出完成提示
```
...
echo "begin to install Debian 12 AKA bookworm ..."    # 输出开始安装提示
echo "check modules ..."                              # 输出模块检查提示
...
chroot ${rootdir} /bin/bash -c "update-locale LC_ALL=zh_CN.UTF-8 LANG=zh_CN.UTF-8" # 设置中文 UTF-8 语言环境
echo "Now you can run '#chroot /compat/debian/ /bin/bash' Into debian 12 bookworm"   # 输出完成提示
```
