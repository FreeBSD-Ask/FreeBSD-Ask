...
以下教程在 14.2-RELEASE 测试通过。兼容层使用技术实际上是 Linux jail，并非 chroot。

类似的方法可以构建 Debian 兼容层。

更多其他系统请看`/usr/local/share/debootstrap/scripts/`。

### 开始构建（基于 Ubuntu 22.04 LTS）
...
### Shell 脚本（基于 Ubuntu 22.04 LTS，FreeBSD 14.2 RELEASE 测试通过）
...
## 附：Debian 12（bookworm）（FreeBSD 14.2 RELEASE 测试通过）
...
## 安装 Windows 11 字体（可选。自制包，打包同时兼容 debian、低版本 Ubuntu）
