...
```
如果遇到类似sh: warning: setlocale: LC_ALL: cannot change locale (zh_CN.uTF-8)这样的错误：<br>
以OpenSUSE为例，安装glibc-lang和glibc-locale这两个包，退出兼容层以后重新进入即可。<br>
其他系统可以搜索相似的包，例如CentOS兼容层，应该安装glibc-locale-source glibc-langpack-zh  
