# 21.4 Arch Linux 兼容层（基于 ArchLinux bootstrap）
...
对于滚动发行版，Linux 兼容层默认内核通常较低。直接构建的话，Arch 兼容层会在 chroot 的时候报错 `FATAL: kernel too old`。需要将 Linux 兼容层的内核版本改为 6.12.20（或其他较高版本）才可以。
