...
>
>该 Bug 声称由 [linux: support termios2 ioctls](https://github.com/freebsd/freebsd-src/pull/1949) 在 FreeBSD 16 解决。
...
# tar --use-compress-program=unzstd -xpvf archlinux-bootstrap-x86_64.tar.zst --strip-components=1 -C /compat --numeric-owner   # 解压压缩包到 /compat，保持数字所有者，出现 tar 错误可忽略
```

`--strip-components=1` 即在 `archlinux-bootstrap-x86_64.tar.zst` 解压时去掉外层路径 `root.x86_64`
...
- [pacman.conf(5)](https://man.archlinux.org/man/pacman.conf.5.en) [备份](https://web.archive.org/web/20260119055516/https://man.archlinux.org/man/pacman.conf.5.en) 指出“在 Linux 系统上，禁用为下载文件进程应用默认的 sandbox。当因当前 Linux 内核不支持该特性而导致下载文件时出现 landlock 相关失败时，这会很有用。”
