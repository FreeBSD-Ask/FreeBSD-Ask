...
本节将介绍如何在 FreeBSD 上构建 openSUSE 兼容层。

## 基于 openSUSE Leap dnf 容器的 shell 脚本

该脚本使用的镜像为 openSUSE Leap dnf 容器镜像，虽然基于 dnf 包管理器构建，但 opi 会自动安装 zypper，实现双包管理器共存。不过，用户仍需 [自行导入 zypper 源](https://mirrors.tuna.tsinghua.edu.cn/help/opensuse/) [备份](https://web.archive.org/web/20260121061427/https://mirrors.tuna.tsinghua.edu.cn/help/opensuse/)。
