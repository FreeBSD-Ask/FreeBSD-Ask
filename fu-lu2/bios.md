...
Gear 1 的性能最佳（最大内存能效，内存延迟最低），Gear 4 的性能最差。但是基本上只有 DDR4 才能支持 Gear 1；DDR5 内存频率很高，内存控制器频率几乎不可能达到同等频率（Gear 1），一般最高只能采用 Gear （否则会无法开机）2。对于频率特别高的 DDR5 内存条，可能只能达到 Gear 4。如果你在 CPU-Z 此类软件中看到你的内存频率是 2400 MHz（这里显示的是内存控制器的频率），但是你实际上是 4800 MHz，那么说明你当前的内存模式为 Gear 2 模式。
