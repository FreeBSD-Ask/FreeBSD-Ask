...
DragonFly BSD（蜻蜓 BSD）是一款基于 FreeBSD 4.8 衍生的类 UNIX 操作系统。该项目由曾参与开发 Amiga（上世纪重要的个人计算平台）的 Matthew Dillon（毕业于加州大学伯克利分校）于 2003 年 6 月启动，并于 2003 年 7 月正式发布于 [FreeBSD 邮件列表](https://lists.freebsd.org/pipermail/freebsd-current/2003-July/006889.html) [备份](https://web.archive.org/web/20251228020117/https://lists.freebsd.org/pipermail/freebsd-current/2003-July/006889.html)。
...
捐赠 DragonFly BSD：[Sponsoring projects](https://www.dragonflybsd.org/donations/) [备份](https://web.archive.org/web/20260117173454/https://www.dragonflybsd.org/donations/)，目前仅支持国际 PayPal。
...
了解 DragonFly BSD 的基本信息后，我们可以开始进行系统安装。下面介绍 DragonFly BSD 的安装步骤。
...
准备好安装介质后，我们可以开始启动安装程序。首先将制作好的安装盘插入计算机并启动。
...
完成初始引导后，我们需要进行硬盘写入和引导配置。本文基于 UEFI，我们选择 `UEFI`。新电脑（2016+）都应选择 UEFI。UEFI 是统一可扩展固件接口，是传统 BIOS 的替代方案，提供更现代的启动机制。
...
选择文件系统，此处我们选择 `HAMMER2`。HAMMER2 是 DragonFly BSD 开发的下一代文件系统，支持快照、校验和等特性。
...
文件安装完成后，我们需要对系统进行基本配置。选择 `Configure this System`（配置此系统）。
...
在安装和使用 DragonFly BSD 的过程中，可能会遇到一些问题。以下是一些已知的问题和注意事项。
...
DragonFly BSD 对 Intel 显卡有特定的支持要求。[根据硬件说明](https://www.dragonflybsd.org/docs/supportedhardware) [备份](https://web.archive.org/web/20240701033415/https://www.dragonflybsd.org/docs/supportedhardware/)，DragonFly BSD 6.4 显卡只支持到英特尔第八代（Coffeelake）处理器。
