...
Dillon 启动 DragonFly BSD 项目的核心动因是对 FreeBSD 5 中采用的 SMP（对称多处理）并行计算架构存在不同的技术判断。SMP 是指多个处理器共享同一个内存空间的架构设计。他认为该设计可能引入不必要的性能开销。这一技术分歧导致了与 FreeBSD 核心开发团队的讨论，并最终促成了独立项目的形成。尽管存在技术路径的差异，DragonFly BSD 与 FreeBSD 项目在错误修复和驱动程序更新等领域仍保持着协作关系。
