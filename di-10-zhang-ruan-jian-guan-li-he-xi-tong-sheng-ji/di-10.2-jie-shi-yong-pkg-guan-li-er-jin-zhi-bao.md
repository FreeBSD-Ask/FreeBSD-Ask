...
> **注意**
>
> pkg 只能管理第三方软件包，并不能起到升级系统、获取安全更新的作用。这是因为 FreeBSD 项目是将内核与用户空间作为一个整体来进行维护的，而不是像 Linux 那样由 Linus Torvalds 负责维护内核，各个发行版的人负责维护 GNU 工具（这些软件实际上被设计为单个软件包，因此可使用包管理器更新与升级系统）。
>
