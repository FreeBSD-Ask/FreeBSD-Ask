...
作为严格遵循 ACID（原子性、一致性、隔离性、持久性）特性的数据库系统，PostgreSQL 支持事务处理机制、复杂查询优化和数据完整性约束保障。

在 FreeBSD 操作系统环境下，PostgreSQL 提供了多个稳定版本供用户选择。

## 安装 PostgreSQL

PostgreSQL 提供多个稳定版本供选择，本节以 PostgreSQL 18 为例进行说明，其他版本的安装与配置方法类似。
...
初始化数据库建议使用 `service postgresql initdb` 命令，这是最简便的方式。但其中仍有技巧可用。
