...
pgAdmin4 是一款用于管理 PostgreSQL 数据库服务器的开源软件，作为 PostgreSQL 官方推荐的图形化管理工具。

pgAdmin4 提供了图形用户界面，便于数据库管理。pgAdmin4 使用 Python（Flask 框架）和 React 编写，支持多种操作系统环境（如 Windows、UNIX、Linux 等），可在桌面模式和服务器模式下运行。

> **注意**
>
> 在安装 pgAdmin4 前，应先安装 PostgreSQL 数据库，否则 pgAdmin4 会安装失败。

pgAdmin4 需要在 Python 环境下运行，且安装时需使用 Python 的 pip 包管理工具。因此，应先安装 Python。本节以系统默认的 Python 版本为例进行说明，请注意，FreeBSD 系统可能默认未安装 Python。可通过启动 Python 解释器确认是否已安装：
...
说明：pip 是 Python 的包管理工具，用于安装和管理 Python 包及其依赖关系。
...
可见 shell 已经变为（pgadmin4）（以下操作均在该 shell 下进行）：
...
这是由于时间不正确引发的，使用命令 `ntpd -q -g -p pool.ntp.org` 同步系统时间到 NTP 服务器池 pool.ntp.org：

```sh
(pgadmin4) root@ykla:~# ntpd -q -g -p pool.ntp.org
```

## 配置并运行 pgAdmin4

安装完成后，需要进行配置才能正常运行 pgAdmin4。
...
> **警告**
>
> 请不要直接将 `./pgadmin4/lib/python3.11/site-packages/pgadmin4/config.py` 复制为 `./pgadmin4/lib/python3.11/site-packages/pgadmin4/config_local.py`，无论如何修改配置均不会生效，请直接创建空文件。
...
现在已经安装并运行了 pgAdmin4，并可以通过 `http://ip:5050` 访问 Web 控制面板：
...
2. 编写 Port pgAdmin4。
3. pgAdmin4 从 Qt/C++ 桌面应用转型为基于 Python Flask 的 Web 应用，这一架构变更体现了从胖客户端向浏览器界面的整体迁移趋势。分析数据库管理工具 Web 化对安全边界（数据库凭据在浏览器端的存储）和运维便利性（远程管理无需 SSH 隧道）的双重影响。
