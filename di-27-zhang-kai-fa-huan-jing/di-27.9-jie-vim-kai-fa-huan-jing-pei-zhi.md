...
coc.nvim 是一款基于 Node.js 的代码补全插件，适用于 Vim 和 Neovim，支持完整的 LSP（Language Server Protocol，语言服务协议）。LSP 是一种用于编辑器与语言服务之间通信的开放协议，它将代码分析、补全、跳转定义等功能从编辑器中分离出来，由独立的语言服务提供。其配置方式与插件系统的整体风格与 VS Code 相似。clangd 用于为 C/C++ 提供 LSP 支持。
