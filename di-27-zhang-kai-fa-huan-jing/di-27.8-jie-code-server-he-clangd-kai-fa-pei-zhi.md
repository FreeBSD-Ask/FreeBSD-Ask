...
回到 code-server，它作为一款 Linux 程序，在尝试访问文件或目录时，默认会先在 **/compat/linux** 下查找。假设需要打开 **/usr/src** 目录以查看 FreeBSD 的源代码树，如果 **/compat/linux/usr/src** 存在，那么实际被打开的是 **/compat/linux/usr/src**，而非真正需要的 **/usr/src**。因此需要删除 **/compat/linux/usr/src**，确保 FreeBSD 内核能够 fallback 到正确的 **/usr/src**。
