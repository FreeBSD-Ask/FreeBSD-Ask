...
计算机系统结构是研究计算机硬件组成与互连的学科领域。总线、接口与协议是现代计算机系统中实现数据传输的核心要素，三者共同构成了计算机硬件之间的通信基础设施。从计算机工程的视角审视，正确理解它们之间的关系，有助于我们更好地认识计算机的硬件架构和数据传输机制。
...
- `V30`：新产品一般将 `U3` 标为 **V30**。不必刻意选择 V60，该等级产品价格较高（普通 A2 存储卡约 1 元/GB，V60 约 3 元/GB，且 V60 与 A2 规格通常不共存）。V90 等级的 microSD 卡目前较为少见。低于 V30 的也较为少见，一般就标注为 `U1`。速率标准换算：***C10 = U1 = V10*** 这三个等级其实是同一回事，但通常会同时标注。
...
移速（MOVE SPEED）这张卡的测试速度甚至高于部分三星产品。~~这是用空间换时间了吗？~~ 某些 A2 存储卡，实际测试 4k 读写只有不到 1.5MB/s，这已经超出了单纯参数虚标的问题。
...
在选购固态硬盘时，不能仅关注读写速度，更重要的是要看固态硬盘的主控、NVMe 协议版本及支持状态。例如，大多数小众品牌固态硬盘都不会支持 ASPM（Active State Power Management，活动状态电源管理）技术。此技术能在保证固态硬盘运行效率的情况下尽可能地对固态硬盘进行降温，在实际测试中可使硬盘温度降低约 20 ℃。而一些小众品牌固态硬盘，因为无法很好地适配，开启后就会掉盘，于是主动对该技术在固态硬盘的固件中予以关闭。还有一些小众品牌固态硬盘仍在使用 NVMe 1.4 协议版本。甚至存在多块硬盘使用相同序列号的情况，而硬盘序列号的重要性不亚于网卡的 MAC 地址，原则上不应重复，否则可能导致系统无法正确识别多块硬盘。
...
可以明显地看到，雷克沙 1066x A2 U3 128GB 存储卡实际测试与页面标称严重不符：标称 A2，应达到读 4000 IOPS，写 2000 IOPS。实际上，无论随机读还是随机写都只有一半。这是为什么？
