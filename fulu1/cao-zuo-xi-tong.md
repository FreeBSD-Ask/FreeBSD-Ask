...
计算机系统是可以按用户的要求接受和存储信息、自动进行数据处理并输出结果信息的系统。广义上计算机系统包含 **机械式系统** 和 **电子式系统** 两种。在电子式系统中，又可以分为模拟式计算机系统和数字式计算机系统两种。
...
中央处理器（CPU，Central Processing Unit，中央处理器）、内存储器（又称主存，即内存）、外存储器（磁盘、磁带、CD-ROM、DVD-ROM、U 盘〔USB Flash Disk〕）以及各种输入输出设备（键盘、鼠标、打印机、显示器、扫描仪、数位板等）组成了计算机硬件系统；各种程序和数据则组成了计算机软件系统。
...
并发性是操作系统最重要的特征之一，它使得多个程序能够在同一时间段内同时运行，提高了系统的资源利用率和响应速度。
...
操作系统运行在一个充满不确定性的环境中，它需要随时应对各种随机事件，确保系统的稳定运行。
