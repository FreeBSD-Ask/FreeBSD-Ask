...
开源软件开发的协作模式是现代软件工程的重要范式之一。FreeBSD 作为历史悠久的开源操作系统项目，其开发流程与贡献机制体现了分布式协作的最佳实践。参与 FreeBSD 开发不仅是技术贡献，更是融入 ~~命运共同体~~ 全球技术共同体的过程。

翻译文档、提交 Port 都是不错的方法，提交 Bug 也是可行的方案。

首先需要阅读一下：<https://wiki.freebsd.org/Phabricator>，FreeBSD 项目代码评审与协作开发平台说明文档。
