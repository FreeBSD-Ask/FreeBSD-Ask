...
# tar --use-compress-program=unzstd -xpvf /tmp/archlinux-bootstrap-x86_64.tar.zst -C /tmp --numeric-owner   # 解压 Arch Linux bootstrap 镜像到 /tmp
...
/
├── tmp
│   ├── archlinux-bootstrap-x86_64.tar.zst  # Arch Linux bootstrap 压缩包
│   └── root.x86_64                          # 解压后的 Arch Linux 文件
└── compat
    └── linux                                # Linux 兼容层目录
```
...
tar --use-compress-program=unzstd -xpvf /tmp/archlinux-bootstrap-x86_64.tar.zst -C /tmp --numeric-owner
