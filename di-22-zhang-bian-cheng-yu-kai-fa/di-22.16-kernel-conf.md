...
详情请参见 [ktr(4)](https://man.freebsd.org/cgi/man.cgi?query=ktr&sektion=4&manpath=FreeBSD+7.3-stable) 和 [ktrdump(8)](https://man.freebsd.org/cgi/man.cgi?query=ktrdump&sektion=8&format=html)。
...
- [ALQ(9)](https://man.freebsd.org/cgi/man.cgi?query=alq&sektion=9&manpath=FreeBSD+8.2-RELEASE) 是一款用于将内核记录异步排队到 vnode 的机制。
- [ktr(4)](https://man.freebsd.org/cgi/man.cgi?query=ktr&sektion=4&manpath=FreeBSD+7.3-stable) 等服务使用它根据内核事件流生成跟踪文件。记录由工作线程异步写入。
...
STACK 将启用 [stack(9)](https://man.freebsd.org/cgi/man.cgi?query=stack&sektion=9&manpath=FreeBSD+8.2-RELEASE) 功能，可捕获内核栈，用于 [procinfo(1)](https://man.freebsd.org/procstat(1)) 等。若内核编译中包含 [DDB(4)](https://man.freebsd.org/cgi/man.cgi?ddb(4))，[stack(9)](https://man.freebsd.org/cgi/man.cgi?query=stack&sektion=9&manpath=FreeBSD+8.2-RELEASE) 也会被自动编译进内核。
...
基本上，你必须启用 [ktr(4)](https://man.freebsd.org/cgi/man.cgi?query=ktr&sektion=4&manpath=FreeBSD+7.3-stable) 来使用这些功能，然后通过 sysctl 设置打开或关闭各种日志位。使用 [ktrdump(8)](https://man.freebsd.org/cgi/man.cgi?query=ktrdump&sektion=8&format=html) 提取日志，再通过显示程序处理，就能生成图表等可视化内容。
