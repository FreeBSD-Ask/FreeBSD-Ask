...
>**思考题**
>
>`bxe` Broadcom NetXtreme II (BCM5771X/BCM578XX) PCIe 10Gb 以太网适配器。条目缺失。请研究 PR。

在其他位置（暂存）：

- bwi:  Broadcom BCM430* 和 BCM431* 系列无线网卡。
- bwn:  Broadcom BCM43xx 系列无线网卡。
- Chelsio T3 系列 1GbE/10GbE PCIe 以太网适配器。
- cxgbe:Chelsio T4/T5/T6 系列 1/10/25/40/100GbE PCIe 适配器。
- cxgbev: Chelsio T4/T5/T6 系列 PCIe 虚拟功能。
- em:   Intel Pro/1000 千兆以太网 82542/82543/82544 系列适配器。
- le:   AMD Am7900 LANCE 和 Am79C9xx PCnet。
- malo: Marvell Libertas 无线网卡。
- mwl:  Marvell 88W8363 802.11n 无线网卡（需要 mwl 固件模块）。
- mwlfw: Marvell 88W8363 固件。
- oce:  Emulex 10G 适配器（OneConnect Ethernet）。
- ral:  Ralink Technology IEEE 802.11 无线适配器。
- rtwn: RealTek 无线适配器。
- rtwnfw: RealTek 无线固件。
- ti:   支持基于 Alteon Tigon 1/2 芯片的 PCI 千兆以太网适配器，包括 Alteon AceNIC、3Com 3c985、Netgear GA620 等。使用此驱动时建议大幅增加 kern.ipc.nmbclusters。
...
支持基于 Attansic/Atheros L2 PCI-Express FastEthernet 控制器的千兆以太网适配器。
