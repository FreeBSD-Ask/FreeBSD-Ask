...
新增对著名的 Linux 文件系统 EXT2 的 支持。使用此功能需格外谨慎—— ext2 的代码往往滞后于内核更新且缺乏充分测试，因此以读写模式挂载可能存在风险（哪怕以只读模式挂载也可能导致系统崩溃）。
...
1993 年 POSIX 标准新增的实时扩展 `_KPOSIX_PRIORITY_SCHEDULING` 内置了 `_POSIX_PRIORITY_SCHEDULING` 支持。
