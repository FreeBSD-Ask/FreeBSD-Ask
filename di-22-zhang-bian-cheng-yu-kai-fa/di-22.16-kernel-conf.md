...
`IPFIREWALL_PMOD` 支持协议修改模块。目前仅支持 TCP MSS 修改。这里有问题。原文缺失此条目。参见 [Add ipfw protocol modification module ipfw_pmod](https://reviews.freebsd.org/D10150)。
...
`MBUF_STRESS_TEST` 选项启用用于创建与 mbuf 函数相关的各种随机故障或极端情况的选项。可参见 [mbuf(9)](https://man.freebsd.org/mbuf(9)) 获取可用测试用例列表。

`MBUF_PROFILING` 启用对系统输出的 mbuf 链（通过参与接口）进行分析的代码，并返回监控参数的对数直方图（例如数据包大小、浪费空间、链中 mbuf 数量）。
...
`NETDUMP` 选项将在内核中启用对 [netdump(4)](https://man.freebsd.org/netdump(4)) 客户端支持。它可内核在 panic 时将内核转储发送到远程主机。
