...
Coc.nvim 是一个基于 NodeJS 的适用于 Vim, Neovim 的 Vim 智能补全插件。
拥有完整的 LSP (语言服务协议 Language Server Protocol，简称 LSP ） 支持。配置、使用方式及插件系统的整体风格类似 VSCode. clangd 用于支持 c/c++ 的 LSP 。
