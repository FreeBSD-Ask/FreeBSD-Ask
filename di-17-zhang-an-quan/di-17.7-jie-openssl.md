...
另一文件 **cert.key** 为证书私钥，务必妥善保管。私钥一旦落入他人之手，即可冒充用户或服务器。

### 自签名证书

若无需 CA 签署，可自行创建自签名证书。
...
这样会在目录 **/etc/ssl/certs** 下创建证书文件 **cert.crt**。
...
```sh
# openssl x509 -noout -modulus -in /etc/ssl/certs/cert.crt | openssl md5
MD5(stdin)= d599f0044313c5219caf938f269117f1
# openssl rsa -noout -modulus -in /etc/ssl/cert.key | openssl md5
Enter pass phrase for /etc/ssl/cert.key: # 输入之前设置的密码
MD5(stdin)= d599f0044313c5219caf938f269117f1
```

如果 MD5 值相同，则说明私钥与证书匹配。
