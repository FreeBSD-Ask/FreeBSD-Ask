...
> **技巧**
>
> 本节示例中出现的用户名 `ykla` 为示例，请根据自身环境替换为实际用户名。
...
若必须使用密码，则密码应当足够复杂，并以强哈希机制加密后存入密码数据库。FreeBSD 的 `crypt()` 库支持 SHA-256、SHA-512、Blowfish 等多种算法。
...
注意，此更改不会影响已有的密码哈希。换言之，所有用户都需要运行 `passwd` 修改一次密码，才能让新算法生效。
