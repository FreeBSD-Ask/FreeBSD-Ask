...
大量使用第三方工具也会增加系统入侵风险。如需审计 Ports 中的第三方工具，管理员可更新数据库并查看已安装软件包的已知漏洞：
...
- 安全公告名称始终以 `FreeBSD-SA-` 开头（意为 FreeBSD 安全公告），后接两位数字年份（`26:`，2026 年）、当年公告序号（`08.` 为第 8 份），再后为受影响应用程序或子系统名称（`rpcsec_gss`）。
- `Topic` 字段概括该漏洞。
