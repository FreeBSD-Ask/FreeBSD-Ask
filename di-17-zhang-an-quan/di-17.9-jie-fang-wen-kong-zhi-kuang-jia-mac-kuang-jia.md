...
若 `multilabel` 标志未在根（**/**）分区上保持启用，以下步骤可解决此临时错误：
...
策略限制对 **/etc/master.passwd** 的访问时也可能发生此情况。通常因管理员在不符系统通用策略的标签下更改文件所致。此时系统会读取用户信息，但因文件继承了新标签而阻止访问。用 sysctl(8) 禁用该策略后应可恢复。
