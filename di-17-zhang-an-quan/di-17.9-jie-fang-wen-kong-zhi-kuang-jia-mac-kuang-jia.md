...
MAC 策略 **不能覆盖** 传统 Unix 安全机制（文件权限和超级用户检查），仅在 DAC 基础之上施加额外限制。即使用 MAC 配置了允许访问，DAC 一旦拒绝，操作仍会失败。MAC 策略可限制 `root` 权限，此为其和 DAC 的关键区别之一。
...
各支持标签化的策略模块均有可调参数，可用于在网络接口上禁用 MAC 标签。将标签设为 `equal` 效果类似。可调参数详情见 `sysctl` 输出、策略手册页和本章后续内容。
...
实施 MAC 类似实施防火墙，需防止管理员自身完全排除在外。应考虑恢复先前配置的能力，通过远程连接实施 MAC 时应格外小心。
