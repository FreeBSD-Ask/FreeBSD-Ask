...
执行 openssl 搭配以下参数，可生成供 CA 签署的证书。该命令在当前目录创建两个文件：证书请求文件 req.pem 可递交至 CA，由 CA 验证凭据、签署后返回已签名证书；另一文件 cert.key 为证书私钥，务必妥善保管。私钥一旦落入他人之手，即可冒充用户或服务器。
...
| `-new` | 创建一个新的证书签名请求 |
| `-nodes` | 不为私钥设置密码（no DES），生成的私钥不会加密保护。在 OpenSSL 3 中该选项已弃用，推荐改用 `-noenc` |
...
上述命令生成的是证书签名请求（CSR），需提交给 CA 签署后方可获得证书。若无需 CA 签署，可自行创建自签名证书（参见下方 `-x509` 命令）。
...
建议将上述文件存放于 **/etc/ssl/** 目录，权限设为 `0700`（仅 `root` 可读），用 `chmod` 设置。
...
OpenSSL 的 FIPS 相关部分受到[重点关注](https://www.openssl.org/docs/fips.html)，附有[已知安全问题列表](https://www.openssl.org/news/fips-cve.html)，并定期接受 [FIPS 140 验证流程](https://github.com/openssl/openssl/blob/master/README-FIPS.md)审查。[FIPS 已验证版本清单](https://www.openssl.org/source/)亦已公开。这些措施使用户得以确保满足 FIPS 合规要求。
