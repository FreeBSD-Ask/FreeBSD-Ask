...
Steam 没有原生 FreeBSD 客户端，但其 Linux 版本可通过 FreeBSD Linux 兼容层运行。本节介绍 games/linux-steam-utils Port 的安装与配置方法，使 FreeBSD 用户可接入 Linux 游戏生态。
...
Steam 的程序文件位于 /home/steam/Steam 目录下。
...
本小节以游戏 Beholder 为例测试 Steam 的运行情况。
...
该问题可通过安装中文字体解决，推荐安装 `wqy-fonts` 或 `noto-fonts-cjk` 等字体包。
