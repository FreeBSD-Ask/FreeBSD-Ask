...
Minecraft 是 Java 开发的沙盒游戏，不同版本对 JDK 版本有特定要求（1.20.5+ 需 Java 21）。本节介绍 FreeBSD 上 OpenJDK 安装、启动器配置及服务器端部署。

## 桌面端

### 安装 OpenJDK

Minecraft 对 JDK 版本有特定要求，不同游戏版本所需的 Java 版本不同：1.17 需要 Java 16，1.18 至 1.20.4 需要 Java 17，1.20.5 及更高版本需要 Java 21。本节测试使用 JDK 21 可正常运行最新版 Minecraft。
...
FreeBSD 中有两种常见启动器，分别是 HMCL 和 Prism Launcher。其中 Prism Launcher 可通过 FreeBSD Ports 获得，而 HMCL 只能手动下载 `.jar` 文件运行。
...
并非所有 Minecraft 版本都受支持，适配情况请参考 [平台支持状态](https://github.com/HMCL-dev/HMCL/blob/main/docs/PLATFORM_zh.md) 文档。其他设置与其他平台相同，关键配置包括 Java 路径设置、游戏目录选择和内存分配三项。
...
Prism Launcher 是使用 C++ 和 Qt 框架开发的开源启动器。该启动器默认禁止离线登录，且限制[绕过方法](https://github.com/antunnitraj/Prism-Launcher-PolyMC-Offline-Bypass)，仅推荐正版用户使用。
...
如果使用 KDE 桌面环境，需更新第三方软件包到最新版本。KDE 桌面环境对 Qt 版本有特定要求，Prism Launcher 依赖的 Qt 库需与系统库保持一致。
...
过旧的 JDK 版本不受服务器端支持，本节测试使用 JDK 21 可正常运行。
...
经过测试，[Minecraft 官方服务器端](https://www.minecraft.net/en-us/download/server)为纯 Java 程序，在 FreeBSD 15.0 上安装 OpenJDK 后即可直接运行。Ports 版本则提供了 FreeBSD 特定的集成配置，包括服务管理脚本和默认配置文件。
...
2. 优化 Minecraft 服务器，使其资源占用最优，达到性能最优。
3. Minecraft 的 Java 版服务端具有“一次编写，到处运行”的特性，理论上应是 FreeBSD 游戏服务器的理想选择。比较 FreeBSD 与 Linux 在运行 Minecraft 服务端时的 JVM 内存管理与网络栈性能差异，并通过定量测试验证 FreeBSD 作为 Minecraft 服务器的竞争力。
