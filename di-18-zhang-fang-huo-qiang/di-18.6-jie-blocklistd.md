...
blocklistd 是一个守护进程，它通过套接字监听，接收其他守护进程发送的连接成功或失败通知。主要用于封锁发起过多连接尝试的来源地址。典型的例子是互联网上运行的 SSH 收到大量来自机器人或脚本的请求，这些请求试图猜测密码以获取访问权限。使用 blocklistd 后，守护进程可通知防火墙，在单个来源地址尝试多次后创建过滤规则，以封锁过多的连接尝试。

blocklistd 最初在 NetBSD 上开发，并在 NetBSD 7 中出现，最初名为 blacklistd，后更名为 blocklistd。自 FreeBSD 11 起从 NetBSD 引入。
...
1. 启用 blocklistd 并配置 SSH 防护：设置 5 次失败后封禁 1 小时，从另一台主机模拟多次错误登录，使用 `blocklistctl dump -b` 验证封禁是否生效。
2. 修改 **/etc/blocklistd.conf**，为 SSH 添加自定义锚点名称（如 `-ssh`），用 `pfctl` 查看锚点 `blocklistd/22` 下的规则表，验证规则是否正确写入。
3. 编写一条 `remote` 规则，对特定 IP 段（如 `192.168.1.0/24`）设置更宽松的登录尝试限制（如 10 次），测试规则是否按预期生效。
4. 模拟一次误封：使用 `pfctl` 从 blocklistd 的 PF 表中手动删除被封禁的 IP 地址，观察 `blocklistctl` 输出中该条目何时过期消失。
