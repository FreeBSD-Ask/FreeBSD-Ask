...
```sh
# echo 'net.inet.ip.fw.default_to_accept="1"' >> /boot/loader.conf.local   # 设置防火墙默认策略为接受，并开机生效（推荐使用 /boot/loader.conf.local 做本地配置扩展）
# reboot # 重启生效
```
...
在 PF 中，table 用于存储需要封禁的 IP 地址列表，anchor 用于组织和管理特定的规则集。Fail2Ban 向 PF 的 table 中添加 IP 地址，再通过 anchor 应用相应规则实现封禁。
...
1. 分析 Fail2Ban 中 **action.d/bsd-ipfw.conf** 与 **action.d/ipfw.conf** 的差异，说明为何在 FreeBSD 上须使用 `bsd-ipfw`。

2. 为除 SSH 之外的另一项服务（如 Nginx）配置 Fail2Ban 防护，验证封禁效果。

3. 调整 Fail2Ban 的封禁策略（如修改 maxretry 或 bantime），测试行为变化并分析攻击者可能的规避方式。
