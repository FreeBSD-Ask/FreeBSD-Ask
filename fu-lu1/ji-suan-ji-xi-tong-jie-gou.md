...
上述概念可简化为两类。通道或协议本质上是总线协议（或总线通道）与物理总线（例如 PCIe、RJ45、Type-C 等物理接口、连接器、连接线）的组合。通道或协议是一个泛指术语，既可以指二者的组合，也可以单独指其中之一。
...
总线协议（本文将直接连接到 CPU 的通道称为总线，而总线在本质上也是一种通道）。现代 x86 计算机通常主要使用三种总线：**USB**、**SATA**、**PCIe**。
...
但在 PCIe 和新一代酷睿处理器大规模应用之后，北桥芯片逐渐从主板上消失。曾经的北桥负责高速设备（如内存、集显），南桥负责低速设备（如网卡、声卡、摄像头等）。南桥与北桥之间通常通过 PCI 或 PCIe 总线进行连接。
...
同时需要注意，PCIe 通道并不是无限的，这由 CPU 的规格决定。英特尔处理器提供的 PCIe 通道数量是有限的，例如 PCIe 3.0 ×20，其中 3.0 表示 PCIe 版本，20 表示通道数量。
...
从抽象层面看，总线可视为一种协议，其特点在于作为公共通道，被多个设备直接或间接共享。真正存在的只有两种类别：物理接口和软件协议。这与协议分层无关。硬件信号可通过电气和逻辑转换进行适配，但在实际应用中仍受成本、性能和稳定性等因素限制。
...
一方面因为使用的不是超频读卡器（即雷克沙配套的读卡器，用于支持其自定义协议）；另一方面，因为 UHS-I 协议的理论速度上限为 104 MB/s（SDR104）。任何存储卡在理论上无法超越这个速度，除非使用 UHS-II（两排金手指）。但是对于 microSD 来说，几乎没有使用 UHS-II 的，只有标准大小的 SD 卡才有（相机使用）。
