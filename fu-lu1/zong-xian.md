...
计算机体系结构是研究计算机硬件组成与互连的学科领域。总线、接口与协议是现代计算机系统中实现数据传输的核心要素，三者共同构成了计算机硬件之间的通信基础设施。从计算机工程的视角审视，正确理解它们之间的关系，有助于我们更好地认识计算机的硬件架构和数据传输机制。

我们知道 M.2 只是一个物理接口，它本质上是一种 Mini PCIe 接口，可以直接连接物理引脚而无需额外芯片。
...
从抽象层面看，总线可以视为一种协议，其特点在于作为公共通道，被多个设备直接或间接共享。真正存在的东西只有两种，物理的接口和软件的协议。这与协议分层实际上无关。从理论上讲，硬件信号可以通过电气和逻辑转换进行适配，但在实际应用中仍受成本、性能和稳定性等因素限制。
