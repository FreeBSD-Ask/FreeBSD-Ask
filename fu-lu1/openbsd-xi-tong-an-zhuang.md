...
1. 在 QEMU 中安装 OpenBSD CURRENT，使用自定义分区（仅 / 和 swap），记录安装过程并验证系统能否正常启动，分析这种极简分区方案的使用场景与限制。

2. 检出 OpenBSD CURRENT 源代码，修改 `sbin/disklabel/editor.c` 中的 `alloc_big` 数组，将 / 分区最大限制从 1 GB 调整为 10 GB，重新编译 disklabel 工具并在测试环境中验证其效果。

3. 从 OpenBSD -release 版本通过源代码升级到 -stable，记录整个升级过程中遇到的问题（如 _shutdown 用户组缺失等），并分析为什么 OpenBSD 团队选择不保持向下兼容的设计决策及其对用户的约束。
