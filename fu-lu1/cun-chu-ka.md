...
microSD 通常也称 TF 卡（TF 为 SanDisk 最初的商品名 TransFlash，后被 SD 协会采纳并更名为 microSD，二者系同一物理产品），也常被称作手机存储卡。
...
移速（MOVE SPEED）这张卡的测试速度甚至高于部分三星产品。~~这是用空间换时间了吗？~~ 某些 A2 存储卡实测 4K 读写不足 1.5 MB/s，已超出参数虚标的范畴。
...
部分小众品牌固态硬盘因无法良好适配，开启 ASPM 后即出现掉盘，故在固件中主动关闭该技术。另有部分小众品牌固态硬盘仍在使用 NVMe 1.4 协议版本。甚至存在多块硬盘使用相同序列号的情况。硬盘序列号的重要性不亚于网卡的 MAC 地址，原则上不应重复，若重复则系统可能无法正确识别多块硬盘。
...
其原因在于：其一，未使用超频读卡器（即雷克沙配套的读卡器，用于支持其自定义协议）；其二，UHS-I 协议的理论速度上限为 104 MB/s（SDR104），任何存储卡在理论上无法超越该速度，除非使用 UHS-II（两排金手指）。但是 microSD 极少使用 UHS-II（虽然市场上有少量 UHS-II microSD 产品，如至誉 Catalyst 系列和 Sabrent Rocket 系列，但价格较高且选择有限），UHS-II 主要用于标准大小的 SD 卡（相机使用）。

因此，市面上无论是三星还是闪迪，只要其标称速度超过 UHS-I 的理论上限且并非 UHS-II 产品，通常均通过非标准协议实现。**这种非标准协议仅能通过配套的官方读卡器支持（售价较高且通常捆绑销售），其他设备均不支持该速率，因此不具备实际意义。**
