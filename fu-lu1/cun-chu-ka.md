...
SD 卡标准之所以复杂，是因为：随着技术发展，SD 卡协会既不会弃用旧标准，也很少升级原有标准（如提升版本号），而是另行制定更高等级的新标准。不同标准采用不同的速度等级体系（如 Speed Class、UHS Speed Class、Video Speed Class 等），各体系间存在重叠与差异。
...
| `microSDXC` | 表示该卡容量位于超过 32 GB 至 2 TB 范围内（市场上常见产品从 64 GB 起步）；4 GB–32 GB 的规格称为 microSDHC。其余规格目前已极为少见，包括容量不超过 2 GB（microSD）或容量超过 2 TB 至 128 TB（microSDUC）的规格。由于存储卡本身已明确标注容量，该参数在实际选购中参考价值有限。SD 协会（SD Association）定义：SD 标准为不超过 2 GB，SDHC 标准为超过 2 GB 至 32 GB，SDXC 标准为超过 32 GB 至 2 TB，SDUC 标准为超过 2 TB 至 128 TB |
...
早期，廉价的大容量存储卡多为扩容卡。但由于树莓派启动盘制作工具内置了镜像校验程序，此类卡无法通过其验证。如果标称容量和实际容量不符，则无法完成镜像写入。因此，对于树莓派无须考虑此问题。
...
多数小众品牌固态硬盘不支持 ASPM（Active State Power Management，活动状态电源管理）技术。此技术可在保证固态硬盘运行效率的同时为其降温，有测试表明在特定条件下开启 ASPM 可使硬盘温度有一定程度的降低。

部分小众品牌固态硬盘因无法良好适配，开启 ASPM 后即出现掉盘，因此在固件中主动关闭该技术。另有部分小众品牌固态硬盘固件较旧，使用的 NVMe 协议版本较低。甚至存在多块硬盘使用相同序列号的情况。硬盘序列号的重要性不亚于网卡的 MAC 地址，原则上不应重复，若重复则系统可能无法正确识别多块硬盘。
