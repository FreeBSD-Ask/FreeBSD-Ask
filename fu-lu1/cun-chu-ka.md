...
SD 卡的标准较为复杂。与 USB-IF 协会制定的 USB 标准相比，SD 卡标准的复杂程度相当。

SD 卡标准之所以复杂，是因为：随着技术发展，SD 卡协会既不会弃用原有旧标准（如放弃英制单位、改用公制单位），也很少升级原有标准（如提升版本号），而是另行制定更高等级的新标准。**SD 卡存在多种并行的度量衡体系，量程各有重叠和差异。**
...
该闪迪 microSD 存储卡上标注 7 项参数，其中有 4 项在实际使用中的参考价值较低。
...
部分小众品牌固态硬盘因无法良好地适配，开启 ASPM 后即出现掉盘，因此在固件中主动关闭该技术。另有部分小众品牌固态硬盘仍在使用 NVMe 1.4 协议版本。甚至存在多块硬盘使用相同序列号的情况。硬盘序列号的重要性不亚于网卡的 MAC 地址，原则上不应重复，若重复则系统可能无法正确识别多块硬盘。
...
因此，市场上只要标称速度超过 UHS-I 理论上限且并非 UHS-II 的产品，通常都通过非标准协议实现。**这种非标准协议只能通过配套的官方读卡器支持（售价较高且通常捆绑销售），其他设备均不支持该速率，不具备实际意义。**
