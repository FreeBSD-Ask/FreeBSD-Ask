...
SD 卡标准之所以显得复杂，原因在于：随着技术发展，SD 卡协会既不会弃用既有旧标准（如放弃英制单位、改用公制单位），也很少对原有标准进行升级（如提升版本号），而是另行制定更高等级的新标准。**SD 卡存在多种并行的度量衡体系，其量程各有重叠和差异。**
...
- `microSDXC`：表示该卡容量位于超过 32 GB 至 2 TB 范围内（市场上常见产品从 64 GB 起步）；4 GB–32 GB 的规格称为 microSDHC。其余规格目前已极为少见——容量不超过 2 GB（microSD）或容量大于 2 TB（microSDUC）。该参数在实际选购中参考价值有限，因为存储卡本身已明确标注了容量大小。SD 协会（SD Association）定义：SDHC 标准为超过 2 GB 至 32 GB，SDXC 标准为超过 32 GB 至 2 TB。
...
- `V30`：新产品通常将 `U3` 标为 `V30`。不必刻意选择 V60，该等级产品价格较高（普通 A2 存储卡约 1 元/GB，V60 约 3 元/GB，且 V60 与 A2 规格通常不共存）。V90 等级的 microSD 卡目前较为少见；低于 V30 的亦较为少见，通常标注为 `U1`。

速率标准换算：***C10 = U1 = V10*** 这三个等级含义相同，但通常会同时标注。

这块闪迪 microSD 存储卡上标注了 7 项参数，其中有 4 项在实际使用中的参考价值较低。从参数来看，这款存储卡的整体性能表现中规中矩。
...
在过去，廉价的大容量存储卡往往是扩容卡。但此类卡无法通过树莓派启动盘制作工具的验证，因为该工具内置了镜像校验程序。如果标称容量和实际容量不符，则无法完成镜像写入。因此，对于树莓派无须考虑此问题。

如今的存储颗粒已十分廉价，通常不会再采用扩容手段。若标称容量并非明显异常（如超过 128 GB 却价格极低），通常不存在此问题。
