...
此界面无需任何操作，等待十秒，可自动进入 `1. Boot Multi user [Enter]`；亦可以直接按 **回车键** 进入。
...
|     选项     |                                   解释                                    |
| :----------: | :----------------------------------------------------------------------- |
|`1. Boot Multi user [Enter]`|默认的多用户模式，可用于安装系统|
| `2. Boot Sigle user` |  单用户模式，找回 root 密码和修复磁盘时会用到 |
...
询问你是否需要创建普通用户。如需创建，请按 **回车键** 选 `Yes`；如果不需要普通用户，只需要 root，请用 **方向键** 选 `No`


>**警告**
>
>绝大多数登录管理器都是默认禁止 root 用户登录的。换言之，如果不进行一定的修改（参见其他章节），在默认状态下，你可能无法用 root 登录桌面。
