...
本章作为 FreeBSD 系统部署的前置性章节，旨在为读者提供系统安装前的必要知识框架。操作系统的成功部署首先依赖于硬件兼容性评估、介质准备及安装介质制作等环节的系统性规划。本节将从硬件需求分析入手，逐步引导读者完成系统安装的前期准备工作。

## 硬件支持情况

在开始安装前，了解 FreeBSD 的硬件支持情况很有必要。以下将介绍最低硬件需求与实测硬件支持情况。
