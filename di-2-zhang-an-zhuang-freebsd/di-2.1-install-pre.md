...
本章作为 FreeBSD 系统部署的前置性章节，旨在为读者提供系统安装前的必要知识框架。操作系统的成功部署首先依赖于硬件兼容性评估、介质准备及安装介质制作等环节的系统性规划。本节将从硬件需求分析入手，逐步引导读者完成系统安装的前期准备工作，为后续系统部署奠定坚实基础。

## 硬件支持情况

在开始安装前，了解 FreeBSD 的硬件支持情况很有必要。以下将系统地介绍最低硬件需求与实测硬件支持情况，为硬件选型提供参考依据。
...
**不建议** 使用 FreeBSD 手册中提到的 win32diskimager，因其有时会出现校验错误（尽管实际文件校验值正确）。类似地，**亦不建议** 使用 [Ventoy](https://www.ventoy.net/) [备份](https://web.archive.org/web/20260115143659/https://www.ventoy.net/en/index.html) 直接加载 ISO 或 IMG 镜像文件。
