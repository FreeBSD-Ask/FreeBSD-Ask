...
当使用 Rufus、Win32DiskImager 等软件制作 U 盘启动盘用于安装系统后，用户可能会发现 U 盘的可见容量只有 31.9 MB（即 EFI 分区）。
...
在桌面新建文件夹 `1`（路径为 `C:\Users\用户名\Desktop\1`），将压缩包内所有文件提取至该文件夹中。
...
启动 DiskGenius 时，双击 `DiskGenius.exe` 即可（路径如 `C:\Users\用户名\Desktop\1\DiskGenius\DiskGenius.exe`，具体路径可能有所不同）。
