...
在此界面无需任何操作，等待十秒可自动进入 `1. Boot Installer [Enter]`；也可以直接按 **回车键** 进入。
...
>**警告**
>
>需要指出的是，某些教程要求关闭 ACPI，但这种做法在当前环境下缺乏依据。除非不是 x86 架构，否则现代计算机没有关闭 ACPI 的理由。ACPI 和电源状态、设备节能、多处理器支持等功能密切相关。应将关闭 ACPI 视为一种遗留手段，通常仅在不支持 UEFI 的老旧计算机上才需要考虑。
>
>如果出现 ACPI 报错，也是非常正常的事情，大部分情况下并不会影响使用。一般通过更新 BIOS 即可解决。在极个别情况下可能需要修补 SSDT 和 DSDT，黑苹果用户对此应该并不陌生。
...
89→选中左侧 `install`，按下 **回车键** 可进行安装；中间 `Shell` 为命令行模式；右侧 `Live System` 为 LiveCD 模式。
...
>**警告**
>
>**不要** 选择 `kernel-dbg`、`lib32`、`src` 以外的组件，这些组件需要联网安装，速度较慢。如有需要可以自己后期安装。
>
> 如果在安装中出现选择镜像站的问题，通常是因为您全选了组件，建议避免此操作。
...
>
>如果不确定硬盘的具体标识（例如 `da0` 或 `nvme0`），可使用图片中的命令查看。
>
...
近十年的计算机通常应选择 `GPT+UEFI`。请勿使用默认选项，否则将创建一个 512 KB 的 `freebsd-boot` 分区。

较老的计算机（如 2013 年以前的）可选择选项 `GPT(BIOS)`——此默认选项同时兼容二者。
...
>**注意**
>
>请慎重设置交换分区（`Swap Size`）的大小。通常建议为物理内存的 1-2 倍，但考虑到设计限制，不建议超过 64 GB。因为 ZFS 和 UFS 文件系统创建后不易缩小，而使用 `dd` 命令创建交换文件或后续调整可能带来性能开销或复杂度。
...
此处输入 root 密码，密码不会显示在屏幕上（无任何字符显示）。要求重复输入两次以确认一致性，若两次输入的密码不同则提示密码不匹配。root 密码强度无强制要求，但不可为空。
...
自动检测安装所需固件（14.2 [引入](https://cgit.freebsd.org/src/commit/?id=03c07bdc8b31) [备份](https://web.archive.org/web/20260115144114/https://cgit.freebsd.org/src/commit/?id=03c07bdc8b31)）。
