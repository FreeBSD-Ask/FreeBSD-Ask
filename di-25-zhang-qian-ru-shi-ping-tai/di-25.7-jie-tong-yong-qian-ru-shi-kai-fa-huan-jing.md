...
本节在 FreeBSD 上搭建 STM32（STM32CubeMX 经 Linux 兼容层）、ESP32（ESP‑IDF）及 Arduino 三套主流嵌入式开发环境，覆盖工具链安装到程序编译烧录的完整流程。
...
在 FreeBSD 平台进行 STM32 嵌入式开发，推荐采用 STM32CubeMX 初始化配置。STM32CubeMX 官方仅支持 Windows、Linux、macOS 三大平台，因此在 FreeBSD 上运行该软件需要借助 Linux 兼容层。
...
> **注意**
>
> 建议在 **/compat/linux/home** 目录下为当前用户创建专用的主目录，并以非特权用户身份完成 STM32CubeMX 的安装流程，以降低 root 权限操作的安全风险。
...
配置好兼容层后，从 [STM32CubeMX 官网](https://www.st.com/en/development-tools/stm32cubemx.html) 下载最新压缩包并解压。本节撰写时，下载无需注册登录，以访客身份下载即可。下载链接将通过邮件发送，请确保邮箱地址可正常收信。
...
PyPI 和 Espressif 的 extra-index 中并不包含 FreeBSD-amd64 的预编译 wheel（官方只提供了 linux-amd64、win-amd64、macosx 等）。同时 `espidf.constraints.v5.5.txt` 文件里还强制写了 `--only-binary cryptography` 和 `--only-binary tree-sitter-c`，导致 pip 拒绝从源代码编译。
...
完成 ESP‑IDF 环境配置后，即可开始编译和烧录示例程序。
...
Arduino 项目的愿景与 FreeBSD 项目的开源理念具有共通之处，[Arduino 的官方愿景](https://www.arduino.cc/en/about/)明确提出要让 Arduino 平台可为任何人所使用。
...
> **注意**
>
> FreeBSD ports 上的 Arduino IDE 只有 1.8.x 版本，而最新的 Arduino IDE 已经到了 2.x 版本，这是因为 1.8.x 是基于 Java 编写的，易于移植，而 Arduino IDE 2.x（从 2.0 开始）基于 Eclipse Theia 框架和 Electron/Chromium，Theia 虽与 VS Code 共享 Monaco Editor、LSP 等底层技术，但并非 VS Code 的衍生品，而是由 Eclipse 基金会独立开发的 IDE 平台。Electron 在 FreeBSD 上移植更为复杂，因为其依赖 Linux 专有组件，且 FreeBSD 的 Chromium port 存在兼容性问题。
>
> 如果对此感兴趣，可以尝试移植。
