...
2. 重构 Arch Linux 兼容层，并将其打包至 FreeBSD Ports。
3. Linux 兼容层通过对 Linux 系统调用的翻译实现二进制兼容，这一机制属于接口兼容而非源代码兼容策略。比较 FreeBSD Linuxulator 与 Wine 在兼容策略上的相似性与差异，并分析二者的翻译开销对 ARM 架构下性能敏感型嵌入式应用的影响。
