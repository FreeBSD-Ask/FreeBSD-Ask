...
> **注意**
>
> FreeBSD 默认提供的 IMG 镜像使用 UFS 文件系统。需使用 ZFS 的用户可先在存储卡上写入 img 镜像并正常启动，再插入 U 盘，加载 ZFS 模块，运行命令 `bsdinstall` 安装（安装位置选择 U 盘）。在 `bsdinstall` 界面中，需选择目标磁盘、设置分区方案并配置 ZFS 池。如需在存储卡上使用 ZFS，可反过来用 U 盘作为启动盘安装。
...
内置网卡等硬件暂无驱动（可使用 USB 网卡）。风扇由固件控制，默认会持续转动。HDMI 显示正常，USB 2.0 和 3.0 接口均正常工作。经过测试，KDE Plasma 5 桌面环境可以正常输出到 HDMI 显示器上。
