...
五笔输入法是中文输入领域中一种常用的形码输入方法。在 FreeBSD 上可通过 IBus + Rime 或 Fcitx 5 + 五笔拼音两种组合实现五笔支持。
...
- 将 `98五笔/98/wbx.main.dict` 词库放到 **/usr/local/share/libime/** 目录下。
...
推荐使用方法二进行设置；方法一在复杂场景下需对配置文件格式有一定了解。
...
上述操作涉及基本系统文件，建议仅修改用户级配置，直接修改系统文件会影响全局设置且在系统更新时易被覆盖。
