...
在当代语境下，UNIX 不仅意味着法律上的 **[商标](https://www.opengroup.org/openbrand/register/index2.html)**，更是一种 **哲学思想** 和一系列 **软件工程原则** 的集合。
...
这些原则在当时的软件设计中协同作用，帮助开发者构建出简洁、高效、可维护的系统。FreeBSD 的开发实践深受 UNIX 哲学影响。FreeBSD 提供先进的网络功能、性能、安全与兼容性特性，这一目标与 UNIX 哲学中"可移植性先于高效性"及"小即美"等原则一脉相承。
...
为验证该算法的有效性，需向磁盘写入数据，Thompson 遂编写了一款批量写入数据的程序。

他以每周完成一个程序的速度编写了三项成果：创建代码的编辑器、将代码转换为 PDP-7 可执行机器语言的汇编器，以及"内核的外层，即操作系统"，最终完成了操作系统的初步构建。
