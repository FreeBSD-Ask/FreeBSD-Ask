...
在 PDP-7 上开发新操作系统后不久，Thompson 与几位同事讨论时，系统尚未命名。直到 1970 年，布莱恩·柯尼汉（Brian Kernighan）建议将其命名为**UNIX**：一个对 **Multics** 略带调侃的双关语。该名称后来被逆向扩展 **UNICS**（**Uniplexed** Information and Computing Service, **非复用** 信息和计算服务），与 Multics（**Multiplexed** Information and Computing Service, **多路复用** 信息和计算服务）形成对照。
