...
CURRENT 分支中的代码在经过充分测试后（需满足 MFC 最短三天的要求，MFC 指 `Merge From CURRENT`，类似于 `backporting` 即向后移植）会推送到 STABLE 分支，但这并不保证两个分支均无重大缺陷。尽管 FreeBSD-STABLE 分支应该始终能够编译并运行，但并无保证。
