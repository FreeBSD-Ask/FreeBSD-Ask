...
FreeBSD 项目的源代码中包含部分受 GNU 通用公共许可证（GPL）和 GNU 宽通用公共许可证（LGPL）授权的软件，项目正在持续努力减少所占比重。尽管这些许可证要求开源而非闭源，但它们仍带来一定的法律挑战和额外复杂性。为充分实现 FreeBSD 的宗旨，即尽可能提供无附加条件的软件以降低商业使用中的复杂性，FreeBSD 项目在可能的情况下更倾向于采用限制更少的 BSD 许可证。
...
FreeBSD 社区由来自世界各地的开发者和用户组成。FreeBSD 社区并不是一个法律实体，也无固定办事处。FreeBSD 社区不仅是英文社区，还有中文、俄语、韩语、日语等社区。
...
PF（Packet Filter）最初设计用于 OpenBSD，于 2003 年 3 月被移植到 FreeBSD，2004 年 2 月 26 日集成到基本系统，随 5.3-RELEASE 一同发布。
