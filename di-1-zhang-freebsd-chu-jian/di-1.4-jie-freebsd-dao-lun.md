...
FreeBSD 还有两个开发分支：FreeBSD-CURRENT 和 FreeBSD-STABLE。
...
3. 由于仓库的大小，有些用户选择仅同步他们感兴趣的部分源代码或他们正在贡献补丁的部分。但计划从源代码编译操作系统的用户必须下载 *所有* 的 FreeBSD-CURRENT，而不仅仅是选定的部分。请阅读 [FreeBSD-CURRENT 邮件列表](https://lists.freebsd.org/subscription/freebsd-current) 和 **/usr/src/UPDATING** 以保持更新，了解有时会成为必要的启动过程。
...
2. 安装新的 FreeBSD-STABLE 系统，可以从 FreeBSD 镜像站安装最新的 FreeBSD-STABLE 发布版本，或使用基于 FreeBSD-STABLE 的月度快照。
   若要编译或升级现有的 FreeBSD 系统到 FreeBSD-STABLE，请使用 `git` 检出所需分支的源代码。分支名称（如 `stable/13`）在 <https://www.freebsd.org/releng> 上列出。
