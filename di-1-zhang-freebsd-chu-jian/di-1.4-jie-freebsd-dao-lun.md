...
因为预发布功能尚未经过充分测试，且很可能存在缺陷，FreeBSD-CURRENT 不应被视为提前获取新功能的捷径。因为其他提交同样有可能引入新缺陷，而不是修复既有缺陷，它也不是获取缺陷修复的捷径。FreeBSD-CURRENT 并未得到“正式支持”。
...
FreeBSD 项目历史悠久，其版本控制工具历经 CVS、SVN、Git。多年来，FreeBSD 的中央源代码树由 CVS（Concurrent Versions System）维护。CVS 免费提供源代码控制功能。因为随着源代码树的快速扩张和已存储历史记录的大量增加，CVS 的技术局限性日益明显，项目于 2008 年 6 月切换至 SVN（Subversion）。文档项目和 Ports 集合的存储库也分别于 2012 年 5 月和 2012 年 7 月从 CVS 迁移至 SVN。2020 年 12 月项目完成 Git 仓库转换测试，2021 年 4 月正式将源代码和文档存储库迁移至 Git，Ports 集合也同步完成迁移。目前使用 Git 进行协作开发。
