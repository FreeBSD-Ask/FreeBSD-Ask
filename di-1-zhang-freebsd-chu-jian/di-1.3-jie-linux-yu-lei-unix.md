...
该术语用于描述在设计理念和技术实现上与 UNIX 高度相似但缺乏正式认证的系统。

当前主流操作系统中，除 Windows 外，绝大多数操作系统均可称为 UNIX-like，包括 Linux 和 FreeBSD。

## 课后习题

1. 在现代 FreeBSD 环境中配置交叉编译工具链，构建并在 QEMU 中运行 4.4BSD-Lite2，记录交叉编译过程中需解决的工具链兼容性问题。
2. 查阅 SUS 与 POSIX 标准的正式文档，从系统调用接口、C 标准库函数和编译器约定三个维度列举两者的异同。
3. 分析 FreeBSD 现有的 OpenRC 部分实现（`src/etc/rc.d/`），梳理其与 FreeBSD 原生 rc.d 框架在服务依赖解析机制上的差异。
