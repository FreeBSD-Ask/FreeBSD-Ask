...
可见，参数量越大的模型，通常其体积也就越大。Ollama 默认存储位置是 `~/.ollama/models`

输入 `/bye` 或者按 **Ctrl** + **d** 退出。下次调用时执行相同命令即可。

## Claude Code

Claude Code 是一款 AI 编程助手和自动化编程工具，可读取和理解完整的代码库，编辑文件、运行命令，并与开发工具协作，可以在终端、IDE、桌面应用和浏览器中使用，帮助用户快速开发功能、修复漏洞和自动处理开发任务。Claude Code 需要付费订阅使用。
...
订阅正常后即可开始使用。连续按两次 **Ctrl** + **C** 即可退出该界面
