...
输入 `/exit` 或者按 **Ctrl** + **C** 退出。下次调用执行相同命令。
...
输入 `/bye` 或者按 **Ctrl** + **d** 退出。下次调用执行相同命令。

## Claude Code

Claude Code 是一款 AI 编程助手和自动化编程工具，可读取和理解完整的代码库，编辑文件、运行命令，并与开发工具协作。它适用于终端、IDE、桌面应用和浏览器环境，帮助快速开发功能、修复漏洞和自动处理开发任务。Claude Code 需要付费订阅使用。
