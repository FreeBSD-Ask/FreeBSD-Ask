...
打开 `/usr/local/etc/pam.d/gdm-password` 文件，注释掉 `account requisite pam_securetty.so` 这一行（即往最前面加 `#`）
...
编辑 `/usr/local/etc/pam.d/lightdm` 文件，注释掉 `account requisite pam_securetty.so` 这一行（即往最前面加 `#`）
