...
| 显卡 | 近十年的 Intel 及 AMD 集成/独立显卡 | 英特尔锐炬® Xe 显卡、英特尔 HD Graphics 4000 | 支持程度与 DRM 驱动程序移植进度相关；截至写作时，drm-kmod 元 Port 默认安装 drm-61-kmod（基于 Linux 6.1），适用于 FreeBSD 14.x 及更高版本；FreeBSD 15.0+ 可另行选用 drm-66-kmod（基于 Linux 6.6）；最新非 LTS 版本 drm-latest-kmod 基于 Linux 6.12，适用于 FreeBSD 15.0+。最新进展参见 FreeBSD Project. drm-kmod: drm driver for FreeBSD[EB/OL]. [2026-03-25]. <https://github.com/freebsd/drm-kmod/pulls>. 该仓库提供 FreeBSD 图形驱动程序内核模块更新，追踪 Linux DRM 驱动程序移植进度。 |
