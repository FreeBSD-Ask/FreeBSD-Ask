...
- 内存：
  - 统一可扩展固件接口（UEFI）模式下：最小 256 MB
  - 基本输入输出系统（BIOS）模式下：最小 96 MB
...
> **注意**
>
> FreeBSD 尚未完全支持安全启动。在安装 FreeBSD 前，必须关闭安全启动（Secure Boot）。同时，FreeBSD 也不支持 Fake RAID（伪 RAID），需将控制器模式修改为 AHCI。
>
...
> **警告**
>
> 使用非 RELEASE 版本的用户应有时间和意愿关注开发动态，浏览邮件列表与问题追踪系统。同时要求用户具备一定的探索和实践能力。否则，建议使用 RELEASE 版本。
...
> **技巧**
>
> 如果不明确 `amd64`、`aarch64`、`armv7` 这些架构的区别，请选择 `amd64`（适用于大多数标准个人计算机，Apple 电脑除外）。
...
- RELEASE 正式版镜像下载地址
  - 虚拟机用：<https://download.freebsd.org/releases/amd64/amd64/ISO-IMAGES/15.0/FreeBSD-15.0-RELEASE-amd64-disc1.iso>
...
默认分配了足够接收巨型帧的缓冲区。巨型帧是指大于标准以太网帧（1500 字节）的帧，通常为 9000 字节。巨型帧可减少网络开销、提升传输效率，但在某些网络环境下可能导致兼容性问题。如不需要巨型帧，可缩小接收缓冲区以降低内存需求并避免内存碎片导致的驱动程序挂起：

```sh
# echo 'hw.re.max_rx_mbuf_sz="2048"' >> /boot/loader.conf # 设置 re 网卡接收缓冲区最大尺寸为 2048，以降低内存需求并避免驱动程序挂起
```
...
除了 2.5 G 网卡外，Intel 还有其他多种型号的以太网卡。i210 和 i211 网卡由 em 驱动，通常无需额外配置便可使用，但未经实际测试。
