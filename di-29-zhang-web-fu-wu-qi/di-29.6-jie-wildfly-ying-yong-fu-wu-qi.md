...
> **技巧**
>
> 可通过 Ports 获取 WildFly 历史版本。历史版本曾存在 Bug [java/wildfly: service start fail, illegal group name](https://bugs.freebsd.org/bugzilla/show_bug.cgi?id=285956)，该 Bug 报告了 WildFly 服务启动时出现非法组名的问题（RC 脚本中 `%%USER%%` 和 `%%GROUP%%` 未被正确替换），该 Bug 已修复，历史版本需自行应用补丁。
