...
> **注意**
>
> FreeBSD 默认提供的 IMG 镜像使用 UFS 文件系统。需要使用 ZFS 的用户可先在存储卡上刻录好 img 镜像并正常启动，再插入 U 盘，加载 ZFS 模块，运行命令 `bsdinstall` 进行安装（安装位置选择 U 盘）。在 `bsdinstall` 界面中，需选择目标磁盘、设置分区方案并配置 ZFS 池。如果想在存储卡上使用 ZFS，可反过来用 U 盘作为启动盘进行安装。
