...
嵌入式系统开发的技术成果广泛应用于物联网（Internet of Things，IoT）、工业自动化控制、消费类电子等诸多领域。
...
完成开发环境搭建后，即可开始进行 STM32 程序的编译和烧录工作。本节介绍如何编译和烧录 STM32 程序。使用 STM32CubeMX 创建项目，在 `Project Manager` 的 `Project` 里的 `Toolchain/IDE` 栏选择 CMake，在 `Code Generator` 里选择 `copy all used libraries into project folder` 和 `Generate peripheral initialization as a pair of '.c/.h' files per peripheral`。
...
完成 ESP‑IDF 环境配置后，即可开始编译和烧录示例程序。本节介绍如何编译和烧录 ESP‑IDF 程序。
...
擦除 Flash 后，即可创建并编译 ESP‑IDF 项目。本节介绍如何编译 ESP‑IDF 项目。执行编译操作：
...
Arduino 是一款面向电子爱好者与教育工作者的开源硬件平台，其设计理念以简易性与可访问性为核心。本节介绍 Arduino 开发平台的配置与使用方法。
...
Arduino 项目的愿景与 FreeBSD 项目的开源理念具有共通之处，[Arduino 的官方愿景](https://www.arduino.cc/en/about/)明确提出要让 Arduino 平台能够为任何人所使用。
