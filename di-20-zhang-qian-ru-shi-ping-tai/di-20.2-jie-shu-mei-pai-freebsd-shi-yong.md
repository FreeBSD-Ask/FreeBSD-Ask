...
2. 在树莓派 5 上对比 FreeBSD 原生 ARM64 性能与通过 Linux 兼容层运行 Debian 的性能差异，设计基准测试方案，测量系统调用延迟、内存带宽与 I/O 吞吐量三个指标。
3. 嵌入式设备的封闭性（无 GPIO 标准驱动、无 GPU 文档）对自由操作系统移植构成结构性障碍。分析 Broadcom SoC 的开放性策略对 FreeBSD/ARM 生态发展的制约，并讨论开放硬件运动（RISC-V）能否从根本上解决此问题。
