...
使用 pkg 安装的二进制 Node.js 依赖于特定版本的 **/lib/libcrypto.so.xxx** 文件，这意味着在 FreeBSD 上使用 Node.js 时，需要关注 FreeBSD 系统的版本，尤其是在配置 pkg 使用 latest 源的情况下。
...
2. 适配 Bun 包管理器。
3. 对比 Node.js、Deno 和 Bun 三种 JavaScript 运行时在 FreeBSD 上的启动速度与内存占用差异。
