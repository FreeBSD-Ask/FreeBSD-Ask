...
Rust 以内存安全和线程安全为核心设计目标，并发编程亦是其重要特性之一。
...
rustup 是 Rust 官方的版本管理工具，可从官方渠道安装 Rust 编程语言，并能管理和切换不同版本的 Rust，从而绕过 Ports 中的 lang/rust 及 `lang/rust-nightly`。类似其他编程语言的版本管理器，rustup 可便捷地获取和管理各种 Rust 版本。
...
3. Rust 以所有权模型实现编译期内存安全，Go 以垃圾回收实现运行时内存安全，二者代表了两种不同的安全哲学。分析这两种内存安全策略在内核级代码（低延迟、无 GC 停顿）和用户级代码（开发效率优先）中的适用边界，并讨论 FreeBSD 内核是否应同时接纳两种语言。
