...
因安全机制差异，FreeBSD 上仅 `root` 用户可以使用 DTrace。Solaris 中存在一些低级安全检查，而 FreeBSD 尚未实现。因此，**/dev/dtrace/dtrace** 设备文件严格限定为 `root` 用户可访问。
...
DTrace 工具包使用 DTrace 特有的脚本语言 D 语言，与 C++ 相似。本节不对 D 语言做深入讲解。如需概览，请参阅 FreeBSD 的 d 手册页。D 语言在 [illumos 动态跟踪指南](https://www.illumos.org/books/dtrace/bookinfo.html) 中也有详细介绍。
...
1. 使用 `dtrace -l` 列出系统中所有可用探针，分别统计 `fbt`、`syscall` 和 `proc` 三种 provider 的探针数量，比较它们各自关注的追踪层面。
2. 运行 `/usr/local/share/dtrace-toolkit/hotkernel` 脚本，找出当前系统中最消耗内核 CPU 时间的前三个函数，简要说明这些函数的功能。
3. 使用 `/usr/share/dtrace/hotopen` 脚本监控系统中各 UID 打开文件的频率，分别在空闲状态和编译软件（如 `make buildworld`）时运行，对比两次输出结果。
4. 编写一个简单的 D 脚本，追踪 `read` 系统调用：记录调用进程名、PID 和文件描述符，运行任意程序触发 `read` 调用，验证脚本输出。
