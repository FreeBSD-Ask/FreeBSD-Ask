...
C 与 C++ 在语法、工具链及编译流程上高度相似，本节以 C 语言为例介绍开发环境的配置方法，C++ 开发可参照调整。
...
本节选取三种典型组合作为示例，分别对应调试适配器 `lldb-mi`，以及构建系统 `CMake + Ninja` 和 `Meson + Ninja`。本节以 Visual Studio Code 作为前端开发环境，说明如何将这些底层工具在编辑器中集成。
