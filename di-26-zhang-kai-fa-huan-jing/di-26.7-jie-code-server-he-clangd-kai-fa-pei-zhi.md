# 26.7 code-server 和 clangd 开发环境
...
基于 FreeBSD 内核的路径劫持特性，虽然为了运行 code-server 使用了 Linux 兼容层，但 clangd 以及其他所有开发工具仍然完全由 FreeBSD 提供。

首先需要明确，Linux 二进制兼容模式并非 Linux 模拟器，也并非 Linux 虚拟机。运行 Linux 程序的主体仍然是 FreeBSD 内核本身。

由于主体仍是 FreeBSD 内核，这必然涉及 Linux 程序和 FreeBSD 程序的混合运行，而混合运行又会带来动态链接库调用的问题。对于已编译的二进制程序，它所需的动态链接库路径是写死的，比如如果某个 Linux 二进制程序依赖 **/lib/glibc.so**，那么它一定会去 **/lib/glibc.so** 查找这个文件，而非其他地方。但在 FreeBSD 下，Linux 的运行时环境位于 **/compat/linux** 目录下。
