...
由于主体仍是 FreeBSD 内核，这必然涉及 Linux 程序和 FreeBSD 程序的混合运行，而混合运行又会带来动态链接库调用的问题。对已编译的二进制程序，它所需的动态链接库路径是写死的，比如如果某个 Linux 二进制程序依赖 **/lib/glibc.so**，那么它一定会去 **/lib/glibc.so** 查找这个文件，而非其他地方。但在 FreeBSD 下，Linux 的运行时环境位于 **/compat/linux** 目录下。
...
同样的逻辑也适用 clangd。由于 **/compat/linux/bin/clangd** 并不存在，当 code-server 试图启动 clangd 时，它最终会调用 FreeBSD 提供的 **/usr/local/bin/clangd**，其他开发工具也同理。
