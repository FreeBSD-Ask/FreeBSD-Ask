...
> **技巧**
>
> `libbz2.so.1` 该库本身已存在，但文件名不同。如找不到，可输入 `ls /compat/linux/lib64/libbz2`，随后按 **Tab** 键补全，查看实际文件名。

- 安装依赖库 `libxkbcommon-x11.so.0`：

```sh
# fetch https://dl.rockylinux.org/pub/rocky/9/AppStream/x86_64/os/Packages/l/libxkbcommon-x11-1.0.3-4.el9.x86_64.rpm	# 下载所需的依赖库
# cd /compat/linux/
...
> **技巧**
>
> 找不到 Rocky Linux 的某库时，可到 <https://rockylinux.pkgs.org/> 搜索。FreeBSD Ports 已打包了一部分，可参照 pkg 章节使用 `pkg-provides` 搜索。
...
如以 root 权限运行 Rocky Linux 兼容层中的微信，中文输入法将无法使用（原因同 QQ：输入法框架依赖用户会话的 D-Bus 和环境变量）。请以普通用户权限运行。
