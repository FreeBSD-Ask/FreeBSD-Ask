...
Git 的代理配置方法，请见本书其他章节。Git 作为分布式版本控制系统（Distributed Version Control System，DVCS），其网络请求需独立配置代理参数。
...
为支持此参数的应用程序指定代理服务器和端口，格式遵循统一资源定位符（Uniform Resource Locator，URL）规范：
...
默认使用超文本传输协议（HyperText Transfer Protocol，HTTP），为支持此参数的应用程序指定套接字安全（Socket Secure，SOCKS）代理服务器和端口，通过协议前缀明确标识代理类型：
...
首先，找到桌面环境为 Chromium 创建的桌面（desktop）文件，通常位于 `~/.local/share/applications/` 目录下，该目录遵循 X 桌面组（X Desktop Group，XDG）规范。
