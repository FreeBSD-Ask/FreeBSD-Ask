...
Mihomo 是一款常用的代理软件，它已在 FreeBSD 的 Ports 中提供。本节介绍 Mihomo 的安装与配置方法。Mihomo 作为 Clash 系列的衍生版本，在保持核心功能的同时进行了性能优化和功能扩展。

## 安装 mihomo

以下是安装 Mihomo 的方法，同样提供了二进制包安装和源码编译两种标准途径。

- 使用 pkg 安装，这是推荐的快速部署方式：

```sh
# pkg install net/mihomo
```

- 或者使用 Ports 安装，适用于需要自定义编译选项的高级用户：
...
已向 Ports 维护者发出请求合并（[Bug 291295 - net/mihomo: Add rc.conf and some Post-installation](https://bugs.freebsd.org/bugzilla/show_bug.cgi?id=291295) [备份](https://web.archive.org/web/20260115020637/https://bugs.freebsd.org/bugzilla/show_bug.cgi?id=291295)），该请求旨在为 mihomo 添加系统服务管理支持，但目前尚未收到回应。在官方集成完成前，可使用下文提供的自定义 RC 脚本实现服务化管理。

### RC 脚本

为了方便管理 Mihomo 服务，可以使用以下脚本。RC 脚本是 FreeBSD 系统服务管理的标准机制，通过遵循 rc.d 框架实现服务的统一管理。将以下脚本保存为名为 mihomo 的文件，并放置到路径 `/usr/local/etc/rc.d/` 下。最后使用 root 账户为该文件赋予可执行权限：`chmod +x /usr/local/etc/rc.d/mihomo`。
...
pidfile="${mihomo_datadir}/mihomo.pid"    # 用于检测服务进程，存储主进程的 PID
...
- 如何指定订阅链接中的代理组中的某个代理？（比如仅使用位于美国的某个代理 A）这涉及到代理节点的精细化选择。
...
如果读者遇到类似问题，建议使用其他解决方案，如上述的 Mihomo 方案。
...
仓库克隆完成后，可直接运行脚本文件 start.sh 即可启动程序，这些脚本封装了复杂的启动流程。
...
[wenyinos/clash-for-freebsd](https://github.com/wenyinos/clash-for-freebsd) [备份](https://web.archive.org/web/20260115134506/https://github.com/wenyinos/clash-for-freebsd) 提供 FreeBSD 下 Clash 代理的完整部署方案，支持订阅链接管理。

### 未竟事宜

以下是 Clash for FreeBSD 项目有待改进的地方，这些改进方向有助于提高项目的通用性和易用性：

- 需要更新支持所有类型的订阅链接，扩大项目的适用范围。
