...
reconfig_cmd="mihomo_reconfig"    # 指定让 reconfig 命令调用 mihomo_reconfig 函数，以下载 config.yaml 文件
regeoip_cmd="mihomo_regeoip"    # 指定 regeoip 命令调用 mihomo_regeoip 函数。用于下载 geoip.dat 文件，可以用 mihomo_extra_flags="-m" 指定使用该文件
init_cmd="mihomo_init"    # 指定 init 命令调用 mihomo_init 函数。创建数据文件目录，指定属主，避免普通用户身份执行时的读写权限问题

mihomo_start()
{    # 使用 daemon 启动 mihomo。指定 -p 参数将使用 pidfile，使 mihomo 成为 daemon 的子进程，daemon  程序会自动管理 pidfile，从而在 mihomo 进程退出时自动清理 pidfile
