...
Mihomo 是一款常用的代理软件，它已在 FreeBSD 的 Ports 中提供。本节介绍 Mihomo 的安装与配置方法。
...
已向 Ports 维护者发出请求合并（[Bug 291295 - net/mihomo: Add rc.conf and some Post-installation](https://bugs.freebsd.org/bugzilla/show_bug.cgi?id=291295) [备份](https://web.archive.org/web/20260115020637/https://bugs.freebsd.org/bugzilla/show_bug.cgi?id=291295) ），但目前尚未收到回应。

### RC 脚本

为了方便管理 Mihomo 服务，可以使用以下脚本。将以下脚本保存为名为 mihomo 的文本文件，并放置到路径 /usr/local/etc/rc.d/ 下。最后使用 root 账户为该文件赋予可执行权限：chmod +x /usr/local/etc/rc.d/mihomo。
...
RC 脚本提供了多个命令行参数和配置选项，以下是一些常用参数。这些命令会直接将配置写入 /etc/rc.conf，如配置有误，可直接修改对应行。
...
除了 Mihomo 之外，还有 Clash for FreeBSD 项目可以使用。
...
仓库克隆完成后，可直接运行脚本文件 start.sh 即可启动程序。
...
程序启动后，可以通过 Web 界面进行管理。访问 http://127.0.0.1:9090/ui 在网页中输入上面输出的 Secret 值即可。

### 停止程序

如果需要停止 Clash for FreeBSD 服务，请按以下步骤操作：
...
以下是 Clash for FreeBSD 项目有待改进的地方：
