...
: ${mihomo_user:="root"}    # 默认用户,如果是其它用户，确保 /etc/mihomo 目录及 $pidfile 与 log 文件可写
...
```sh
sysrc mihomo_geoip="https://ghfast.top/https://github.com/MetaCubeX/meta-rules-dat/releases/download/latest/geoip.dat" # 可选,建议用。
```

- 指定其他 mihomo 参数

```sh
sysrc mihomo_extra_flags="-m" # 可选,建议用
```

- 指定调用 mihomo 的用户

```sh
sysrc mihomo_user="mihomo"  # 可选，默认root
```

- 指定 mihomo 的数据路径

```sh
sysrc mihomo_datadir="/var/run/mihomo" # 可选
```
...
```sh
service mihomo regeoip # 可选,首次建议使用，这个不需要经常更新
```
...
```ini
mihomo_config="https://xxx.yyy" # 你的订阅链接地址
