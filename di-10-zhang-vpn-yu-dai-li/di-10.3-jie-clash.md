...
已向 Ports 维护者发出请求合并（[Bug 291295 - net/mihomo: Add rc.conf and some Post-installation](https://bugs.freebsd.org/bugzilla/show_bug.cgi?id=291295)），但未予回应。

### RC 脚本

将以下脚本保存为文本文件 `mihomo`，然后将其放到路径 `/usr/local/etc/rc.d/` 下面。最后用 root 账户赋予该文件可执行权限：`chmod +x /usr/local/etc/rc.d/mihomo`，即可。
...
: ${mihomo_user:="root"}    # 默认用户。如果是其它用户，请确保 /etc/mihomo 目录及 $pidfile 与 log 文件可写
: ${mihomo_extra_flags:=""}    # mihomo 的额外参数

procname="/usr/local/bin/mihomo"    # 与 pidfile 一起，用于检测服务进程
...
extra_commands="init reconfig regeoip"    # 设置额外自定义命令
reconfig_cmd="mihomo_reconfig"    # 指定 reconfig 命令调用 mihomo_reconfig 函数，用于下载 config.yaml 文件
regeoip_cmd="mihomo_regeoip"    # 指定 regeoip 命令调用 mihomo_regeoip 函数,用于下载 geoip.dat 文件，可以用 mihomo_extra_flags="-m" 指定使用这个文件
init_cmd="mihomo_init"    # 指定 init 命令调用 mihomo_init 函数,创建数据文件目录，指定属主，避免普通用户身份执行时的读写权限问题

mihomo_start()
{    # 使用 daemon 启动 mihomo,指定 -p 参数使用 pidfile，使 mihomo 成为 daemon 的子进程，daemon  程序自动管理 pidfile，mihomo 进程退出时自动清理 pidfile
...
mihomo_reconfig()
{
	...
	if ( fetch -o ${mihomo_datadir}/config.yaml.new "${mihomo_config}" );then
		mv ${mihomo_datadir}/config.yaml.new ${mihomo_datadir}/config.yaml    # 下载成功则覆盖配置，失败保留原配置   
...
这里是一些命令行参数，会直接将结果写入文件 `/etc/rc.conf`，如果执行错了直接改对应行即可。
...
- 在这里指定你的订阅链接地址（显然照抄不管用。请读者自行探索）

```sh
sysrc mihomo_config="https://xxxx.yyy"
```

- 用于地理位置判断的 GeoIP 数据或功能，主要用于根据 IP 地址的地理归属来做流量分流或规则匹配。

```sh
sysrc mihomo_geoip="https://ghfast.top/https://github.com/MetaCubeX/meta-rules-dat/releases/download/latest/geoip.dat" # 可选，但建议
```

- 指定其他 mihomo 参数

```sh
sysrc mihomo_extra_flags="-m" # 可选。但建议用
```
...
这里是一个最简单的使其跑起来的例子，可在理解、修改后将其写入 `/etc/rc.conf` 文件：

```ini
mihomo_config="https://xxx.yyy" # 你的订阅链接地址，照抄不管用！
