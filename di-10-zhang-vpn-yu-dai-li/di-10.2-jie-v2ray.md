...
FreeBSD 系统中，`xray-core` 的 `pkg` 安装的资源文件位于 `/usr/local/share/xray-core/`，但程序默认会在可执行文件同级目录（`/usr/local/bin/`）查找。若启动时报 `open /usr/local/bin/geosite.dat: no such file or directory` 错误，可采用以下两种方案：
