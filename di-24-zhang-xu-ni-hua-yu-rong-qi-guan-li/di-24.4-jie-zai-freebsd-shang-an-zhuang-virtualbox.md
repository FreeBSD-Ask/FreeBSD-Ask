...
虚拟机系统访问宿主机 DVD/CD 驱动器是通过共享物理设备实现的。在 VirtualBox 中，可通过虚拟机设置中的 Storage 窗口配置。如有需要，先创建空的 IDE CD/DVD 设备。然后在虚拟 CD/DVD 选择菜单中选择 Host Drive。此时会出现 Passthrough 复选框，勾选该项后虚拟机会直接访问硬件。例如，音频 CD 或刻录功能仅在该选项启用时可用。
