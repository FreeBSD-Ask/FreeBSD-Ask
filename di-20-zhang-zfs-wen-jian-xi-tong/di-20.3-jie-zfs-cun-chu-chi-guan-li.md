...
ZFS 在各 vdev 之间条带化数据。例如，使用两个镜像 vdev 实质上构成了 RAID 10，写入会条带化到两个镜像组上。ZFS 分配空间时力求各 vdev 同步达到 100% 占用。由于更多写入会流向剩余空间较多的 vdev，vdev 之间的可用空间不均衡会降低性能。
...
升级 FreeBSD 后，或从运行旧版系统的计算机导入池时，需要手动将池升级到最新的 ZFS 版本以启用新功能标志。由于升级是单向操作——旧池可以升级，但升级后的池不支持降级，升级前应评估旧系统未来是否会再次导入该池。
