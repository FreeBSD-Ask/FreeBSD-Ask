...
在大规模部署场景中，如何高效管理多个 Jail 实例是一个关键的系统工程问题。
...
需要创建多个 Jail 时，只需重复创建数据目录和项目目录，这样所有 Jail 都可以共用 `/jail/mroot`。
...
创建所需 Jail 路径并放入基本目录，同时将 Ports 树和系统源代码放入模板。注意源代码对应的版本要与 `/jail/mroot` 的版本相同。FreeBSD 源代码可参照相关章节获取：
