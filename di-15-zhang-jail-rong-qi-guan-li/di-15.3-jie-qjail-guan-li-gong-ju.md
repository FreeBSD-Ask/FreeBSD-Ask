...
Qjail 是用于部署 Jail 环境的工具，源自 ezjail 3.1，在继承其设计理念的基础上进行了功能扩展与优化。
...
ezjail 和 Qjail 使用 sh（shell 脚本）编写，iocage 使用 Python 编写，实现语言的不同带来了可维护性和扩展性方面的差异。
...
部分教程中会提到使用 `qjail config -k jailname` 打开 raw_sockets 功能来实现外网访问，这是一种误解。raw_sockets 仅为 ping 等工具所需，并非网络访问的必要条件。在 Jail 中打开 raw_sockets 存在安全风险，这是 Jail 环境默认的安全设计。因此除非确实需要在 Jail 中使用 ping 等工具，否则不建议打开 raw_sockets 功能。
