...
Jail 技术于 1999 年提交到 FreeBSD 源代码树，随 2000 年 3 月发布的 FreeBSD 4.0 首次正式引入。它是首个实现文件系统、进程空间和网络资源三重隔离的操作系统级虚拟化方案，设计目标是通过这三重隔离机制实现安全的多租户环境。
...
devfs 文件系统提供对系统设备节点的访问接口，使 Jail 内的应用程序能够与底层硬件设备进行受控交互。推荐挂载 devfs 以便 Jail 访问必要的设备文件。
...
创建配置文件 **/etc/jail.conf**。也可写入 **/etc/rc.conf** 文件，但单独配置更便于管理，且符合将服务配置与系统配置分离的现代推荐实践。
...
使用 `jls` 命令可查看当前运行中的 Jail 信息，包括 Jail 的标识符、网络地址、主机名和根目录路径等关键信息。
