...
- [one ZFS file system always starts with readonly=on temporary on boot](https://github.com/openzfs/zfs/issues/2133) [备份](https://web.archive.org/web/20260115041705/https://github.com/openzfs/zfs/issues/2133)，提供了 ZFS 文件系统启动时只读属性问题的解决方案
...
1. 进入单用户模式后，尝试修改 ZFS 文件系统的只读属性并重置 root 密码，对比 UFS 和 ZFS 在系统恢复时的操作差异。

2. 使用 FreeBSD Live 镜像挂载并访问系统的 ZFS 池，如遇到权限限制则尝试 Ubuntu Live 镜像。尝试修改 FreeBSD Live 镜像生成脚本，使其可挂载，并将其贡献至 FreeBSD 项目。

3. 在单用户模式下尝试修改系统的关键配置文件（如 rc.conf），观察重启后的行为变化。
