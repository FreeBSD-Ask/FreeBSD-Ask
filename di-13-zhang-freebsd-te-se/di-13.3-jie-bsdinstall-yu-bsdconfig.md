...
本小节将介绍 `bsdconfig` 的主菜单及其功能。

如果你已经安装了 FreeBSD，可以使用此菜单对其进行一定程度的自定义，以适应你的自定义配置。最重要的是，你可以使用 pkg 工具安装额外的第三方软件，这些软件不包含在基本系统中。
...
本小节将介绍 `bsdconfig` 的使用说明。选中该项会弹出窗口，展示详细的使用指引，说明内容如下：
...
本小节将介绍如何通过 `bsdconfig` 安装 FreeBSD 文档。
...
> **注意**
>
> 不必安装中文文档（无论简体或繁体），其内容更新远落后于英文文档。

## 3 Packages 软件包

本小节将介绍如何通过 `bsdconfig` 设置 FreeBSD 软件包的安装方式。
...
本小节将介绍如何选择 HTTP 直接安装的站点。

请选择最适合你的站点，或者选择“其他”以指定不同站点。“Main Site”会通过 GeoDNS 将用户引导至最近的官方镜像站（提供完整分发文件并支持 IPv4 和 IPv6）。其他站点称为“社区镜像”，并非所有站点都提供基础分发包之外的内容。请选择一个站点。
...
本小节将介绍如何通过 `bsdconfig` 修改 root 用户密码。
...
本小节将介绍如何使用 `bsdconfig` 进行登录管理和组管理。
...
- “2 Font” 对中文用户无实际作用，仅适用于西欧国家。
- “5 Saver” 配置屏幕保护程序参见其他章节。
- “6 Screenmap” 对中文用户无实际作用，仅适用于西欧国家。
- “7 Ttys” 对中文用户无实际作用，适用于西欧或俄罗斯等区域。


## 8 Timezone 时区

本小节将介绍如何通过 `bsdconfig` 配置系统时区。

配置同 `bsdinstall`，不再赘述。

## 9 Mouse 鼠标

本小节将介绍如何通过 `bsdconfig` 配置鼠标。

你可以运行 mouse 守护进程以在文本控制台中进行剪切和粘贴。需指定端口和鼠标协议类型后启用守护进程，如不使用此功能，可选择 6 禁用守护进程。
...
本小节将介绍如何使用 `bsdconfig` 配置操作系统安全策略。

此菜单用于配置操作系统安全策略。修改设置前，请仔细阅读系统文档，以免操作不当导致服务中断。
...
本小节将介绍如何通过 `bsdconfig` 查看和编辑系统启动配置。
...
1. 通过 bsdconfig 的 "View/Edit Startup Configuration" 添加一个自定义 sysctl 配置，追踪该配置如何写入 rc.conf 并最终生效。

2. 尝试使用 bsdconfig 的命令行模式配置网络接口，对比图形界面与命令行两种方式的操作路径差异。
