...
在 FreeBSD src 源代码中，上述文件主要位于 [libexec/rc](https://github.com/freebsd/freebsd-src/tree/main/libexec/rc) 路径下。

`rc.conf` 文件管理所有系统服务，其相关文件和路径如下：

默认配置位于 `/etc/defaults/rc.conf`。**此文件仅供参考，请勿直接修改。** 如需修改，请编辑 `/etc/rc.conf` 文件来覆盖默认的设置。

用户自定义的配置位于 `/etc/rc.conf`。例如，如果想让系统自动启动 ssh、ipfw、nginx 等服务，就要修改该文件。
...
>**注意**
>
>关键字 `enable`、`disable`、`delete` 最早出现在 13.0-RELEASE，参见 [Add new rc keywords: enable, disable, delete](https://reviews.freebsd.org/D17113) [备份](https://web.archive.org/web/20260115041256/https://reviews.freebsd.org/D17113)

系统服务安装后默认未启用，以上命令无法直接执行，需要先启用服务。编辑 `/etc/rc.conf` 文件：在文件中添加一行：`XXX_enable="YES"`，其中 `XXX` 表示服务名称（例如 `nginx`、`samba` 等），这是固定格式。
...
服务对应的脚本路径为：`/usr/local/etc/rc.d/`。当然也可以直接调用 `/etc/rc.d/` 和 `/usr/local/etc/rc.d/` 下的那些脚本。
...
原代码位于 [libexec/rc/rc.conf](https://github.com/freebsd/freebsd-src/blob/main/libexec/rc/rc.conf)，版本 [Set virtual_oss_enable="NO" in /etc/defaults/rc.conf](https://github.com/freebsd/freebsd-src/commit/1b2d495a24c36d81b14178a2f898025946bff2d8)。
...
apm_enable="NO"        # 设置为 YES 可启用 APM BIOS 功能，否则禁用
...
#cloned_interfaces="gif0 gif1 gif2 gif3" # 预先克隆 GENERIC 配置
#ifconfig_lo0="inet 127.0.0.1/8"         # 默认回环设备配置
#ifconfig_lo0_alias0="inet 127.0.0.254/32" # 回环设备别名示例
#ifconfig_em0_ipv6="inet6 2001:db8:1::1 prefixlen 64" # IPv6 地址示例
#ifconfig_em0_alias0="inet6 2001:db8:2::1 prefixlen 64" # IPv6 别名示例
#ifconfig_em0_name="net0"           # 将接口 em0 重命名为 net0
#vlans_em0="101 vlan0"              # em0 设备的 vlan(4) 接口
#create_args_vlan0="vlan 102"       # vlan0 设备的 vlan tag
#wlans_ath0="wlan0"                 # ath0 设备的 wlan(4) 接口
#wlandebug_wlan0="scan+auth+assoc"  # 使用 wlandebug(8) 设置调试标志
#ipv4_addrs_em0="192.168.0.1/24 192.168.1.1-5/28" # IPv4 地址示例
#
...
#route6d_flags="-l"               # 示例：仅使用 IPv6 site local 地址
#route6d_flags="-q"               # 终端节点运行路由守护进程时应停止广告
#ipv6_network_interfaces="em0 em1" # 路由器或终端节点静态配置示例
#ipv6_prefix_em0="fec0:0000:0000:0001 fec0:0000:0000:0002"  # 路由器示例
#ipv6_prefix_em1="fec0:0000:0000:0003 fec0:0000:0000:0004"  # 路由器示例
...
cursor="NO"                # 光标类型 {normal|blink|destructive}（或 NO）
...
kern_securelevel="-1"		# 范围：-1..3；-1 最不安全。注意：将 securelevel 设置为 0 会导致系统以 securelevel 1 启动，因为 init(8) 会在 rc(8) 完成后提升级别
...
watchdogd_shutdown_timeout=""	# 停止 watchdogd 后使用的超时，仅对系统关机有效，覆盖 watchdogd_flags 中的 -x。
...
jail_reverse_stop="NO"		# 以相反顺序停止 jail
