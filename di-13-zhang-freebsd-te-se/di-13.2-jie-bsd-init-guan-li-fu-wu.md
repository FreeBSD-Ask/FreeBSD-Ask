...
默认 rc 配置位于 `/etc/defaults/rc.conf`。**此文件仅供参考，请勿直接修改。** 如需修改，请编辑 `/etc/rc.conf` 文件来覆盖默认的设置。`/etc/rc.conf` 是用户自定义的 rc 配置。例如，如果想让系统自动启动 sshd、apache、pf 等服务，就要修改该文件。
...
# 添加服务并设置开机自启
# service xxx enable	# 该方法并非适用于所有服务，仍有局限，参见 https://bugs.freebsd.org/bugzilla/show_bug.cgi?id=285543
