...
长期在后台运行的服务通常命名为 `xxxd`，例如 `sshd`、`ntpd`，其中的 `d` 表示 FreeBSD 的守护进程标志“[daemon](https://www.freebsd.org/copyright/daemon/) ”。在 Windows 系统中，这类程序被称为“服务”，可在任务管理器中查看。
...
# 添加服务并设置开机自启
# service xxx enable	# 该方法并非适用于所有服务，仍有局限，参见：rc keywords: enable, disable, delete cannot manage certain built-in rc startup items.[EB/OL]. [2026-03-26]. https://bugs.freebsd.org/bugzilla/show_bug.cgi?id=285543
...
>**注意**
>
>关键字 `enable`、`disable`、`delete` 最早出现在 13.0-RELEASE，参见：Add new rc keywords: enable, disable, delete[EB/OL]. [2026-03-26]. <https://reviews.freebsd.org/D17113>

系统服务安装后默认未启用，以上命令无法直接执行，需先启用服务。
