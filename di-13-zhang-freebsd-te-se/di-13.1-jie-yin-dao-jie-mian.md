...
根据 [loader.conf(5)](https://man.freebsd.org/cgi/man.cgi?query=loader.conf) 所述，loader.conf 文件包含了关于系统引导过程的配置信息。通过 loader.conf，可以指定要启动的内核、传递给内核的参数以及需要加载的附加模块；通常还可以设置 loader(8) 中所述的一切变量。
