...
根据 [loader.conf(5)](https://man.freebsd.org/cgi/man.cgi?query=loader.conf) 所述，loader.conf 是 FreeBSD 系统引导过程中的配置文件。该文件在引导加载器 loader(8) 阶段被读取，用于指定要启动的内核、传递给内核的参数以及需要加载的附加模块，同时可设置 loader(8) 支持的所有变量。
...
loader.conf：系统启动配置（写入此处的配置比 `rc.conf` 更早生效，但可能会妨碍启动），位于 `/boot/loader.conf`，请参考 [loader.conf](https://man.freebsd.org/cgi/man.cgi?query=loader.conf)。

> **技巧**
>
> 不建议直接修改 `/boot/defaults/loader.conf`，如需自定义配置，应使用 `/boot/loader.conf` 或 `/boot/loader.conf.local` 进行本地配置扩展，其中 `/boot/loader.conf.local` 优先级最高，用于机器特定设置。
...
引导选择界面等待时间由 `autoboot_delay` 参数控制，该参数定义了系统在自动启动默认内核前等待用户干预的时长。修改该参数可根据需求调整启动流程的用户交互窗口。
...
精简启动输出可通过多维度配置实现：引导器层面减少内核加载信息、服务启动阶段关闭状态提示、网络配置优化等待逻辑。以下配置从引导到多用户启动阶段逐步减少输出信息。
...
- [FreeBSD Desktop – Part 1 – Simplified Boot](https://vermaden.wordpress.com/2018/03/29/freebsd-desktop-part-1-simplified-boot/) [备份](https://web.archive.org/web/20260115135302/https://vermaden.wordpress.com/2018/03/29/freebsd-desktop-part-1-simplified-boot/)，提供了 FreeBSD 启动过程简化配置的完整指南
- [rc.conf -- system configuration information](https://man.freebsd.org/cgi/man.cgi?rc.conf(5) )，系统配置文件标准手册页
- [acpi -- Advanced	Configuration and Power	Management support](https://man.freebsd.org/cgi/man.cgi?acpi(4) )，高级配置与电源管理接口技术手册
...
- [How to find the valid values of efi_max_resolution](https://forums.freebsd.org/threads/how-to-find-the-valid-values-of-efi_max_resolution.84840/) [备份](https://web.archive.org/web/20260115135345/https://forums.freebsd.org/threads/how-to-find-the-valid-values-of-efi_max_resolution.84840/) ，提供了 gop 方法参考及实用动图演示
...
- [customize boot loader logo](https://forums.freebsd.org/threads/customize-boot-loader-logo.72903/) [备份](https://web.archive.org/web/20260114075554/https://forums.freebsd.org/threads/customize-boot-loader-logo.72903/)，提供了自定义引导加载程序 Logo 的详细方法
- [How to change the FreeBSD logo which appears as soon it boots with that of the little devil.](https://forums.freebsd.org/threads/how-to-change-the-freebsd-logo-which-appears-as-soon-it-boots-with-that-of-the-little-devil.85934/) [备份](https://web.archive.org/web/20260114075527/https://forums.freebsd.org/threads/how-to-change-the-freebsd-logo-which-appears-as-soon-it-boots-with-that-of-the-little-devil.85934/)，介绍了修改 FreeBSD 启动 Logo 的尝试及问题说明
- [loader: Load a splash screen if "splash" variable is defined](https://reviews.freebsd.org/D45932) [备份](https://web.archive.org/web/20260119174807/https://reviews.freebsd.org/D45932) ，提出了引导程序启动画面加载功能的技术实现方案
