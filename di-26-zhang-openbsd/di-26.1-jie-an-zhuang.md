...
以 OpenBSD 7.3，AMD 64 位架构为例，访问：<https://cdn.openbsd.org/pub/OpenBSD/7.3/amd64> 获取系统镜像。若是刻录 U 盘安装，就下载 `install73.img` 和 `miniroot73.img`（同时支持 UEFI+BIOS，即需要两个 U 盘）；若是虚拟机体验，请下载 `install73.iso`（ISO 镜像不支持 UEFI 启动，只支持 BIOS）。（注：截止 OpenBSD 7.3 时，请不要使用 ventoy 引导实体机安装。）
