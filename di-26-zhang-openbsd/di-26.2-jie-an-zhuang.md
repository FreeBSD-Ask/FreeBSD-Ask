...
假设磁盘容量为 80 GB，可规划分区为：`EFI 260 MB`、`/` 75 G、其余空间分配给 `swap`。**顺序不可动，否则无法启动！必须先分一个 /，再分 swap。基本思路：自动分区 → 删去 i 分区以外的分区 → 分 / → 分 swap**
...
2. 检出 OpenBSD 7.7 源码，修改 `sbin/disklabel/editor.c` 中的 `alloc_big` 数组，将 / 分区最大限制从 1 GB 调整为 10 GB，重新编译 disklabel 工具并在测试环境中验证其效果。
