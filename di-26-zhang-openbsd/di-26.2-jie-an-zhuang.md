...
以 OpenBSD 7.7、AMD64 架构为例，请访问 [https://cdn.openbsd.org/pub/OpenBSD/7.7/amd64](https://cdn.openbsd.org/pub/OpenBSD/7.7/amd64) [备份](https://web.archive.org/web/20260120171222/https://cdn.openbsd.org/pub/OpenBSD/7.7/amd64/) 获取系统镜像。
...
> **技巧**
>
> OpenBSD 安装程序实际上调用的是位于 [distrib/miniroot/install.sub](https://github.com/openbsd/src/blob/master/distrib/miniroot/install.sub) [备份](https://web.archive.org/web/20260121065244/https://github.com/openbsd/src/blob/master/distrib/miniroot/install.sub) 的脚本。该安装脚本高度耦合，截至写作时，有三千两百余行。
...
直接回车，选择 `cd0`，即使用安装介质作为软件源。

```sh
Pathname to the sets? (or 'done') [7.7/amd64]
```
...
```sh
Location of sets? (cd0 disk http nfs or 'done') [done]  # 直接回车
Time appears wrong.   Set to 'Mon Dec 16 22:43:37CST 2824' ? [yes]  # 直接回车
```
...
本附录将详细介绍 OpenBSD 的自动分区机制，并提供手动分区的方法供读者参考。

### OpenBSD 自动分区的源代码分析

OpenBSD 默认的自动分区实际调用的是 [sbin/disklabel/editor.c](https://github.com/openbsd/src/blob/master/sbin/disklabel/editor.c) [备份](https://web.archive.org/web/20260120213618/https://github.com/openbsd/src/blob/master/sbin/disklabel/editor.c) 文件中的相关函数。
...
// 磁盘容量介于 700M（含）到 2.38 GB（不含）之间时执行下面的分区策略
...
// 磁盘容量介于 1MB 到 700M（不含）之间时执行下面的分区策略
struct space_allocation alloc_stupid[] = {
	{    MEG(1),      MEG(2048), 100, "/"		}
};
```

根据 [sbin/disklabel/extern.h](https://github.com/openbsd/src/blob/master/sbin/disklabel/extern.h) [备份](https://web.archive.org/web/20260120170920/https://github.com/openbsd/src/blob/master/sbin/disklabel/extern.h) 头文件中的宏：

```c
#define MEG(x)	((x) * 1024LL * (1024 / DEV_BSIZE))    // 实际上是 1MB
#define GIG(x)  (MEG(x) * 1024LL)    // 实际上是 1GB
```
...
- 若磁盘总大小介于 700M（含）到 2.38 GB 之间（不含）：swap

> **思考题**
>
> 在实际使用中，`/` 分区很快就会被填满，而 `/home` 几乎完全为空。并且原生的文件系统不支持磁盘的再分配，如缩减分区，调整顺序。
>
> 你认为这种设计的根本缺陷在哪？请读者尝试改正并通过邮件列表向 OpenBSD 项目提交改进建议。

实际执行计算的是同 [sbin/disklabel/editor.c](https://github.com/openbsd/src/blob/master/sbin/disklabel/editor.c) [备份](https://web.archive.org/web/20260120213618/https://github.com/openbsd/src/blob/master/sbin/disklabel/editor.c) 文件内的函数 `allocate_space()`，其中 `if (xtrablks < sa[i].minsz)` 表明先验证磁盘总大小是否满足初次分配的最小要求。从结构体 `alloc_big[]`、`alloc_medium[]`、`alloc_small[]` 最后到 `alloc_stupid[]` 依次降级计算。

函数 `allocate_partition()` 则执行实际写入。

> **思考题**
>
> 请读者分析在 UEFI 下，OpenBSD 自动生成的 `EFI` 分区在哪个位置？
...
了解自动分区的机制后，读者可以根据实际需求执行手动分区，以更合理地分配磁盘空间。

在系统分区阶段，选择 `C`（Custom），即“自定义设置”。

> `p m`（注意之间的空格）

输入 `p m`（注意之间的空格）来显示硬盘。其它选项如下表：
...
除了标准的 release 版本外，OpenBSD 还提供了 stable 和 current 版本供高级用户选择。本节将介绍如何从 release 版本升级到这些版本。
...
OpenBSD [不建议](https://www.openbsd.org/faq/current.html) [备份](https://web.archive.org/web/20260113181957/https://www.openbsd.org/faq/current.html) 从 release 升级到 current，建议直接使用 [快照版本](https://cdn.openbsd.org/pub/OpenBSD/snapshots/) [备份](https://web.archive.org/web/20260120213647/https://cdn.openbsd.org/pub/OpenBSD/snapshots/)（即预构建的 current）。这是因为 current 版本的 ABI 变动频繁，从 release 直接升级可能遇到兼容性问题。

经过测试如果直接从 release 升级到 current 可能会遇到问题，例如：
...
- [Anonymous CVS](https://www.openbsd.org/anoncvs.html) [备份](https://web.archive.org/web/20260111205832/http://www.openbsd.org/anoncvs.html) 官方 CVS 匿名访问指南，提供代码仓库获取方法。
- [FAQ - Building the System from Source](https://www.openbsd.org/faq/faq5.html) [备份](https://web.archive.org/web/20260113175054/https://www.openbsd.org/faq/faq5.html) 详细说明从源码编译系统的步骤与配置。
- [release — building an OpenBSD release](https://man.openbsd.org/release) [备份](https://web.archive.org/web/20260120170946/https://man.openbsd.org/release) 官方手册页，介绍发布版本构建流程。

## 课后习题

1. 在 QEMU 中安装 OpenBSD 7.7，使用自定义分区（仅 / 和 swap），记录安装过程并验证系统能否正常启动，分析这种极简分区方案的适用场景与限制。

2. 检出 OpenBSD 7.7 源码，修改 `sbin/disklabel/editor.c` 中的 `alloc_big` 数组，将 / 分区最大限制从 1GB 调整为 10GB，重新编译 disklabel 工具并在测试环境中验证其效果。

3. 从 OpenBSD 7.7 release 版本通过源码升级到 7.7-stable，记录整个升级过程中遇到的问题（如 _shutdown 用户组缺失等），并分析为什么 OpenBSD 团队选择不保持向下兼容的设计决策及其对用户的约束。
