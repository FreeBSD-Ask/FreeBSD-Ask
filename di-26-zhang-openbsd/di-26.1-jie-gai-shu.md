...
OpenBSD 至今仍然和传统开发一样，**仅** 使用邮件列表进行开发。任何 GitHub PR 都会被直接关闭。整个项目仍然使用传统的 CVS 进行协作，CVS 连 HTTP 代理都很难支持。而现代开发者甚至连 SVN 都不掌握，更遑论前前代技术产品 CVS。这种极其保守都不技术路线对年轻开发者造成了严重困扰。
