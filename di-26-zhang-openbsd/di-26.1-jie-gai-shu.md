...
OpenBSD 至今仍然和传统开发一样，**仅** 使用邮件列表进行开发。任何 GitHub PR 都会被直接关闭。整个项目仍然使用传统的 CVS 进行协作，CVS 连 HTTP 代理都很难支持。而现代开发者甚至连 SVN 都不掌握，更遑论前前代技术产品 CVS。这种极其保守的技术路线对年轻开发者造成了严重困扰。

OpenBSD 的源代码可以称得上是“一行注释、一个说明文件也没有”。普通人难以学习。

OpenBSD 的 Bug 报告也缺乏统一的追踪、报告平台，仅通过邮件列表进行反馈，而这很快就会丢失。

发行报告和报菜名一样难以阅读。也没有 wiki 网站对某具体子项目的开发进度进行追踪。
...
在 OpenBSD，你几乎找不到任何在现代操作系统上理所应当的安全模型或者框架：
...
OpenBSD 的安全策略是这样的：我们知道超线程必然存在安全问题，那么我们要做的事情不是去打微码，而是直接将之禁用。其在安全模型和安全理念方面的洞察究竟是高深还是粗浅，这需要更多研究来支撑。
...
[Donating to the Foundation](https://www.openbsdfoundation.org/donations.html)，看起来中国大陆的人很难对其进行捐赠，其只接受国际 PayPal，`此国家或地区不支持捐赠给此收款人`。我方已经发过邮件反馈过此事。笔者甚至写了一封挂号信给远在加拿大的 OpenBSD 基金会，但是很遗憾，还是没有任何回应。哪怕是采用 GitHub Sponsors 也是好的！后多方沟通仍遭拒绝。我们对此深表遗憾和怀疑——其拒绝增加捐款渠道的动机究竟是什么？
