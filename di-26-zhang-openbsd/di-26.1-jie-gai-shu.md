...
以上问题笔者已向相关开发者报告过多个实际存在的错误，但未获得积极回应。在 KDE 5 移植相关的讨论中，部分维护者未能正视技术问题，反而与笔者发生争执。这种处理用户反馈的方式，与 OpenBSD 一贯强调的严谨性和可靠性存在一定差异。

### 项目过于保守

OpenBSD 至今仍沿用传统开发方式，主要通过邮件列表进行协作。项目不接受 GitHub 上的 Pull Request（PR）。整个项目仍然使用 CVS（Concurrent Versions System）进行版本控制，CVS 对 HTTP 代理支持有限。对于许多现代开发者而言，SVN（Subversion）尚不熟悉，更不用说更早期的 CVS 工作流。这种保守的技术路线对新开发者造成了严重困扰。
