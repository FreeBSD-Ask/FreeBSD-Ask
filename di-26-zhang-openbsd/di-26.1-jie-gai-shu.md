...
>
>- 我们面临的最大问题是人们会将 GPL 套在我们的代码上，以同样的方式（商业闭源）将我们排除在外。……我们有很多公司一直在回馈代码。但是一旦代码被 GPL 化，我们就无法再获取它。
>
...
在与几位 OpenBSD 的维护者和社区进行交流过程中，笔者感受到部分维护者在技术讨论中态度较为固执。例如，笔者曾反馈 KDE5 无法正常启动、出现黑屏的情况，但对方始终坚持这是我自身配置的问题。然而，数月之后我才发现，问题实际上出现在虚拟机环境中，尤其是在启用 UEFI 的情况下才会触发该 Bug。
...
此外，OpenBSD 的安装器在网络处理和错误恢复方面也存在严重问题。例如，在安装过程中需要在线下载驱动，但下载失败时并没有合理的错误处理机制；若用户尝试使用 Ctrl-C 中断，系统往往直接崩溃，只能重新开始安装；断网状态下，安装器也会卡死在某一步，无法继续。

以上我已经向相关开发者报告过多个实际存在的 bug，然而并未获得积极回应。甚至在 KDE5 移植相关的讨论中，有维护者不愿正视技术问题，反而与我争执。这种对待用户反馈的方式，与 OpenBSD 一贯强调的严谨性和可靠性存在一定反差。

### 项目过于保守

OpenBSD 至今仍沿用传统开发方式，仅通过邮件列表进行协作。任何 GitHub PR 都会被直接关闭。整个项目仍然使用传统的 CVS 进行协作，CVS 连 HTTP 代理都很难支持。而对许多现代开发者而言，SVN 尚且不熟悉，更遑论更早期的 CVS 工作流。这种极其保守的技术路线对年轻开发者造成了严重困扰。

OpenBSD 的源代码可以称得上是“一行注释、一个说明文件也没有”。普通人难以学习。

OpenBSD 的 Bug 报告也缺乏统一的追踪、报告平台，仅通过邮件列表进行反馈，而这很快就会丢失。

发行公告在结构和可读性方面相对欠佳，像报菜名一样，难以阅读。也没有 Wiki 网站对某具体子项目的开发进度进行追踪。
...
在 OpenBSD 中，一些现代操作系统常见的安全模型或框架尚未得到支持：

- 不支持安全启动和 TPM
- 系统和软件包的构建流程不够清晰透明，且技术栈明显落后时代（就 CVS 而言明显缺乏维护）
- 几乎没有做过任何合规认证，甚至没有 CIS 基准
- 不支持 ACL，不支持 MAC 框架
- 没有运行时完整性
- 没有 MLS/Biba
- 缺乏 RBAC 的概念
- 没有任何前沿或权威的学术论文证实其安全性
- 没有外部审计
- 没有任何安全认证

一项多年前的研究表明，OpenBSD 的代码漏洞在逐年减少，且被发现的漏洞难以攻击。

OpenBSD 的安全策略基于这样一种判断：超线程机制可能带来潜在的安全风险。那么我们要做的事情不是去打微码，而是直接将之禁用。其在安全模型和安全理念方面的洞察究竟是高深还是粗浅，这需要更多研究来支撑。

>**思考题**
>
>>有的观点认为：“如果任何一款操作系统像 OpenBSD 这样采取激进偏执的安全策略，都会比 OpenBSD 要安全得多。但是事实果真如此吗？安卓的安全策略应该是所有操作系统里最全面的，而且是软硬件结合的。但是，现实是什么？”
>
>你怎么看？
...
至此，事件的主要经过已介绍完毕。从结果来看，其中不乏引发争议的内容。

## 捐赠方式

根据 OpenBSD 基金会的页面 [Donating to the Foundation](https://www.openbsdfoundation.org/donations.html)，该组织接受 PayPal 捐赠、银行转账、PayPal 转账、Benevity 和支票。中国内地的 PayPal 账号无法发起捐赠，但可以向境外 PayPal 账户转账。中国内地捐赠者可以注册国区 PayPal 并绑定银联卡，向 PayPal 账号 obsd-paypal@openbsdfoundation.org 转账。
...
FuguIta 是一款基于 OpenBSD 开发的 Live 系统。亦支持部分树莓派。
