...
完成 OpenBSD 系统安装后，本节将介绍初次配置、硬件驱动获取、系统更新与升级等关键配置步骤，确保系统能够稳定运行并满足使用需求。

## 初次进入系统后获取驱动

系统安装完成后，首次进入 OpenBSD 时需要进行一些基本配置。本节将介绍如何获取硬件驱动程序。
...
由于各种原因，新版本在下载驱动时可能会遇到超时错误，可多次尝试下载。运行 `fw_update` 后，如有驱动下载失败，可记录其名称，然后直接访问 [OpenBSD 官方固件网站](http://firmware.openbsd.org/firmware/) [备份](https://web.archive.org/web/20260121064913/https://firmware.openbsd.org/firmware/) 手动获取，并注意匹配版本。解压驱动包，将内部驱动文件复制到 `/etc/firmware/` 目录下，然后重启系统。
...
在 OpenBSD 中，doas 是默认用于获取管理员权限的工具。本节将介绍如何配置 doas。
...
为确保系统安全和稳定，需要定期更新 OpenBSD 系统和软件包。本节将介绍常用的更新和升级命令。
...
在 OpenBSD 中使用可移动磁盘需要手动进行挂载操作。本节将详细介绍相关操作步骤。
...
新近安卓系统的存储设备通常采用 MTP 协议映射，需要相应的 MTP 软件来管理手机文件。MTP（Media Transfer Protocol）是一种用于在设备之间传输媒体文件的协议。与 Linux 不同，OpenBSD 上可用的软件较少，此处使用 `simple-mtpfs` 完成挂载操作。
...
无线网络是现代计算机常用的联网方式，本节将介绍在 OpenBSD 中配置 Wi-Fi 的方法。
...
正确关闭系统是保护数据安全的重要操作。本节将介绍 OpenBSD 中的关机命令。
...
- [OpenBSD FAQ](https://www.openbsd.org/faq/) [备份](https://web.archive.org/web/20260121064805/https://www.openbsd.org/faq/)—[中文版在此](https://openbsd-zh-association.github.io/docs-openbsd-zh/) [备份](https://web.archive.org/web/20260120212942/https://openbsd-zh-association.github.io/docs-openbsd-zh/) 官方系统配置与使用指南，提供完整参考。
- *Absolute OpenBSD, 2nd Edition: Unix for the Practical Paranoid*，ISBN 978-1593274764，No Starch Press 权威 OpenBSD 技术参考书，深入讲解系统原理与实践。
- [Installing OpenBSD 7.3 on your laptop is really hard (not)](https://www.k58.uk/openbsd.html) [备份](https://web.archive.org/web/20260112185607/https://www.k58.uk/openbsd.html) 笔记本电脑安装 OpenBSD 的实践指南。

## 课后习题

1. 在 OpenBSD 7.7 中配置 Wi-Fi 自动连接，使用 `hostname.if` 文件，测试网络连接并分析该配置机制与 NetworkManager 等现代网络管理工具在设计理念上的差异。

2. 修改 `/etc/doas.conf`，实现仅允许特定用户执行特定命令（如仅允许用户 ykla 执行 pkg_add），并验证该权限控制机制的有效性，思考这种极简设计如何限制或扩展系统管理员的权力。
