...
本章节主要介绍 BIOS 中保存与退出相关的设置选项，包括保存更改、放弃更改、恢复默认值等操作。作为 BIOS 配置流程的最终环节，保存与退出操作确保了配置变更的持久化，是系统设置过程中不可或缺的关键步骤。

以下是保存与退出菜单的界面示例，通过该菜单可以完成各项保存与退出操作。

![Save & Exit 保存与退出菜单](../.gitbook/assets/image-20250726132151-iqo9ym1.png)

本表格列出了保存与退出菜单中的各项功能及其英文和中文对照说明。
...
1. 在 FreeBSD 中使用工具读取并备份当前 BIOS 设置，修改几项核心配置后保存并重启，验证修改是否生效，然后使用 Restore Defaults 恢复，对比备份与恢复后的配置差异。

2. 分析保存与退出机制的设计，重构其最小概念模型，探讨为什么需要区分“保存更改并退出”、“保存更改并重启”等多个选项。

3. 配置 Save as User Defaults 保存自定义设置，修改多项配置后使用 Restore User Defaults 恢复，分析这种“用户默认值”机制如何在厂商预设与个人偏好之间建立平衡，以及它如何改变用户与系统配置的关系。
