...
FreeBSD 提供两个相关 Port：`godot-tools` 为引擎编辑器，`godot` 为导出模板。

## 安装 Godot

pkg 提供预编译二进制包，安装速度快；Ports 从源代码编译，可进行自定义配置。
...
还需要注意项目的创建方式。如果遇到上述卡顿问题并使用了 OpenGL 参数，在创建项目时应选择“兼容”，而不是 Forward+ 或“移动”。Forward+ 和“移动”模式使用 RenderingDevice，这是一种更现代的渲染抽象层，其特性与兼容性要求可在创建窗口的说明中查看。只有“兼容”模式使用 OpenGL 3 后端。
