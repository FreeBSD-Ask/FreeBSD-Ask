...
在 FreeBSD 中搭建 FTP 服务器，可在局域网或互联网环境中实现文件的快速传输与共享。
...
通过 pkg 安装的 Pure-FTPd 不包含数据库支持，因此需通过 Ports 安装以启用 MySQL 集成。
...
> **技巧**
>
> 其设计思路为：数据库中的虚拟用户继承 `pw useradd` 所创建用户的权限及 GID/UID，并通过数据库中的用户信息执行 FTP 操作。
...
> **技巧**
>
> ProFTPD 通过 Windows 自带 FTP 客户端连接时不会出现乱码。
...
> **警告**
>
> 如果不手动创建目录（即使系统已自动创建，也建议再次手动创建），系统将提示 `proftpd[3435]: warning: handling possibly truncated configuration data at line 65 of '/usr/local/etc/proftpd.conf'`。
...
vsftpd（Very Secure FTP Daemon，即"非常安全的 FTP 守护进程"）注重安全性设计，在 Linux 系统中应用较广。实测表明，Windows 环境的 FTP 客户端不会出现乱码。
