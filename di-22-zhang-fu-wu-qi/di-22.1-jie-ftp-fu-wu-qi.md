...
标准 FTP 协议的传输过程不安全：密码与数据均以明文发送，默认部署不加密。
...
Pure-FTPd 已移除 RFC 2640 支持，该 RFC 定义了 FTP 协议的国际化框架。因此，在 Windows 系统中使用 FTP 命令行访问非英文字符文件时可能出现乱码。具体变更可参见 Pure-FTPd 1.0.48 版本发布公告。
