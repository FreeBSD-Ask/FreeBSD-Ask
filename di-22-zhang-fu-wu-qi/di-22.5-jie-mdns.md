...
该功能在嵌入式设备上较为实用。在某些场景下，用户无法通过屏幕或串口获取树莓派的 IP 地址，却需以 SSH 方式连接。树莓派 OS 默认启用 Avahi 服务且支持多播 DNS，此时用户可直接使用 `ping raspberrypi.local` 获取 IP 地址，或直接通过 SSH 连接 `raspberrypi.local`。
