...
在 FreeBSD 平台开发 STM32 嵌入式系统，推荐采用 STM32CubeMX 初始化配置。STM32CubeMX 官方仅支持 Windows、Linux、macOS 三大平台，在 FreeBSD 上运行该软件需借助 Linux 兼容层。
...
请将上述路径 `Exec`、`Icon` 改为实际路径。随后授予可执行权限。
