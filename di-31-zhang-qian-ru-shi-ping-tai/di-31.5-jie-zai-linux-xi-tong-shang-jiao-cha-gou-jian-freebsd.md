# 31.5 在 Linux 系统上交叉构建 FreeBSD

在 Linux（以 Ubuntu 24.04 LTS 为例）上交叉构建 FreeBSD 需要至少 12 GB 内存与 16 GB swap。本节给出从环境准备到完整构建的步骤。

## 设备环境

本节以 Ubuntu 24.04 LTS 为例进行说明，其他 Linux 发行版亦可参考。构建过程内存消耗较大，建议配置如下：
...
○ systemd-oomd.service # 注意此处应为黑色圆圈，表示正常状态；如果为绿色则说明服务正在运行
...
○ systemd-oomd.socket # 注意此处应为黑色圆圈，表示正常状态；如果为绿色则说明服务正在运行
...
此参数显式指定 strerror_r 返回类型，避免因 glibc 同时支持 POSIX 和 GNU 两种规范导致的编译错误。否则构建过程将中止于 krb5 阶段。
