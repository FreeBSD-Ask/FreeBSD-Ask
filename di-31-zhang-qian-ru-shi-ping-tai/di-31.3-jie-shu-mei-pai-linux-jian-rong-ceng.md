...
case $answer in
	[Nn][Oo]|[Nn])
		echo "configuration not completed"           # 用户选择不修改
		exit 4
		;;
	[Yy][Ee][Ss]|[Yy]|"")
...
echo ""
echo "the script can also perform some initial configuration"    # 提示是否进行初始化配置
echo "if agree:"
...
case $answer in
	[Nn][Oo]|[Nn])
		echo "set your Arch Linux by yourself."   # 用户选择不进行初始化
		exit 0
		;;
	[Yy][Ee][Ss]|[Yy]|"")
		...
		echo "Enabling DisableSandbox in pacman.conf to avoid landlock error on this kernel"   # 提示将启用 DisableSandbox 以避免 pacman 错误
