...
自树莓派 3B+ 型号起，系统原生支持从 U 盘启动，无需额外硬件或软件修改。根据实际测试结果，FreeBSD 12、13、14 版本均支持该功能。其性能瓶颈主要源于两方面因素：一是树莓派硬件本身受总线速率限制，接口性能存在上限，树莓派 5 支持 SDR104（UHS-I）高速模式，树莓派 4 支持 DDR50（UHS-I），而更早的型号（树莓派 1/2/3）仅支持默认 SD 模式（约 20–22 MB/s），不支持 UHS-I；二是部分 U 盘产品的实际性能与标称值存在较大差距。因此启动速度较为缓慢。
...
该问题主要出现在使用 ZFS 存储卡安装系统到 U 盘时，目前还没有解决方案，只能使用另一块采用 UFS 文件系统的 U 盘启动盘安装 U 盘系统。
