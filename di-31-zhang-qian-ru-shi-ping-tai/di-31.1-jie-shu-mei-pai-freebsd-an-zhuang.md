...
根据文献记载，该系教学主管部门分析后认为，当时青少年群体缺乏价格低廉且架构开放的计算设备用于学习实践，是剑桥大学计算机科学与技术系教学困境的主要原因。正因如此，树莓派项目的初始动机与解决这一教学困境密切相关。
...
自树莓派 3B+ 型号起，系统原生支持从 U 盘启动，无需额外硬件或软件修改。根据实际测试结果，FreeBSD 12、13、14 版本均支持该功能。其性能瓶颈主要源于两方面因素：一是树莓派硬件本身受总线速率限制，接口性能存在上限，树莓派 5 支持 SDR104（UHS-I）高速模式，树莓派 4 支持 DDR50（UHS-I），而更早的型号（树莓派 1/2/3）仅支持默认 SD 模式（约 20–22 MB/s），不支持 UHS-I；二是部分 U 盘产品的实际性能与标称值存在较大差距。因此系统启动速度相对较慢。
...
FreeBSD 对不同硬件体系结构的支持采用等级划分机制。截至目前，AArch64（ARM64）架构属于 [一级架构](https://www.freebsd.org/platforms/)，该架构获得了 FreeBSD 项目的核心支持。但在软件生态的完整性方面，AArch64 架构仍略逊于 AMD64（x86-64）架构，部分软件包目前还无法通过 FreeBSD Ports 以源代码形式成功构建。
...
> **警告**
>
> 本节不适用于 D0 步进版本以及 16 GB 内存版本。具体可参考 <https://rpicn.bsdcn.org>。如果使用较新版本的固件，可能需要降级固件版本才能使用本节方法。
