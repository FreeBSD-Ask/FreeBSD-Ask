...
FreeBSD 对不同硬件体系结构的支持采用等级划分机制。截至目前，ARM 架构属于 [一级架构](https://www.freebsd.org/platforms/)，该架构获得了 FreeBSD 项目的核心支持。但在软件生态的完整性方面，ARM 架构仍略逊于 AMD64（x86-64）架构，部分软件包目前还无法通过 FreeBSD Ports 以源代码形式成功构建。
