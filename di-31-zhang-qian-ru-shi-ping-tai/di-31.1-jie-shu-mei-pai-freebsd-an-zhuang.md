...
剑桥大学计算机科学专业时任教学主管 Eben Upton 发现，当时青少年群体缺乏价格低廉且架构开放的计算设备用于学习实践，是剑桥大学计算机科学与技术系教学困境的主要原因。正因如此，树莓派项目的初始动机与解决这一教学困境密切相关。

~~“为了守护我们最爱的院系——我们所能做的，就是开发一块每个人都买得起的电路板！！”~~

虽然《LoveLive!》中的学园偶像未能达成目标，但树莓派项目最终取得了巨大成功，成为全球销量第三的通用计算机。
...
自树莓派 3B+ 型号起，系统原生支持从 U 盘启动，无需额外硬件或软件修改。经实测，FreeBSD 12、13、14 版本均支持该功能。其性能瓶颈主要源于两方面因素：一是树莓派硬件本身受总线速率限制，接口性能存在上限，树莓派 5 支持 SDR104（UHS-I）高速模式，树莓派 4 支持 DDR50（UHS-I），而更早的型号（树莓派 1/2/3）默认仅运行在 High Speed 模式（约 20–22 MB/s），不支持 UHS-I（树莓派 3 的 SD 卡控制器理论上支持 UHS-I，但存在已知的可靠性问题）；二是部分 U 盘产品的实际性能与标称值存在较大差距。因此系统启动速度相对缓慢。
