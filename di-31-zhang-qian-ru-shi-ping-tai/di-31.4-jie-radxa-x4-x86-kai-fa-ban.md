...
FreeBSD 在 Alder Lake 处理器上存在一个长期未修复的 Bug（Bug 271548）：无法正确处理混合架构（P-core/E-core），导致选择了所有核心中最低的最大频率，主频无法达到标称值。此问题源于 FreeBSD 内核的频率管理缺陷，并非 CPU 硬件故障。该 Bug 最早于 13.2-STABLE 被报告，截至 2026 年 6 月仍未修复。如需完整性能，可在 BIOS 中禁用 E-core 以规避此问题。

FreeBSD 14.3-RELEASE 存在潜在问题：如果使用 eMMC 存储版本且将系统安装在 eMMC 上，则固态硬盘的分区数量不得超过 5 个，否则 ZFS 文件系统将无法正常启动。
...
完成无线网卡驱动安装后，还需进行配置以优化无线连接的稳定性与性能。当前驱动尚不支持 Wi-Fi 5/6 标准。

在 2.4 GHz 频段，传输速率仅能达到 802.11g 标准；在 5 GHz 频段，传输速率仅能达到 802.11a 标准。rtw89 驱动虽然硬件支持 802.11a/b/g/n/ac/ax 模式，但当前兼容层代码仅支持 802.11a/b/g 模式，802.11n/ac/ax 支持尚待实现。
