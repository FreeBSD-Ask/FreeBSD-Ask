# 31.4 Radxa X4 x86 开发板

除树莓派等 ARM 架构单板计算机外，市场上亦存在基于 x86 架构的开发板产品。
...
建议安装 FreeBSD 15.1-RELEASE（2026 年 6 月发布）或更新版本。

FreeBSD 在采用混合架构（P-core/E-core）的 Alder Lake 处理器上存在一个长期未修复的 Bug（Bug 271548）：无法正确处理混合架构（P-core/E-core），导致选择了所有核心中最低的最大频率，主频无法达到标称值。此问题源于 FreeBSD 内核的频率管理缺陷，并非 CPU 硬件故障。该 Bug 最早于 13.2-STABLE 被报告，截至 2026 年 6 月仍未修复。如需完整性能，可在 BIOS 中禁用 E-core 以规避此问题。注意：Radxa X4 搭载的 N100 处理器属于 Alder Lake-N 系列，全部采用 Gracemont 能效核心（E-core），不含性能核心（P-core），因此不受此 Bug 影响。

FreeBSD 14.3-RELEASE 存在潜在问题：若使用 eMMC 存储版本并将系统安装于 eMMC，则固态硬盘的分区数量不得超过 5 个，否则 ZFS 文件系统将无法正常启动。

#### 参考文献

- [Bug 271548 - Alder lake CPU not running at full speed](https://bugs.freebsd.org/bugzilla/show_bug.cgi?id=271548)
...
完成无线网卡驱动安装后，需进一步配置以优化无线连接的稳定性与性能。当前驱动尚不支持 Wi-Fi 5/6 标准。

在 2.4 GHz 频段，传输速率仅可达到 802.11g 标准；在 5 GHz 频段，传输速率仅可达到 802.11a 标准。rtw89 驱动虽然硬件支持 802.11a/b/g/n/ac/ax 模式，但当前兼容层代码仅支持 802.11a/b/g 模式，802.11n/ac/ax 支持尚待实现。
...
为确保每次系统启动后均可正常监控 CPU 温度，需将温度监控模块的加载配置持久化。为便于 htop 等系统监控软件读取 CPU 温度信息，编辑 **/boot/loader.conf** 配置文件，添加以下内容：
...
除了 FreeBSD 系统外，部分用户需在 Windows Server 系统上使用 Radxa X4 开发板。
...
1. 下载并解压 [适用于 Windows Server 2025* 的英特尔 ® 网络适配器驱动程序](https://www.intel.cn/content/www/cn/zh/download/838943/intel-network-adapter-driver-for-windows-server-2025.html)
2. 在设备管理器中定位以太网控制器
3. 在其属性页面选择“更新驱动程序”
4. 点击“浏览我的计算机以查找驱动程序软件”
5. 选择“让我从计算机上的可用驱动程序列表中选取”
6. 在“从以下列表选择设备的类型”中选择“网络适配器”
