...
在这 4 个 Jail 中分别安装 xclock、Firefox、Chrome 和 jwm，其中 jwm 用于界面管理。Alpine 中 VNC 方法未能成功，Firefox 和 Chrome 也未能运行，但 xclock、xterm 等可正常运行。
...
将 `DISPLAY` 设为 `:0.0`、**127.0.0.1:0.0**、**10.0.2.15:0.0** 均可成功，上述几种方法均可尝试。
...
TCP 监听默认关闭的原因之一是宿主机 X Server TCP 监听加 xhost 连接管理的方式安全性较差。
