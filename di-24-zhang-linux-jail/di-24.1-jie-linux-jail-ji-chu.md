...
Linux Jail 是 FreeBSD 操作系统中的一项功能，能在 Jail 中运行 Linux 二进制文件和应用程序。其原理是在 FreeBSD 内核中集成兼容层，将部分 Linux 系统调用和库转换为可执行形式。Linux Jail 的目的是在无需单独部署 Linux 虚拟机或运行环境的情况下，使 FreeBSD 系统能运行 Linux 软件。
...
即使不使用防火墙规则，也需要启用 pf 服务来实现 NAT 功能。请参照其他章节自行启用 pf 防火墙。
