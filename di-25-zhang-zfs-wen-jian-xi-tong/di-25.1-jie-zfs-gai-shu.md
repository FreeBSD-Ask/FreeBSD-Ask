...
作为现代存储技术的重要里程碑，ZFS 融合了文件系统与卷管理器功能，提供了数据完整性保障、可扩展性与高级管理特性的综合解决方案。
...
在 OpenSolaris 关停 3 年后（2013 年），ZFS 的主要开发者迁移到了 ZFSonLinux，后重命名为如今的 OpenZFS。OpenZFS 旨在统一 ZFS 的开源开发（此前还有若干类似移植到 Linux 的项目）。由于 Oracle Solaris ZFS 的闭源开发，OpenZFS 很难再兼容 Oracle Solaris ZFS。
