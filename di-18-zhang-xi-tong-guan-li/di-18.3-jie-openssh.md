...
sshd 首次在 FreeBSD 系统启动时，会自动生成主机密钥并将指纹显示在控制台上。应将这些指纹告知用户，以便用户在首次连接时校验。
...
-rw-------  1 root  wheel    99 Mar 22 18:39 /root/.ssh/authorized_keys # 检查权限是否为 600，如果不是需要手动修改权限
```
...
PasswordAuthentication yes   #（可选）设置是否使用 SSH 密码认证，默认值为 no；注意 FreeBSD 下 PAM 的 challenge-response 机制可能绕过此限制
```
...
随后可通过 SSH 连接远程主机，之后即使关闭终端窗口，会话也不会中断。
...
`tmux`（Terminal Multiplexer，终端多路复用器）来自 OpenBSD，采用 ISC 许可证，是 GNU Screen 的现代替代品。与 screen 相比，tmux 原生支持分屏、更灵活的配置体系以及客户端-服务器架构，在 SSH 远程管理场景中同样能保持会话不中断。
...
tmux 采用客户端-服务器模型，核心概念包括：

- **会话（session）**：一个服务器可包含多个会话，会话可脱离（detach）后在后台运行。
- **窗口（window）**：一个会话可包含多个窗口，类似于标签页。
- **面板（pane）**：一个窗口可划分为多个面板，实现分屏操作。
...
使用 `-s` 可指定 `123` 为会话名称，便于后续查找。

随后可通过 SSH 连接远程主机，即使关闭终端窗口，会话也不会中断。
...
```sh
# tmux attach -t 123	# 重新附着会话 123（名称或编号）
```

如果会话仍处于附着状态，需先脱离再重新附着：

```sh
# tmux detach -t 123	# 将会话 123 从当前终端脱离，保持会话在后台运行
# tmux attach -t 123	# 恢复
```

tmux 的所有快捷键均以 **前缀键** `Ctrl+b` 触发（键入前缀键后松开再键入功能按键），常用操作如下：
...
> **技巧**
>
> tmux 的示例配置文件位于 **/usr/local/share/examples/tmux/example_tmux.conf**，可将其复制为 **/usr/local/etc/tmux.conf**。
...
客户端也需做相同配置。由于 Mosh 是为移动终端设计的，本节使用 Android 设备上的 [JuiceSSH](https://play.google.com/store/apps/details?id=com.sonelli.juicessh) 软件测试。
...
- FreeBSD Project. FreeBSD 13.1-RELEASE Announcement[EB/OL]. [2026-04-17]. <https://www.freebsd.org/releases/13.1R/announce/>. 13.1 起内置 OpenSSH 8.8p1。
...
2. 分别创建一个 screen 会话和一个 tmux 会话运行长时间任务，模拟网络断开后重连恢复会话，对比有会话管理器与无会话管理器两种场景下进程的生命周期差异，并比较 screen 与 tmux 在操作方式上的异同。
