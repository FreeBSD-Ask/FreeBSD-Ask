...
```sh
  方式一：交换分区                方式二：交换文件                方式三：ZFS 卷
  ┌──────────────────┐         ┌──────────────────┐         ┌──────────────────┐
  │  /dev/nda0p3     │         │  /usr/swap0      │         │  zroot/swap      │
  │  (原始分区)       │         │  (普通文件)       │         │  (ZFS zvol)      │
  └────────┬─────────┘         └────────┬─────────┘         └────────┬─────────┘
           │                            │                            │
           │ swapon                     │ mdconfig + swapon          │ swapon
           ▼                            ▼                            ▼
  ┌──────────────────┐         ┌──────────────────┐         ┌──────────────────┐
  │  直接作为交换设备  │         │  /dev/md0        │         │  /dev/zvol/      │
  │                  │         │  (内存磁盘映射)    │         │  zroot/swap      │
  └──────────────────┘         └──────────────────┘         └──────────────────┘
           │                            │                            │
           └────────────────────────────┼────────────────────────────┘
  ...
  适用场景：                      适用场景：                      适用场景：
  · 安装时已规划                  · 安装后动态添加                · ZFS 原生环境
  · 固定大小                      · 灵活调整大小                  · 支持快照/数据校验
  · 不支持在线扩容                · 性能略低于分区                · 极端内存压力下可能锁死
```
