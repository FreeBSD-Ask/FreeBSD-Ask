...
Jail 的发音：[dʒeɪl]（英/美），接近汉语文字“杰诶尔”的发音（或“宅奥”），“炸儿”不是正确发音。
...
使用 `jexec` 命令可在指定 Jail 中执行命令。以下示例在 Jail ID 为 1 的环境中启动 shell：
...
使用 freebsd-update 可对指定 Jail 进行系统更新。需将 `/here/is/the/jail` 替换为实际的 Jail 路径：
