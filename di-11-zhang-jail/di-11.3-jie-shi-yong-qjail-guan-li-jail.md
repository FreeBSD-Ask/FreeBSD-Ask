...
Qjail 提供 `qjail config` 命令以对每个 jail 进行定制化配置，运行该命令前须先停用目标 jail，这是因为 jail 的核心配置参数在其运行状态下无法安全地进行修改。

`qjail config` 命令提供丰富的配置选项，以下列出几个常用参数，完整说明请参考手册页，手册页是获取命令完整技术文档的权威来源。

[qjail -- Utility for deployment of jail environments](https://www.freebsd.org/cgi/man.cgi?query=qjail&manpath=FreeBSD+15.0-RELEASE+and+Ports) [备份](https://web.archive.org/web/20260114074343/https://man.freebsd.org/cgi/man.cgi?query=qjail&manpath=FreeBSD+15.0-RELEASE+and+Ports)。官方手册提供完整命令参数说明，是 Qjail 使用的权威技术文档。
...
启用该 jail 的 System V IPC（进程间通信）机制，System V IPC 是一套经典的 UNIX 进程间通信机制，包括共享内存、信号量和消息队列，在 jail 中部署 PostgreSQL 等依赖共享内存和信号量的应用时，必须启用该选项。
