...
Qjail 是用于部署 Jail 环境的工具，源自 ezjail 3.1。常见的 Jail 管理工具包括 ezjail、Qjail 和 iocage。ezjail 在 2015 年更新至 3.4.2 后未再进行关键更新，其 ports 更新依赖 portsnap，现已废弃。iocage 依赖 ZFS 文件系统，因此使用 UFS 文件系统的用户无法使用。Qjail 在这些方面不存在限制。ezjail 不支持 Jail 的 vnet 功能，而 iocage 和 Qjail 支持该功能。ezjail 和 Qjail 使用 sh 编写，iocage 使用 Python 编写。
...
ifconfig_lo1_alias0="inet 192.168.1.0-9" # 宿主机 IP 为 10.0.2.15，选择该网段是为了与宿主机网段分离，请根据实际情况调整
```
