...
在 pf 防火墙中建立名为 `jails` 的表，并指定 `persist` 标志，使防火墙始终保留该表（即使没有相关规则引用它）。`persist` 标志确保表在防火墙规则重新加载时不会被自动清空。

“表”是 pf 防火墙的一种命名结构，可保存地址和网络的集合，是 pf 实现高效地址匹配和管理的核心机制。`jails` 表中的地址可以通过 NAT（网络地址转换）访问网络。
...
> **警告**
>
> 无论是否使用防火墙，都需要进行此项设置，否则在 Jail 中将无法访问网络。
...
X Server 里的 X Server，这里使用 Xnest/Xephyr，这是嵌套式 X Server 技术，对 X11 应用来说是 X Server，对宿主 X Server 来说是 X Client，这种嵌套结构可在一个 X 窗口中运行另一个完整的 X 会话。
...
- [https://wiki.freebsd.org/LinuxApps](https://wiki.freebsd.org/LinuxApps) [备份](https://web.archive.org/web/20260114170114/https://wiki.freebsd.org/LinuxApps)。列出可在 FreeBSD 上运行的 Linux 应用及方法，为兼容性实践提供参考。
- [12.2. 配置 Linux 二进制兼容层](https://handbook.bsdcn.org/di-12-zhang-linux-er-jin-zhi-jian-rong-ceng/12.2.-pei-zhi-linux-er-jin-zhi-jian-rong-ceng.html) [备份](https://web.archive.org/web/20260115022832/https://handbook.bsdcn.org/di-12-zhang-linux-er-jin-zhi-jian-rong-ceng/12.2.-pei-zhi-linux-er-jin-zhi-jian-rong-ceng.html)。FreeBSD 中文手册中关于 Linuxulator 配置的详细指南。
- [linux -- Linux ABI support](https://www.freebsd.org/cgi/man.cgi?linux) [备份](https://web.archive.org/web/20260115022830/https://man.freebsd.org/cgi/man.cgi?linux)。官方手册详细说明 Linux 兼容层的系统调用转换机制。
- [wiki/Linuxulator](https://wiki.freebsd.org/Linuxulator) [备份](https://web.archive.org/web/20260114165714/https://wiki.freebsd.org/Linuxulator)。提供 Linuxulator 技术原理与实现细节的权威文档。
- [wiki/LinuxJails](https://wiki.freebsd.org/LinuxJails) [备份](https://web.archive.org/web/20260114074439/https://wiki.freebsd.org/LinuxJails)。介绍如何在 FreeBSD Jail 中部署 Linux 用户空间环境。
- [12.3. Linux 用户空间](https://handbook.bsdcn.org/di-12-zhang-linux-er-jin-zhi-jian-rong-ceng/12.3.-linux-yong-hu-kong-jian.html) [备份](https://web.archive.org/web/20260115134806/https://handbook.bsdcn.org/di-12-zhang-linux-er-jin-zhi-jian-rong-ceng/12.3.-linux-yong-hu-kong-jian)。中文手册中关于构建完整 Linux 用户空间环境的教程。
