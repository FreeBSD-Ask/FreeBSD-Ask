...
- [https://wiki.freebsd.org/LinuxApps](https://wiki.freebsd.org/LinuxApps) 。列出可在 FreeBSD 上运行的 Linux 应用及方法，为兼容性实践提供参考。
- [12.2. 配置 Linux 二进制兼容层](https://handbook.bsdcn.org/di-12-zhang-linux-er-jin-zhi-jian-rong-ceng/12.2.-pei-zhi-linux-er-jin-zhi-jian-rong-ceng.html) 。FreeBSD 中文手册中关于 Linuxulator 配置的详细指南。
- [linux -- Linux ABI support](https://www.freebsd.org/cgi/man.cgi?linux) 。官方手册详细说明 Linux 兼容层的系统调用转换机制。
- [wiki/Linuxulator](https://wiki.freebsd.org/Linuxulator) 。提供 Linuxulator 技术原理与实现细节的权威文档。
- [wiki/LinuxJails](https://wiki.freebsd.org/LinuxJails) 。介绍如何在 FreeBSD Jail 中部署 Linux 用户空间环境。
- [12.3. Linux 用户空间](https://handbook.bsdcn.org/di-12-zhang-linux-er-jin-zhi-jian-rong-ceng/12.3.-linux-yong-hu-kong-jian.html) 。中文手册中关于构建完整 Linux 用户空间环境的教程。
