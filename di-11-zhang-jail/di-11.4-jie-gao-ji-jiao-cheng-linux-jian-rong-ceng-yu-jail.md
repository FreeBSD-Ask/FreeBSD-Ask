...
本章详细探讨了如何在 FreeBSD Jail 中部署完整的 Linux 用户空间环境。
...
另外可以在 `/etc/rc.conf` 文件中用 `jail_list` 变量指定在开机时启动的 Jail 的名称，编辑 `/etc/rc.conf` 文件写入：
...
如果已经在文件 `/etc/rc.conf` 中设置过 `jail_enable=YES`，也可以用：
...
在 `/etc/jail.conf` 文件中写入（这里只展示 antiX 相关部分）：
...
然后在 Jail 的 `fstab` 文件中加入下面内容，这里以 ubuntu Jail 的 `fstab` 为例，其他 Jail 参照修改即可：
...
在 FreeBSD 内新建 `/usr/local/etc/sddm.conf` 文件（如果没有的话），修改 `ServerArguments` 内容如下：
