...
如果在 FreeBSD 之后安装了其他操作系统，它们将覆盖现有的 MBR。如果发生这种情况，或需要以 FreeBSD MBR 替换现有 MBR，请使用以下命令：
...
loader 提供一种交互式配置方式，使用内置命令集，并辅以更强大的解释程序，该程序具备更复杂的命令集。
...
内核被 loader 或 boot2 加载后（boot2 可绕过 loader 直接加载内核），内核会检查启动标志并根据需要调整其行为。常用的启动标志如下：
