...
> **警告**
>
> 写入 MBR 将修改磁盘的主引导记录，操作不当可能导致系统无法启动。请确保 `设备` 参数指向正确的启动磁盘。

```sh
# fdisk -B -b /boot/boot0 设备
```

`设备` 是启动磁盘，例如 `ada0` 表示第一块 SATA 磁盘，`ada2` 表示第三块 SATA 磁盘，`nvd0` 表示第一块 NVMe 磁盘。要创建 MBR 的自定义配置，请参阅 boot0cfg(8)。
...
阶段 2 的 boot2 稍微复杂一些，它能够识别 FreeBSD 文件系统，找到其中的文件。它可以提供一个简单的界面来选择要运行的内核或加载器。它运行 loader，后者更加复杂，能够读取引导配置文件。如果在阶段 2 中断启动过程，将显示以下交互式屏幕：
...
> **警告**
>
> 如果只使用磁盘名称（如 `ada0`），bsdlabel 将以“危险专用模式”创建磁盘，不使用分区，这可能导致数据丢失。请务必使用正确的 `diskslice` 格式。

```sh
# bsdlabel -B diskslice
```

如果只使用磁盘名称（如 `ada0`），bsdlabel 将以“危险专用模式”创建磁盘，不使用分区。这可能并非预期行为，因此按回车键前仔细检查 diskslice。
...
最后，默认情况下，loader 等待用户按键 10 秒，若无中断则启动内核。若用户中断此过程，将看到一个提示符，该提示符支持命令集操作，用户可以在其中调整变量、卸载所有模块、加载模块，然后最终启动或重启。
...
内核加载 loader 或 boot2（绕过 loader）后，内核会检查启动标志并根据需要调整其行为。常用的启动标志如下：
...
内核完成启动后，内核将控制权传递给用户进程 init(8)，该进程位于 **/sbin/init**，或 loader 中 `init_path` 变量指定的程序路径。这是启动过程的最后阶段。

启动序列确保系统中可用的文件系统是一致的。如果 UFS 文件系统不一致，并且 fsck 无法修复不一致之处，init 会将系统降级到单用户模式，以便系统管理员直接解决问题。如果文件系统正常，系统将启动进入多用户模式。

## 单用户模式

用户使用 `-s` 启动或在 loader 中设置 `boot_single` 变量，即可指定单用户模式。也可以在多用户模式下运行 `shutdown now` 进入单用户模式。单用户模式以此内容开始：
...
虽然单用户模式对于修复系统很有用，但除非系统位于物理安全的位置，否则它会带来安全风险。默认情况下，任何能够获得系统物理访问权限的用户启动进入单用户模式后都将拥有系统的完全控制权。

## 多用户模式

如果 init 发现文件系统正常，或者待用户在单用户模式下完成操作并输入 `exit` 离开单用户模式，系统进入多用户模式，启动系统的资源配置。
