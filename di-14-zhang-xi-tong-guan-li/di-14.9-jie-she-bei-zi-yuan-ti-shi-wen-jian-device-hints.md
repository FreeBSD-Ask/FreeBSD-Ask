...
设备资源提示文件（**device.hints**）是 FreeBSD 引导过程中的配置文件。系统启动时，boot loader(8) 读取该文件，其内容被传递给内核，用于控制内核的引导行为，但也可包含任何内核可调参数值。
...
可以在第 3 阶段引导加载器提示符处指定设备提示，将覆盖写在 **/boot/device.hints** 中的变量。引导加载器中输入的设备提示仅在当次启动会话中生效，重启后即失效。
