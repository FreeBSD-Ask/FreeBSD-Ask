...
可按 **设施 facility** 和 **级别 level** 决定日志消息的记录位置，也可按发送消息的应用程序筛选日志消息。在远程日志记录场景下，还可按生成日志事件的机器主机名进行过滤。

该配置文件每行定义一个操作，由选择器字段和操作字段组成。
- 选择器字段的语法是 **设施.等级**，匹配来自 **设施 facility** 且级别为 **级别 level** 或更高的日志消息。
...
以下是 FreeBSD 默认的 **/etc/syslog.conf** 文件示例，其源代码是 **usr.sbin/syslogd/syslog.conf 文件**：
...
- ④ 示例展示了指定程序的用法：后续规则仅对指定程序生效。本例中，仅将 devd(8) 生成的消息记录到 **/var/log/devd.log** 文件。

## 日志记录设施

设施（facility）标识生成消息的子系统，用于将不同来源的消息分开，方便查阅日志。
...
| daemon | 系统守护进程，例如 routed(8)，这些进程没有专门的设施提供日志记录。 |
...
- **Syslog/RFC 5424 格式**（`-O syslog` 或 `-O rfc5424`）：较新的标准格式，使用 [RFC 3339](https://datatracker.ietf.org/doc/html/rfc3339) 时间戳（含微秒精度），并在每条消息中包含结构化的设施与严重级别字段。
...
可以通过运行以下命令在 syslogd(8) 中启用详细日志记录，这样每条消息上将添加设施和级别：
...
日志文件增长迅速，占用磁盘空间，且增加查找有效信息的难度。为了解决这一问题，FreeBSD 使用 newsyslog(8) 来管理日志文件。

newsyslog 定期轮替并压缩日志文件，可选择性地创建缺失的日志文件，并在日志文件移动时向程序发送信号。newsyslog 源代码位于 **usr.sbin/newsyslog/newsyslog.conf**。
...
- `mode` - 指定日志文件和归档文件的文件模式。有效的模式位是 0666（可以为归档日志指定所有者、组和其他用户的读写权限。）
- `count` - 指定可存在的最大归档文件数。
- `size` - 当日志文件的大小达到指定的大小（以 KB 为单位）时，将轮替日志文件。如果该字段包含星号（`*`），则不会根据大小轮替。
- `when` - 包含时间间隔、特定时间或两者。
...
虽然 newsyslog(8) 默认由 cron(8) 每小时调用一次，但也可以使用选项 `-F` 选项手动触发轮替：

```sh
# newsyslog -F
```

强制轮替所有满足条件的日志文件。示例轮替文件如下：
...
系统数量增多后，监控多个主机的日志文件会带来额外管理负担。配置集中式日志记录可以减轻这一负担。

在 FreeBSD 中，可以使用 syslogd 和 newsyslog 配置集中式日志文件聚合、合并和轮替。

本节展示的示例配置，其中主机 `A`，命名为 `logserv.example.com`，将收集本地网络的日志信息。

```sh
# hostname logserv.example.com
```

**/etc/hosts** 文件修改示例：

```ini
::1                     localhost logserv.example.com localhost.my.domain
127.0.0.1               localhost logserv.example.com localhost.my.domain
```

主机 `B`，命名为 `logclient.example.com`，将配置传递日志信息的日志客户端。

```sh
# hostname logclient.example.com
```

**/etc/hosts** 文件修改示例：

```ini
::1                     localhost logclient.example.com localhost.my.domain
127.0.0.1               localhost logclient.example.com localhost.my.domain
```
...
配置好日志服务器后，在日志客户端上执行以下命令，启用系统启动时启动 syslogd：

```sh
# service syslogd enable
```

防止此客户端接收来自其他主机的日志（`-s`），并增加日志消息的详细程度：

```sh
# sysrc syslogd_flags="-s -v -v"
```

接下来，在客户端的 **/etc/syslog.conf** 文件中定义日志服务器。在此示例中，所有记录的设施都通过 `@` 符号发送到指定的主机：
...
- 日志服务器和所有客户端机器必须在本地 DNS 中具有正向和反向条目。如果网络没有 DNS 服务器，请在每个系统的 **/etc/hosts** 中创建条目。正确的名称解析是必要的，以确保日志条目不会被日志服务器拒绝。

在日志服务器上，编辑 **/etc/syslog.conf** 文件，指定接收日志条目的客户端名称、要使用的日志设施和存储主机日志条目的日志文件名称：
...
以上示例在日志服务器的配置中，添加了日志客户端的主机名作为过滤条件。它告诉服务器：凡是来自主机 **logclient.example.com** 的、所有设施（Facilities）和级别的日志条目，都统一存储在服务器本地的 **/var/log/logclient.log** 文件中。

为每个客户端添加同样类似的两行条目便可添加多个日志客户端。

接下来，执行以下命令。在系统启动时启动 syslogd：
...
如果返回 PID，则表示服务器已成功重新启动，可以开始客户端配置。
...
```sh
May 17 19:46:46 <user.notice> ykla ykla[1958]: Test Message by logclient.
```

### 调试日志服务器

如果日志服务器没有接收到任何消息，通常是网络连接问题、主机名解析问题或配置文件中的拼写错误。为排除故障，请确保日志服务器和日志客户端能够使用 **/etc/rc.conf** 文件中指定的主机名互相 `ping`。如果失败，请检查网络布线、防火墙规则集，以及日志服务器和客户端中的 DNS 服务器或 **/etc/hosts** 中的主机名条目。重复检查，直到 `ping` 成功为止。

如果两个主机的 `ping` 都成功，但日志消息仍未接收，可临时提高日志详细度以缩小配置问题的范围。在以下示例中，日志服务器上的 **/var/log/logclient.log** 为空，而日志客户端上的 **/var/log/messages** 未显示失败原因。
...
本例中，日志消息因拼写错误导致主机名不匹配而被拒绝。客户端的主机名应为 `logclient`，而非 `logclien`。修正拼写错误后，执行重启并验证结果：
...
同所有网络服务一样，在实施日志服务器之前应考虑安全需求。日志文件可能包含有关本地主机上启用的服务、用户账户和配置数据的敏感信息。

**网络安全**：

从客户端发送到服务器的网络数据不会加密或进行密码保护。如果需要加密传输，可考虑以下方案：
...
- 强烈建议使用 `-a` 选项明确限制允许发送日志的主机，而非接受所有来源。
- 如果不需要接收远程日志，应使用 `-s` 参数启用安全模式。指定两次 `-ss` 将完全不打开网络套接字。
- 可在防火墙上限制 UDP 514 端口的访问来源。

**本地安全**：

日志文件在使用过程中或日志轮替后并未加密。本地用户可能访问日志文件以获取系统配置的额外信息。因此，对日志文件设置适当的权限十分必要。newsyslog 支持对新创建的和已轮替的日志文件设置权限。将日志文件设置为模式 `600` 可防止本地用户的未经授权访问。对于 `auth.log` 等包含认证信息的日志，默认即应使用 `600` 权限。
...
4. 手动触发 `newsyslog` 轮转 `/var/log/auth.log` 文件，观察轮转结果。
