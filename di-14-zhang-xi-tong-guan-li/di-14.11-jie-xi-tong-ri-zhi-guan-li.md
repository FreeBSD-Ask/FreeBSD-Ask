...
- **BSD/RFC 3164 格式**（默认，`-O bsd` 或 `-O rfc3164`）：传统 BSD syslog 格式。
...
若要切换到 RFC 5424 格式，可以在 `/etc/rc.conf` 中配置：
...
虽然 newsyslog(8) 默认由 cron(8) 每小时调用一次，但也可以手动触发轮换：

```sh
newsyslog -F
```

`-F` 选项强制轮换所有满足条件的日志文件。使用 `-v` 可启用详细输出，查看正在处理哪些日志。若要轮换特定文件，可指定文件路径：
...
从客户端发送到服务器的网络数据不会被加密或进行密码保护。如果需要加密传输，可考虑以下方案：
...
syslogd(8) man 手册明确指出，通过 UDP 接收日志消息等效于"未经认证的远程磁盘填充服务"。因此：
...
日志文件在使用过程中或日志轮换后并未加密。本地用户可能访问日志文件以获取系统配置的额外信息。因此，对日志文件设置适当的权限十分必要。newsyslog 支持对新创建的和已轮换的日志文件设置权限。将日志文件设置为模式 `600` 可防止本地用户的未经授权访问。对于 `auth.log` 等包含认证信息的日志，默认即应使用 `600` 权限。
