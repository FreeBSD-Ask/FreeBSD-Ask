...
- ④ 示例展示了程序指定用法：后续规则仅对指定程序生效。本例中，仅 [devd(8)](https://man.freebsd.org/cgi/man.cgi?query=devd\&sektion=8\&format=html) 生成的消息记录到 **/var/log/devd.log**。
...
本地安全也是一个问题。日志文件在使用过程中或日志轮换后并未加密。本地用户可能访问日志文件以获取有关系统配置的额外信息。因此，对日志文件设置适当的权限至关重要。内置的日志轮换工具 newsyslog 支持对新创建的和已轮换的日志文件设置权限。将日志文件设置为模式 `600` 应该可以防止本地用户的未经授权访问。
