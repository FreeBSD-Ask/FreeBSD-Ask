...
本节展示的示例配置，主机 `A`（`192.168.5.18`）作为日志客户端，命名为 `logclient.example.com`.
...
在日志客户端 A 上执行以下命令，启用系统启动时启动 syslogd：

```sh
# service syslogd enable
```

防止此客户端接收来自其他主机的日志（`-s`），同时增加日志消息的详细程度：
...
日志文件在使用过程中或日志轮替后并未加密。本地用户可能访问日志文件以获取系统配置的额外信息。因此，对日志文件设置适当的权限十分必要。newsyslog 支持对新创建的和已轮替的日志文件设置权限。将日志文件（如 `auth.log`）设置为模式 `600` 可防止本地用户的未经授权访问。
