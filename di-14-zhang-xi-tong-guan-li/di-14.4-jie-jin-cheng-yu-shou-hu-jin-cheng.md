...
有些程序并非设计为持续接收用户输入，并在启动后即与终端断开连接。例如，Web 服务器响应 Web 请求而非用户输入，邮件服务器是此类应用程序的另一个例子。这类程序被称为守护进程（daemon）。术语“daemon”来自希腊神话，代表一种非善非恶的实体，它无形中执行有用的任务。正因如此，BSD 的吉祥物设计为身着运动鞋、手持草叉的快乐守护进程形象。在 Windows 系统中，这类程序被称为“服务”，可在任务管理器中查看。
...
要查看系统上运行的进程，可以使用 ps(1) 或 top(1)。FreeBSD 的 `ps` 基于 4.4BSD，BSD 风格关键字 **允许不带** `-`，如 `ps auxww`。
...
两个信号可用于停止进程：SIGTERM 和 SIGKILL。因为进程可以读取信号、关闭可能打开的任何日志文件，并尝试在关闭前完成正在执行的操作，SIGTERM 是终止进程的温和方式。在某些情况下，如果进程正在执行某些无法中断的任务，它可能会忽略 SIGTERM。
