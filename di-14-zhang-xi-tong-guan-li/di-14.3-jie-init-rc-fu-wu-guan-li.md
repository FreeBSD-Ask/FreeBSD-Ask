...
> **注意**
>
> 不应编辑 **/etc/defaults/rc.conf** 文件中包含的默认设置。所有系统特定的更改应写入 **/etc/rc.conf**。**/etc/rc.conf** 文件的优先级高于 **/etc/defaults/rc.conf** 文件。也就是说，**/etc/rc.conf** 文件将覆盖 **/etc/defaults/rc.conf** 文件中的同名配置项。
...
`reboot` 与 `halt` 在 FreeBSD 中属于同一程序（参见 reboot(8)），根据调用名称行为不同；`poweroff` 则等价于 `shutdown -p now`（参见 shutdown(8)）。而在 Linux 中，这些命令可能是 systemd 的符号链接，行为也不同。
...
- FreeBSD 下 `reboot` 直接刷新文件系统缓存并向所有进程发送 SIGTERM/SIGKILL 后重启，不会执行 **/etc/rc.shutdown**；推荐的关机/重启方式是 `shutdown -r now`，它会通过 init(8) 运行 rc.shutdown 脚本以有序关闭服务
