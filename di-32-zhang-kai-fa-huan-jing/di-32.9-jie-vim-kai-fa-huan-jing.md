...
代码格式化可统一代码风格，提升代码可读性。vim-clang-format 插件不兼容新版 clang-format 的命令行接口变更，且多年未更新，对新版 clang-format 支持存在问题（clang-format15 可正常使用，而 clang-format17 和 clang-format19 可能存在异常），因此推荐使用 vim-codefmt。
