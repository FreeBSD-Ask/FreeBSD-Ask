...
code-server 是 VS Code 的开源服务器版，可通过 Arch Linux 兼容层运行（因为上游 FreeBSD 版 code-server 存在兼容问题）。
...
编辑 **/compat/linux/etc/pacman.conf** 配置软件源（需添加 archlinuxcn 仓库以安装 code-server），然后执行：
