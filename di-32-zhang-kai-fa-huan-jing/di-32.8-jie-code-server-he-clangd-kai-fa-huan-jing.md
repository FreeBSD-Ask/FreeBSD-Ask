...
借助 FreeBSD 内核的路径劫持特性，虽然运行 code-server 使用了 Linux 兼容层，但 clangd 以及其他所有开发工具仍然完全由 FreeBSD 提供。关于 Linux 兼容层的路径重定向机制（即 Linux 程序访问文件时优先查找 `/compat/linux/` 路径、未命中则回退至宿主系统原生路径的机制），参考本书其他相关章节。
