...
code-server 是 VS Code 的开源服务器版，可通过 Arch Linux 兼容层运行（因上游 FreeBSD 版 code-server 存在兼容问题）。

## 概述

code-server 是 Visual Studio Code 的开源服务器版本，可在远程服务器上运行并通过浏览器访问。

由于上游 FreeBSD 版 code-server 存在问题，通过 Linux 兼容层运行 code-server 仍为当前可行的方案。本节使用 Arch Linux bootstrap 镜像构建 Linux 兼容层环境。
