...
在 FreeBSD 中，不同的 Python 版本分别封装。换言之，Python 3.11 和 Python 3.13 属于不同的包，例如 Port `lang/python311` 对应 Python 3.11，`lang/python313` 对应 Python 3.13，`lang/python314` 对应 Python 3.14。

在 FreeBSD Ports 中，不同版本 Python 的 pkg 二进制包支持程度随版本演进而有所不同。

例如，当前 `lang/python`（实际上仅为元包，即指向其他软件包的依赖包）指向 Python 3.14（2026 年 5 月起），则通过 pkg 安装的其他 Python 包（如 Python-XX）在默认情况下也会以此版本为基础构建；其他依赖 Python 的软件同样依赖该版本。
