...
本章介绍 ZFS 快照（ZFS snapshot）技术的原理与实践。ZFS 快照采用写时复制（Copy-on-Write, CoW）机制，能够在不中断系统运行的情况下捕获文件系统的时间点状态，是数据备份、版本控制与灾难恢复的核心技术手段。与虚拟机快照相比，ZFS 快照在文件系统层面实现，具有更轻量、更高效的特性。本节介绍 ZFS 快照的创建、还原和销毁操作。
