...
本节介绍 ZFS 快照（ZFS snapshot）技术的原理与实践。ZFS 快照采用写时复制（Copy-on-Write, CoW）机制，能够在不中断系统运行的情况下捕获文件系统的时间点状态，是数据备份、版本控制与灾难恢复的核心技术手段。与虚拟机快照相比，ZFS 快照在文件系统层面实现，具有更轻量、更高效的特性。本节介绍 ZFS 快照的创建、还原和销毁操作。

- 目前 FreeBSD 中文社区（CFC）提供了 ZFS 脚本，可用于查看、创建、删除和恢复 ZFS 快照（ZFS snapshot）。[ZFS 脚本项目地址](https://github.com/FreeBSD-Ask/zfs-snap) [备份](https://web.archive.org/web/20260114055056/https://github.com/FreeBSD-Ask/zfs-snap)
...
>**思考题**
>
>如果读者有更优方案，可尝试实现；已知网络上存在一些相关脚本，读者可考虑将其作为功能请求或提交 Pull Request（PR）至 OpenZFS 项目。或者使用中文社区的 [ZFS 脚本项目](https://github.com/FreeBSD-Ask/zfs-snap) [备份](https://web.archive.org/web/20260114055056/https://github.com/FreeBSD-Ask/zfs-snap)。

与虚拟机快照不同，在默认状态下，`zfs rollback` 命令无法回滚到除最新快照之外的快照（[参考手册](https://docs.oracle.com/cd/E19253-01/819-7065/gbcxk/index.html) [备份](https://web.archive.org/web/20251207043848/https://docs.oracle.com/cd/E19253-01/819-7065/gbcxk/index.html)，Oracle 官方 ZFS 回滚命令文档）；使用 `-r` 参数可以递归回滚，但会删除该快照创建后的所有快照。
...
3. 修改 ZFS 快照策略，禁用自动快照功能并手动创建一个快照，执行 `rm -rf /*` 后恢复，验证其恢复能力。
