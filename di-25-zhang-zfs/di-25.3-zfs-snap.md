...
> **思考题**
>
> 探索更优的快照回滚方案；已知网络上存在一些相关脚本，可考虑将其作为功能请求或提交 Pull Request（PR）至 OpenZFS 项目。或者使用中文社区的 [ZFS 脚本项目](https://github.com/FreeBSD-Ask/zfs-snap)。

与虚拟机快照不同，在默认状态下，`zfs rollback` 命令无法回滚到除最新快照之外的快照（[参考手册](https://docs.oracle.com/cd/E19253-01/819-7065/gbcxk/index.html)，Oracle 官方 ZFS 回滚命令文档）；使用 `-r` 参数可以递归回滚，但会删除该快照创建后的所有快照。
