...
ZFS 委托管理（ZFS delegation）是一种细粒度的权限控制机制，能让系统管理员将特定的 ZFS 管理权限授予非特权用户，而无需提供完整的 root 访问权限。该机制基于最小权限原则（Principle of Least Privilege）设计，为多用户环境下的存储管理提供了灵活且安全的解决方案。本节介绍 ZFS 委托管理的基本用法和用户级操作。
