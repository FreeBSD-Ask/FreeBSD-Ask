...
长期在后台运行的服务通常命名为 `xxxd`，例如 `sshd`、`ntpd`，其中的 `d` 表示守护进程（daemon），这是 UNIX 系统的通用命名约定“[daemon](https://www.freebsd.org/copyright/daemon/) ”。在 Windows 系统中，这类程序被称为“服务”，可在任务管理器中查看。
