...
> **技巧**
>
> 视频教程见 FreeBSD 中文社区. 004-FreBSD14.2允许root登录ssh[EB/OL]. (2024-12-04)[2026-04-04]. <https://www.bilibili.com/video/BV1gji2YLE2o>.

编辑文件 `/etc/ssh/sshd_config` 文件，去掉相应行首的 `#` 并根据需要将参数设置为 `yes` 或 `no`：
...
> **技巧**
>
> 如果找不到 `#PasswordAuthentication no` 这一行，请确认修改的是 `/etc/ssh/sshd_config` 文件，而不是 `/etc/ssh/ssh_config` 文件。只有 `sshd_config` 是 SSH 服务的配置文件。
...
如果提示找不到 `sshd`，请执行以下命令启用 sshd 服务：
...
可以看到，断开后会有提示，重连网络后会自动恢复，像是没有断开一样（已结合 `screen` 使用）。

## 参考文献

- mosh. mosh FAQ[EB/OL]. [2026-03-26]. <https://mosh.org/#faq>.
- silbertmonaphia. ssh && mosh[EB/OL]. [2026-03-26]. <https://silbertmonaphia.github.io/ssh%E7%99%BB%E5%BD%95%E3%81%AE%E5%91%A8%E8%BE%BA-&&-Mosh.html>.
