...
`login.conf` 在 FreeBSD 源代码中的位置为 [usr.bin/login/login.conf](https://github.com/freebsd/freebsd-src/blob/main/usr.bin/login/login.conf)，该文件即为默认配置，默认设置实际上禁用了资源配额，以便用户开箱即用并便于进一步配置。
