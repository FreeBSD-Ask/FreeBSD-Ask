...
安装完成后新建项目并进入编辑器，可能会发现界面响应延迟且 CPU 占用率较高。这是因为在 FreeBSD 默认配置下，Godot 的渲染由 CPU 完成软件光栅化处理，并且仅使用单个线程，性能开销较大。
...
1. 查找 Godot 引擎的官方源代码，在 FreeBSD 上构建其开发版，并分析其构建系统如何处理不同渲染后端（如 OpenGL 3、Vulkan）的配置与选择。
2. 使用 Godot 引擎运行一款游戏。
