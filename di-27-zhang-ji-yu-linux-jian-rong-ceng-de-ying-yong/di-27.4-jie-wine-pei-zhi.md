...
Wine 通过实现 Windows API 接口使 FreeBSD 可运行 Windows 程序。本节给出 pkg 安装及基础配置方法，并说明字体与图形硬件加速（DRM 驱动）的依赖关系，仅在物理机测试通过。由于 Wine 对图形硬件加速的依赖（需要 DRM 驱动支持），虚拟机环境下可能无法正常运行（通常会提示不支持 3D 加速）；虚拟机可能对某些特定 CPU 指令集的支持不完善，这也会导致程序启动失败。
