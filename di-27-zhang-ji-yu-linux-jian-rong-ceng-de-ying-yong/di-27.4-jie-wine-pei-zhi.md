...
Wine（全称“Wine Is Not an Emulator”）是一款兼容层，可在类 UNIX 操作系统（如 Linux、macOS 和 FreeBSD）上运行 Windows 应用程序。本节给出 pkg 安装及基础配置方法，并说明字体与图形硬件加速（DRM 驱动）的依赖关系。以下内容仅在物理机测试通过：由于 Wine 依赖 DRM 驱动提供图形硬件加速，虚拟机环境下可能无法正常运行（通常会提示不支持 3D 加速）；虚拟机对某些特定 CPU 指令集的支持也可能不完善，同样会导致程序启动失败。
