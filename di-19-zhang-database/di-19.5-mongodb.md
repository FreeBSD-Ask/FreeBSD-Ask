...
MongoDB 是一款流行的 NoSQL 文档数据库（Document-oriented Database），采用 BSON（Binary JSON）格式存储数据，适合处理半结构化数据和大规模分布式存储场景。与传统关系型数据库相比，其具有灵活的 schema 设计和水平扩展能力。本节将介绍如何在 FreeBSD 上安装 MongoDB 8.0。使用 pkg 安装：
