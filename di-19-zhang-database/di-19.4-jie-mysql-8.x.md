...
MySQL 8.x 是 MySQL 数据库的主流版本系列，作为 Oracle 公司主导开发的开源关系型数据库管理系统（Relational Database Management System, RDBMS），在产业界应用广泛。该版本在架构层面提供了多项性能优化与安全增强，包括窗口函数（Window Functions）支持、通用表表达式（Common Table Expressions, CTE）、改进的 JSON 数据处理能力以及默认采用 caching_sha2_password 认证机制等技术特性。本节将系统性介绍如何在 FreeBSD 操作系统环境下部署 MySQL 8.x。

### MySQL 8.0 LTS

MySQL 8.0 是长期支持版本，具有稳定的性能和广泛的兼容性。使用 pkg 安装：
...
为了数据库安全，首次登录后建议立即修改 root 用户密码。以下命令将数据库的 root 用户密码设置为 `z`，然后刷新权限，最后退出 MySQL。
