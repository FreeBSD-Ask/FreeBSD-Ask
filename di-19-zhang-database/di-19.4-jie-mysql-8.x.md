...
MySQL 8.x 是 MySQL 数据库的最新版本系列，作为 Oracle 公司主导开发的开源关系型数据库管理系统，在产业界应用广泛。该版本提供了多项性能和安全改进，包括窗口函数支持、通用表表达式（CTE）、改进的 JSON 处理以及默认使用 caching_sha2_password 认证机制等。本节将介绍如何在 FreeBSD 系统上安装 MySQL 8.x。
