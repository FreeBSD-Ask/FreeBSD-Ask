...
8.x 版本在架构层面提供了多项性能优化与安全增强，包括窗口函数（Window Functions）支持、通用表表达式（Common Table Expressions, CTE）、改进的 JSON 数据处理能力以及默认采用 caching_sha2_password 认证机制等技术特性。

MySQL 8.0 是长期支持版本，具有稳定的性能和广泛的兼容性。

本节将系统性介绍如何在 FreeBSD 操作系统环境下部署 MySQL 8.x。
