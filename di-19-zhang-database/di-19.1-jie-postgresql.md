...
PostgreSQL 是一款自由的对象关系型数据库（Object-Relational Database，ORDB），在现代数据管理系统中具有重要学术价值与产业应用价值。其起源于加州大学伯克利分校的 Ingres 项目，最早发布于 1989 年 6 月（最初名为 POSTGRES）。作为 ACID 兼容的数据库系统，PostgreSQL 在事务处理、复杂查询和数据完整性保障方面表现出色。

在 FreeBSD 上，PostgreSQL 有多个版本可选。在吞吐量、源代码可读性和响应时间等方面，PostgreSQL 相较于 MySQL 有一定优势，尤其在复杂查询处理和事务完整性保障方面表现突出。

值得一提的是，PostgreSQL 也渊源于加州大学伯克利分校。PostgreSQL 的许可证是 [PostgreSQL 许可证](https://www.postgresql.org/about/licence/) [备份](https://web.archive.org/web/20260115143345/https://www.postgresql.org/about/licence/)，与 BSD 许可证类似，属于宽松的开源许可。


## 安装 PostgreSQL

以 PostgreSQL 18 为例，其他版本安装与配置方法类似。
...
- 示例输出（已设置国际化，如果你的输出是英文也是正常的）：初始化 PostgreSQL 数据库实例。
