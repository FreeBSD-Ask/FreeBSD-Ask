...
初始化数据库建议使用 `service postgresql initdb` 命令，这是最简便的方式。但其中仍有一些技巧可用。
...
- ② `postgresql_data`、`postgresql_enable` 等参数仍然可以使用。这些参数作为实例对应参数的默认值使用，在找不到实例参数时使用。但当你显式配置了实例参数后，就不会使用默认参数了。
