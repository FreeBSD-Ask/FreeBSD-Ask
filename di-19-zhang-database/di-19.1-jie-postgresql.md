...
PostgreSQL 是一款自由的对象关系型数据库（Object-Relational Database，ORDB）。PostgreSQL 最早发布于 1989 年 6 月（最初名为 POSTGRES）。

在 FreeBSD 上，PostgreSQL 有多个版本可选。在吞吐量、源代码可读性和响应时间等方面，PostgreSQL 相较于 MySQL 有一定优势。

值得一提的是，PostgreSQL 也渊源于加州大学伯克利分校。PostgreSQL 的许可证是 [PostgreSQL 许可证](https://www.postgresql.org/about/licence/) [备份](https://web.archive.org/web/20260115143345/https://www.postgresql.org/about/licence/)，与 BSD 许可证类似。
...
设置 PostgreSQL 初始化数据库的参数：UTF-8 编码、C 排序规则、SCRAM-SHA-256 认证，并要求输入密码。
