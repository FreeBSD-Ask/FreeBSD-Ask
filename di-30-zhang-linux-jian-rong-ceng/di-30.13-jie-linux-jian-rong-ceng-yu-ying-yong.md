...
- `compat.linux32`: 32 位 Linux 兼容
- `compat.linux32.emulate_i386`: 兼容真正的 i386
- `compat.linux32.maxvmem`:
- `compat.linux32.maxssiz`:
- `compat.linux32.maxdsiz`:
- `compat.linux`: Linux 模式
- `compat.linux.use_real_ifnames`: 使用 FreeBSD 接口名称而不生成 ethN 别名（即使用 wlan0、ue0 等真实的接口名称，不用 eth0、eth1）
...
- `kern.features.linuxulator_v4l2`: Linux 兼容层中的 V4L2 ioctl wrapper 支持
- `kern.features.linuxulator_v4l`: Linux 兼容层中的 V4L ioctl wrapper 支持
- `vm.uma.linux_dma_object`:
- `vm.uma.linux_dma_object.stats`:
