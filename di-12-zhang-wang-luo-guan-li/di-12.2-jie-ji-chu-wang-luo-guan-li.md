...
- **12.1.1.0**、子网掩码为 **255.255.255.0**；
- **202.0.75.16**、子网掩码为 **255.255.255.240**。

我们需要为系统配置两个网段的 IP 地址范围：**12.1.1.1** 到 **12.1.1.5**，以及 **202.0.75.17** 到 **202.0.75.20**。

只有给定网络范围中的第一个地址应具有实际的子网掩码。其余地址（**12.1.1.2** 到 **12.1.1.5** 和 **202.0.75.18** 到 **202.0.75.20**）必须配置为子网掩码为 **255.255.255.255**。

以下 **/etc/rc.conf** 条目将接口配置为适应此场景：

```sh
# sysrc ifconfig_em0="inet 12.1.1.1 netmask 255.255.255.0"
# sysrc ifconfig_em0_alias0="inet 12.1.1.2 netmask 255.255.255.255"
# sysrc ifconfig_em0_alias1="inet 12.1.1.3 netmask 255.255.255.255"
# sysrc ifconfig_em0_alias2="inet 12.1.1.4 netmask 255.255.255.255"
# sysrc ifconfig_em0_alias3="inet 12.1.1.5 netmask 255.255.255.255"
...
```sh
# sysrc ifconfig_em0_aliases="inet 12.1.1.1-5/24 inet 202.0.75.17-20/28"
```
...
示例：如需访问网络 **12.88.200.0/24** 地址块（可用主机 IP 从 **12.88.200.1** 到 **12.88.200.254**），则将数据包发送至 12.10.12.254，由其转发：

```ini
route_static2="-net 12.88.200.0/24 12.10.12.254"
```
