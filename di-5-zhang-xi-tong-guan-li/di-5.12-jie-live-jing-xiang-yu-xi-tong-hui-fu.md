...
FreeBSD 系统恢复可根据故障严重程度选择不同的方法论，按侵入性从低到高排列如下：

1. **单用户模式（Single-User Mode）**：系统启动至运行级 1（run level 1），仅挂载根文件系统（`/`），不启动网络服务和多用户环境。单用户模式下系统以 root 权限运行，进入单用户模式后，可进行系统修复、密码重置等操作。开机按 2 进入 `single user` 模式，即可进入单用户模式。默认无需输入密码，可使用 `passwd` 命令自行重置密码。
...
3. **灾难恢复**：当整个存储子系统不可用时，需依赖预先创建的备份（如 ZFS 快照的远程复制、`dump`/`restore` 备份）进行系统重建。此场景超出本节范围。
