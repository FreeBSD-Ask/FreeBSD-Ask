...
如果数据以明文形式在网络上传输，处于客户端与服务器之间任一位置的网络嗅探器都能窃取用户/密码或在会话中传输的数据。OpenSSH 则提供多种认证与加密机制来规避此类风险。
...
除内置的 SSH 客户端工具外，同样可以将 FreeBSD 系统配置成 SSH 服务器，接受来自其他 SSH 客户端的连接。

要让 SSH 服务器在系统重启后自动运行，请执行以下命令启用 sshd 服务：
...
sshd 首次在 FreeBSD 系统上启动时，会自动生成主机密钥并将指纹显示在控制台上。请将这些指纹告知用户，方便他们在首次连接时进行校验。

关于启动 sshd 时可用的选项，以及认证流程、登录过程和各类配置文件的完整说明。
...
登录 SSH 服务器时，使用 ssh 并指定该服务器上的一个有效用户名及其 IP 地址/主机名或端口。
...
SSH 通过密钥指纹机制在客户端连接时验证服务器的真实性。首次连接输入 `yes` 接受指纹后，其指纹将存入文件 `~/.ssh/known_hosts`。此后的每次登录都会与已保存的密钥进行比对。若服务器密钥与记录不符，ssh 将发出警告。碰到这种情况，请务必先查清密钥变更的原因，再决定是否继续连接。
...
如需交互式地复制文件，请使用 sftp，操作方式形如 FTP 客户端。以下示例将 D 盘的 `FreeBSD-16.0-CURRENT-amd64-20260413-e9fc0c538264-285005-disc1.iso` 文件上传至 `/home/ykla`。
...
虽然 sshd 是 FreeBSD 中使用最广泛的远程管理工具，但只要是暴露在公网上的系统，暴力破解与路过式攻击就几乎是不可避免的。本节将介绍几项附加参数，用以阻断此类攻击。所有配置均写入 `/etc/ssh/sshd_config`。

>**注意**
>
>请勿将 `/etc/ssh/sshd_config` 与 `/etc/ssh/ssh_config` 混淆（注意第一个文件名中多了一个 `d`）。前者配置的是服务器端，后者配置的是客户端。

一个好做法是使用 OpenSSH 服务器配置文件中的 `AllowUsers` 关键字限制哪些用户可以从何处登录 SSH 服务器。例如，要仅允许 `user` 从 `192.168.1.32` 登录，请在 `/etc/ssh/sshd_config` 中添加以下行：
