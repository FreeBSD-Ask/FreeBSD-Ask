...
关于启动 sshd 时可用的选项，以及认证流程、登录过程和各类配置文件的完整说明，请参阅 sshd(8)。
...
虽然 sshd 是 FreeBSD 中使用最广泛的远程管理工具，但只要是暴露在公网上的系统，蛮力攻击与路过式攻击就几乎是不可避免的。本节将介绍几项附加参数，用以阻断此类攻击。所有配置均写入 `/etc/ssh/sshd_config`。
