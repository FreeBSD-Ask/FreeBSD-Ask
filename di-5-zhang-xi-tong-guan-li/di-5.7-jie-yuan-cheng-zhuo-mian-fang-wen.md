...
远程桌面访问技术能让用户通过网络从一台设备远程控制另一台设备的桌面环境。远程桌面协议主要分为两类：基于帧缓冲区的协议（如 VNC，定义于 RFC 6143）和基于指令流的协议（如 RDP，定义于 ITU-T T.128）。本节涵盖两种协议在 FreeBSD 下的配置。
...
x11vnc 提供屏幕镜像功能，用户的操作会同步显示在物理显示器上，物理显示器上的操作在 VNC 客户端中同样可见。建议配合 SSH 隧道或 SSL 加密使用，以避免 VNC 流量被嗅探。

如果没有物理显示器则无法使用 x11vnc，但可使用 HDMI 显卡欺骗器代替物理显示器以解决此问题。
...
非镜像 VNC 在连接时必须指定端口，否则按默认端口 5900 进行连接。由于使用的不是镜像屏幕（非 x11vnc），因此无法连接。
...
```sh
$ xfreerdp 192.168.31.213 # 注意命令为 xfreerdp。
...
提示需要挂载 proc 文件系统，经测试，如果没有挂载该文件系统，程序将无法正常启动。
