...
loader.conf 是系统启动配置的关键文件，位于 `/boot/loader.conf`。写入此处的配置比 `rc.conf` 文件更早生效，但不当配置可能会妨碍系统正常启动。详细说明请参考 [loader.conf](https://man.freebsd.org/cgi/man.cgi?query=loader.conf) 手册页。
