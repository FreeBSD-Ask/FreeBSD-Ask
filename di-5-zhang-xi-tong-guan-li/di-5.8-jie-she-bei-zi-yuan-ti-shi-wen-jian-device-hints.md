...
设备资源提示文件（**device.hints**）是 FreeBSD 引导过程中的配置文件。该文件在系统启动期间由 boot loader(8) 读取，其内容传递给内核，用于控制内核的引导行为，但也可包含任何内核可调参数值。

**device.hints** 储存了称为变量的内核启动信息，有时也被称为“设备提示（device hints）”。这些“设备提示”被设备驱动程序用于设备配置。
...
**device.hints** 语法是一行一个变量，每一行的格式如下：

```ini
hint.驱动.单元编号.关键字="值"
```

将为驱动的某单元编号设备实例指定资源或属性（注释标记是 `#`）。

其中，`驱动` 是设备驱动程序名称，`单元编号` 是设备驱动程序的单元编号，`关键字` 是提示关键字。关键字可以是以下选项：
...
可以在第 3 阶段引导加载器提示符处指定设备提示，将覆盖写在 **/boot/device.hints** 中的变量。在引导加载器中输入的设备提示不是永久性的，并且在下次重启时将失效。
