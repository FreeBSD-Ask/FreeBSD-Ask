...
ntpd 通过读取 **/etc/ntp.conf** 文件来确定要查询的 NTP 服务器。建议选择多个 NTP 服务器，以防其中一个服务器无法访问或其时钟不可靠。ntpd 会根据接收到的响应，优先选择可靠的服务器，而将不可靠的服务器排除。

查询的 `server` 可以是本地网络中的服务器，ISP 提供的服务器，或者从 [公开可访问的 NTP 服务器列表](https://support.ntp.org/Servers/WebHome) 中选择。选择公共 NTP 服务器时，应选择与地理位置接近的服务器，并查看其使用政策。此外，FreeBSD 也提供由项目维护的服务器池，`0.freebsd.pool.ntp.org`。
...
要在启动时启动 ntpd，可以通过以下命令立即启动，无需重启系统：
...
通常，如果时钟偏差超过 1000 秒，ntpd 会记录错误信息并退出。该选项用于绕过此限制，在没有电池支持的实时时钟的系统中尤其有用。
...
首先临时结束现有的 ntpd 服务，以防止既有服务阻碍时间同步。
