...
**/etc/wall_cmos_clock** 文件存在则表示将机器的实时时钟视为地方时，不存在则表示将实时时钟视为 UTC 时间。
...
关键字 `restrict` 控制哪些系统可访问服务器。可包含多个 `restrict` 条目，每个条目都会根据前面的声明细化访问限制。示例中的值授予本地系统完全的查询和控制访问权限，同时允许远程系统仅能够查询时间。
...
当系统时间与 NTP 服务器偏差超过 1000 秒时，ntpd 默认拒绝修正并退出，需使用 `-g` 选项强制执行修正。
