...
本节将介绍 Minecraft 在 FreeBSD 上的客户端与服务器端部署方案。作为最具影响力的沙盒游戏之一，Minecraft 的部署涉及 Java 运行环境配置、启动器选择等多个技术层面。

本小节介绍如何在 FreeBSD 系统上安装和运行 Minecraft 客户端。Minecraft 是一款沙盒类游戏，需要 Java 运行环境支持。
...
FreeBSD 中常见有两种启动器供选择，分别是 HMCL 和 Prism Launcher。其中 Prism Launcher 可在 FreeBSD Ports 中获得，而 HMCL 只能通过手动下载 `.jar` 文件运行。读者可尝试将其移植到 FreeBSD Ports。
...
Prism Launcher 默认禁止离线登录，并且还会主动封禁任何绕过方法。
...
> **注意**
> 
> 读者需要 Minecraft 正版账号才能登录游戏。因此可能需要 [购买](https://www.minecraft.net/zh-hans/store/minecraft-java-bedrock-edition-pc) 该游戏。
...
过旧的 JDK 版本不受服务器端支持，存在安全性与性能问题，本文测试使用 JDK 21 可正常运行，该版本在服务器环境中表现稳定。
...
因为在当前默认配置下，服务器启用了正版验证机制，非正版用户和未登录用户无法连接该服务器。如需在局域网或测试环境中使用，可关闭正版验证功能。在 `/usr/local/etc/minecraft-server/server.properties` 文件中，将 `online-mode=true` 修改为 `online-mode=false` 即可关闭正版验证功能，该操作会降低服务器安全性，请在受控环境中使用。
