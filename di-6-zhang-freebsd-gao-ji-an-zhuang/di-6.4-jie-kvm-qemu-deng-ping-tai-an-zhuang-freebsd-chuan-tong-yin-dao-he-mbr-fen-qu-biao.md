# 6.4 KVM、QEMU 等平台安装 FreeBSD（传统引导和 MBR 分区表）

在 KVM/QEMU 等硬件辅助虚拟化平台上，通过传统 BIOS + MBR 方式安装 FreeBSD 适用于不直接提供 FreeBSD 镜像的云服务环境。本节所述的 MEMDISK 引导方法不支持 UEFI 引导，也不适用于 OpenVZ、LXC 等容器化平台。

> **注意**
>
> 因为容器化技术本质上不属于完整的虚拟化解决方案，宿主机与客户机共享内核，此方法不支持 OpenVZ、LXC 等容器化技术。内核已经是 Linux，无法运行 FreeBSD 系统。
>
> 本节所述的 MEMDISK 引导方法不支持 UEFI 引导模式（BIOS + GPT 分区表同样不支持），仅支持传统 BIOS + MBR 引导方式，请确保环境符合要求。若需 UEFI 引导，可考虑使用 mfsBSD 的 UEFI 支持或其他引导方案。
...
下载 mfsBSD Mini 14.2-RELEASE ISO 镜像：

```sh
# wget https://mfsbsd.vx.sk/files/iso/14/amd64/mfsbsd-mini-14.2-RELEASE-amd64.iso
```

校验和（官网链接指向错误，已反馈但未获回复）：[checksums](https://mfsbsd.vx.sk/files/iso/14/amd64/mfsbsd-mini-14.2-RELEASE-amd64.iso.sums.txt)
...
# netstat -rn -f inet6 # 显示 IPv6 的路由表
```
