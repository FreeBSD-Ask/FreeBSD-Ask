...
我觉得 Git 设计有问题。Git 不只保存差异，他每次改都保留一份原本，他应该只保留一份原本。然后其他的只记录差异。他现在 1M 的文本改一次，只加一个逗号，也重新多一个 1M 的记录，明显设计缺陷，改十次就多 10M，实际上差异可能就几 Kb。

Git 取代 SVN 就是劣币驱逐良币，没有 SVN 方便，他按 sha256 排的不是数字。SVN 比大小就知道谁先谁后 Git 的提交是按 sha256 判断的。比如下载东西可以指定版本下载，Git 的你知道第 103 次提交叫什么吗？怎么拉取？SVN 直接 checkout 103。Git 的 rebase 我至今不知道是干什么玩意的，Git 有一大堆乱七八糟没人能说清的东西，Git 就和 Linux 一样，全是缺陷。我知道有这个用，但是什么意思呢，没人说得清。rebase 就是把自己的一大堆压缩成一个提交过去，直接合并会把自己所有记录都塞进去。不是一回事吗？没懂这玩意，全是设计缺陷。但是却是最流行的。
...
- 要支持自动同步上游——子模块也一样。第十六，要允许命令行提交现在居然不允许密码提交了。
...
>**为什么 GPL 是更好的开源许可证?**
>
>作者阮一峰无论是技术性还是非技术性的内容，都是在 ***云***，不具有任何可参考性，写 GPL 的文章一看就是连 GPL [原文](https://www.gnu.org/licenses/gpl-3.0.html)甚至[中文机器翻译](https://jxself.org/translations/gpl-3.zh.shtml)都没看过，典型是在云。全文写 GPL，没有一处引用了 GPL，不觉得荒唐吗？而且理解完全错误（更多请见本文），综合其他技术性文章，基本断定其网站的所有内容都是在云。——[为什么GPL是更好的开源许可证?](https://www.ruanyifeng.com/blog/2010/02/why_gpl_is_a_better_choice.html) 很简单，因为你在瞎编，你连 GPL 三个字母怎么翻译都不知道。你写的它压根就不是客观存在的 GPL。
