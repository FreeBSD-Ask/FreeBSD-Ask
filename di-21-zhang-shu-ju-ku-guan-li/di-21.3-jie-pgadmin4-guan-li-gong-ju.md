...
virtualenv 用于创建独立的 Python 虚拟环境，各项目之间互不干扰。
...
如遇网络问题，可通过更改 pip 镜像源解决，设置为清华大学镜像源：
...
这是由于时间不正确引发的，可使用命令 `ntpd -q -g -p pool.ntp.org` 同步系统时间到 NTP 服务器池 pool.ntp.org：
...
pgadmin4 更新频率较高，如需升级，须先删除原有用 virtualenv 创建的 pgadmin4 目录，然后用安装用户再次执行如下命令，创建名为 pgadmin4 的 Python 虚拟环境：
