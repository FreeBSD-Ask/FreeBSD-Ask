...
用户可以拉取旧版 Ports 编译安装。以下 commit 来自 FreeBSD ports 仓库历史，是各 MySQL 5.x 版本在官方源中停止支持前的最后提交。旧版本已停止官方维护，存在安全与兼容性风险。
...
将密码修改为 `your_new_password`，然后刷新权限。
...
2. 使 MySQL 5.7 配置现代化。
3. MySQL 5.7 已于 2023 年终止官方支持，但大量遗留系统仍在使用。分析在一个仍受安全更新的操作系统上运行已终止支持的数据库软件所面临的合规风险，并讨论 FreeBSD Ports 是否应对 EOL 软件设置明确的移除时间表。
