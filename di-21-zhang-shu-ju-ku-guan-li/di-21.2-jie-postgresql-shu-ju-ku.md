...
数据库集簇（database cluster）是 PostgreSQL 存储数据的目录集合，包含所有数据库文件和配置，初始化方法如下。
...
初始化数据库建议使用 `service postgresql initdb` 命令，最为简便，但其中仍有技巧。
...
Debian/Ubuntu 系统管理多个数据库实例的常用工具是 `pg_ctlcluster`，它封装了 `pg_ctl`。

FreeBSD 没有此工具，但 FreeBSD 上 PostgreSQL 的 rc 脚本封装了这些管理功能，同样便捷。
