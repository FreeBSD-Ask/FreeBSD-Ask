...
关系型数据库基于关系模型，以二维表格存储数据，通过表间关联关系实现数据的组织、管理与查询。
...
在 MySQL 中，`RENAME DATABASE` 存在于早期版本，但在 MySQL 5.1.23 之后已被移除，原因是该操作存在安全风险。
...
- `char` 是字符类型，如果要存储字符串需使用 `char(字符数)`
...
外键是用于建立并强制两个表之间关联关系的一列或多列，指向另一个表的主键或唯一键，以确保数据的一致性与完整性。数据库管理系统在外键约束上维护参照完整性，确保子表中的外键值必须存在于父表的主键或唯一键中，或者为 NULL（如果允许）。
...
`ON DELETE CASCADE` 是外键约束的一个选项，父表记录被删除时，数据库自动删除子表中所有引用该记录的行。此机制在“自动维护父表和子表之间的参照完整性”一节中有详细说明。
...
SET 子句指定需要修改的列。如需仅修改部分记录，应通过 WHERE 子句限定条件，以明确修改范围。
...
在一个书籍管理库中，使用 LIKE '%设计%' 会返回类似《MySQL 数据库设计》的书名。
...
1. 在 FreeBSD 上安装 MySQL 或 MariaDB，创建一个名为 `library` 的数据库，在其中建立 `books` 表（包含书名、作者、出版社、价格、出版日期字段），设置主键和非空约束。
2. 向 `books` 表插入不少于 10 条测试数据，使用 `UPDATE` 将某出版社的所有图书价格上调 15%，用 `DELETE` 删除出版日期早于 2000 年的记录，每步操作后使用 `SELECT` 验证结果。
3. 创建 `authors` 表（包含作者 ID、姓名、国籍）和 `book_authors` 关联表，通过外键建立多对多关系，编写 JOIN 查询列出每本书的作者姓名和国籍。
4. 使用 `mysqldump` 导出 `library` 数据库，在另一台 FreeBSD 主机上导入，验证数据完整性。
