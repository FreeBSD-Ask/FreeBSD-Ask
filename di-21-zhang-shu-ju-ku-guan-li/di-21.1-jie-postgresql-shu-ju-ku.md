...
PostgreSQL 是一款自由的对象关系型数据库管理系统（Object-Relational Database Management System，ORDBMS）。其学术源流可追溯至加州大学伯克利分校的 Ingres 项目，随后 Michael Stonebraker 教授在此基础上启动了后继项目 POSTGRES（意为“post-Ingres”后 Ingres 时代），首个版本于 1989 年 6 月发布。

PostgreSQL 采用 [PostgreSQL 许可证](https://www.postgresql.org/about/licence/)，这是一种与 BSD 许可证类似的宽松开源许可证。

作为严格遵循 ACID（原子性、一致性、隔离性、持久性）特性的数据库系统，PostgreSQL 支持事务处理机制、复杂查询优化和数据完整性约束。

PostgreSQL 提供多个稳定版供选择。

## 安装 PostgreSQL

PostgreSQL 提供多个稳定版供选择，本节以 PostgreSQL 18 为例进行说明，其他版本的安装与配置方法类似。
...
~postgres/data/postgresql.conf（postgres 用户主目录下的数据目录配置文件）。
...
来改变这一默认行为。更多信息参见该启动脚本头部的说明。

To set limits, environment stuff like locale and collation and other
如果要设置资源限制、区域/排序等环境相关参数，

things, you can set up a class in /etc/login.conf before initializing
可以在初始化数据库前，先在 /etc/login.conf 文件中定义一个 login class，

the database. Add something similar to this to /etc/login.conf:
并将类似如下的条目加入 /etc/login.conf 文件：
...
然后在 /etc/rc.conf 文件中加入 'postgresql_login_class="postgres"'，

set it as the postgres user's login class in /etc/passwd.
或直接在 /etc/passwd 文件里将 postgres 用户的 login class 设为 postgres。

======================================================================

To use PostgreSQL, enable it in rc.conf using
要启用 PostgreSQL，请在 rc.conf 文件中启用服务：
...
数据库集簇（database cluster）是 PostgreSQL 存储数据的目录集合，包含所有数据库文件和配置。以下是初始化方法。
...
初始化数据库建议使用 `service postgresql initdb` 命令，这是最简便的方式。但其中仍有技巧。
...
PostgreSQL 能在一台机器上运行多个实例，适用于测试不同配置、实现环境隔离等场景。

在 Debian/Ubuntu 系统上，管理多个数据库实例常用工具是 `pg_ctlcluster`，它封装了 `pg_ctl`。

FreeBSD 上并没有这个工具，但 FreeBSD 上 PostgreSQL 的 rc 脚本封装了这些管理功能，同样便捷。
...
- ② `postgresql_data`、`postgresql_enable` 等参数仍然可以使用。这些参数作为实例对应参数的默认值使用，在找不到实例参数时使用。但当显式配置了实例参数后，就不会使用默认参数了。
- ③ 在管理多个实例时，建议只使用实例名相关的参数，不使用没有实例名的参数。这样做的好处是清晰明确，控制上也灵活，不同实例可以有不同设置。
...
- The PostgreSQL Global Development Group. A Brief History of PostgreSQL[EB/OL]. [2026-04-17]. <https://www.postgresql.org/docs/current/history.html>. 记载了 POSTGRES 项目从 Ingres 后继发展到 PostgreSQL 的完整历程，包括 1989 年 6 月发布 Version 1 等关键时间节点。

## 课后习题

1. 结合本节内容，编写一份 PostgreSQL 常用操作速查手册，涵盖数据库创建、用户管理、权限配置、备份恢复等核心操作，并为每个操作提供命令示例。

2. 查找 FreeBSD PostgreSQL rc 脚本源代码，分析其多实例管理机制的实现方式，修改默认端口分配逻辑，实现实例名到端口号的自动映射。

3. 使用 PostgreSQL 部署一些常见服务，如 Typecho、WordPress，并完善它们对 PostgreSQL 的支持，贡献给上游。
