...
VirtualBox 是 Oracle 公司开发的跨平台 Type-2 虚拟化平台，采用 GNU General Public License（GPL）协议开源发布，支持 x86 与 AMD64 架构的硬件辅助虚拟化（Intel VT-x 与 AMD-V）。作为广泛使用的桌面虚拟化解决方案，VirtualBox 提供了图形化用户界面与命令行管理工具（VBoxManage），支持多种客户机操作系统。闭源的 VirtualBox 扩展包（VirtualBox Extension Pack）尚未在 FreeBSD 上完全支持，该扩展包提供 USB 2.0/3.0 支持、NVMe 磁盘加密等附加功能。

## 安装 VirtualBox

VirtualBox 是一款功能强大的虚拟化软件，在 FreeBSD 上有多个版本可供选择。截至本文撰写时，包括：`emulators/virtualbox-ose-legacy`（5.X）、`emulators/virtualbox-ose`（6.X）、`emulators/virtualbox-ose-70` 和 `emulators/virtualbox-ose-71`，其中均提供无 X11 的 `-nox11` 版本（例如 `emulators/virtualbox-ose-71-nox11`，无图形界面，仅命令行操作，命令为 `VBoxManage`）。
...
完成 VirtualBox 安装后，需要进行一些必要的配置才能正常使用，具体步骤如下：

- 将以下行写入 `/boot/loader.conf` 文件，设置系统在启动时加载 VirtualBox 内核模块：
...
为了让普通用户能够使用 VirtualBox，需要将用户加入相应的用户组：
