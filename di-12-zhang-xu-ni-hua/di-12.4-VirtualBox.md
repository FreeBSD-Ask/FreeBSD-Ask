...
VirtualBox 是甲骨文（Oracle）公司开发的跨平台 Type-2 虚拟化平台，采用 GNU 通用公共许可证（GNU General Public License，GPL）协议开源发布，支持 x86 与 AMD64 架构的硬件辅助虚拟化（Intel VT-x 与 AMD-V）。作为广泛使用的桌面虚拟化解决方案，VirtualBox 提供了图形化用户界面与命令行管理工具（VBoxManage），支持多种客户机操作系统。闭源的 VirtualBox 扩展包（VirtualBox Extension Pack）尚未在 FreeBSD 上完全支持，该扩展包提供 USB 2.0/3.0 支持、NVMe 磁盘加密等附加功能。
