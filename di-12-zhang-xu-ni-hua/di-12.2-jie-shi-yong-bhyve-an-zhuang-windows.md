...
vm-bhyve 是基于命令行的 bhyve 虚拟机管理框架，简化了虚拟机的创建、配置与生命周期管理。本节详细介绍在 FreeBSD 上通过 vm-bhyve 部署 Windows 11 客户机的完整流程。

本节基于 FreeBSD 14.2 RELEASE 版本和 Windows 11 IoT Enterprise LTSC。

## 安装固件与所需软件

本节将介绍安装 bhyve 虚拟机管理工具、配置环境以及准备 Windows 11 系统镜像的详细步骤。

### 准备 Windows 镜像

Windows 11 IoT Enterprise LTSC，版本 24H2 (x64) - DVD (英文) 系统下载链接（来自 [itellyou](https://next.itellyou.cn/) [备份](https://web.archive.org/web/20260114171228/https://next.itellyou.cn/)）：
...
首先，安装统一可扩展固件接口（UEFI）固件、虚拟网络计算（VNC）与 vm-bhyve。
...
`public` 为模板中指定的虚拟交换机名称，`ue0` 为当前宿主机的物理网络接口名称，请根据实际网卡修改（可通过命令 `ifconfig` 查看）。虚拟交换机通过 bridge(4) 实现网络桥接功能，将虚拟机网络栈与宿主机物理网络接口连接，否则创建将失败，虚拟机会报错 `exit with error 4`：
...
虚拟机模板是创建虚拟机的基础配置文件，本节将介绍如何创建并配置 Windows 虚拟机模板。
...
但是注意该模板是有问题的，需要进行修改，文件路径是 `/home/ykla/vm/winguest/winguest.conf`：

> **注意**
>
>旧版 FreeBSD 中（FreeBSD 14.0 以前），`/home/` 是被软连接到 `/usr/home/` 的，两者是一样的。
>
>自 FreeBSD 14 以降，不再是软连接，而是直接使用 `/home/`。
...
- [vm-bhyve/wiki/Running-Windows](https://github.com/churchers/vm-bhyve/wiki/Running-Windows) [备份](https://web.archive.org/web/20260114075231/https://github.com/churchers/vm-bhyve/wiki/Running-Windows)，教程主要参考此处，详述 Windows 虚拟机部署；中文版本在 [vm-bhyve Wiki](https://book.bsdcn.org/wen-zhang/wen-zhang/vm-bhyve) [备份](https://web.archive.org/web/20260114055841/https://book.bsdcn.org/wen-zhang/wen-zhang/vm-bhyve)
- [The win11 release ISO requires the install-time regedit TPM workaround](https://twitter.com/bhyve_dev/status/1446404943020056581)，提供 Windows 11 TPM 绕过方法
- [windows 11 on bhyve](https://forums.freebsd.org/threads/windows-11-on-bhyve.82371/) [备份](https://web.archive.org/web/20260115022934/https://forums.freebsd.org/threads/windows-11-on-bhyve.82371/)，社区讨论 Windows 11 部署问题与解决方案
- [FreeBSD, bhyve и Windows 11](https://dadv.livejournal.com/209650.html) [备份](https://web.archive.org/web/20260120154947/https://dadv.livejournal.com/209650.html)，提供俄语 Windows 11 部署指南
- [iki/bhyve/Windows](https://wiki.freebsd.org/bhyve/Windows) [备份](https://web.archive.org/web/20260115022938/https://wiki.freebsd.org/bhyve/Windows)，官方 Wiki 提供 Windows 虚拟机支持信息
- [churchers/vm-bhyve/wiki](https://github.com/churchers/vm-bhyve/wiki) [备份](https://web.archive.org/web/20260115040715/https://github.com/churchers/vm-bhyve/wiki)，vm-bhyve 官方文档
- [Using Windows on FreeBSD's vm-bhyve](https://srobb.net/vm-bhyve.html) [备份](https://web.archive.org/web/20260114074951/https://srobb.net/vm-bhyve.html)，提供 Windows 虚拟机实用配置指南

## 课后习题

1. 使用 vm-bhyve 命令行工具创建一个新的虚拟机模板，对比默认模板与你创建的模板配置差异，理解 vm-bhyve 如何通过模板系统控制虚拟机的创建流程。

2. 修改虚拟机配置文件中的网络设备类型（从 e1000 改为 virtio-net），在 Windows 虚拟机中安装对应驱动，验证网络功能是否正常，将步骤记录到本文。
