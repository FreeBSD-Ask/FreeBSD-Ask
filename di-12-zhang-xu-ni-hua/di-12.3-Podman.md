...
容器化技术是操作系统级虚拟化的重要实现形式，通过隔离的用户空间实例提供应用程序运行环境。
...
在 FreeBSD 平台上，Podman 基于原生的 Jail 机制实现容器隔离，Jail 作为 FreeBSD 成熟的操作系统级虚拟化技术，为容器提供了可靠的安全隔离边界，用户可以使用命令 `jls` 查看当前运行的容器。
...
编辑 `/etc/pf.conf` 文件，将 `ix0` 替换为你当前使用的网卡，可使用命令 `ifconfig` 查看：
