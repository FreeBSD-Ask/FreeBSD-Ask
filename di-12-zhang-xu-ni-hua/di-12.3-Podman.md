...
Podman 是一个用于管理容器和镜像的开源工具。本节介绍 Podman 在 FreeBSD 上的安装和配置方法。在安装 sysutils/podman-suite 元包时，会同时安装 sysutils/buildah（用于构建容器镜像）、sysutils/podman（容器运行时）和 sysutils/skopeo（用于操作容器镜像）。
