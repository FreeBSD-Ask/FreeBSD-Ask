...
以下示例演示密钥文件的生成方法，该文件挂载在 **/mnt** 下，是加密提供者主密钥的组成部分。该密钥文件提供随机数据用于加密主密钥。主密钥同时受密码保护。提供者的扇区大小设为 4kB。本示例还包含 `geli` 提供者的附加、文件系统创建、挂载、操作和卸载。
...
生成密钥文件用于主密钥（作为磁盘元数据存储）。以下命令生成密钥文件，所有数据都将以此为凭据加密。此密钥不可更改。用户密钥由密钥文件 `/root/ada0.key` 的随机字节与口令短语组合而成。本例使用 `/dev/random` 作为密钥文件的数据源：
...
然后使用 UFS 文件系统格式化该设备，并将其挂载到现有挂载点：
...
这将把 **/dev/ada0** 配置为 geli 设备，并使用 **/root/ada0.key** 作为主密钥。然后，文件系统将挂载，通常通过 **/etc/fstab** 中的条目挂载（新增下行）：

```ini
/dev/ada0.eli   /mnt   ufs   rw   0   0
```

系统关闭前，系统将自动将提供者从内核中分离。在引导过程中，脚本将提示输入密码，然后附加提供者。引导过程中的其他内核消息可能会在密码提示之前或之后显示。
