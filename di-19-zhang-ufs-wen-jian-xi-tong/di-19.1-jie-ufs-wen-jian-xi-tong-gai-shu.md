...
需要区分本节所述的 UFS 文件系统与手机等设备中使用的 UFS 存储属于完全不同的技术范畴。后者全称为 Universal Flash Storage（通用闪存存储），是一种硬件存储标准，目前已发展至 5.0 版本。FreeBSD 自 8.0 起通过 mmc/mmcsd 驱动提供基本的 MMC/SD 支持，FreeBSD 12.0 实现了完整的 eMMC 支持（包括分区、TRIM 及 HS400 总线速度模式），FreeBSD 15.0 已通过 `ufshci` 驱动支持 UFS 存储。
