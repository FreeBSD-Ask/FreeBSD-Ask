...
当不再使用内存磁盘时，应该将其资源释放回系统。首先，卸载文件系统，随后使用 `mdconfig` 从系统中分离磁盘并释放其资源。继续此示例：
...
- FreeBSD Project. pkg(8) -- package manager[EB/OL]. [2026-04-17]. <https://man.freebsd.org/cgi/man.cgi?query=pkg&sektion=8&manpath=freebsd-16.0-current>. FreeBSD 包管理器手册页。
