...
ext2fs 是 FreeBSD 内核提供的 EXT 文件系统支持模块，与 fusefs-ext2 配合使用可增强性能。首先，确保加载 FUSE 内核模块，它为所有基于 FUSE 的文件系统提供底层支持。将 ext2fs 和 fusefs 内核模块添加至开机加载列表：
...
加载这些内核模块可提供更底层的 EXT 文件系统支持，提升 fusefs-ext2 的访问性能。
