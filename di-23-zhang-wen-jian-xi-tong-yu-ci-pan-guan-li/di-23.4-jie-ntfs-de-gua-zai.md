...
UBLIO（User space Block I/O）是一个用户空间块 I/O 库，用于提升 FUSE 文件系统的性能。但由于 [Bug 206978 - sysutils/fusefs-ntfs: Disable UBLIO as it breaks mkntfs](https://bugs.freebsd.org/bugzilla/show_bug.cgi?id=206978) 和 [Bug 194526 - sysutils/fusefs-ntfs: ntfs-3g with libublio lost files](https://bugs.freebsd.org/bugzilla/show_bug.cgi?id=194526) 的存在，UBLIO 在 FreeBSD 上与 ntfs-3g 配合使用时可能导致文件系统损坏和数据丢失，因此请勿使用 pkg 进行二进制安装，而应采用 Ports 系统进行源码编译：
