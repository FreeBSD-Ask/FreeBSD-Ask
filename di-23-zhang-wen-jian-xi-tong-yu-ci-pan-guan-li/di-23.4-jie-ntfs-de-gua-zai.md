# 23.4 Windows 文件系统的挂载

Windows 文件系统（特别是 NTFS 和 FAT32）的跨平台访问是 FreeBSD 与 Windows 互操作性的关键组成部分。本节系统阐述 NTFS 和 FAT32 文件系统在 FreeBSD 上的挂载、格式化与自动配置方法。
...
由于 [Bug 206978 - sysutils/fusefs-ntfs: Disable UBLIO as it breaks mkntfs](https://bugs.freebsd.org/bugzilla/show_bug.cgi?id=206978) 和 [Bug 194526 - sysutils/fusefs-ntfs: ntfs-3g with libublio lost files](https://bugs.freebsd.org/bugzilla/show_bug.cgi?id=194526) 的存在，请勿使用 pkg 进行二进制安装，而应采用 Ports 系统进行源码编译：
...
- [NTFS on FreeBSD](https://www.gridbugs.org/ntfs-on-freebsd/) [备份](https://web.archive.org/web/20260116225729/https://www.gridbugs.org/ntfs-on-freebsd/)，系统阐述了 NTFS 文件系统在 FreeBSD 上的挂载技术实现方案
...
配置中使用 ntfs-3g 驱动以读写模式挂载，`late` 选项表示延迟挂载。

### NTFS 的手动挂载操作

- 使用 ntfs-3g 将 `/dev/da0s1` 挂载至 `/media/NTFS`，设置读写权限，并指定文件所有者和权限掩码：

```sh
# ntfs-3g  /dev/da0s1  /media/NTFS   -o  rw,uid=1000,gid=1000,umask=0
```

- 若不确定哪个磁盘分区为 NTFS 格式，可使用以下命令检测 `/dev/da0s1` 分区的文件系统类型：
...
- 若上述方法仍无法解决问题，可尝试修复 `/dev/da0s1` 上的 NTFS 文件系统错误（轻量级修复，功能类似 Windows 系统的 chkdsk）：
...
- [ntfs-3g manpage](https://www.freebsd.org/cgi/man.cgi?query=ntfs-3g&format=html) [备份](https://web.archive.org/web/20260116205246/https://man.freebsd.org/cgi/man.cgi?query=ntfs-3g&format=html)（ntfs-3g 官方技术文档，详细说明各项参数配置与使用方法）
...
2. 分析为什么 ntfs-3g 需要通过 Ports 源码编译而不能直接使用 pkg 安装，重构一个最小化的 NTFS 挂载脚本。
