...
写入配置后，可使用命令 `mount -al` 进行检查（`-a` 表示挂载 `/etc/fstab` 中所有条目；`-l` 表示仅挂载带有 late 标记的分区，而不挂载普通分区），确保无错误输出。参见 FreeBSD Project. man mount(8)[EB/OL]. [2026-03-26]. <https://man.freebsd.org/cgi/man.cgi?mount(8)>
