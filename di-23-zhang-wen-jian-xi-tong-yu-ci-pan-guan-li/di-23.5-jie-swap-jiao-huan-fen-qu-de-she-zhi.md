...
写入配置后，可使用命令 `mount -al` 进行检查（`-a` 表示挂载 `/etc/fstab` 中所有条目；`-l` 表示仅挂载带有 late 标记的分区，而不挂载普通分区），确保无错误输出。参见 man mount(8)[EB/OL]. [2026-03-26]. <https://man.freebsd.org/cgi/man.cgi?mount(8)>

### 参考文献

- FAQ[EB/OL]. [2026-03-25]. <https://openzfs.github.io/openzfs-docs/Project%20and%20Community/FAQ.html>. OpenZFS 官方常见问题解答，提供 ZFS 卷配置的最佳实践
- Oracle Solaris ZFS 管理指南[EB/OL]. [2026-03-25]. <https://docs.oracle.com/cd/E19253-01/819-7065/givdo/index.html>. Oracle 官方 ZFS 技术文档，系统阐述 ZFS 存储管理方法
...
选项 `-h` 表示以人类可读格式（human-readable）输出。参见 man swapinfo(8)[EB/OL]. [2026-03-26]. <https://man.freebsd.org/cgi/man.cgi?swapinfo(8)>。
