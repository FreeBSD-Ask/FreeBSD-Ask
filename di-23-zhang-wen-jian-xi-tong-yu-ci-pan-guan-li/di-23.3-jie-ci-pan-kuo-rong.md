...
UFS 全称为 Unix File System（UNIX 文件系统），起源于 UNIX v7。过去，macOS 也使用该文件系统作为根文件系统。目前 FreeBSD 使用的是 UFS2。Linux 对 UFS 的读写支持不完整。
...
> **警告**
>
> 如果你使用的是 GPT 分区表，此处的扩容操作在虚拟机或云服务器上可能会破坏 GPT 分区表，所以要恢复 `da0` 磁盘的分区表：
>
> ```sh
> # gpart recover da0
> ```
