...
MySQL 8.x 是 MySQL 数据库的主流版本系列。MySQL 是由 Oracle 公司主导开发的开源关系型数据库管理系统（Relational Database Management System，RDBMS）。

MySQL 8.4 是长期支持版本，具有稳定的性能和广泛的兼容性。
...
1. 分别安装 MySQL 8.0（已 EOL）和 8.4 LTS 版本，创建相同的测试数据库并执行相同的 SQL 查询，对比两个版本在性能与默认配置上的差异。

2. 查找 FreeBSD MySQL rc 脚本源代码，分析其配置文件加载机制，修改默认配置文件路径，实现从 /usr/local/etc/mysql84.conf 加载配置。
