...
1. 查找 FreeBSD MySQL rc 脚本源码，分析其配置文件加载机制，修改默认配置文件路径，实现从 /usr/local/etc/mysql84.conf 加载配置。

2. 修改 MySQL 8.x 的默认认证方式从 caching_sha2_password 改为 mysql_native_password，验证其兼容性变化。
