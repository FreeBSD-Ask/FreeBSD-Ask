...
- ① 用于调试的 Linux 伪文件系统，参见：lindebugfs(4)[EB/OL]. [2026-03-26]. <https://man.freebsd.org/cgi/man.cgi?query=lindebugfs&sektion=4>.
- ② GCOV 用于内核代码覆盖率分析。
...
- ① 提供 MPSPEC 多处理器表的内核支持。参见：mptable(1)[EB/OL]. [2026-03-26]. <https://man.freebsd.org/cgi/man.cgi?query=mptable&sektion=1>.
...
Microchip smartpqi 控制器。这些控制器提供类 SCSI 接口，且需要 CAM 基础设施。

```ini
device		mpi3mr			# Broadcom MPIMR 3.0
```

Broadcom MPIMR 3.0 IT/IR 24Gb/s SAS Tri-Mode RAID 控制器，仅支持 64 位架构。
...
device		qlnxe		# Marvell QLogic 41000/45000 系列 10/25/40/100 GbE 以太网与 CNA 适配器
device		sfxge		# AMD/Xilinx Solarflare SFC9000 系列 10Gb 以太网
```
...
> **注意**
>
> 该功能依赖 crypto、cryptodev 和 ZSTDIO 内核组件。
...
`AUTO_EOI_2` 为从 8259A 中断控制器启用“自动 EOI”功能。每次中断可节省约 0.7-1.25 微秒。文档记载自动 EOI 在原始 i8259A 芯片的从控制器上无效，但部分克隆芯片和集成版本可正常运行。
...
syscons 控制台驱动程序（兼容 SCO 彩色控制台接口）。syscons 已被 vt(4) 取代为默认控制台驱动，且不兼容 UEFI 启动的系统；建议新部署使用 vt(4)。参见：syscons(4)[EB/OL]. [2026-06-10]. <https://man.freebsd.org/cgi/man.cgi?query=syscons&sektion=4>。
...
```ini
options 	VGA_SLOW_IOACCESS	# 对 TS 与 GDC 寄存器执行字节宽度 I/O 操作
```
...
```ini
device		bxe		# QLogic NetXtreme II BCM5771X/BCM578XX 10GbE 以太网卡
...
device		iwm		# Intel 无线网卡（Intel Dual Band Wireless AC 系列），依赖 iwm 固件
...
- `amdtemp`：AMD Family 0Fh-17h（K8 至 Zen）系列处理器晶圆的内置传感器
