...
本节介绍用户级 ZFS 加密功能的配置和使用方法。在 FreeBSD 14.1 中，若要在用户级使用 ZFS 加密功能，必须为用户授予特定权限。
...
本节介绍如何使用 adduser 命令创建加密的用户主目录。adduser 命令中可以直接使用加密的用户主目录数据集，但默认给出的权限不足，一经卸载就无法由普通用户直接挂载。
