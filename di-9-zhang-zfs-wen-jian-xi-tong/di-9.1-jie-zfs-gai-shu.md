...
- **ONLINE**：处于 ONLINE 状态的池或 vdev，其所有成员设备已连接且完全正常运行。

- **OFFLINE**：管理员可将单个设备设为 OFFLINE 状态，前提是存在足够冗余以维持池或 vdev 可用。此操作通常用于准备更换磁盘或便于识别设备。

- **DEGRADED**：处于 DEGRADED 状态的池或 vdev 有一个或多个磁盘丢失或故障，池仍可使用，但若再有设备故障可能导致不可恢复。重新连接丢失设备或更换故障磁盘并完成重建后，池将恢复 ONLINE 状态。

- **FAULTED**：处于 FAULTED 状态的池或 vdev 已不可操作、数据无法访问。当丢失或故障设备数量超过 vdev 冗余上限时进入此状态。若重新连接丢失设备，池将恢复 ONLINE；若冗余不足以弥补故障磁盘数量，池中数据将永久丢失，须从备份恢复。

- **REMOVED**：设备已被管理员通过 `zpool remove` 命令从池中移除。被移除设备上的数据已重新分布到剩余设备中（若池支持设备移除功能）。设备移除完成后，该 vdev 从配置中彻底删除。

- **UNAVAIL**：设备无法打开或访问。此状态与 FAULTED 不同：FAULTED 通常表示设备已损坏，而 UNAVAIL 表示设备暂时不可访问（如设备掉电、连接松动等）。ZFS 不会假定 UNAVAIL 设备的数据已永久丢失，若设备恢复访问且数据完整，设备将自动恢复健康状态。
