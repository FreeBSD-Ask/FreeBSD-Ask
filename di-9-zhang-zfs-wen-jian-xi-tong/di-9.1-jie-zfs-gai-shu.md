...
- **vdev 类型**：
  ...
  - **备用 (Spare)**：备用盘属于辅助 vdev（auxiliary vdev）类型，用于跟踪可用的热备盘。当活动设备故障时，热备盘会自动替换故障设备；待使用 `zpool replace` 将故障设备永久替换后，热备盘自动释放并恢复为可用状态。热备盘可在多个池之间共享，但不能替换日志设备。正在使用的共享热备盘会导致池无法导出。

  - **日志 (Log)**：日志 vdev（SLOG，Separate ZFS Intent Log）用于将 ZIL（ZFS 意图日志）从常规池设备转移到专用存储设备（通常为 SSD）。专用日志设备可提升数据库等高同步写入负载的性能。日志设备可镜像，但不支持 RAID-Z；使用多个日志设备时写入将负载均衡。

  - **缓存 (Cache)**：向池中添加缓存 vdev 会将缓存存储加入 L2ARC。缓存设备不可镜像。由于缓存设备仅存储现有数据的副本，不存在数据丢失风险。
  ...
  - **特殊分配类 (Special)**：特殊 vdev 专用于存储内部元数据（如文件系统元数据、重复数据删除表、间接块），也可按数据集粒度接受小文件块。通常使用高性能 SSD 以加速元数据密集型操作。若特殊 vdev 写满，后续分配将回退至普通 vdev。
...
- **校验和**：每个数据块均附带校验和，所用算法为数据集属性（通过 `zfs set` 设置）。读取时校验和自动透明验证，使 ZFS 能够检测静默数据损坏。若数据与预期校验和不匹配，ZFS 会尝试从可用冗余（如镜像或 RAID-Z）中恢复。`scrub` 可触发全量校验和验证。支持的校验和算法包括 `on`（默认，非去重数据集为 `fletcher4`，去重数据集为 `sha256`）、`fletcher2`、`fletcher4`、`sha256`、`sha512`（SHA-512/256，64 位硬件上比 SHA-256 快约 50%，需启用池特性）、`skein`（SHA-3 候选算法，比 SHA-256 快约 80%，需启用池特性）、`edonr`（高性能 SHA-3 候选算法，比 SHA-256 快约 350%，需启用池特性；当配合去重使用时自动采用 `verify` 模式）和 `blake3`（高性能安全哈希，针对现代 CPU 优化，需启用池特性）。另有 `noparity` 禁用校验和且不维护奇偶校验，仅供 RAID-Z 上的 dump 设备内部使用。其中 `fletcher` 类算法速度更快但不提供密码学安全性，`sha256` 及以上为强加密哈希。`sha512`、`skein`、`edonr` 和 `blake3` 均采用带盐校验和（salted checksum）。可设为 `off` 禁用校验和，但极不推荐。
