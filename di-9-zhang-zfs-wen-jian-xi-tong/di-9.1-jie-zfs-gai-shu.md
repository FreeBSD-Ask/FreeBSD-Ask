...
- **校验和**：每个数据块均附带校验和，所用算法为数据集属性（通过 `zfs set` 设置）。读取时校验和自动透明验证，使 ZFS 能够检测静默数据损坏。若数据与预期校验和不匹配，ZFS 会尝试从可用冗余（如镜像或 RAID-Z）中恢复。`scrub` 可触发全量校验和验证。支持的校验和算法包括 `on`（默认，当前为 `fletcher4`）、`fletcher2`、`fletcher4`、`sha256`、`sha512`（SHA-512/256，64 位硬件上比 SHA-256 快约 50%，需启用池特性）、`skein`（SHA-3 候选算法，比 SHA-256 快约 80%，需启用池特性）、`edonr`（高性能 SHA-3 候选算法，比 SHA-256 快约 350%，需启用池特性）和 `blake3`（高性能安全哈希，针对现代 CPU 优化，需启用池特性）。另有 `noparity` 禁用校验和且不维护奇偶校验，仅供 RAID-Z 上的 dump 设备内部使用。其中 `fletcher` 类算法速度更快但不提供密码学安全性，`sha256` 及以上为强加密哈希。`skein` 和 `edonr` 采用带盐校验和。可设为 `off` 禁用校验和，但极不推荐。
