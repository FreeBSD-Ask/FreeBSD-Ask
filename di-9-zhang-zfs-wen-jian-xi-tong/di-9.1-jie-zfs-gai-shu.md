...
```
                                           ...
                                           └───────┬───────┘
                                                   ...
                                                   ▼
              ┌──────────────────┐              ┌──────────────┐              ┌──────────────┐
              │  快照 (Snapshot)   │   创建/回滚   │ 数据集(Dataset)│    派生      │  克隆 (Clone) │
              │   pool/fs@snap    │◄────────────►│                │────────────►│  可写副本      │
              │   只读历史状态     │              │  COW(写时复制)  │              │              │
              └────────┬─────────┘              │  Checksum(校验) │              └──────────────┘
                       │                        │  Compression    │
  ...
  写路径: 写入 → ZIL(意图日志) → [Log VDEV可选] → TXG(事务组) → 数据 VDEV → 磁盘
  读路径: 读取 → ARC(RAM缓存) → 命中返回 → 未命中 → L2ARC(Cache VDEV) → 命中返回
           → 仍未命中 → 数据 VDEV → 磁盘读取
  校验路径: Scrub/日常读取 → Checksum验证 → 损坏 → 利用冗余(Mirror/RAID-Z)自动修复
  快照流程: 数据集 ──创建──► 快照(只读) ──派生──► 克隆(可写) ──轻量引用──► 书签
```
