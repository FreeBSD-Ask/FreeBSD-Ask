...
- **压缩**：每个数据集均有压缩属性。设为 `on`（默认值）时表示使用当前默认压缩算法，该算法平衡了压缩与解压速度及压缩比，适用于广泛的工作负载。与所有其他压缩设置不同，`on` 不选择固定的压缩类型——随着新压缩算法加入 ZFS 并在池中启用，默认压缩算法可能发生变化。当前默认压缩算法为 `lzjb`，或在启用了 `lz4_compress` 功能标志时为 `lz4`。启用压缩算法后，ZFS 会压缩所有新写入数据。除节省空间外，由于读写块数减少，吞吐量通常也会提升。任何非 `off` 的压缩设置均自动检测全零块（NUL 字节）并存储为空洞（hole）。压缩后的数据会按扇区（2^`ashift` 字节）向上取整；如果节省空间不足一个扇区，该块将不压缩存储。此外，还有 12.5% 的默认压缩阈值。
