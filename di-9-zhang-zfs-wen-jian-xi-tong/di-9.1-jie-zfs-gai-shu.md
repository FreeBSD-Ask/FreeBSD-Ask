...
类型：filesystem | volume | snapshot | bookmark | clone
  ...
  ─────────────────────            ─────────────────────
  * 挂载为 POSIX 目录树            * 呈现为原始块设备
  * 存储空间 动态分配              * 固定大小 (refreservation)
  * 不可再格式化                  * 可构建 UFS / ext4 / NTFS
               ...
               ─────────────        ─────────────        ─────────────────────
               @snap (只读)         #bkmk (元数据)◄────── 从快照派生的可写数据集
               回滚 / 克隆源        Send 增量参照         与原快照共享数据块 (COW)
...
数据在各顶级 VDEV 间条带化，冗余在单个 VDEV 内实现
...
────────────────────────────────────────────────────────────────────────────
  ...
  物理磁盘                    磁盘分区                普通文件
  /dev/nvme0                 /dev/nvme0p3             /tmp/vdev
  推荐使用整盘               一般不推荐               测试用途
                             GEOM 保证缓存一致性      无掉电保护

注：FreeBSD 下 GEOM 提供 BIO_FLUSH 一致性，分区与整盘通常无性能差异。
```
