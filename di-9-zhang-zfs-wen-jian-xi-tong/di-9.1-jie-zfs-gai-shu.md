...
| 校验和 | 每个块均附有校验和。使用的校验和算法是每个数据集的属性。读取时校验和会自动透明验证，使 ZFS 能检测静默数据损坏。若读取数据与预期校验和不匹配，ZFS 会尝试从可用冗余（如镜像或 RAID-Z）中恢复数据。可通过 `scrub` 触发所有校验和的全面验证。支持的校验和算法包括 fletcher2、fletcher4 和 sha256，其中 `fletcher` 速度更快，`sha256` 为强加密哈希，碰撞概率更低但性能稍逊。可禁用校验和，但极不推荐。 |
