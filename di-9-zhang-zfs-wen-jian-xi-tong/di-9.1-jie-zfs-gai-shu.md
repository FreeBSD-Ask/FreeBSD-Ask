...
- **校验和**：每个数据块均附带校验和，所用算法为数据集属性（通过 `zfs set` 设置）。读取时校验和自动透明验证，使 ZFS 能够检测静默数据损坏。若数据与预期校验和不匹配，ZFS 会尝试从可用冗余（如镜像或 RAID-Z）中恢复。`scrub` 可触发全量校验和验证。支持的校验和算法包括 `fletcher2`、`fletcher4`（当前默认）、`sha256`、`sha512`（SHA-512/256，64 位硬件上比 SHA-256 快约 50%）、`skein`（SHA-3 候选算法，64 位硬件上比 SHA-256 快约 80%）、`edonr`（高性能 SHA-3 候选算法，比 SHA-256 快约 350%）和 `blake3`（高性能安全哈希，针对现代 CPU 优化）。其中 `fletcher` 类算法速度更快但不提供密码学安全性，`sha256` 及以上为强加密哈希，碰撞概率更低。`skein` 和 `edonr` 采用带盐校验和（salted checksum），以池内存储的 256 位随机密钥预置种子，可防止重复数据删除系统中的哈希碰撞攻击。虽可禁用校验和，但极不推荐。
- **压缩**：每个数据集均有压缩属性，默认为关闭。启用压缩算法后，所有写入数据将被压缩。除节省空间外，由于读写块数减少，吞吐量通常也会提升。
...
- **复制**：`copies` 属性设置为大于 1 的值时，ZFS 将在文件系统或卷中为每个块维护多份副本。为重要数据集设置此属性可提供额外冗余，以便从校验和不匹配的块中恢复。在无冗余的池中，副本功能是唯一的冗余形式。它可恢复单个坏扇区或其他小范围故障，但不保护池免受整盘丢失的影响。注意：加密数据集不支持 `copies=3`，最高仅支持 `copies=2`。
