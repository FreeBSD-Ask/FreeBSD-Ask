...
- **设备状态**：
  ...
  - **DEGRADED**：设备出现过多校验和错误或 I/O 错误，但因仍有足够冗余，系统继续降级使用该设备。
...
- **压缩**：每个数据集均有压缩属性，默认为关闭（`off`）。启用压缩算法后，所有新写入数据将被压缩。除节省空间外，由于读写块数减少，吞吐量通常也会提升。任何非 `off` 的压缩设置均会自动检测全零块（NUL 字节）并存储为空洞（hole）。压缩后的数据会按扇区（2^`ashift` 字节）向上取整；若节省空间不足一个扇区，该块将不压缩存储。
  ...
  - **ZSTD**：一种高性能压缩算法（GUID：`org.freebsd:zstd_compress`），兼具高压缩比与高速度。相比 GZIP 在更高速度下提供略好的压缩比，相比 LZ4 提供更好的压缩比而速度仅略慢。压缩速度约 250–500 MB/s/线程，解压速度超过 1 GB/s/线程。可通过 `zstd-N`（N=1~19）指定压缩级别，`zstd-fast-N` 指定快速模式，默认级别为 3。
  - **GZIP**：流行压缩算法，主要优势在于可配置压缩级别。设置 `compress` 属性时，管理员可从 `gzip1`（最低）到 `gzip9`（最高）之间选择，以在 CPU 时间与磁盘空间之间取得平衡。
  - **ZLE**：零长度编码，仅压缩连续零块，适用于包含大量零块的数据集。

- **加密**：ZFS 支持原生数据集级别加密（GUID：`com.datto:encryption`）。启用加密后，ZFS 将对文件与 ZVOL 数据、文件属性、ACL、权限位、目录列表、FUID 映射及用户/组/项目空间使用数据进行加密。支持的加密套件包括 `aes-128-ccm`、`aes-192-ccm`、`aes-256-ccm`、`aes-128-gcm`、`aes-192-gcm` 和 `aes-256-gcm`（当前默认）。加密密钥支持三种格式：原始密钥（raw、32 字节随机生成）、十六进制密钥（hex）和口令短语（passphrase，经 PBKDF2 处理）。密钥可存储于本地文件、通过 HTTPS/HTTP 远程获取，或通过命令行提示输入。

- **复制**：`copies` 属性设置为大于 1 的值时，ZFS 将在文件系统或卷中为每个块维护多份副本（存储在可能的不同磁盘上）。为重要数据集设置此属性可提供额外冗余，以便从校验和不匹配的块中恢复。在无冗余的池中，副本功能是唯一的冗余形式。它可恢复单个坏扇区或其他小范围故障，但不保护池免受整盘丢失的影响。**警告：** 不要将 `copies=2` 设置在双盘条纹池中当作冗余——任何一块磁盘故障都会导致整池无法导入，所有数据将丢失。注意：加密数据集不支持 `copies=3`，最高仅支持 `copies=2`。

- **重复数据删除**：校验和机制使写入时能够检测重复块。通过重复数据删除，已存在块的引用计数递增，从而节省存储空间。ZFS 将重复数据删除表（DDT）存储在磁盘上并在内存（ARC）中缓存，写入时通过它检测重复块，表内包含唯一校验和列表、块位置及引用计数。写入新数据时，ZFS 计算校验和并与列表比对，匹配时复用现有块。`sha256` 校验和配合重复数据删除提供安全的加密哈希。`dedup` 设为 `on` 时，校验和匹配即视为数据相同；设为 `verify` 时，ZFS 将逐字节比对确保一致，发现差异则记录哈希冲突并分别存储。DDT 须存储每个唯一块的哈希，内存消耗巨大，经验值为每重复数据删除 1 TB 数据约需 5–6 GB 内存。若内存不足以容纳完整 DDT，每次写入前须从磁盘读取 DDT，性能将严重下降。重复数据删除可利用 L2ARC 存储 DDT，作为内存与磁盘之间的折中方案。建议优先考虑压缩，通常能以相近的空间节省效果避免额外的内存开销。
...
- **重建 (Resilver)**：更换故障磁盘时，ZFS 须将丢失数据填充到新磁盘中。重建即利用剩余磁盘上的奇偶校验信息计算丢失数据并写入新磁盘的过程。
...
- **配额**：
  ...
  - **用户配额**：限制指定用户使用的空间量。
  - **组配额**：限制指定组可以使用的空间量。
  - **项目配额**：限制指定项目的空间使用量，通过项目 ID 进行标识。与用户和组配额类似，项目配额可对属于同一项目的文件和目录进行空间核算和限制。项目 ID 通过 `chmod` 或 `zfs project` 命令设置。
...
- **保留**：
  ...
  - **参考预留 (refreservation)**：`refreservation` 属性可为特定数据集保留一定空间，但不包括其后代数据集。例如在 `storage/home/bob` 上设置 10 GB 保留后，即使其他数据集尝试占用空闲空间，仍会为该数据集保留至少 10 GB。与常规 `reservation` 不同，快照和后代数据集所占空间不计入该保留。若对 `storage/home/bob` 创建快照，除 `refreservation` 占用的空间外，必须有足够磁盘空间才能成功完成。主数据集的后代不占用 `refreservation` 空间。
