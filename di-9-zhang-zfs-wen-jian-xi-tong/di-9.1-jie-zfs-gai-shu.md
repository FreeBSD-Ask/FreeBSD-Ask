...
类型：filesystem | volume | snapshot | bookmark | clone
  ...
  ─────────────────────            ─────────────────────
  * 挂载为 POSIX 目录树            * 呈现为 原始块设备
  * 存储空间 动态分配              * 固定大小 (refreservation)
  * 不可再 格式化                  * 可构建 UFS / ext4 / NTFS

                ───────────────┬───────────────
                               │

               快照 Snapshot        书签 Bookmark        克隆 (Clone)
               ─────────────        ─────────────        ─────────────────────
               @snap (只读)         #bkmk (元数据)◄────── 从快照派生 的 可写数据集
               回滚 / 克隆源        Send 增量参照        与原快照 共享数据块 (COW)

核心特性：COW / Checksum / Compression / Encryption / Block Cloning (BRT)
...
顶级 VDEV (Top-level VDEVs) × N
数据在各 顶级 VDEV 间 条带化，冗余 在 单个 VDEV 内实现

                              虚拟设备层 (VDEV Layer)
────────────────────────────────────────────────────────────────────────────

[数据设备 Data VDEVs]            [分配类设备 Allocation Classes]
...
设备状态：ONLINE / OFFLINE / DEGRADED / FAULTED / REMOVED / UNAVAIL
...
────────────────────────────────────────────────────────────────────────────
  ...
  推荐 使用整盘              一般 不推荐                 测试 用途
                             GEOM 保证 缓存一致性       无 掉电保护

注：FreeBSD 下 GEOM 提供 BIO_FLUSH 一致性，分区 与 整盘 通常 无 性能差异。
```
