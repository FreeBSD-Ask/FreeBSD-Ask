...
- **配额**：
  ...
  - **用户配额**：限制指定用户使用的空间量。
  - **组配额**：限制指定组可以使用的空间量。
  - **项目配额**：限制指定项目的空间使用量，通过项目 ID 进行标识。与用户和组配额类似，项目配额可对属于同一项目的文件和目录进行空间核算和限制。项目 ID 在 FreeBSD 上通过 `zfs project -s` 命令设置。
...
- **池检查点（Pool Checkpoint）**：在执行包含破坏性操作的关键流程（如 `zfs destroy`）之前，管理员可对池状态建立检查点，并在出错时将整个池回滚到检查点。检查点类似池级快照，包含池的所有状态（属性、vdev 配置等）。存在检查点时，vdev 移除/附加/分离、镜像拆分、更改池 GUID 等操作不可执行。添加新 vdev 虽被允许，但回滚后需重新添加。回滚后检查点将被永久删除。注意：存在检查点时数据集保留可能无法强制生效，且已被释放的检查点数据不会被 scrub 扫描。
