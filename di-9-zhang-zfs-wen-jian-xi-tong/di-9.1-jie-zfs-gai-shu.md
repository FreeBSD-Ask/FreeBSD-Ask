...
```
                              ┌──────────────┐   创建/回滚    ┌──────────────────┐    派生     ┌──────────────┐
                              │数据集(Dataset)│◄─────────────►│ 快照 (Snapshot)   │───────────►│  克隆 (Clone) │
                              │              │              │  pool/fs@snap    │           │  可写副本      │
                              │ COW(写时复制) │              │  只读历史状态     │           └──────────────┘
                              │ Checksum(校验)│              └────────┬────────┘
                              │ Compression  │                       │
                ...
                │            存储池 (zpool / Pool)          │
                │                                         │
                │  Scrub(清洗)──►全量校验所有块  依据手册    │
                │  Resilver(重建)──►故障后重建冗余数据      │
                │                                         │
                │  ┌──────────────────────────────────┐   │
                │  │            Root VDEV              │   │
                │  └────────────────┬─────────────────┘   │
   ...
   │   │                     数据 VDEV (Data VDEVs)                      │  │
   │   │  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐          │  │
   │   │  │磁盘 (Disk)│ │镜像(Mirror)│ │  RAID-Z  │ │  dRAID   │          │  │
   │   │  │ 单盘/分区  │ │ 2+ 磁盘   │ │raidz1/2/3│ │draid1/2/3│          │  │
   │   │  │  无冗余    │ │ N-1 冗余  │ │分布式奇偶 │ │集成热备   │          │  │
   │   │  └─────┬────┘ └──────────┘ └──────────┘ └──────────┘          │  │
   │   │        │                                                        │  │
   │   │        │         活动设备故障时自动替换                             │  │
   │   │        └───────────────────────────┐                            │  │
   ...
   │                                                                       │
   │   ┌────────────────────────────────────────────────────────────────┐  │
   │   │                     辅助 VDEV (Support VDEVs)                   │  │
   │   │   ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐       │  │
   │   │   │日志 (Log) │  │缓存(Cache)│  │特殊(Special)│ │去重(Dedup)│       │  │
   │   │   │  ZIL 设备 │  │  L2ARC   │  │ 元数据加速 │  │ DDT 存储  │       │  │
   │   │   └────┬─────┘  └────┬─────┘  └────┬─────┘  └────┬─────┘       │  │
   │   │        │             │            │            │               │  │
   │   │        ▼             ▼            ▼            ▼               │  │
   │   │   同步写入加速   扩展读缓存   元数据/小文件   去重表存储           │  │
   │   │   降低写延迟    弥补ARC不足  存放于SSD      独立于普通VDEV       │  │
   │   └────────────────────────────────────────────────────────────────┘  │
   │                                                                       │
   │  设备状态: ONLINE(在线) / OFFLINE(离线) / DEGRADED(降级) /               │
   │            FAULTED(故障) / REMOVED(已移除) / UNAVAIL(不可用)             │
   └────────────────────────────────┬──────────────────────────────────────┘
                                    │
   ┌────────────────────────────────┼──────────────────────────────────────┐
   │                      物理存储层 (Physical)                              │
   │                                │                                      │
   │      ┌────────────┐   ┌────────────┐   ┌────────────┐                │
   │      │ 硬盘 (Disk) │   │分区(Partition)│  │ 文件 (File) │                │
   │      │ /dev/ada0   │   │ /dev/ada0p3 │   │ /path/file  │                │
   │      │ /dev/da0    │   │             │   │  仅测试用    │                │
   │      │ 推荐使用整盘 │   │             │   │             │                │
   │      └────────────┘   └────────────┘   └────────────┘                │
   └──────────────────────────────────────────────────────────────────────┘
