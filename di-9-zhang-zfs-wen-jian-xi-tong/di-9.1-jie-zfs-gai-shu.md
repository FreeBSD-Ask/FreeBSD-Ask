...
| 离线 | 管理员可将单个设备设为 `Offline` 状态，前提是存在足够冗余以维持池或 vdev 可用。此操作通常用于准备更换磁盘或便于识别设备。 |
| 降级 | 处于 `Degraded` 状态的池或 vdev 有一个或多个磁盘丢失或故障，池仍可使用，但若再有设备故障可能导致不可恢复。重新连接丢失设备或更换故障磁盘并完成重新同步后，池将恢复 `Online` 状态。 |
| 故障 | 处于 `Faulted` 状态的池或 vdev 已不可操作、数据无法访问。当丢失或故障设备数量超过 vdev 冗余上限时进入此状态。若重新连接丢失设备，池将恢复 `Online`；若冗余不足以弥补故障磁盘数量，池中数据将永久丢失，须从备份恢复。 |
