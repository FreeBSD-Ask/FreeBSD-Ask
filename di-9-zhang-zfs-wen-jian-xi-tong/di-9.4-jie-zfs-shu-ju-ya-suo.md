...
另一项重要优势来自压缩 ARC 功能——ZFS 的自适应替换缓存（ARC）会将数据的压缩版本缓存到 RAM 中，并在每次请求时解压。这使得相同容量的缓存能够存储更多的逻辑数据和元数据，从而提升缓存命中率与访问性能。如果从 LZ4 升级到 Zstd 增加了磁盘上的压缩比，这些增益会直接放大压缩 ARC 中每个字节的效能。
