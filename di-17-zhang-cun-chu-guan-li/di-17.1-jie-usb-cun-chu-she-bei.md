...
许多外部存储解决方案，如硬盘、USB 闪存设备、CD 和 DVD 刻录机，均采用通用串行总线（USB）。FreeBSD 支持 USB 1.x、2.0 和 3.0 设备。
...
若要验证 USB 配置，插入 USB 设备。使用 `dmesg` 命令确认设备出现在系统消息缓冲区中，输出类似如下：
...
> **警告**
>
> 虽然启用 `vfs.usermount` 会让不受信任的用户挂载任意介质，但从安全角度而言并不推荐。大多数文件系统并非为防范恶意设备而设计。
