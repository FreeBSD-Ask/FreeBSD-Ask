...
> **注意**
>
> `automount` 对普通用户的权限控制较为有限，主要以 root 身份执行挂载操作；如果需要更精细的权限控制机制，建议使用 DSBMD。
...
* **-media**
  查询还未挂载但包含有效文件系统的设备。通常用于访问可移动媒体上的文件。
...
此后，将以下内容添加到设备状态监测进程文件 **/etc/devd.conf** 中：
...
在 **/etc/auto_master** 中 **/media** 行末尾追加 `-L=zh_CN.UTF-8`。需注意 `-L=zh_CN.UTF-8` 是 mount_msdosfs(8) 的专有参数，而其他文件系统的挂载程序无法识别该参数，将导致其他设备无法挂载。
...
客户端可根据实际需求选择安装其中之一。桌面环境下推荐使用 `dsbmc` 图形界面客户端，便于操作。
...
该配置以后台方式启动 `dsbmc-cli` 并启用自动连接。`-a` 参数用于启用自动连接模式，客户端持续监控设备事件并自动执行相应操作，此方式不提供图形界面提示。
...
- FreeBSD Project. autofs -- filesystem automounter[EB/OL]. [2026-04-14]. <https://man.freebsd.org/cgi/man.cgi?query=autofs&sektion=4&manpath=FreeBSD+16.0-RELEASE>. autofs 文件系统手册页，描述自动挂载机制。
- FreeBSD Project. auto_master -- auto_master database[EB/OL]. [2026-04-14]. <https://man.freebsd.org/cgi/man.cgi?query=auto_master&sektion=5&manpath=FreeBSD+16.0-RELEASE>. 自动挂载主配置文件格式手册页。
- FreeBSD Project. automountd -- daemon handling autofs mount requests[EB/OL]. [2026-04-14]. <https://man.freebsd.org/cgi/man.cgi?query=automountd&sektion=8&manpath=FreeBSD+16.0-RELEASE>. 自动挂载守护进程手册页。
- FreeBSD Project. mount -- mount file systems[EB/OL]. [2026-04-14]. <https://man.freebsd.org/cgi/man.cgi?query=mount&sektion=8&manpath=FreeBSD+16.0-RELEASE>. 文件系统挂载命令手册页。
- FreeBSD Project. fstab -- static information about the filesystems[EB/OL]. [2026-04-14]. <https://man.freebsd.org/cgi/man.cgi?query=fstab&sektion=5&manpath=FreeBSD+16.0-RELEASE>. 文件系统表配置文件格式手册页。
