...
创建一个大小为 8 GiB 的交换文件 **/usr/swap0**（1 GiB = 1024 MiB，如需更大容量，可按此比例换算）：

```sh
# dd if=/dev/zero of=/usr/swap0 bs=1M count=8192 status=progress  # bs=1M 表示使用 1 MiB 块写入零；status=progress 用于显示写入进度
