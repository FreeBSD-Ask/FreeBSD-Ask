...
- `LINT-NOVIMAGE`：调试开发用。LINT 变体文件，内容不足五行，禁用了 VIMAGE(9)[EB/OL]. [2026-03-26]. <https://man.freebsd.org/cgi/man.cgi?query=vimage&sektion=9>（实际上是 VNET(9) 的别名，网络子系统虚拟化基础设施）。参见：sys: add LINT-NOVIMAGE[EB/OL]. [2026-03-26]. <https://reviews.freebsd.org/D50780>
...
`std.debug` 文件的实际文本内容位于 [sys/conf/std.debug](https://github.com/freebsd/freebsd-src/blob/main/sys/conf/std.debug)。撰写本文时，`std.debug` 文件约包含 20 行。
