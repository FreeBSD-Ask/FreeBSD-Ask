# 24.5 GENERIC 内核选项注解（AMD64）

本节阐述 amd64 架构下 GENERIC 内核的配置选项。

GENERIC 内核作为 FreeBSD 发行版的默认配置，经过了广泛的硬件兼容性测试和性能优化，是绝大多数用户场景的首选配置。
...
为了便于说明，并避免引入对大多数非开发者无用的调试选项，本节选择 15.0-RELEASE（releng/15.0 分支）的 GENERIC 内核选项进行说明。
...
1. GENERIC 内核默认使用哪种调度器？
2. 哪个选项用于兼容 i386 二进制程序？
3. bpf 设备为 DHCP 提供了什么支持？
