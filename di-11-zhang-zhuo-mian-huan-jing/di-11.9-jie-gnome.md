...
GNOME 曾是 GNU 项目的组成部分，目标是提供一套功能完备的桌面环境，目前主要由 Red Hat 主导开发与维护。GNOME 是 Linux 与类 UNIX 系统中广泛采用的主流桌面环境，以简洁现代的设计理念著称。需注意的是，在英语发音中，GNOME 的首字母 `G` 不发音（音标为 /ˈnoʊm/）。
...
FreeBSD 安装桌面环境后，默认采用简洁朴素的色调，该风格可能与部分用户的预期不符。以下内容适用于基于 `GTK` 库的桌面环境，涵盖 `GNOME`、`Xfce`、`Cinnamon` 和 `LXDE` 等。
