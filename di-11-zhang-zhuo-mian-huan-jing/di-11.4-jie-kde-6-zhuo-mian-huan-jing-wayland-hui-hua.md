...
由于 FreeBSD DRM 驱动移植仅覆盖了 Intel、AMD 和 NVIDIA 等 GPU，vmwgfx 和 virtio 等虚拟化 GPU 驱动尚未得到支持（参见：freebsd/drm-kmod. Request to restore support for vboxvideo and vmwgfx DRM drivers #356[EB/OL]. [2026-04-04]. <https://github.com/freebsd/drm-kmod/issues/356>），因此在 VMware、VirtualBox 或任何基于 Virtio 的虚拟机中均无法复现本节内容，需要在真实物理机上进行操作。
...
初次进入 KDE Wayland 桌面时，KDE 会在右下角提示需在设置的虚拟键盘中进行配置才能启用输入法。请留意该提示。如果没有进行此设置，将无法切换输入法或输入中文。
