...
要理解 FreeBSD 对 Linux 应用程序的兼容机制，需要考察 Linuxulator 这一核心组件。Linuxulator 这一术语的字面含义为“Linux Emulator”，该名称极易引起误解，使人将其与传统意义上的指令集模拟器相混淆。然而，Linuxulator 并非传统模拟器范畴，它甚至不是一个具有独立可执行文件形态的 FreeBSD 用户空间应用程序。确切地说，Linuxulator 只是 FreeBSD 官方文档中对某一特定内核模块的非正式称谓，该内核模块的正式标识符为 `linux`。
