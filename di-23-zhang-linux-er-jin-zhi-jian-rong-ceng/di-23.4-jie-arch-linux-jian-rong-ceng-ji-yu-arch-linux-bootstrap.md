# 23.4 Arch Linux 兼容层（基于 Arch Linux bootstrap）
...
Arch Linux 兼容层占用的系统资源略高于 Ubuntu 兼容层，这是因为系统中正在运行谷歌 Chrome 浏览器。
...
由于新安装的 Arch 没有任何文本编辑器，需要在 FreeBSD 中编辑相关文件，设置 Arch Linux 的 pacman 使用清华大学镜像源：
...
需要为 pacman 启用 DisableSandbox，否则将触发错误 `error: restricting filesystem access failed because landlock is not supported by the kernel!`，因为 FreeBSD 未实现此沙箱。
...
> **技巧**
>
> 如果为用户 `test` 设置了密码后仍提示密码错误，需要新开一个终端，执行 `reboot` 命令重启 FreeBSD，然后继续操作。
...
2. 在 Arch Linux 兼容层中，保持 `DisableSandbox` 启用或在 FreeBSD 内核中实现 landlock，以实现 pacman 的沙箱机制。
