...
在 FreeBSD 系统中，本地化环境变量可以在不同的配置文件中设置，根据所使用的登录管理器和 shell 的不同，配置文件的路径也会有所区别。
...
了解了配置文件的路径后，接下来需要掌握与本地化相关的环境变量。下面的变量用于控制环境本地化，这些变量大都是由 POSIX 规范定义的。
...
通过组合使用这些变量，用户可轻松地调整操作系统的语言、界面消息以及本地化相关设置，以适应不同的地域和语言环境。
