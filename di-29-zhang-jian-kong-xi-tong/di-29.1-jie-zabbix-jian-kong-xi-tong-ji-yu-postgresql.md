# 29.1 Zabbix 监控系统（基于 PostgreSQL）
...
上述安装过程将自动安装 PHP，本节采用的版本为 PHP 8.4（Zabbix 7.0.10 起已正式支持 PHP 8.4）。具体可用版本可查阅 [zabbix7-frontend](https://www.freshports.org/net-mgmt/zabbix7-frontend)，该站点提供最新的包信息与兼容性说明。
...
DBPassword=your_strong_password  # 数据库用户密码（请与上方创建用户时设置的密码保持一致）
