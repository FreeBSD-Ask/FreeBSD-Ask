...
客户和用户期望其数据得到保护，因此 CIA 三要素是计算机安全的基石。例如，客户期望其信用卡信息得到安全存储（机密性），他们的订单不会被暗中篡改（完整性），并且他们随时可以访问订单信息（可用性）。
...
系统和网络可能遭到未授权访问，有时是意外发生的，也可能是远程攻击者所为，甚至可能由企业间谍活动或前员工所为。用户应当在安全漏洞发生时做好准备，承认自己的错误，同时将可能的问题报告给安全团队。作为管理员，了解威胁并做好应对准备同样重要。
...
作为 FreeBSD 用户，理解如何防范攻击和入侵是不可或缺的。
