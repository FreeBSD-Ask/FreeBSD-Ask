...
①手写 10 万字以上悔过书（要求使用“简体中文，现代汉语”，且字数不含标点符号）。必须予以录制视频（1080P 30FPS 为底线，要求看清所写内容与双手），以防机器抄写，且不允许视频快放。同时将电子版附上以便于查重。我们将视其情节严重与悔过程度（文本重复率等）考虑解禁，并将在 30 个工作日内给出解禁与否的回复。悔过书和视频以及电子文本请提交到 issue，附上被封渠道与账号。请注意，每次我们只能解禁一个渠道的账号，也即一份悔过书只能解禁一个平台的一个账号。每个人终身只有两次申诉的机会（包括申诉失败）。
