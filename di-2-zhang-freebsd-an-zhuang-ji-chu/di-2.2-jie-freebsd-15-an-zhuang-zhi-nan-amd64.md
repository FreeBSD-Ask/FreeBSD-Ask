...
> **技巧**
>
> 经测试，在 UEFI 环境下，256 MB 内存也可以运行 ZFS；如果使用传统 BIOS，128 MB 内存即可。但此配置仅能完成安装，不具备实用价值。
...
>
> 此问题可能与分区表调整有关（参见 FreeBSD 中文文档项目. FreeBSD 手册：磁盘分区调整[EB/OL]. [2026-03-25]. <https://handbook.bsdcn.org>。该章节介绍磁盘分区调整方法），可能由程序缺陷所致。
...
> **注意**
>
> 请慎重设置交换分区（`Swap Size`）的大小。对于现代系统，交换分区一般设置为 2～8 GB 即可满足多数场景；若物理内存充裕（如 16 GB 以上）且无休眠到磁盘的需求，可直接设为 `0` 跳过创建。物理内存较小的旧式系统如需运行大型应用，可视情况适当增大。因 ZFS 和 UFS 文件系统创建后不易缩小，而使用 `dd` 命令创建交换文件或后续调整可能带来性能开销或复杂度。
...
> **注意**
>
> 若硬盘为 eMMC 类型，可能会出现 `mmcsd0`、`mmcsd0boot0`、`mmcsd0boot1` 等选项，请选择 `mmcsd0`。此外，在多硬盘与 eMMC 共存的情况下，如果另一块硬盘的分区数量超过 5 个，安装在 eMMC 中的 FreeBSD 可能会在启动时停留在 `Mounting from zfs:zroot/ROOT/default failed with error 22: retrying for 3 more seconds` 提示处。如果手动指定参数，则可能导致内核恐慌（Panic）。此问题可能为程序缺陷，但目前尚无更详细的报告信息。
...
如果在安装时对 ZFS 根分区启用了加密，后续如何挂载该磁盘？
