...
FreeBSD 15.0-RELEASE 使用 bsdinstall 工具进行安装，流程涵盖磁盘分区、网络配置、系统组件选择与用户账户创建。本节以 AMD64 架构的 disc1.iso 镜像为例，记录从系统启动到首次登录的完整操作。
...
>
> 若为物理机安装，可考虑使用 Rufus <https://rufus.ie/zh/> 工具配合 <https://download.freebsd.org/ftp/releases/amd64/amd64/ISO-IMAGES/15.0/FreeBSD-15.0-RELEASE-amd64-memstick.img> 镜像进行刻录。该文件为 FreeBSD 15.0-RELEASE amd64 架构的 U 盘安装镜像。
...
- 由于系统直接从安装介质运行，性能可能明显弱于安装在硬盘上的系统。~~除非你的硬盘是机械硬盘，外接了固态 U 盘~~
...
- 可能影响稳定性：基本系统被破坏的风险有所增加（该问题在社区广泛使用后反馈增多，逐渐受到重视）；
...
| `M Mirror Swap? NO` | 交换分区镜像？否 | 是否在多块磁盘之间镜像交换分区，若选否，则每块磁盘的交换分区是独立的 |
| `W Encrypt Swap? NO` | 加密交换分区？否 | |

> **技巧**
>
> 若将此项设置为 `GPT (UEFI)` 而非其他，后续分区与系统更新过程会更加简单。
...
> **注意**
>
> 若硬盘为 eMMC 类型，可能会出现 `mmcsd0`、`mmcsd0boot0`、`mmcsd0boot1` 等选项，请选择 `mmcsd0`。此外，在多硬盘与 eMMC 共存的情况下，若另一块硬盘的分区数量超过 5 个，安装在 eMMC 中的 FreeBSD 可能会在启动时停留在 `Mounting from zfs:zroot/ROOT/default failed with error 22: retrying for 3 more seconds` 提示处。若手动指定参数，则可能导致内核恐慌（Panic）。此问题可能为程序缺陷，但目前尚无更详细的报告信息。
...
如果需要要在 LiveCD 环境中挂载该加密分区，操作较为便捷（不需要密钥）。假设根分区为 **/dev/nda0p2**，执行以下命令：
