...
桥接是宿主机与虚拟机互通的便捷方式，虚拟机可获得与宿主机处于同一网段的 IP 地址。例如，若宿主机 IP 为 `192.168.5.123`，则虚拟机 IP 将为 `192.168.5.x`。
...
当桥接模式无法生效时，可改用双网卡方案。若要通过宿主机（如 Windows 11）控制虚拟机中的 FreeBSD 系统，需配置两块网卡：一块为 NAT 模式，用于连接互联网；另一块为仅主机模式，用于与宿主机互通。
...
执行 `ifconfig` 查看网络状态。若第二块网卡 `em1` 未获取 IP 地址，可通过 DHCP 临时获取：`dhclient em1`。为保证长期有效，可在 **/etc/rc.conf** 中添加 `ifconfig_em1="DHCP"`。
