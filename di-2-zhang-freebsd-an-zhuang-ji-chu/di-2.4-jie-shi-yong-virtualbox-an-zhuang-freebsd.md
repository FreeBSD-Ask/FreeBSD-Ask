...
Oracle VirtualBox 是 Type-2 虚拟机监视器（Hypervisor），通过虚拟设备模拟（device emulation）和半虚拟化（paravirtualization）技术为虚拟机提供计算、存储和网络资源。VirtualBox 支持多种虚拟磁盘镜像格式，默认使用 VDI（Virtual Disk Image），也兼容 VMDK（VMware）、VHD（Microsoft）等格式。
...
2. 对比 VirtualBox、VMware Workstation 与 Hyper-V 在虚拟化 FreeBSD 时对 UEFI 固件行为的模拟差异，设计实验方案测量三种平台下 EFI 运行时服务的接口一致性。
3. 分析 VirtualBox Guest Additions 与 FreeBSD 内核模块的加载优先级冲突（参见 `vboxguest.ko`），提出一套最小修改方案以兼容 VirtualBox 7.1.x 与 FreeBSD 的当前发行版。
