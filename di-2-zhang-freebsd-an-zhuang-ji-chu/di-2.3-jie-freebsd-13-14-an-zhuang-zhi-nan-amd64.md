...
本节聚焦于 FreeBSD 13.x 和 14.x 系列在 AMD64 架构下的标准化安装流程。其安装流程与 15.x 存在差异。以下介绍基于 bsdinstall 安装工具，提供从系统启动到首次登录的完整操作指南。
...
> **警告**
>
> 若为物理机安装，可考虑使用 Rufus. Rufus - Create bootable USB drives the easy way[EB/OL]. [2026-03-25]. <https://rufus.ie/zh/> 工具配合 FreeBSD Project. FreeBSD-14.3-RELEASE-amd64-memstick.img[EB/OL]. [2026-03-25]. <https://download.freebsd.org/ftp/releases/amd64/amd64/ISO-IMAGES/14.3/FreeBSD-14.3-RELEASE-amd64-memstick.img>。该文件为 FreeBSD 14.3-RELEASE amd64 架构的 U 盘安装镜像。
...
![启动界面](../.gitbook/assets/boot-screen.png)

在此界面无需任何操作，等待十秒可自动进入 `1. Boot Installer [Enter]`；也可直接按 **回车键** 进入。

若按下任意其他键将暂停启动流程，此时可按数字键继续，或按 **ESC** 键进入 **OK 提示符**。在 **OK 提示符** 下输入 `menu` 并按 **回车键** 可返回主菜单。输入 `?` 可查看所有可用命令。
...
![键盘布局选择](../.gitbook/assets/keyboard-layout.png)
...
![选择安装组件](../.gitbook/assets/select-components.png)
...
![磁盘分区菜单](../.gitbook/assets/disk-partition-menu.png)
...
![ZFS 配置](../.gitbook/assets/zfs-config.png)
...
![ZFS 配置 2](../.gitbook/assets/zfs-config-2.png)
...
![选择虚拟设备类型](../.gitbook/assets/zfs-vdev-type.png)
...
![选择目标硬盘](../.gitbook/assets/zfs-target-disk.png)
...
![确认格式化](../.gitbook/assets/zfs-confirm-format.png)
...
![设置 root 密码](../.gitbook/assets/set-root-password.png)
...
![选择网络接口](../.gitbook/assets/select-network-if.png)
...
![选择启动服务](../.gitbook/assets/select-services.png)
...
>
> ![固件安装](../.gitbook/assets/firmware-install-prompt.png)
>
> **此图片来自物理机安装界面（使用采集卡）。**
...
![填写用户信息](../.gitbook/assets/user-info.png)
...
![完成安装](../.gitbook/assets/install-complete.png)
...
![是否进入 Shell](../.gitbook/assets/enter-shell.png)
...
![系统启动](../.gitbook/assets/system-boot.png)
...
![登录界面](../.gitbook/assets/login-screen.png)

输入用户名 `root` 及安装时设置的 root 密码以登录系统。

> **技巧**
>
> 密码是不会显示在屏幕上的（包括 `*` 号），即屏幕无任何显示，输入完毕后直接按回车键即可。

![登录成功](../.gitbook/assets/login-success.png)
