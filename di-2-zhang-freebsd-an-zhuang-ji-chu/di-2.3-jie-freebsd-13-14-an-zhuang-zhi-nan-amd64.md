...
FreeBSD 13.x 和 14.x 系列作为稳定的长期支持版本，在生产环境中仍被广泛采用。尽管其安装流程与 15.x 系列具有高度一致性，但在 pkgbase 支持等特性上存在版本差异。以下是针对这些版本的标准化安装指南。
...
> **技巧**
>
> 注意观察图片中的红色加粗大写首字母，如 `Install`、`Shell` 和 `Live System` 中的 **`I`**、**`S`**、**`L`** 分别是红色加粗大写的。若直接按键盘上面的对应按键（无论大小写），均会选定并直接进入该界面。
...
`请选择此机器的主机名。如果正运行在受管理的网络上，请向的网络管理员询问合适的名称。`
...
![ZFS 配置2](../.gitbook/assets/ins8.2.png)
...
- ① 如果用户全名为空（即不设置），系统会分配一个默认值 `User &`。这是早期 Unix 的行为（Gecos 字段）。由 FreeBSD Project. freebsd-src/usr.sbin/pw/pw_user.c[EB/OL]. [2026-03-25]. <https://github.com/freebsd/freebsd-src/blob/main/usr.sbin/pw/pw_user.c>，该文件包含用户管理工具 pw 的源代码，文件中的 `static struct passwd fakeuser` 这部分代码实现。
