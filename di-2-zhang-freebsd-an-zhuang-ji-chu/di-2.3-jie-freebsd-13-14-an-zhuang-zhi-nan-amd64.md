...
>
> 若出现 ACPI 相关报错，属于常见现象，多数情况下不影响系统使用。通常可通过更新 BIOS 解决。在极少数情况下，可能需要对 SSDT（辅助系统描述表）和 DSDT（差分系统描述表）进行修补，黑苹果用户可能对此过程较为熟悉。
...
- FreeBSD Project. Bug 286847: If the hostname is not set for the host, the value "Amnesiac" should be written to rc.conf.[EB/OL]. [2026-03-25]. <https://bugs.freebsd.org/bugzilla/show_bug.cgi?id=286847>。该 Bug 报告提出空主机名时应写入默认值，该 Bug 为笔者发现。
...
| `P Partition Scheme` | GPT (UEFI) 分区方案 | 只有老电脑才需要 `GPT (BIOS+UEFI)` 等选项 |
| `S Swap Size 2g` | 交换分区大小 2g | 如果不需要 Swap，`Swap Size` 输入 `0` 或 `0G` 即可不设置交换分区。 |
...
如果在安装 FreeBSD 的时候选择了 ZFS 磁盘加密，那么如何解密并挂载该磁盘呢？
...
此处输入 root 密码，密码会在屏幕上显示为 `*`（bsdinstall 的 dialog 界面特性；注意：系统登录时的密码输入则不会显示任何字符）。
...
使用中国标准时间：China Standard Time（CST），没有问题，按 **回车键** 选定 `Yes`。
...
> **警告**
>
> **请勿全选！** 尤其 **不要** 选择 `local_unbound`，否则可能影响系统 DNS 解析（参见 FreeBSD Project. Bug 262290: After a normal FreeBSD installation and reboot, /etc/resolv.conf will be changed[EB/OL]. [2026-03-25]. <https://bugs.freebsd.org/bugzilla/show_bug.cgi?id=262290>。该 Bug 报告记录 local_unbound 服务导致 resolv.conf 变更问题）。除非明确了解其用途。
...
- ① 如果用户全名为空（即不设置），系统会分配一个默认值 `User &`，其中 `&` 会自动展开为首字母大写的用户名（如用户 `ykla` 的全名将显示为 `User Ykla`）。这是早期 Unix 的 GECOS 字段行为。由 FreeBSD Project. freebsd-src/usr.sbin/pw/pw_user.c[EB/OL]. [2026-03-25]. <https://github.com/freebsd/freebsd-src/blob/main/usr.sbin/pw/pw_user.c>，该文件包含用户管理工具 pw 的源代码，文件中的 `static struct passwd fakeuser` 这部分代码实现。
