...
> **警告**
>
> KDE 6 Ports 默认通过 PulseAudio（脉冲音频，PA）在全局占用音频，请勿手动切换到其他音频后端（如 PipeWire），以免造成不必要的问题。

## 声音设置

声卡驱动 `snd_hda` 通常会默认加载；当默认内核未包含该驱动时，需要手动加载对应的内核模块以启用音频功能。
...
FreeBSD 中大部分软件的音频输出接口为 OSS（Open Sound System，开放声音系统）。有些软件默认使用 PulseAudio（如 Firefox，火狐浏览器），这些软件的相关设置请参见后文提示。Firefox 可以通过 `about:support` 页面查看当前使用的音频后端。Firefox 支持多种音频后端，可根据系统中已安装的后端按优先顺序自动选择，也可以手动指定。
...
上文中的 `pcm6: <Realtek ALC892 (Rear Digital)> (play)` 为数字输出接口。一般集成声卡的模拟接口采样率最高为 48kHz，数字接口最高采样率可以达到 192kHz。如果存在数字输出，但机箱面板上没有对应接口，而主板上提供了 S/PDIF（Sony/Philips Digital Interface，索尼/飞利浦数字接口）接口插针，可以安装一块 S/PDIF 挡板（通常包含 S/PDIF 光纤口和同轴口，价格较低），连接后即可使用。


## man 示例

以下节选自 [man snd_hda](https://man.freebsd.org/cgi/man.cgi?snd_hda) 官方手册页。
...
已知在 APU（加速处理单元，Accelerated Processing Unit）上使用 drm‑kmod 时，打开空播放器窗口可能会触发 mode 2 reset 报错（即驱动重置），进而导致 Kernel Panic（内核恐慌）。
