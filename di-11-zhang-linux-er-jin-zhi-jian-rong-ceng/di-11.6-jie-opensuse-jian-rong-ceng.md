...
使用 openSUSE Tumbleweed 时，dnf 和 zypper 均不可用。该问题不影响 Leap 对应的脚本，但影响 Tumbleweed 对应的脚本，导致无法正常使用。已提交 [Bug 报告](https://bugzilla.opensuse.org/show_bug.cgi?id=1213158)。
...
在 LXC Leap 中，用 rpm 替换 rpm-ndb 可以解决该问题，dnf 不受影响，该问题仅存在于 zypper 中。然而，在 openSUSE Tumbleweed LXC 中，dnf 和 zypper 都存在该问题，替换 rpm-ndb 无法解决。
