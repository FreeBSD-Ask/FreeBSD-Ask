...
以下介绍在 FreeBSD Jail 中部署 Debian 用户空间的步骤，含 debootstrap 的使用与初始化配置。
...
至此，基于 Debian 12 的 Linux Jail 已成功建立，同样的方法可用于创建基于不同版本的 Debian 或 Ubuntu 的多个 Jail。
...
antiX Linux 是基于 Debian 的轻量级发行版，采用 SysVinit 初始化系统而非 systemd。SysVinit 是传统的 UNIX 初始化系统，依赖运行级别（runlevels）和 init 脚本进行服务管理，于 Jail 环境中兼容性更好。
...
antiX 不使用 systemd，而是使用 rc 进行初始化。此处 **/etc/init.d/rc 3** 指定 antiX Jail 在运行级别 3 启动，因此 Debian Jail 中的服务启动问题不会出现，可直接在 Jail 启动时运行服务（如 sshd）。
...
> **警告**
>
> `xhost +` 会关闭访问控制，存在安全风险。更安全的方式是指定允许访问的 IP 地址。
