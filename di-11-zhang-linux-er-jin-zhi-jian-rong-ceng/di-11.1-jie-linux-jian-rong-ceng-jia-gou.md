...
在文件系统层面，Linux 兼容层实现了一套备用根路径重定向机制。当 Linux 进程请求路径查找时，系统会优先尝试在 **/compat/linux/** 路径下定位文件；若未命中，则回退至宿主系统的原生路径。由此，Linux 程序能够无缝加载其专有的共享库，同时在必要时仍能访问 FreeBSD 的系统资源。通过在 **/compat/linux** 中提供定制的 uname(1) 等工具，环境伪装得以闭环。
