...
1. 在 Gentoo 兼容层中修改 `FEATURES` 变量，移除 `-sandbox` 相关选项，验证 Portage 的行为变化。

2. 对比 Gentoo Portage 的 USE 标记与 FreeBSD Ports 的 OPTIONS 对话框在编译时功能选择层面的差异。

3. 在 FreeBSD 兼容层中安装一个需要编译的 Gentoo 软件包，记录完整的编译过程和遇到的问题。
