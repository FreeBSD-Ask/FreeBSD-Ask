...
**路由** 是使系统能够找到通往另一个系统的网络路径的机制。每条路由用一对地址定义，分别表示“目标”和“网关”。路由表明，当尝试连接指定目标时，数据包应经指定的网关发送。有三种目标类型：单台主机、子网和“默认”。“默认路由”在没有其他路由适用时使用。网关也有三种类型：单台主机、接口（也叫做链路）和以太网硬件（MAC）地址。已知的路由会存储在路由表中。

### 路由基础

通过 netstat(1) 可查看 FreeBSD 系统的路由表：
...
- **default**：
  指定了默认路由。当本地系统需要连接到远程主机时，它会检查路由表以确定是否有已知的路径。如果远程主机匹配表中的某个条目，系统将检查是否可以通过该条目指定的接口连接。
  默认路由在没有更具体前缀匹配时使用，不依赖于其他路径失败。对于本地局域网上的主机，默认路由中的 `Gateway` 字段指向直接接入互联网的系统。在读取此条目时，请确保 `Flags` 列指示网关是可用的（`UG`）。
...
RACK（Recent ACKnowledgment）是一种基于时间戳的快速丢包检测算法，最初源自 Google Yuchung Cheng、Neal Cardwell 等人的提案（标准化为 RFC 8985）。Netflix 的 Randall Stewart 将其实现为 FreeBSD 的 tcp_rack 栈，通过改进丢包恢复机制优化网络传输性能。与传统基于 4.4BSD 的 TCP 栈相比，RACK 在高延迟丢包时恢复速度更快，在现代网络环境中具有更好的性能表现。
...
TCP BBR（Bottleneck Bandwidth and Round-trip propagation time）是 Google 公司推出的一种基于模型的拥塞控制算法。与传统基于丢包的拥塞控制算法（如 CUBIC、NewReno）不同，BBR 通过主动探测网络路径的瓶颈带宽和往返传播时间构建网络性能模型，其核心优化目标包括：
...
FreeBSD 平台上的 TCP BBR 实现出自 Netflix 技术团队，作为 `tcp_bbr` 拥塞控制模块提供。部分网络场景下（如长距离传输、高带宽链路），启用 TCP BBR 可提升网络传输性能。
