...
**路由** 是系统寻找通往其他系统网络路径的机制。每条路由由一对地址定义，分别表示“目标”与“网关”。路由指明：连接指定目标时，应经由指定网关发送数据包。目标分为三种类型：单台主机、子网与“默认”。“默认路由”于其他路由均不适用时生效。网关同样分为三种类型：单台主机、接口（亦称为链路）与以太网硬件（MAC）地址。已知路由存储于路由表中。
...
- **default**：
  默认路由。本地系统需要连接远程主机时，检查路由表以确定是否存在已知路径。若远程主机匹配表中某条目，系统即检查是否可经由该条目所指定接口完成连接。
...
- **localhost**：第二条路由。`Netif` 列中为 `localhost` 指定的接口为 **lo0**，亦称为回环设备。所有发往该目标的流量均保留于本机内部，不通过网络发送。
...
传输控制协议（Transmission Control Protocol，TCP）是互联网协议族（Internet Protocol Suite）中的核心传输层协议，其软件实现体系称作 TCP 栈（因采用层次化结构组织，因此称"栈"）。Vint Cerf 和 Bob Kahn 于 1974 年在论文《A Protocol for Packet Network Intercommunication》中首次提出了 TCP 的核心思想（当时为传输与网络转发合一的单一协议），后经迭代，于 1981 年随 RFC 793 将 TCP 与 IP 拆分为两个独立协议。RFC 793 已于 2022 年 8 月被 RFC 9293 废止，后者为当前 TCP 协议的最新标准规范。
