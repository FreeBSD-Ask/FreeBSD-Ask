...
当地址空间分配给一个网络时，服务提供商会配置其路由表，以确保所有流量都发送至该站点的链路。那么，外部站点如何知道将数据包发送到网络的 ISP 呢？

互联网通过全球路由系统跟踪所有分配的地址空间，并定义它们与互联网主干网（承载互联网流量的核心线路）连接的节点。每台主干路由器都保存一份主路由表，将特定网络的流量指向对应的主干运营商，再经由一系列服务提供商逐级传递至目标网络。

服务提供商需向主干站点通告自身的连接点身份，从而成为流量的进入路径。这一过程称为路由传播。

有时，路由传播会出现问题，导致某些站点无法连接。查找路由断点的常用命令是 `traceroute`（IPv6 使用 `traceroute6`），尤其在 `ping` 失败时十分有用。
...
调整相关参数或可优化该性能差异，具体的参数调优方法尚需进一步研究与验证。
