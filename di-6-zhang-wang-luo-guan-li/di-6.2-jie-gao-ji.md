...
- **default**：
  ...
  默认路由于无更具体前缀匹配时采用，不依赖其他路径的成败。对于本地局域网上的主机，默认路由中的 `Gateway` 字段设置为可直接连接互联网的系统。读取该条目时，应确认 `Flags` 列指示网关可达（`UG`）。
...
在测试环境中，RACK 和 BBR 在局域网表现良好，但在互联网环境下带宽显著下降：RACK 约为默认栈的三分之一，BBR 约为默认栈的六分之一。该测试在特定网络条件下完成，实际性能可能因网络环境而异。
