...
- **子网**
  ...
  由 IPv4 路由表可知，该设备同时接入两个局域网：`192.168.5.0/24` 与 `192.168.179.0/24`。对应网关分别为 `link#2` 与 `link#1`，标志仅含 U（Up），表明二者均为“直连网络”。当该机器需与这两个网段内其他设备（如 `192.168.5.x` 范围内计算机）通信时，数据包无须经由路由器，而是直接分别通过 em1 与 em0 接口广播发送。
