...
**路由** 是系统寻找通往其他系统网络路径的机制。每条路由由一对地址定义，分别表示“目标”与“网关”。路由指明：连接指定目标时，应经由指定网关发送数据包。目标分为三种类型：单台主机、子网与“默认”。“默认路由”于其他路由均不适用时生效。网关同样分为三种类型：单台主机、接口（也称为链路）与以太网硬件（MAC）地址。已知路由存储于路由表中。
...
fe80::20c:29ff:fe8 link#3             UHS             lo0   # lo0 接口链路本地 IPv6 地址（与第 37 行重复，属路由表缓存机制的正常现象），静态主机路由
...
- **default**：
  默认路由。本地系统需要连接远程主机时，检查路由表以确定是否存在已知路径。如果远程主机匹配表中某条目，系统即检查是否可经由该条目所指定接口完成连接。
  默认路由在无更具体前缀匹配时采用，不依赖其他路径的成败。对于本地局域网上的主机，默认路由中的 `Gateway` 字段设置为可直接连接互联网的系统。读取该条目时，应确认 `Flags` 列指示网关可达（`UG`）。

  对于作为外部世界网关的机器，默认路由指向与互联网服务提供商（ISP）相连的网关。

  对于 IPv4 网络，默认网关为 `192.168.179.2`，标记为 UGS（Up、Gateway、Static），即所有发往未知外部网络（如互联网）的 IPv4 流量，均经 `em0` 接口发送至该路由器予以转发。

  IPv6 部分中，默认路由以链路本地地址 `fe80::5%em1` 指定下一跳，该下一跳通常为 ISP 提供的路由器。标志为 UG，表明所有未匹配的 IPv6 流量经 em1 接口发出。链路本地地址为 IPv6 特性，仅用于本地链路通信，并不等同于公网可达地址。

  可见该机器具备双栈网络能力，IPv4 与 IPv6 出口分别依赖不同网卡（em0 与 em1）。

- **localhost**：第二条路由。`Netif` 列中为 `localhost` 指定的接口为 **lo0**，也称为回环设备。所有发往该目标的流量均保留于本机内部，不通过网络发送。

- **子网**

  由 IPv4 路由表可知，该设备同时接入两个局域网：`192.168.5.0/24` 与 `192.168.179.0/24`。对应网关分别为 `link#2` 与 `link#1`，标志仅含 U（Up），表明二者均为“直连网络”。当该机器需与这两个网段内其他设备（如 `192.168.5.x` 范围内计算机）通信时，数据包无须经由路由器，而是通过 ARP 解析目标 MAC 地址后，直接经 em1 与 em0 接口以链路层单播帧发送。
...
- **标志**
  ...
  `Flags` 列展示各路由的属性。**常见的路由表标志** 汇总了这些标志及其含义。
  ...
  **常见的路由表标志**
  ...
  | S | 该路由是静态配置的 |
  | C | 克隆路由：基于此路由为连接目标自动生成新路由（常用于本地网络） |
  | W | 已克隆：该路由是由克隆路由自动生成的 |
  | B | 静默丢弃数据包（更新期间） |
  | D | 由重定向动态创建 |
  | L | 有效的协议到链路地址转换 |
  | R | 匹配时发送 ICMP 不可达消息（拒绝路由） |
  | b | 路由表示广播地址 |

### 跟踪路由信息

地址空间分配给某网络后，服务提供商将配置自身路由表，确保所有流量均发往该站点链路。外部站点如何获知将数据包发往该网络 ISP？

互联网经由全球路由系统跟踪所有已分配地址空间，并界定其与互联网主干网（承载互联网流量的核心线路）的连接节点。每台主干路由器均保存一份主路由表，将特定网络的流量指向对应的主干运营商，再经由一系列服务提供商逐级传递至目标网络。

服务提供商须向主干站点通告自身连接点身份，从而成为流量进入路径。该过程称为路由传播。

路由传播偶有故障，导致部分站点无法连通。查找路由断点的常用命令为 `traceroute`（IPv6 使用 `traceroute6`），尤其在 `ping` 失败时十分有用。

使用 `traceroute` 时，须提供远程主机的地址。输出将显示路径上的网关主机，最终或抵达目标主机，或因连接问题而终止。
...
传输控制协议（Transmission Control Protocol，TCP）是互联网协议族（Internet Protocol Suite）中的核心传输层协议，其软件实现体系称作 TCP 栈（采用层次化结构组织，因此称“栈”）。Vint Cerf 和 Bob Kahn 于 1974 年在论文《A Protocol for Packet Network Intercommunication》中首次提出了 TCP 的核心思想（当时为传输与网络转发合一的单一协议），后经迭代，于 1981 年随 RFC 793 将 TCP 与 IP 拆分为两个独立协议。RFC 793 已于 2022 年 8 月被 RFC 9293 废止，后者为当前 TCP 协议的最新标准规范。
...
在测试环境中，RACK 和 BBR 在局域网表现良好，但在互联网环境下带宽显著下降：RACK 约为默认栈的三分之一，BBR 约为默认栈的六分之一。该测试在特定网络条件下完成（如高延迟跨境链路），实际性能可能因网络环境而异。需注意，RACK 与 BBR 的设计目标是提升而非降低吞吐量，上述结果可能反映了特定丢包/延迟环境的结构性偏差，不代表这些栈在典型部署场景中的表现。
