...
大多数新款 Android 手机，可在开启 Wi-Fi 并关闭移动数据的同时将网络共享给 FreeBSD。该功能通过将手机已建立的 Wi-Fi 连接经由 USB 接口转发给 FreeBSD 实现，不会产生移动数据流量消耗。
...
如果需要要系统启动时自动加载上述模块，可根据设备类型选择相应条目写入 **/boot/loader.conf** 文件：
