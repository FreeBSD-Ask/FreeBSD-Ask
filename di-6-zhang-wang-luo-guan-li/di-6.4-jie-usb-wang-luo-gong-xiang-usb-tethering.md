...
大多数新款 Android 手机可在开启 Wi-Fi 并关闭移动数据的同时将网络共享给 FreeBSD。该功能通过将手机已建立的 Wi-Fi 连接经由 USB 接口转发给 FreeBSD 实现，不会产生移动数据流量消耗。
...
首先需要加载相应的内核模块，以便系统能够识别 USB 网络共享设备（如果默认情况下未生效）。
