...
蓝牙是一种无线技术，用于在 2.4 GHz 非授权频段内创建个人网络，通常覆盖 10 米范围。网络通常由便携设备（如手机、手持设备和笔记本电脑）按需形成。与 Wi-Fi 不同，蓝牙提供更高级别的服务协议，例如 FTP 类文件服务器、文件推送、语音传输、串行线路仿真等。
...
以罗技 M337 鼠标为例，需长按鼠标底部的配对按钮直至指示灯快速闪烁，使其进入可发现的配对模式。
