...
FreeBSD 作为一款理想的互联网或内网服务器操作系统，能够在最重的负载下提供稳健的网络服务，并高效使用内存以维持数千个并发用户进程的良好响应时间。
...
route_static1="-net 10.0.10.0/24 10.0.1.1"  # 若需访问 10.0.10.0/24 网络（10.0.10.1 到 10.0.10.254），就将数据包发送给 10.0.1.1，由它来转发
route_static2="-net 172.16.30.0/24 172.16.1.254"  # 若需访问 172.16.30.0/24 网络（172.16.30.1 到 172.16.30.254），就将数据包发送给 172.16.1.254，由它来转发
```
