...
如果无法扫描到 Wi-Fi 网络，可能需要修改无线区域设置或路由器信道。此时可按以下步骤重新配置：
...
第一条命令销毁现有 `wlan0` 接口并释放其占用的资源，避免出现 `ifconfig: SIOCS80211: Device busy` 错误；第二条命令重新创建无线接口并绑定到物理设备；第三条命令设置无线国家码为 HR 并使用 ETSI 无线频段规范，在目标网络使用大于 48 的信道（DFS 信道）时需要进行此设置，如信道小于 48，可省略该步骤。
