...
本节以 COMFAST CF-912AC 1200M 802.11AC 无线网卡为例，介绍一般无线网卡的驱动配置方法。该网卡采用 Realtek 芯片组，在 FreeBSD 中具有较好的兼容性。注意，其他采用 Realtek 芯片的网卡配置方法类似。
...
若无法扫描到 WiFi 网络，可能需要修改无线区域设置或路由器信道。此时可按以下步骤重新配置：
...
若无法获取无线网络的服务集标识符（SSID）和预共享密钥（PSK），可尝试联系产品客服或在确认稳妥后重置网络设备。
...
若不希望重启整个系统，可以通过以下命令重启网络服务来应用配置：
...
在配置无线网络的过程中，可能遇到一些常见问题，以下为相关的故障排除方法。
...
3. 查阅 FreeBSD 源代码中 `wlan` 接口的创建逻辑，复现其最小实现，分析为什么系统采用“物理网卡 + 虚拟接口”的分层架构而非直接管理物理设备。
