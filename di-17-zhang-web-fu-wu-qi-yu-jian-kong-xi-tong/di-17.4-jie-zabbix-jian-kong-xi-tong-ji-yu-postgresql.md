...
Zabbix 是一款开源的企业级监控解决方案，用于监控网络服务、服务器硬件及应用程序，其架构采用服务器-代理模式，能够实现全面的基础设施监控。
...
Zabbix 需要数据库以存储监控数据，数据库是监控系统的重要组成部分，负责持久化存储所有监控指标和配置信息。
...
上述安装过程将自动安装 PHP，本文采用的版本为 PHP 8.3（截至撰稿时 PHP 8.4 可能存在兼容性问题）。具体可用版本可查阅 [zabbix7-frontend](https://www.freshports.org/net-mgmt/zabbix7-frontend)，该站点提供最新的包信息与兼容性说明。

亦可采用 Ports 方式安装，该方式提供更大的配置灵活性，需先进入对应目录再执行编译安装命令：
...
Zabbix 前端需要通过 PHP 访问 PostgreSQL 数据库，因此需要安装相应的 PHP 数据库扩展，PDO（PHP Data Objects）是 PHP 中用于数据库访问的统一接口。所使用的 PHP 版本须与 Zabbix 前端兼容，此为系统正常运行的关键前提。
...
zabbix7-agent 是 Zabbix 的代理程序，负责在被监控主机上收集监控数据并发送给 Zabbix Server，是监控数据采集的关键组件。应安装在需要被监控的服务器上，可部署在多台主机上实现分布式监控。
...
为确保 Zabbix 相关服务在系统启动时自动运行，保障监控系统的持续性与可靠性，需将相关服务配置为开机自启，此为生产环境部署的标准实践。
...
需要在 PostgreSQL 中创建专门的数据库和用户供 Zabbix 使用，并导入 Zabbix 的初始数据，此为配置监控系统数据库的必要步骤。PostgreSQL 数据库初始化完成后，执行以下命令：
...
以下为与 PostgreSQL 配置相关的参考文献，供读者进一步查阅，这些资料提供了更深入的数据库配置说明。
...
需要配置 Zabbix Server 的主要配置文件，使其能够连接到 PostgreSQL 数据库，此为确保监控系统正常工作的关键配置步骤。
...
- 找到 `;date.timezone =` 修改为 `date.timezone = Asia/Shanghai`（须删除原行首的分号 `;`）
...
服务启动成功后，可通过浏览器访问 Zabbix Web 前端进行登录和配置。Zabbix Web 前端的默认用户名和密码如下：
...
登录 Zabbix 后，可将界面语言设置为中文，以适应中文使用环境。
...
在 Zabbix 运行过程中若出现异常，可通过查看相关日志文件进行排查。本节同时列出了若干待完善的事项。
...
1. 将本节中尚未在 FreeBSD 上实现的监控模块重新实现。
