...
为了方便管理 OpenList 服务，可以编写一个 FreeBSD RC 脚本。创建并编辑 `/usr/local/etc/rc.d/openlist` 文件，加入以下内容：
...
新建 `userapp-xxx.desktop` 文件（只可改动字段 `xxx` 为所需的名称，其他字段不可改变。如 `userapp-abc.desktop`）文件，以便 xdg-open 调用上面的脚本，在本节中为 `~/.local/share/applications/userapp-vlc.desktop` 文件：
