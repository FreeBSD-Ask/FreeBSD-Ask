...
- `vfs.zfs.arc.max` - ARC 的最大大小。默认值为 `0` 实际取总内存的 1/2。此值不得低于 64 MiB。如果系统运行其他守护进程或进程可能需要内存，可以使用较小的值。
...
- `vfs.zfs.l2arc.write_max` - 限制每个 L2ARC 设备每秒写入的最大数据量。默认值为 `67108864` 字节（32 MiB）。总 L2ARC 吞吐量随池中缓存设备数量线性增长。
...
- `vfs.zfs.vdev.rebuild_min_active` - resilver 操作时每个设备的最小并发 I/O 数。默认值为 `1`。
- `vfs.zfs.vdev.rebuild_max_active` - resilver 操作时每个设备的最大并发 I/O 数。默认值为 `3`。增大此值可加快 resilver 完成速度，但会增加读写延迟。
...
- `vfs.zfs.vdev.nia_credit` - 某些机械硬盘会以较高优先处理顺序 I/O，导致并发随机 I/O 延迟达到数秒。为防止非交互式 I/O（如 scrub）独占设备，当存在未完成的交互式 I/O 时，最多只能发送 `vfs.zfs.vdev.nia_credit` 个非交互式操作。此强制等待确保机械硬盘在合理时间内处理交互式 I/O。默认值为 `5`。
