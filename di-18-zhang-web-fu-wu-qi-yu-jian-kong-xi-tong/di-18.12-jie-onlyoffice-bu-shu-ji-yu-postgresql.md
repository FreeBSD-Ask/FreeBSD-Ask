# 18.10 OnlyOffice 部署（基于 PostgreSQL）

OnlyOffice 是一个开源的在线文档编辑器，可以与 Nextcloud 等平台集成。本节将介绍基于 PostgreSQL 数据库在 FreeBSD 上部署 OnlyOffice 的方法。

## 安装 Nextcloud 和 Nginx

OnlyOffice 需要与 Nextcloud 配合使用，首先请在另一台机器上安装 Nextcloud 和 Nginx。准备好后，就可以开始安装 OnlyOffice 文档服务器了。
...
OnlyOffice 使用 supervisord 来管理其服务进程。编辑 `/usr/local/etc/supervisord.conf` 文件，翻到最底下，找到 `;[include]`，并删去前面的分号：
