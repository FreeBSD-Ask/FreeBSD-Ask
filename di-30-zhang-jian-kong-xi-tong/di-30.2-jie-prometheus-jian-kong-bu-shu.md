...
```sh
# service prometheus enable   # 将 Prometheus 服务配置为系统启动时自动运行
...
```sh
# service grafana enable   # 将 Grafana 服务配置为系统启动时自动运行
...
```sh
# service node_exporter enable   # 将 Node Exporter 服务配置为系统启动时自动运行
...
> **技巧**
>
> 上述示例中的 `10.0.55.1`、`node-exporter-server.example.com` 为占位符，需要替换为实际的值。
...
> **注意**
>
> InfluxDB v1.8 系列已于 2021 年停止维护（v1.8.10 为该系列的最终版本）。截至 2026 年，InfluxDB OSS v1 主线仍处于维护状态，最新版本为 v1.12.x。FreeBSD Ports 中的 InfluxDB 仍为 v1.8.10，使用 v1 API 配置。生产环境亦可考虑使用 InfluxDB v3 或 VictoriaMetrics 等替代时序数据库。
