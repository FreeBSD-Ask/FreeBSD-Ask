...
# 如果要使 WildFly 绑定到所有网络接口，在 rc.conf 中添加以下内容：
...
# 如果要修改 JVM 参数，请编辑相应的 standalone.conf 文件。
...
- 打开 `http://localhost:9990`（对应的局域网也可以连接）可登录管理界面。
...
2. 分别配置独立模式和域模式。
3. 在 WildFly 上部署一个示例 Java Web 应用（WAR 包），验证应用是否正常运行。
4. 对 WildFly 进行安全加固：修改默认管理端口、配置用户角色权限。
