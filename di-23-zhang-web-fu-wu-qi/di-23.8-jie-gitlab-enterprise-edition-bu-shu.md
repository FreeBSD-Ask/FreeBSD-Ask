# 23.8 GitLab Enterprise Edition 部署
...
# 如果是主版本升级（例如从 17.5.x 升级到 17.6.x），
# 请按照上面链接的升级指南操作。
...
# 如果只是小版本升级（例如从 17.8.1 升级到 17.8.2），
# 请参阅上面的补丁版本更新指南。
```

GitLab 的 FreeBSD 版本由维护者提供了完整的安装说明，请参阅：<https://gitlab.com/mfechner/freebsd-gitlab-docu/blob/master/install/17.8-freebsd.md>。
...
GitLab 需使用 PostgreSQL 数据库。当前 GitLab 17.x 要求 PostgreSQL 最低版本为 14，GitLab 18.x 要求最低版本为 16（参见 GitLab. GitLab installation requirements — PostgreSQL[EB/OL]. [2026-04-16]. <https://docs.gitlab.com/ee/install/requirements.html>. 该页面列出了各版本 GitLab 对 PostgreSQL 的版本要求）。
...
Redis 是开源的内存数据结构存储系统，GitLab 使用其作为缓存和会话存储。Redis 已自动作为依赖项安装。
...
# /usr/local/etc/redis.conf
      ...
      in your /etc/rc.conf.
# 要让 Redis 在系统启动时自动运行，需在 /etc/rc.conf 文件中添加以下行：
# redis_enable="YES"
```
...
……此处省略。该步骤大概需要几十分钟；如果提示内存溢出，请增加 swap。通常需执行两次；如果出现内存溢出，则需再重复执行一次……
...
Nginx 是高性能 Web 服务器和反向代理，GitLab 使用其作为前端 Web 服务器。
...
1. 在 FreeBSD 上安装 GitLab CE（社区版），完成完整配置流程并验证代码仓库创建、提交与克隆功能。
2. 对 GitLab 进行安全加固：配置 HTTPS、启用双因素认证。
