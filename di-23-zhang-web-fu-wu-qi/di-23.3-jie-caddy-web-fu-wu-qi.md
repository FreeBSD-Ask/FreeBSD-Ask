...
# - 运行 'service caddy enable' 启用 Caddy 服务，使其在系统启动时自动启动
...
#   使用该命令将 Caddy 配置为以 www:www 用户和组运行
...
- Note if Caddy has been started as root previously, files in
  ...
  changing manually.
# 注意，如果 Caddy 曾以 root 身份启动过，/var/log/caddy、/var/db/caddy 和 /var/run/caddy 中的文件可能需要手动更改其所有权。
...
# 可更改这些默认设置。请查看 /usr/local/etc/rc.d/caddy 文件了解更多信息
...
users.
# 之前的默认设置（尤其是与 root 身份搭配使用时）可能在存在不信任用户的共享机器上引发严重安全隐患。
...
> **注意**
>
> 由于非特权用户无法开启 443 端口，将报错 `listen tcp :443: bind: permission denied`。
...
1. 将 Caddy 与其他服务器结合使用。
2. 加固 Caddy：配置访问日志、设置请求频率限制。
3. 对比 Caddy 的自动 HTTPS 机制与 Nginx 手动配置 TLS 的运维差异，说明两种方案的适用场景。
