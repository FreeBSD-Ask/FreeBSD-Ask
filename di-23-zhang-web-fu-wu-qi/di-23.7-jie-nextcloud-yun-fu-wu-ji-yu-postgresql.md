# 23.7 Nextcloud 云服务（基于 PostgreSQL）

Nextcloud 是自托管云存储与协作平台，提供文件管理、日历、联系人、在线办公功能。
...
Nextcloud 前端需通过 PHP 访问，因此需安装 PHP 的 Apache 模块。可通过 `php -v` 命令检查当前 PHP 版本，确保与系统安装的版本一致：
...
如果使用 Apache 作为 Web 服务器，需完成相应配置，使 Apache 正确处理 Nextcloud 的 PHP 文件。请参考其他章节完成 Apache 的安装和服务自启。
...
然后重启 Apache。Nextcloud 会在首次访问时启动捆绑的安装程序。安装程序完成后，可通过 'Settings' 和 'Administration' 进一步配置。更多配置选项可在以下位置找到：
...
Nextcloud 还有许多常用插件可安装使用。其他常用插件可使用命令 `pkg search -x nextcloud | grep php83` 查找，然后通过 pkg 安装。
...
2. 对 Nextcloud 进行安全加固：配置 HTTPS、启用双因素认证。
3. 比较自托管云存储方案与商业云存储（Google Drive、OneDrive）在数据主权和运维成本上的差异。
