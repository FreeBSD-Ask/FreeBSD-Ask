...
Tomcat 的主要配置文件为 **/usr/local/apache-tomcat-11.0/conf/server.xml**，该文件定义了服务端口、连接器、引擎和主机等核心组件。默认情况下，Tomcat 监听 8080 端口用于 HTTP 连接。AJP（Apache JServ Protocol）连接器默认未启用，如需使用可在 `server.xml` 中取消注释开启 8009 端口。如需修改端口或配置 HTTPS 支持，可编辑该配置文件。
