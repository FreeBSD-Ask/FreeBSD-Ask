...
Tomcat 的主要配置文件为 **/usr/local/apache-tomcat-11.0/conf/server.xml**，该文件定义了服务端口、连接器、引擎和主机等核心组件。在默认情况下，Tomcat 监听 8080 端口用于 HTTP 连接。AJP（Apache JServ Protocol）连接器默认未启用，如需使用可在 `server.xml` 中取消注释开启 8009 端口。如需修改端口或配置 HTTPS 支持，可编辑该配置文件。
...
1. 为 Tomcat 新增 HTTPS 支持及 SSL 自动续签。
2. 修改 Tomcat 默认的 server.xml 配置，将默认 HTTP 端口从 8080 改为 8081，同时配置线程池最大线程数从 200 改为 50，验证配置变更效果。
3. 测试在不同 GC 算法（G1、ZGC）下调整 Tomcat 线程池大小的性能影响，对比结果。
