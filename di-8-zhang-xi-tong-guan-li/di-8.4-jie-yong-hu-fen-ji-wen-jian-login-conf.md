...
普通用户可通过主目录下的 **~/.login_conf** 文件覆盖系统级配置。记录 ID 为“me”的条目只能覆盖该用户所属的用户分级配置，系统级配置文件 **/etc/login.conf** 的优先级低于用户本地配置。
...
- FreeBSD Project. login.conf(5)[EB/OL]. [2026-03-26]. <https://man.freebsd.org/cgi/man.cgi?query=login.conf>.
