...
该命令会快速开启 jail1 的 ssh 服务，新建一个 wheel 组用户，用户名和密码均与 jail1 名称相同，首次用这个用户登录要求修改密码。也可在登录 jail1 控制台后，自行配置 sshd 服务。
...
以下命令均在 Jail 控制台下运行，pkg 安装时是否使用镜像请自行决定，若使用镜像可在 Jail 控制台中如同宿主机般进行设置，请参考相关文章。
