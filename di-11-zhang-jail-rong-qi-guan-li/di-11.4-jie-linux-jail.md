...
在 `/etc/jail.conf` 文件中，加入以下内容（若无则新建）。关键配置项说明：devfs_ruleset 定义 devfs 的规则集，enforce_statfs 控制 Jail 中挂载点的可见性（0：无限制，1：仅根目录下可见，2：默认，仅根目录所在挂载点可操作）：
...
root@antix:/# echo "nameserver 223.5.5.5" > /etc/resolv.conf    # Jail 中，注意提示符变化，先设置地址解析，这里使用阿里云 DNS
...
root@antix:/# mandb       # apt upgrade 的时候显示 mandb 有几个权限错误，多执行几次 `mandb` 命令即可。
```
...
├── alpine/          # JID 3, IP 192.168.5.3
└── freebsd-jail/    # JID 4, IP 192.168.5.4
```
...
在 Ubuntu 22.04 中，Firefox、Chrome 要求用 snap 安装，snap 需要 systemd，不能使用，所以使用 deb 包安装。方法如下：
...
将 `DISPLAY` 设为 `:0.0`、`127.0.0.1:0.0`、`10.0.2.15:0.0` 均可成功，上述几种方法也可一一尝试。
