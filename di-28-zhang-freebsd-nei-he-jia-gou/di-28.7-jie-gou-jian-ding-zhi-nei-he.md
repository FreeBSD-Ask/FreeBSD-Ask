...
传统上，FreeBSD 使用的是宏内核。宏内核是一款大型程序，支持固定的设备列表，并且如果要改变内核的行为，必须编译并重新启动到新内核。

如今，FreeBSD 内核中的大部分功能都包含在可以根据需要动态加载和卸载的模块中。这使得运行时内核能够立即适应新硬件。因此将这种内核称为模块化内核。

有时，仍须对内核作静态配置。某些功能与内核紧密绑定，无法动态加载。一些安全环境会阻止加载和卸载内核模块，要求只能将所需功能静态编译进内核。
...
> **警告**
>
> 非默认配置的测试不如 GENERIC 配置彻底。虽然定制内核可以带来特定的好处，但它也增加了遇到构建或运行时问题的风险。只有那些有明确理由作出更改并愿意在必要时进行调试的专业用户，才建议使用定制内核配置。
...
> **注意**
>
> 使用 `include` 指令时，其行为类似 C 语言的 `#include`，会将目标文件的全部内容插入当前位置。以 `GENERIC` 为例，其首行为 `include "std.amd64"`，引入了标准的 amd64 基础配置。使用这种方法，本地配置文件表示与 GENERIC 内核的本地差异。在执行升级时，除非使用 `nooptions` 或 `nodevice` 明确防止，否则 GENERIC 中新增的特性也会添加到本地内核。
...
实际上，上面的例子出自 16.0-CURRENT 的 GENERIC 内核配置文件，-CURRENT 与 -STABLE、-RELEASE 的重要差异就在于 -CURRENT 引入了大量的调试功能。该功能即由 `include "std.debug"` 引入。因此部分用户会体验到 -CURRENT 系统性能显著下降。
...
在默认情况下，当编译自定义内核时，所有内核模块都会重新编译。要更快地更新内核或仅构建自定义模块，可以在开始构建内核之前编辑 **/etc/make.conf** 文件。
...
如果内核版本与系统工具的构建版本不同，例如，对 -RELEASE 系统安装了从 -CURRENT 源代码构建的内核，那么许多系统状态命令，如 ps 和 vmstat 将无法工作。为了解决这个问题，请重新编译并安装与内核版本相同的用户空间（世界）。使用与操作系统其他部分版本不同的内核是不推荐的。
