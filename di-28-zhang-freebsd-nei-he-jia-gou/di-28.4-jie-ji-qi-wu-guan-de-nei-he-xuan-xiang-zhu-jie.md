...
>>
>>以 `envvar hint.` 开头的行应放入 hints 文件中。参见 **/boot/device.hints**，config(8) 中的 `hints` 指令。
>>
>>若需进行内核测试构建，请使用 `make LINT` 创建旧式的 LINT 文件。
>>
...
- ②：`CONF_CFLAGS` 提供了一些额外的编译器标志，这些标志会在大多数其他标志之后添加到 `${CFLAGS}` 中。在此处，使用它来禁止使用非最优的 gcc 内建函数（例如 `memcmp`）。
...
`SCHED_4BSD` 和 `SCHED_ULE` 必须二选一。这些选项用于选择需编译进内核的调度器。
...
在发生 panic 时不进入调试器。适用于无人值守的操作环境，若需通过控制台手动进入调试器，但仍希望系统能够在 panic 后自行恢复，可启用此选项。
...
`EARLY_PRINTF` 可在内核非常早期（在调用 `cn_init()` 之前）使用一种特殊的 `printf`（`eprintf`）。这通常仅用于启动早期调试。通常情况下，它未被定义。此处将其注释掉是因为该功能通常不可用，而且所需的 `eputc()` 也未定义。
...
这个选项可开启各种详细打印功能。它由一个位掩码控制（可以通过 socket 选项和 sysctl 设置）。包含该选项本身不会立即产生日志，必须先设置相应的位。但打印信息可能非常冗长。因此，若未启用该选项，代码就不会进行位检测和打印，如此可提高运行速度。如果并非在调试，请不要使用。
...
>> **思考题**
>>
>>读者可能希望进一步研究此处的缺失条目 `IPFIREWALL_PMOD`，请仔细查阅源代码及 [Add ipfw protocol modification module ipfw_pmod](https://reviews.freebsd.org/D10150) 等相关提及，并提交 PR 到 FreeBSD src。
...
`SND_FEEDER_RATE_HP`（feeder_rate）将尽可能使用高精度 64 位运算（默认情况下会尽量避免）。可能造成运算速度下降。
