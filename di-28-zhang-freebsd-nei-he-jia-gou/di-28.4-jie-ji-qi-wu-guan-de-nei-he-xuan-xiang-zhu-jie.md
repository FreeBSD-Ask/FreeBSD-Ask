...
这两个选项分别用于调整 mbuf 集群大小和 mbuf 大小。在将它们的默认值更改为其他值时，须格外谨慎处理网卡驱动模块，因为这可能导致内核假定的 mbuf 大小与模块假定的 mbuf 大小不匹配。目前，唯一能够检测这种不匹配的驱动是 [ti(4)](https://man.freebsd.org/cgi/man.cgi?query=ti&sektion=4)。
...
使 `mi_startup()` 执行的 SYSINIT 过程输出更为详细的信息。该功能在移植新架构时十分有用。若同时启用了 DDB 调试器，将显示函数名称而非地址。若定义值为 `0`，则详细输出代码会被编译进内核但默认禁用，可通过可调参数 `debug.verbose_sysinit=1` 启用。
