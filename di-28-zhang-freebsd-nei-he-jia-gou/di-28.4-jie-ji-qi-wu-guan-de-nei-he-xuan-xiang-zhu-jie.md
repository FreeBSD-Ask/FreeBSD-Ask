...
调度程序是内核中负责分配 CPU 时间给不同线程和进程的核心组件。以下是相关的内核配置选项。
...
性能监控选项提供了测量和分析系统性能的能力，特别是利用 CPU 内置的性能监控计数器。以下是相关的内核配置选项。
...
这个选项可开启各种详细打印功能。它由一个位掩码控制（可以通过 socket 选项和 sysctl 设置）。包含该选项本身不会立即产生日志，必须先设置相应的位。但打印信息可能非常冗长。因此，如果没有启用该选项，代码就不会进行位检测和打印，如此可提高运行速度。如果并非在调试，请不要使用。
