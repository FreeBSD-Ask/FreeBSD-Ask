...
传统上，FreeBSD 使用宏内核。宏内核是一种大型程序，支持固定的设备列表，若需改变内核行为，须重新编译并重启至新内核。
...
构建定制内核前，应明确其目的。具体而言，若需特定硬件支持，该支持可能已以模块形式存在。
...
若需在启动时以模块方式加载驱动程序 u3g，可在 **/boot/loader.conf** 文件中加入以下行：
...
若要创建自定义内核配置文件并构建自定义内核，必须先安装完整的 FreeBSD 源代码树。
...
实际上，上述示例出自 16.0-CURRENT 的 GENERIC 内核配置文件，-CURRENT 与 -STABLE、-RELEASE 的重要差异在于 -CURRENT 引入了大量的调试功能。该功能即由 `include "std.debug"` 引入，部分用户因而会察觉到 -CURRENT 系统性能显著下降。
...
**对于 pkgbase 系统**：必须先构建完整的用户空间。须将内核和用户空间构建为 repo 软件源后方可安装。使用以下命令构建用户空间：
...
在默认情况下，编译自定义内核时所有内核模块都会重新编译。若需加快内核更新速度或仅构建自定义模块，可在构建内核之前编辑 **/etc/make.conf** 文件。
