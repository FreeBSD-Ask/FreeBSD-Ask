...
`MAXMEM` 用于指定主机内存容量。如果没有指定该参数，FreeBSD 将首先从 CMOS RAM 读取内存容量，初始内存将被限制在 64 MB 或 16 MB（具体取决于 BIOS）。若 BIOS 报告 64 MB，系统将通过内存探测尝试检测实际安装的内存容量。若探测失败且内存大于 64 MB，则必须使用 `MAXMEM` 参数。容量值以千字节为单位，例如 128 MB 内存需设置为 131072（128 * 1024）。
