...
MAC 策略 **不能覆盖** 传统 Unix 安全机制（文件权限和超级用户检查），仅在 DAC 基础之上施加额外限制。换言之，即使用 MAC 配置为允许访问，DAC 一旦拒绝，操作仍会失败。MAC 策略可限制 `root` 权限，此为其与 DAC 的关键区别之一。
...
MAC 框架设计参考 IEEE POSIX.1e 草案 17（曾称 POSIX.1e），该草案旨在为操作系统定义安全扩展，包含访问控制列表（ACL）、审计和强制访问控制。POSIX.1e 最终未成正式标准，FreeBSD 的 MAC 实现是 TrustedBSD 项目按实际需求独立演进的结果，不完全等同于任何已批准的国际或行业标准。
...
在适当情形下可向 tunefs(8) 传递 `multilabel` 为 UFS 文件系统设置多标签策略。多标签策略允许每个主体或客体拥有独立 MAC 标签。单标签或多标签的决策仅对实现标签化功能的策略必需，如 `biba`、`lomac` 和 `mls`。部分策略（如 `seeotheruids`、`portacl`、`partition`）完全不使用标签。`multilabel` 功能原生支持于 UFS2 文件系统，需在单用户模式下运行 `tunefs -l enable` 启用。ZFS 文件系统的 MAC 标签支持因实现方式不同而有所差异。
...
对于当前运行进程（如 sshd），通常改用 `getpmac` 获取其标签。此命令以进程 ID（PID）代替命令名。如果用户试图访问不在其权限范围内的文件，将因已加载策略模块的限制而显示 `Operation not permitted` 错误。
...
| 标签 | 说明 |
| ---- | ---- |
| `low` | 客体或主体可拥有的最低标签设置。设置此标签将阻止其访问标记为 high 的客体或主体。 |
...
需考虑 MAC 框架如何从整体增强系统安全性。MAC 框架提供的各种安全策略模块可用于保护网络和文件系统，或阻止用户访问特定端口和套接字。策略模块的最佳用法或许是同时加载多个以提供 MLS 环境，此方法有别于仅针对特定用途、强化系统局部元素的策略。MLS 的缺点在于管理开销增加。
...
该策略通过 **/usr/bin/mdo** 可执行文件生效，详情参阅 mdo(1)。规则以 `from>to` 格式的字符串指定，每条规则包含 `from` 部分（匹配当前凭据）和 `to` 部分（目标凭据），多条规则以分号 `;` 分隔。
...
> **警告**
>
>将 `net.inet.ip.portrange.reservedlow` 和 `net.inet.ip.portrange.reservedhigh` 均设为 `0` 会彻底移除内核级特权端口保护机制，使原本仅 `root` 可绑定的 1-1023 端口不再受任何内核限制。此配置的含义是：**用 mac_portacl 的规则层保护完全替代传统 UNIX 特权端口保护**。
>
> 若 mac_portacl 模块因任何原因未加载、被禁用（`security.mac.portacl.enabled=0`）或规则配置不正确，则系统将回退为 **所有用户均可绑定任意端口** 的状态——这包括 SSH（22）、HTTP（80）、HTTPS（443）等关键服务端口，可能导致服务劫持或权限提升攻击。务必确保 `mac_portacl` 始终可靠加载且规则完备。
...
用户从 `root` 切换到系统中其他用户时，可能出现错误信息 `_secure_path: unable to stat .login_conf`。此消息通常发生在用户标签高于欲切换的目标用户时。例如，如果 `joe` 默认标签为 `biba/low`，`root` 标签为 `biba/high`，则 `root` 无法查看 `joe` 的主目录，因为 Biba 完整性模型不允许 `root` 查看处于更低完整性级别的客体——无论 `root` 是否用 `su` 切换为 `joe`，均会发生此情况。
