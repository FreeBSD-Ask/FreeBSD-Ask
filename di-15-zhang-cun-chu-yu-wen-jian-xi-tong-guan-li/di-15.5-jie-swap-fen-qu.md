...
在 FreeBSD 中，swap 可通过传统分区、交换文件或 ZFS 卷（ZVOL）等多种方式实现。本节介绍系统安装后添加 swap 空间的方法。

如果在系统安装阶段未配置 swap（交换分区），则只能通过 dd 命令创建交换文件或 ZFS 卷，因为 UFS 及 ZFS 文件系统均不支持分区收缩操作。
...
选项 `-h` 表示以人类可读格式（human-readable）输出。参见 FreeBSD Project. swapinfo(8)[EB/OL]. [2026-03-26]. <https://man.freebsd.org/cgi/man.cgi?query=swapinfo&sektion=8>。
...
2. 对比传统交换文件和 ZFS 卷作为 swap 的性能差异，分别测试两种方式下的读写速度。
