...
讲解 FreeBSD 的系统管理功能，包括启动引导、服务管理、用户分级、SSH、远程桌面、定时任务、系统配置以及 Live 镜像与系统恢复等。

**第 6 章 网络管理**

介绍 IPv4、IPv6 配置及常用网络命令，包括网络基础配置、TCP/IP 协议栈、无线网络和 USB 网络共享等内容。

**第 7 章 网络代理工具**

介绍系统代理配置及常用代理工具，包括 V2Ray、Mihomo 等的配置和使用方法。

**第 8 章 存储与文件系统管理**

主要介绍 UFS 文件系统、自动挂载、常见的 Windows 和 Linux 文件系统的使用方法，以及 Swap 分区配置。

**第 9 章 ZFS 文件系统**

介绍划时代的新型文件系统——ZFS，包括其概述、快照与还原、磁盘扩容、数据压缩、启动环境、存储池更新及委托管理等。
...
尽可能列举可在 FreeBSD 上运行的桌面环境及窗口管理器，包括 GNOME、KDE、Xfce 等常见环境，以及 bspwm、Hyprland 等平铺式窗口管理器。若读者所需之桌面环境未列其中，欢迎提交合并请求（PR）。
...
包括中文输入法框架及系统字体配置等本地化内容。若读者所需之软件未列其中，欢迎提交合并请求（PR）。

**第 14 章 多媒体和外部设备**

介绍 Web 浏览器、音视频播放与处理，以及配置和使用蓝牙、打印机、人机输入设备等常用外部设备。
...
介绍一些非 Web 服务器的搭建和配置，如 FTP 服务器、Samba 文件共享、NFS 服务器、零配置网络等。

**第 19 章 Web 服务器与监控系统**

介绍常用的 Web 服务器、应用服务器、监控平台及云服务，如 Apache、Nginx、Caddy、Prometheus、Grafana、Nextcloud、GitLab 等。

**第 20 章 嵌入式平台**

主要介绍如何在树莓派等 ARM 设备，以及 RISC-V 和 x86 架构的嵌入式开发板上运行 BSD 系统。
...
介绍如何在 FreeBSD 上搭建常用开发环境，包括 C/C++、Java、Python、Rust 等。

**第 25 章 社区参与**

介绍如何参与 FreeBSD 社区贡献，包括 Bug 报告、邮件列表和开发参与。

**第 26 章 基于 Linux 兼容层的应用**

介绍基于 Linux 兼容层运行的常用应用，如 QQ、WPS Office、微信和 Wine 等。

**第 27 章 FreeBSD 内核架构**

介绍 FreeBSD 内核选项，提供完整注释和翻译，帮助读者理解内核配置，并指导构建定制内核。
