...
两个经典问题是：“vi 到底怎么退出”（作为和记事本相同类型的文本编辑软件，却连保存退出都是困难的，而编辑更困难）、以及“我的密码为什么输入不了？因为屏幕上的密码区域始终是空白的，连 `****` 都不会有，就是什么也没有，他们就是如此设计的。实际上在 2007 年 sudo 就有选项 `pwfeedback` 可以启用该功能，但是默认启用的寥寥无几，该软件也满是 CVE。
