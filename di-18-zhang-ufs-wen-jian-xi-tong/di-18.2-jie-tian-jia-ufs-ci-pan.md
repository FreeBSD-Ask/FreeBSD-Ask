...
然后添加一个单一的分区。为了提高在具有更大硬件块大小的较新硬盘上的性能，分区对齐到 1MB 边界：
...
> **技巧**
>
> 此处的 `-t` 仅是为了写入元数据与文件系统的 UUID，并非实际执行格式化。允许与格式化的文件系统无关。
...
验证了之前的命令创建了 GPT 分区表，新建了 10G 容量的磁盘分区 **nda1p1**，文件系统使用 UFS。

## 创建多个分区

为方便使用，可以创建多个分区。

以下将为磁盘 **nda1** 创建 4 个分区，容量分别为 1GB、2GB、3GB 和 4GB。使用 `-s` 参数来指定分区大小（支持 G、M 等单位）。
...
注意到，之前的命令分别创建了分区 **nda1p1**、**nda1p2**、**nda1p3**、**nda1p4**。
...
临时挂载，将 **/dev/nda1p3** 挂载到 **/mnt/newdisk**：
