...
FreeBSD 提供了对 macOS 文件系统的支持，包括 HFS/HFS+、APFS，从而可以访问为 Apple 系统格式化的存储设备。
...
通过指定其 FreeBSD 分区名和一个已存在的挂载点来挂载 HFS/HFS+ 卷。以下示例将 **/dev/ada1p1** 挂载到 **/mnt**：

```sh
# hfsfuse /dev/ada1p1 /mnt
```

> **注意**
>
> 当前 `fusefs-hfsfuse` 主要提供只读支持，无法进行写入操作。
...
- libfsapfs Project. GitHub Repository[EB/OL]. [2026-03-25]. <https://github.com/sgan81/libfsapfs>. libfsapfs 开源项目仓库。
