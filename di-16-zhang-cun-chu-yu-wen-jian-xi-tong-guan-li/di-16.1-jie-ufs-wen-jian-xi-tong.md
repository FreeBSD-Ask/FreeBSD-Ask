...
必须明确区分，本节所述的 UFS 文件系统与手机等设备中使用的 UFS 存储属于完全不同的技术范畴。后者全称为 Universal Flash Storage（通用闪存存储），是一种硬件存储标准，目前已发展至 4.1 版本。FreeBSD 自 9.x 版本起通过 mmc/mmcsd 驱动支持 eMMC，FreeBSD 15.0 已通过 `ufshci` 驱动支持 UFS 存储。
