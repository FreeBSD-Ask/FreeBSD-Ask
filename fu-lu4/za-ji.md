...
不存在绝对的发展，你现在的功法，不就是以后的古法。作者没有考虑过这样一个问题，即他的古法定义也是浮动的，最后的结果就是把修仙界变成地球，因为最后没有任何功法可用了。他这个设定逻辑结果就是末法时代——就是地球上的中世纪，然后发现伊斯兰的文献开始文艺复兴，但是没用，按他这个设定，功法最后都会消失。地球就是修仙界的最终结局。你要是一直说古法有用，今法平庸，是古法退化、弱化、甚至残缺、错乱的版本。那么发展到最后就没有功法可用了，修仙界退化成地球，造成逻辑悖论。
