...
池的状态显示 md1 设备出现了校验和错误，但应用程序读取数据时未收到错误数据——ZFS 从完好的 md0 镜像磁盘提供了数据：
