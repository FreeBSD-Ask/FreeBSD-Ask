...
> **危险警告**
>
> 以下 `dd` 命令会向磁盘写入随机数据，**将不可逆地摧毁该磁盘上的所有数据**。此操作仅用于演示 ZFS 自愈能力，**绝不能在真实的池成员磁盘上运行**，也不要在任何具有 ZFS 文件系统以外分区的磁盘上执行。执行前必须确认已导出池并拥有可验证的完整备份。命令中的 `</dev/故障磁盘>` 是**占位符**，必须替换为实际要模拟损坏的磁盘设备名——本示例中为 `/dev/nda3`。请再三确认目标设备是否正确，因为该命令**没有任何确认提示**。
...
```sh
# dd if=/dev/random of=/dev/<故障磁盘> bs=1m count=200
200+0 records in
200+0 records out
...
池的状态显示某个设备出现了错误。请注意，应用程序读取数据时未收到任何错误数据。ZFS 从未受影响的 **nda2** 镜像磁盘提供了数据。要查找校验和错误的设备，请查看 `CKSUM` 列包含非零值的设备。
...
ZFS 检测到错误并利用未受影响的 **nda2** 镜像磁盘中的冗余数据进行了修复。对比原始校验和，可以确认池数据是否一致。
...
对比有意破坏数据前后生成的校验和，可以看到数据仍然一致。这说明 ZFS 即使在校验和不一致的情况下，也能自动检测并修复错误。注意，此能力仅在池具备足够冗余时才成立——单独设备构成的池没有自我修复能力。正因如此，校验和在 ZFS 中不可或缺，不应以任何理由禁用。ZFS 无需借助传统的文件系统一致性检查程序（fsck）即可检测并修复这些问题，同时在发生问题时保持池的可用性。接下来需要执行一次 scrub 操作，以覆盖 **nda3** 上的损坏数据。
...
config:
    ...
    healer      ONLINE       0     0     0
      mirror-0  ONLINE       0     0     0
       nda2     ONLINE       0     0     0
       nda3     ONLINE       0     0   627  (repairing)

errors: No known data errors
```

scrub 操作从 **nda2** 读取数据，并在 **nda3** 上重写任何具有错误校验和的数据，显示为 `zpool status` 中的 `(repairing)` 输出。操作完成后，池状态将变为：
...
config:
    ...
    healer      ONLINE       0     0     0
      mirror-0  ONLINE       0     0     0
       nda2     ONLINE       0     0     0
       nda3     ONLINE       0     0 2.72K

errors: No known data errors
```

在 scrub 操作完成并将所有数据从 **nda2** 同步到 **nda3** 后，使用 `zpool clear` 清除池状态中的错误消息。
