...
创建 ZFS 存储池需要做出不可更改的决策。最重要的一项决策是物理磁盘按何种 vdev 类型分组，具体选项请参阅 vdev 类型列表。创建池后，镜像（mirrors）允许向 vdev 添加新磁盘；条带（stripe）也可以通过将新磁盘附加到 vdev 来升级为镜像。启用 `raidz_expansion` 功能标志后，RAID-Z vdev 也可扩展。尽管添加新的 vdev 可以扩展池，vdev 类型在创建后无法更改。此时只能备份数据、销毁池，然后重新创建。
...
以下示例展示了这种自我修复行为，先创建一个由 **/dev/nda2** 和 **/dev/nda3** 构成的镜像池 `healer`。

```sh
# zpool create healer mirror /dev/nda2 /dev/nda3
```
...
healer  59.5G   384K  59.5G        -         -     0%     0%  1.00x    ONLINE  -
```

将一些重要数据复制到 `healer` 存储池中，以防数据因错误而损坏，并生成池的校验和以供后续比较。

```sh
# cp /home/ykla/.cshrc /healer
```

```
# zfs list healer
NAME     USED  AVAIL  REFER  MOUNTPOINT
healer   448K  57.7G   100K  /healer
```

```
# sha256 /healer/.cshrc  > checksum.txt
```

```
# cat checksum.txt
SHA256 (/healer/.cshrc) = 523d96d41067f380469d4f3d68b540edb63b127e91a9955fb7f6a01c53545258
```
...
> **警告**
>
> 这是个危险操作，可能会销毁重要数据，仅用于演示。**不要在正常操作的存储池中尝试**。不要在任何具有 ZFS 文件系统以外分区的磁盘上运行此有意损坏示例。不要使用池外的任何其他磁盘设备名。确保存在正确的池备份，并在运行命令之前测试它们！
...
```sh
# dd if=/dev/random of=/dev/nda3 bs=1H count=60
200+0 records in
200+0 records out
209715200 bytes transferred in 0.638596 secs (328400638 bytes/sec)
```
