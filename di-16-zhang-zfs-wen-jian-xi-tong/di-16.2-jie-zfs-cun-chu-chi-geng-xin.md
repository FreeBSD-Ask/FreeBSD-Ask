...
ZFS 的自愈能力源于校验和与存储冗余的结合。以下示例在一个两磁盘镜像池 `healer` 上演示自愈行为——先模拟磁盘数据损坏，再观察 ZFS 如何自动检测并修复。

以下示例展示了这种自愈行为，先创建一个由 **/dev/nda2** 和 **/dev/nda3** 构成的镜像池 `healer`。
...
对比有意破坏数据前后生成的校验和，可以看到数据仍然一致。这说明 ZFS 即使在校验和不一致的情况下，也能自动检测并修复错误。注意，此能力仅在池具备足够冗余时才成立——单独设备构成的池没有自愈能力。正因如此，校验和在 ZFS 中不可或缺，不应以任何理由禁用。ZFS 无需借助传统的文件系统一致性检查程序（fsck）即可检测并修复这些问题，同时在发生问题时保持池的可用性。接下来需要执行一次 scrub 操作，以覆盖 **nda3** 上的损坏数据。
