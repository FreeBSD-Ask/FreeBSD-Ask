...
```sh
# dd if=/dev/random of=/dev/nda3 bs=1m count=200
200+0 records in
200+0 records out
...
ZFS 检测到错误并利用未受影响的 **nda3** 镜像磁盘中的冗余数据进行了修复。对比原始校验和，可以确认池数据是否一致。
...
scrub 操作从 **nda1** 读取数据，并在 **nda3** 上重写任何具有错误校验和的数据，显示为 `zpool status` 中的 `(repairing)` 输出。操作完成后，池状态将变为：
