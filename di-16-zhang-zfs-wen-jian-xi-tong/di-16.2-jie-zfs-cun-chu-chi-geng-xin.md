...
`zpool offline` 可将物理设备脱机——脱机期间 ZFS 不会对设备进行任何读写。此操作不适用于备盘。如果仅有一个设备，将报错：`cannot offline nda1: no valid replicas`。
...
1. 在 FreeBSD 14.3 虚拟机中创建一个 ZFS 池，不启用任何新功能标志，然后升级到 FreeBSD 15.0 并执行 zpool upgrade，验证新旧系统的兼容性。
2. 分析 OpenZFS 中 zpool upgrade 命令的核心处理流程，说明升级过程涉及哪些磁盘上的数据结构变更。
3. 修改 ZFS 池的 compatibility 功能标志，禁用所有可能影响兼容性的新功能，验证旧版 FreeBSD 仍能访问该池。
