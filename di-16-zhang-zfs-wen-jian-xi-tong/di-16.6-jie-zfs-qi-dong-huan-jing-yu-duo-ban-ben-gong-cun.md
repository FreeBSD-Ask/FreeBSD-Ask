...
借助 ZFS 启动环境，可在同一台机器上保留多个独立的系统版本，升级失败时无需重新安装即可回滚到先前状态，从而实现系统更新的原子回滚。
...
pkgbase 的设计初衷是让 stable、current 和 release（包括 BETA、RC 等）都能使用统一的二进制工具进行更新。此前，stable 和 current 只能通过编译源代码来更新。
...
前面以一次性启动环境进行了验证。如果验证通过并且不需要再保留旧版本，可以将新环境设置为永久默认。
...
1. 阅读 pkgbasify 脚本的源代码，说明其转换流程的关键步骤。
