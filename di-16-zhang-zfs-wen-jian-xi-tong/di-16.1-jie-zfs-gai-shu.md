...
- **vdev 类型**：
  ...
  - **日志 (Log)**：日志 vdev 是独立的意图日志设备（separate intent log device），将 ZIL（ZFS 意图日志）从常规池设备转移至专用存储设备（通常为 NVRAM 或专用 SSD）。专用日志设备可提升数据库等高同步写入负载的性能。日志设备可镜像，但不支持 RAID-Z；使用多个日志设备时，写入负载会均匀分布。可通过 `zpool remove` 命令移除 日志设备，也可添加或替换日志设备。
