...
执行重命名，将数据集从 zroot/usr 移动到 zroot/var 下：
...
查看 snapdir 属性的当前设置，并将其切换为 visible：
...
书签功能需要存储池启用 `bookmarks` 功能标志。在现代版本的 ZFS 中，该标志默认已启用。

### 创建书签

使用 `zfs bookmark` 命令为快照创建书签：
...
# zfs list -t bookmark
NAME                          USED  AVAIL  REFER  MOUNTPOINT
zroot/var/tmp#mybookmark         -      -      -  -
zroot/var/tmp#mybookmark2        -      -      -  -
```
...
书签的主要价值在于增量复制。当源系统的快照已被销毁以节省空间，但仍需向备份系统发送增量流时，书签可替代快照作为增量源：

```sh
# zfs send -i zroot/var/tmp#mybookmark zroot/var/tmp@recent | zfs receive backup/var/tmp
```

这允许管理员在销毁中间快照后仍能继续增量备份链，极大节省了存储空间。

### 销毁书签

销毁书签使用 `zfs destroy` 命令，与销毁快照的语法类似，但使用 `#` 分隔符：
...
当尝试回滚到非最新快照时，ZFS 会检测是否存在更新的快照和书签，并提示用户使用 `-r` 选项强制删除：
...
ZFS 提供了不同的压缩算法，各有优劣。不同压缩算法在压缩比、压缩速度与解压速度方面各有优劣，需根据工作负载特征进行选择。
