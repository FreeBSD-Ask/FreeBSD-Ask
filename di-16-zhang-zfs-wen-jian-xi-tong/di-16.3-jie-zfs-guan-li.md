...
快照是 ZFS 最强大的功能之一。快照提供了数据集的只读、时间点副本。借助写时复制（COW）技术，ZFS 通过在磁盘上保留数据的旧版本来快速创建快照。如果没有快照，ZFS 会在数据覆盖或删除时回收空间以备用。快照通过记录当前数据集与先前版本之间的差异来节省磁盘空间。快照作用于整个数据集，而非单个文件或目录。数据集的快照会复制其包含的所有内容，包括文件系统属性、文件、目录、权限等。快照在创建时不会额外占用空间，但会随其引用的块发生变化而消耗空间。使用 `-r` 创建的递归快照会在数据集及其所有子数据集上创建同名快照，从而提供文件系统的一致时间点副本。跨多个相关数据集或依赖文件工作时，这一点尤为重要。如果没有快照，备份将是散布在不同时间点的文件副本。

ZFS 中的快照具备许多其他文件系统所不具备的功能。快照的一个典型用途是，在执行软件安装或系统升级等高风险操作之前，快速备份当前文件系统状态。如果操作失败，可回滚到快照，将系统恢复到快照创建时的状态。如果升级成功，则删除快照以释放空间。如果没有快照，升级失败通常需要从备份恢复，不仅繁琐耗时，还可能导致系统长时间不可用。快照回滚速度很快，即使系统在正常运行期间执行回滚，也几乎不会造成停机。在多 TB 的存储系统上，考虑到从备份复制数据所需的时间，快照节省的时间极为可观。快照不能替代完整的池备份，但它提供了一种快速便捷的手段，在特定时刻保存数据集的副本。
...
ZFS 提供了不同的压缩算法，各有优劣。`lz4_compress` 功能标志引入了 LZ4 压缩，可在不牺牲其他算法性能的前提下压缩整个池。LZ4 的最大优势是*提前中止*特性：如果 LZ4 在数据的头部未达到至少 12.5% 的压缩率，ZFS 会将该块数据按未压缩状态写入，避免将 CPU 周期浪费在已压缩或不可压缩的数据上。有关 ZFS 中不同压缩算法的详细信息，请参见术语部分的压缩条目。
...
启用去重后，ZFS 通过每个块的校验和来检测重复块。新块与现有块重复时，ZFS 写入一个指向现有数据的引用，而非写入整个重复块。若数据包含大量重复文件或重复信息，可节省可观的存储空间。但需注意：去重需要大量内存，而压缩在不增加额外开销的情况下即可提供大部分空间节省。
...
ZFS 快照采用写时复制（Copy-on-Write, COW）机制，创建过程瞬时完成且不占用额外磁盘空间。快照创建后，原文件系统中的数据块发生修改时，ZFS 将新数据写入新位置，旧数据块原地保留，快照继续指向旧数据块，从而捕获文件系统在快照创建时刻的时间点状态。与虚拟机快照相比，ZFS 快照在文件系统层面实现，具有更轻量、更高效的特性，广泛用于数据备份、版本控制与灾难恢复。
