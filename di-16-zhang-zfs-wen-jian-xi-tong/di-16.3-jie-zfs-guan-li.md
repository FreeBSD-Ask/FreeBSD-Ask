...
克隆创建时，它是快照创建时刻数据集的完整副本。此后，克隆可独立于原始数据集进行更改。两者之间由快照关联，ZFS 在 `origin` 属性中记录这一关联。使用 `zfs promote` 提升克隆后，克隆将变为独立的数据集，`origin` 属性的值已清除，克隆与快照之间的关联也随之断开。以下示例展示了这一过程：
