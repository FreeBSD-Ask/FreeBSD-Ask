...
> **思考题**
>
> 探索更优的快照回退方案。可参考中文社区的 [ZFS 脚本项目](https://github.com/FreeBSD-Ask/zfs-snap)，或向 OpenZFS 项目提交功能请求与 Pull Request。

与虚拟机快照不同，在默认状态下，`zfs rollback` 命令只能回退到最新快照（[参考手册](https://docs.oracle.com/cd/E19253-01/819-7065/gbcxk/index.html)，Oracle 官方 ZFS 回退命令文档）；使用 `-r` 参数可以销毁比目标快照更新的所有快照，从而允许回退到非最新的快照。ZFS 不支持一次性递归回退所有子数据集，需要对每个子文件系统单独执行回退操作。
...
- FreeBSD Project. zfs -- configures ZFS datasets[EB/OL]. [2026-04-14]. <https://man.freebsd.org/cgi/man.cgi?zfs(8)>. ZFS 数据集管理工具手册页，涵盖快照、克隆与回退操作。
...
- OpenZFS Project. zfs-rollback(8)[EB/OL]. [2026-04-18]. <https://openzfs.github.io/openzfs-docs/man/master/8/zfs-rollback.8.html>. 详细说明了 -r 参数用于销毁更新的快照，而非递归回退子数据集。
...
2. 选取 ZFS 快照与回退的核心机制，编写一个最小化脚本实现递归回退所有子文件系统的功能，分析为什么 ZFS 默认不支持一次性递归回退，尝试回溯上游。
