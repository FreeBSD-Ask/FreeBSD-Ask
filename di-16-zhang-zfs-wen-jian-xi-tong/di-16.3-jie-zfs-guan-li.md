...
ZFS 中的快照具备许多其他文件系统所不具备的功能。快照的一个典型用途是，在执行软件安装或系统升级等高风险操作之前，快速备份当前文件系统状态。如果操作失败，可回滚到快照，将系统恢复到快照创建时的状态。如果升级成功，则删除快照以释放空间。如果没有快照，升级失败通常需要从备份恢复，不仅繁琐耗时，还可能导致系统长时间不可用。快照回滚速度很快，即使系统在正常运行期间执行回滚，也几乎不会造成停机。对于多 TB 的存储系统而言，考虑到从备份复制数据所需的时间，快照节省的时间极为可观。快照不能替代完整的池备份，但它提供了一种快速便捷的手段，在特定时刻保存数据集的副本。
...
ZFS 提供 19 个 Zstd 压缩级别，每个级别逐步增加空间节省量，但压缩速度随之降低。默认级别为 `zstd-3`，其压缩比高于 LZ4，同时速度不会慢太多。级别 10 以上需要大量内存来压缩每个块，因此内存小于 16GB 的系统不应使用。ZFS 还提供一些 `Zstd_fast_` 级别，速度更快但压缩比更低。ZFS 支持 `zstd-fast-1` 至 `zstd-fast-10`、`zstd-fast-20` 至 `zstd-fast-100`（以 10 为增量），以及 `zstd-fast-500` 和 `zstd-fast-1000`，这些级别压缩比最低，但性能极高。
...
ZFS 快照采用写时复制（Copy-on-Write, COW）机制，创建过程瞬时完成且不占用额外磁盘空间。快照创建后，原文件系统中的数据块被修改时，ZFS 将新数据写入新位置，旧数据块原地保留，快照继续指向旧数据块，从而捕获文件系统在快照创建时刻的时间点状态。与虚拟机快照相比，ZFS 快照在文件系统层面实现，具有更轻量、更高效的特性，广泛用于数据备份、版本控制与灾难恢复。
