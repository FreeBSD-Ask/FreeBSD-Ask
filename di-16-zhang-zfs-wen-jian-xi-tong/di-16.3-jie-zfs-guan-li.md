...
要递归重命名快照，可使用 `-r` 选项，这将重命名所有子数据集中具有相同名称的快照。
...
```sh
# zfs list all  # 列出所有 ZFS 文件系统及其属性
NAME                 USED  AVAIL  REFER  MOUNTPOINT
zroot               3.94G  46.0G    96K  /zroot
zroot/ROOT           886M  46.0G    96K  none
zroot/ROOT/default   886M  46.0G   886M  /
...
将输出与表格对照可知，ZFS 在创建 `zroot/var/tmp@test` 快照之后添加了 **COPYRIGHT** 文件，这也导致挂载在 `/var/tmp` 的父目录发生了修改。
