...
>
>一个命令的选项或参数可能有几十上百个，而字典或文档（man 也好 info 也罢）是用来查的不是用来背的。请记住这一点——“吾生也有涯，而知也无涯。以有涯随无涯，殆已”
...
`ls` 命令的基本用法上面已经介绍过了，下面试着让 ls 以人类易读的方式列出文件大小：
...
>
>在该操作中，意为先进入目录 `/home/ykla/test/`，删除 `/home/ykla/test/` 下的所有空目录和文件；然后进入 `/home/ykla/test/` 下的子目录（如有），删除子目录下的所有空目录和文件；然后再进入到子目录的子目录中（如有），删除子目录的子目录下的所有空目录和文件……如此循环。当最后一个子目录为空时，直接删除之，然后向上删除所有上级目录。直至 `/home/ykla/test/`。
...
```sh
root@ykla:/ # mv -v test /home/ykla # -v 选项可以帮我们看到文件的变动，是 verbose 的缩写，即“啰嗦”一些，意为输出详细信息
test -> /home/ykla/test
```
...
```sh
root@ykla:/ # cp -v /usr/ports/editors/vscode /home/ykla
...
```sh
root@ykla:/ # cp -vr /usr/ports/editors/vscode /home/ykla
