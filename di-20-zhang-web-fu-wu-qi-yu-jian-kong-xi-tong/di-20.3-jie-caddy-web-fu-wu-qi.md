...
Caddy 是一个现代化的 Web 服务器，具有自动 HTTPS 配置等特性。

本节说明自动 TLS 证书管理的配置要点。
...
2. 加固 Caddy，使其适用于生产环境。
3. Caddy 以自动 HTTPS 和默认安全配置为差异化特性，其架构决策倾向于减少运维人员在 TLS 管理上的主动性。分析“自动化优先”的 Web 服务器设计对运维人员审计能力的削弱，并讨论可观测性与自动化程度之间是否存在零和关系。
