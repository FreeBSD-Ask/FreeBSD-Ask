# 20.7 Zabbix 监控系统（基于 PostgreSQL）

Zabbix 是一款开源的企业级监控解决方案，用于监控网络服务、服务器硬件及应用程序，其架构采用服务器-代理模式，能够实现基础设施监控。

pkg 安装 Zabbix 7 将额外引入 MySQL 依赖并导致绑定冲突，故本节采用 Ports 方式，基于 PostgreSQL 给出完整安装与配置步骤。
...
也可采用 Ports 方式安装，该方式提供更大的配置灵活性，需先进入对应目录再执行编译安装命令：
...
Zabbix 前端需要通过 PHP 访问 PostgreSQL 数据库，因此需要安装相应的 PHP 数据库扩展，PDO（PHP Data Objects）是 PHP 中用于数据库访问的统一接口。所使用的 PHP 版本须与 Zabbix 前端兼容。
...
需要配置 Zabbix Server 的主要配置文件，使其能够连接到 PostgreSQL 数据库。
