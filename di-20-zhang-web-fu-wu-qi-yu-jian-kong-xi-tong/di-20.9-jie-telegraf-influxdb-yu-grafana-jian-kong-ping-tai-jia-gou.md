# 20.9 Telegraf、InfluxDB 与 Grafana 监控平台架构
...
其中 InfluxDB 作为时序数据库专门用于高效存储时间序列数据，Telegraf 作为插件化的采集代理负责指标收集，Grafana 则提供可视化能力。
...
需要配置 Telegraf 使其能够将采集的数据发送到 InfluxDB。此处使用的是 InfluxDB 1.8 版本。
...
此处将收集系统的 CPU、磁盘（disk）、磁盘 IO（diskio）、内存（memory）、交换空间（swap）等指标。下面是 Telegraf 配置文件中的部分内容，其中部分参数默认启用，部分参数需要手动取消注释。
...
使用 Ports 方式安装 Grafana，需先进入对应目录再执行编译安装命令。
