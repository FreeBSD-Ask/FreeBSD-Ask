...
系统监控是运维管理和性能优化的基础，通过实时采集、存储和可视化系统指标，可完成故障预警、性能分析和容量规划。

Telegraf、InfluxDB 与 Grafana 构成的 TIG 堆栈是业界广泛采用的开源监控解决方案，遵循数据采集、存储与可视化分离的设计原则，构成模块化的监控架构。
...
使用 pkg 包管理器安装 InfluxDB 是最快捷的方式，执行以下命令完成安装：

```sh
# pkg install influxdb
```

或者使用 Ports 方式安装 InfluxDB，需先进入对应目录再执行编译安装命令。
...
需在 InfluxDB 中创建数据库和用户，供 Telegraf 使用。
...
使用 pkg 包管理器安装 Telegraf 是最快捷的方式，执行以下命令完成安装：

```sh
# pkg install telegraf
```

或者使用 Ports 方式安装 Telegraf，需先进入对应目录再执行编译安装命令。
...
使用 pkg 包管理器安装 Grafana 是最快捷的方式，执行以下命令完成安装：
...
> **注意**
>
> 上面使用的是 InfluxDB 1.8，所以选择的查询语言必须选 `InfluxQL`（即为默认选项）
...
数据源配置完成后，可配置 Dashboard 可视化展示监控数据。选择用于展示数据的 Dashboard，可自行开发，也可使用 [官方模板库](https://grafana.com/grafana/dashboards/) 中社区贡献的仪表板资源。
