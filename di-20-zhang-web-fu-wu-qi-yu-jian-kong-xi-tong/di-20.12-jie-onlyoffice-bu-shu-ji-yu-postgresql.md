...
OnlyOffice 默认配置不允许私有 IP 地址访问，需修改配置以允许局域网访问。故需将以下片段加入 **/usr/local/etc/onlyoffice/documentserver/local.json** 文件：
...
在使用 OnlyOffice 过程中如遇到问题，可查看相关日志文件排查。本节还列出了一些有待完善的事项。
