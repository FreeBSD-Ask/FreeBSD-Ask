...
上述方案在属主差异较少时尚可适用，差异较多时则不够便捷。此时可借助 bindfs 更为适宜。Bindfs 类似于 nullfs，但在将目标目录挂载到其他位置时，可覆盖文件的属主和所属组。
...
但是利用好 OpenList 的 markdown（`top.md`、`bottom.md`、`readme.md`）机制，可实现基本的元数据展示功能。
