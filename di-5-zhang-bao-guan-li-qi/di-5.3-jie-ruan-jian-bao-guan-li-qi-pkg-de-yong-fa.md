...
本章系统讲解 FreeBSD 二进制包管理器 pkg 的核心功能与使用方法。pkg 作为 FreeBSD 现代包管理体系的核心组件，提供了高效、可靠的软件安装、更新与卸载机制。
...
Ports 框架可以将源代码编译并打包成 pkg 格式的二进制包，这个过程涉及多个步骤。下面的流程图展示了完整的构建流程。
...
FreeBSD 镜像站（无论是官方的还是非官方的）源存在以下典型现象：
