# 17.1 使用 bhyve 及 vm-bhyve 工具安装 Windows 11
...
安装模式下，`vm-bhyve` 将等待 VNC 客户端连接完毕后才启动虚拟机，以捕捉 Windows 可能弹出的“Press any key to boot from a CD/DVD”提示。此时 `vm list` 中可看到虚拟机显示为锁定状态：
...
安装过程中系统会执行更新，可能可以跳过此步骤（经测试 `oobe\bypassnro` 无效），请根据需要自行探索。
