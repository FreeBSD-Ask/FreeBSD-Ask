...
> **警告**
>
> 根据 OpenZFS 社区文档，对内存压力极高的系统，无论 swap 空间剩余多少，使用 zvol 作为 swap 设备都可能导致系统锁死（参见 [Swap deadlock in 0.7.9](https://github.com/openzfs/zfs/issues/7734)）。而将 swap 放在其他分区又可能影响 ZFS 对 swap 的数据校验，因此上游文档建议在极端内存压力场景下谨慎使用 swap。此外，swap 对系统休眠功能至关重要，如果需要该功能，需至少保证 swap 的容量不小于系统内存容量。
...
- `-o com.sun:auto-snapshot=false`：ZFS 用户自定义属性，供第三方工具 zfs-auto-snapshot（需通过 ports 安装 **filesystems/zfstools**）读取——设 `false` 将禁用该工具对此 swap 卷的自动快照，因 swap 数据无需备份
