...
通过 pkg 安装将额外引入 MySQL 依赖，并可能导致绑定冲突，故建议采用 Ports 方式安装，以更精确地控制依赖关系与配置选项。首先进入对应的 Ports 目录并进行配置。
...
也可采用 Ports 方式安装，配置更灵活，需先进入对应目录再执行编译安装命令：
...
Zabbix 前端需要通过 PHP 访问 PostgreSQL 数据库，因此需要安装相应的 PHP 数据库扩展，PDO（PHP Data Objects）是 PHP 访问数据库的统一接口。所使用的 PHP 版本需要与 Zabbix 前端兼容。
