...
通过 pkg 安装将额外引入 MySQL 依赖，并可能导致绑定冲突，故建议采用 Ports 方式安装，从而更精确地控制依赖关系与配置选项。首先进入对应的 Ports 目录并进行配置。
...
3. 完善 Zabbix 的简体中文翻译。
4. 对 Zabbix 进行安全加固：配置 HTTPS、限制 Agent 访问 IP。
