# 24.2 Prometheus 监控部署

Prometheus 是一款开源的时间序列监测系统和警报工具包，用于实时记录纯数字的时序数据，并提供性能监控和告警功能。以 Prometheus TSDB 为时序数据库，通过 exporters 采集节点系统负载、存储与内存指标，并部署 Alertmanager 实现告警通知。
...
使用 pkg 包管理器安装 Prometheus 是最快捷的方式：

```sh
# pkg install prometheus
```

或者使用 Ports 方式安装 Prometheus，需先进入对应的目录再执行安装命令。
...
或者使用 Ports 方式安装 Grafana，需先进入对应目录再执行编译安装命令。
...
使用 pkg 包管理器安装 Node Exporter 是最快捷的方式：

```sh
# pkg install node_exporter
```

或者使用 Ports 方式安装 Node Exporter，需先进入对应目录再执行编译安装命令。
...
直接查看数据或 Panel 不够便捷，可使用 Grafana 以更友好的方式展示数据。
...
在默认情况下，仅 Grafana 登录需要密码。各组件之间以 HTTP 连接，例如可通过访问 `http://ip:9100/` 直接获取 Node Exporter 的监控数据，Prometheus 可通过 `http://ip:9090/` 直接访问。

在生产环境中直接暴露这些信息存在安全风险，因此需完成安全认证。
...
如果对安全要求较高，还可使用 CA 证书认证方式加强安全，但这不是每个 exporter 都支持的认证方式。
...
指定 SAN（Subject Alternative Name）是必要的，否则可能无法访问。也可以在 Prometheus 中配置忽略证书验证，但这与安全原则相悖，本文不予赘述。
...
- `alert` 指定告警名称。
- `expr` 指定告警触发条件表达式，这里使用 `absent(jail_id{name="dox"})` 表示当 jail dox 的指标（jail_id）不存在时触发告警。
...
出于安全考量，应修改 **/usr/local/etc/influxd.conf** 文件，在 http 段开启 http 认证：
...
Prometheus 提供了查询语言 PromQL，用于查询和分析监控数据。Grafana 可解析 PromQL 并基于其构建仪表板。用户还可使用 PromQL 编写自己的临时查询，从而快速搜索，无需先构建仪表板。

### 导出器

提取、格式化和发送指标的逻辑编码在导出器中。特定软件（如数据库）有多个对应的导出器可供选择。RabbitMQ、GitLab 和 Grafana 等应用程序支持将自身应用状态导出为 Prometheus 兼容的格式以供监控。
...
1. 对 Prometheus 进行安全加固：配置 HTTPS 和基本认证。
2. 完善 Prometheus 的简体中文翻译。
