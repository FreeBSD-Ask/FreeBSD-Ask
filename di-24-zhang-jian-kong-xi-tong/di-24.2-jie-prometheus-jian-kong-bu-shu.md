...
使用 pkg 包管理器安装 Prometheus 是最快捷的方式，执行以下命令完成安装：
...
使用 pkg 包管理器安装 Node Exporter 是最快捷的方式，执行以下命令完成安装：
...
Prometheus 提供了名为 PromQL 的查询语言，用于查询和分析监控数据。Grafana 可解析 PromQL 并基于其构建仪表板。用户还可使用 PromQL 编写自己的临时查询，从而快速搜索，无需先构建仪表板。
...
告警管理器是 Prometheus 的重要组件，当特定事件发生时，它可发送多种类型的通知（电子邮件、短信、寻呼器、聊天消息）。可配置告警规则，在特定事件发生时发送通知（例如系统是否可达），或在特定阈值被触发时（例如剩余磁盘空间仅 10%）发送通知。
