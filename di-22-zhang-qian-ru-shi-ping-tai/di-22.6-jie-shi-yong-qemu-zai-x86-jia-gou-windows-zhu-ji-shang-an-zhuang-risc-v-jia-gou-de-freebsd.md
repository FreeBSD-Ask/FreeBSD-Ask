# 22.6 使用 QEMU 在 x86 架构 Windows 主机上安装 RISC-V 架构的 FreeBSD

QEMU（Quick Emulator）是一款通过纯软件模拟的开源虚拟机监视器（VMM，Virtual Machine Monitor）。
...
此外，该镜像默认没有为普通用户配置 SSH 服务，因此无法直接通过 SSH 连接 FreeBSD 设备。
