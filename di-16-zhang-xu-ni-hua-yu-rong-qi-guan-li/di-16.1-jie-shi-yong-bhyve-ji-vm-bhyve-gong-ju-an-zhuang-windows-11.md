# 16.1 使用 bhyve 及 vm-bhyve 工具安装 Windows 11

vm-bhyve 是 bhyve 的命令行前端工具，封装了底层 bhyve 命令，降低了手动配置虚拟机的复杂度。

本节基于 FreeBSD 14.2-RELEASE 与 Windows 11 IoT Enterprise LTSC，覆盖 bhyve 虚拟机管理工具安装、环境配置与系统镜像准备。
...
编辑 **/etc/rc.conf** 文件，指定启动 vm 与虚拟机的位置：
...
随后配置虚拟网络。`public` 是模板指定的虚拟交换机名称，`ue0` 为当前宿主机的物理网络接口名称，请根据实际网卡修改（可通过命令 `ifconfig` 查看）。虚拟交换机借助 bridge(4) 完成网络桥接，连接虚拟机网络栈与宿主机物理网络接口，虚拟机便可直接访问物理网络。
...
若在 FreeBSD 13.0 及以上版本的宿主机中使用 xHCI 鼠标，需启用 USB HID 子系统，请在 **/boot/loader.conf** 文件中添加：
...
默认模板存在问题，须予修改。配置文件路径为 **/home/ykla/vm/winguest/winguest.conf**：
...
安装模式下，`vm-bhyve` 将等待 VNC 客户端连接完毕方启动虚拟机，以捕捉 Windows 可能弹出的"Press any key to boot from a CD/DVD"提示。此时 `vm list` 中可看到虚拟机显示为锁定状态：
...
安装过程中系统会执行更新，可能可跳过此步骤（经测试 `oobe\bypassnro` 无效），请根据需要自行探索。
...
本节列出 bhyve 虚拟机使用过程可能遇到的问题及解决方法。

如遇问题，请先重启宿主机；若问题仍然存在，可借助 `ifconfig` 检查是否存在多余网卡并予以删除。

若虚拟机长时间保持 stopped 状态，请检查网络配置。
...
3. bhyve 通过 virtio 半虚拟化驱动模型实现 I/O 加速，但 Windows 默认不包含 virtio 驱动。比较 bhyve 的 virtio 方案与 VirtualBox Guest Additions 在跨平台驱动分发策略上的差异，分析将 Windows virtio 驱动签名纳入 FreeBSD Ports 构建流程的可行性。
