...
Podman 是由红帽（Red Hat）主导开发的开源容器运行时，是 Docker 的替代方案，命令行接口与 Docker 高度兼容。Podman 采用无守护进程（daemonless）架构，每个容器作为独立进程运行，无需始终运行后台守护进程，系统安全性因此提高，单点故障风险随之降低。
...
网络配置是确保容器能够正常访问外部网络的关键步骤。Podman 借助 PF（Packet Filter）防火墙完成容器网络的地址转换与流量控制，容器得以通过宿主机网络接口访问外部网络。
