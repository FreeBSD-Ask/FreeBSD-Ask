...
Podman 是由红帽（Red Hat）主导开发的开源容器运行时，是 Docker 的替代方案，命令行接口与 Docker 高度兼容。Podman 采用无守护进程（daemonless）架构，每个容器作为独立进程运行，无需始终运行后台守护进程，从而提高了系统安全性并降低了单点故障风险。

在 FreeBSD 平台上，Podman 基于原生的 Jail 机制实现容器隔离，可使用命令 `jls` 查看当前运行的容器。

## 安装 Podman

在安装 sysutils/podman-suite 元包时，会同时安装 sysutils/buildah（用于构建容器镜像）、sysutils/podman（容器运行时）和 sysutils/skopeo（用于操作容器镜像）。
...
网络配置是确保容器能够正常访问外部网络的关键步骤。Podman 以 PF（Packet Filter）防火墙实现容器网络的地址转换和流量控制，使容器可以通过宿主机网络接口访问外部网络。
...
为了管理容器的存储资源，建议创建专用的 ZFS 文件系统，具体操作如下：
...
服务启动完成后，可以通过测试拉取 Ubuntu 镜像验证 Podman 是否正常工作：
...
除了 Linux 镜像外，Podman 也可以运行 FreeBSD 原生容器。本节测试一个由 FreeBSD 维护者打包的 Nginx 容器：
...
3. OCI 容器镜像标准最初为 Linux cgroups 与命名空间设计，FreeBSD 的容器实现需在此基础上做平台适配。分析 FreeBSD Jail 与 Linux 命名空间在隔离原语上的语义差异，并讨论这些差异是否阻碍了 OCI 镜像在跨平台场景下的“一次构建，到处运行”承诺。
