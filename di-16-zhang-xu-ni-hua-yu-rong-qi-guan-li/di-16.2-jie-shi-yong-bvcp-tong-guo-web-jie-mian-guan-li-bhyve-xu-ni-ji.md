# 16.2 使用 BVCP 通过 Web 界面管理 bhyve 虚拟机

bhyve 是 FreeBSD 原生的虚拟机管理程序（hypervisor），以 vmm 内核模块提供硬件辅助虚拟化支持，客户机操作系统可直接于 CPU 虚拟化扩展模式执行，可在 FreeBSD 宿主机上运行多种客户机操作系统。

BVCP（bhyve Virtual-Machine Control Panel）是 bhyve 的 Web 图形化管理工具，封装了 bhyve 命令行操作，降低了虚拟机管理的复杂度，实现虚拟机的可视化配置与生命周期管理。
...
详细步骤与前文 Ubuntu 24.04 的安装说明类似，可参见前一节“使用 bhyve 及 vm-bhyve 工具安装 Windows 11”，此处仅列出关键的不同之处。
...
如需卸载 BVCP，可参考官方文档进行操作（FreeBSD bhyve Project. Uninstallation of BVCP[EB/OL]. [2026-03-26]. <https://bhyve.npulse.net/uninstall>）。
