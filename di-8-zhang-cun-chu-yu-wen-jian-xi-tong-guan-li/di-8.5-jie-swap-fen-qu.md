...
如果在系统安装阶段未配置 swap（交换分区），则仅能通过 dd 命令创建交换文件或 ZFS 卷来实现，这是因为 UFS 及 ZFS 文件系统均不支持分区收缩操作。
