...
对于快照文件，[unlink(1)](https://man.freebsd.org/cgi/man.cgi?query=unlink&sektion=1&format=html) 是例外，它们可以被删除。

快照是通过 [mount(8)](https://man.freebsd.org/cgi/man.cgi?query=mount&sektion=8&format=html) 创建的。要将 **/home/ykla** 的快照放入文件 **/var/snapshot/snap** 中，请使用以下命令：

```sh
# mount -u -o snapshot /var/snapshot/snap /home/ykla
```

或者，使用 [mksnap_ffs(8)](https://man.freebsd.org/cgi/man.cgi?query=mksnap_ffs&sektion=8&format=html) 创建快照：

```sh
# mksnap_ffs /home/ykla /var/snapshot/snap
```

可以使用 [find(1)](https://man.freebsd.org/cgi/man.cgi?query=find&sektion=1&format=html) 查找文件系统中的快照文件，例如 **/home/ykla**：
...
- 一些管理员会使用快照文件进行备份，因为可以将快照传输到 CD 或磁带。
...
- 可以作为文件系统的冻结镜像挂载快照。要挂载快照 **/var/snapshot/snap**，可以执行以下命令：
