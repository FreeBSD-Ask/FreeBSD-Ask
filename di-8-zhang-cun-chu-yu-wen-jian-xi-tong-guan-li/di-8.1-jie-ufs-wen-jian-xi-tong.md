...
必须在执行操作的文件系统中创建快照文件，每个 UFS 文件系统最多可创建 20 个快照。活动快照会记录在超级块中，因此在卸载和重新挂载操作以及系统重启后仍可保持持久性。当不再需要某个快照时，可使用 [rm(1)](https://man.freebsd.org/cgi/man.cgi?query=rm&sektion=1&format=html) 删除。尽管可以按任意顺序删除快照，但可能无法彻底回收所有已使用的空间，因为其他快照可能会占用部分已释放的块。

可使用 [mksnap_ffs(8)](https://man.freebsd.org/cgi/man.cgi?query=mksnap_ffs&sektion=8&format=html) 创建 `/` 的快照，并存储至 `/.snap/snap1`：
...
快照亦可通过 [mount(8)](https://man.freebsd.org/cgi/man.cgi?query=mount&sektion=8&format=html) 创建。若要将 **/** 的快照存入文件 **/home/ykla/snapshot/snap**，可使用以下命令：
...
不可变更的 `snapshot` 文件标志由 [mksnap_ffs(8)](https://man.freebsd.org/cgi/man.cgi?query=mksnap_ffs&sektion=8&format=html) 在快照文件初始创建时设置。

可使用 [find(1)](https://man.freebsd.org/cgi/man.cgi?query=find&sektion=1&format=html) 查找文件系统中的快照文件，例如 **/**：
...
可作为文件系统的冻结镜像挂载快照。若要只读挂载快照 **/home/ykla/snapshot/snap**，可执行以下命令：
...
通常，在启动时，系统会通过 [quotacheck(8)](https://man.freebsd.org/cgi/man.cgi?query=quotacheck&sektion=8&format=html) 检查每个文件系统的配额完整性。此程序确保配额数据库中的数据正确反映文件系统中的数据。此过程较为耗时，会显著影响系统的启动时间。要跳过此步骤，可在 **/etc/rc.conf** 中添加以下变量：
