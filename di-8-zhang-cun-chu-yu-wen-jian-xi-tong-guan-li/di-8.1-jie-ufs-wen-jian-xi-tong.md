...
磁盘配额可用于限制用户或用户组在每个 UFS 文件系统上可分配的磁盘空间或文件数量，从而防止某个用户或用户组消耗掉所有可用的磁盘空间。
...
硬限制不可超过。当用户达到硬限制时，该用户在该文件系统上将无法再分配任何空间。例如，如果用户在文件系统上的硬限制为 500 KB，并且当前使用了 490 KB，则该用户只能分配额外的 10 KB。尝试分配额外的 11 KB 将失败。
