...
手动编辑 `/etc/resolv.conf` 后，重启系统时该文件会被重置，因为动态主机配置协议（Dynamic Host Configuration Protocol，DHCP）会重写该文件。这一行为是系统网络自动配置机制的一部分。
...
尝试使用 `ping` 命令测试与 163.com 的连通性（按 Ctrl + C 可中断），示例输出如下：
