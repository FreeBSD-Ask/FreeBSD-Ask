...
- resolvconf[EB/OL]. [2026-03-26]. <https://man.freebsd.org/cgi/man.cgi?query=resolvconf>，man 手册。提供 resolvconf 工具的完整技术文档，为 DNS 配置管理提供权威参考。
- 8.8.8.8 or 1.1.1.1 if set in etc resolv conf doesn't stay as an entry in the file after a network restart[EB/OL]. [2026-03-26]. <https://forums.freebsd.org/threads/8-8-8-8-or-1-1-1-1-if-set-in-etc-resolv-conf-doesnt-stay-as-an-entry-in-the-file-after-a-network-restart.85951/>，实际案例分析，提供防止 DNS 配置被覆写的解决方案。
...
- ifconfig(8)[EB/OL]. [2026-03-26]. <https://man.freebsd.org/cgi/man.cgi?ifconfig(8)>，提供 ifconfig 命令的完整技术文档，为网络接口配置提供权威参考。
