...
传输控制协议（Transmission Control Protocol，TCP）是互联网协议套件中的核心协议之一，其实现被称为 TCP 堆栈（按层次结构叠加，因此称为“栈”）。
...
TCP BBR（Bottleneck Bandwidth and Round-trip propagation time）是 Google 开发的一种拥塞控制算法。其主要作用有两个：

- 充分利用带宽；
- 降低延迟。

通常来说，如果使用代理软件，建议启用 TCP BBR 功能，这在提升速度和稳定性方面效果显著。在中国大陆地区，该技术主要用于网络优化。在 FreeBSD 中，TCP BBR 由 Netflix 团队开发。
...
- [netflix/tcplog_dumper](https://github.com/netflix/tcplog_dumper) [备份](https://web.archive.org/web/20260118025036/https://github.com/netflix/tcplog_dumper)
...
RACK 和 BBR 在局域网表现良好，均优于默认堆栈；但在互联网上表现不佳：RACK 的互联网速度约为默认堆栈的三分之一，BBR 则约为默认堆栈的六分之一。
