...
> **技巧**
>
> 如果提示 `pkg` 未找到该命令或未提供 kde 软件包，可能是二进制包尚未构建完成，或需切换软件源分支。参考本书其他相关章节。如无二进制包，则需使用上述 Ports 编译安装。
...
> **注意**
>
> 如果之前以 root 身份执行过上述命令，新用户仍需再执行一次才能正常运行 startx（无需 root 权限或 sudo）。
...
点击应用程序启动器 → System Settings → Language & Time，在 Region & Language 的 Language 栏点击 Modify，找到并选择“简体中文”。如果显示为 `□□□□`，请检查中文字体是否已安装。随后单击 Apply 按钮；注销后重新登录，此时系统语言将变为中文。
