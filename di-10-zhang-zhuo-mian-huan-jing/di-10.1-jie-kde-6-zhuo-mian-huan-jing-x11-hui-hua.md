...
> **技巧**
>
> 如果提示 `pkg` 找不到或未提供 kde，可能是二进制包尚未构建，或需切换软件源分支。参考本书其他相关章节。如无二进制包，则需使用上述 Ports 编译。
...
点击开始 → System Settings → Language & Time，在 Region & Language 的 Language 栏点击 Modify，找到并选择“简体中文”。如果显示为 `□□□□`，请检查中文字体是否已安装。随后单击 Apply 按钮；注销后重新登录，此时系统语言将变为中文。
