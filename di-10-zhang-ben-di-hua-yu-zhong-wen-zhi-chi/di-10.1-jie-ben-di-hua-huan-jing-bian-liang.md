...
- `MM_CHARSET`：设置 MIME 字符集，供应用程序在处理多语言内容时确定字符编码。该变量由 FreeBSD Handbook 推荐设置，但并非 FreeBSD libc 的 `setlocale(3)` 标准变量（参见 FreeBSD `environ(7)` 手册页）
...
将三个环境变量都设置为 `"zh_CN.UTF-8"` 的原因是不同软件读取本地化变量的优先级存在差异，此配置策略可避免由此导致的语言设置不一致。
