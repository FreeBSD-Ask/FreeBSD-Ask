...
使用 `locale` 命令查看当前系统的本地化设置，使用 `locale -a` 可列出系统已安装的所有可用 locale 清单，确认 `zh_CN.UTF-8` 是否在列：
...
将三个环境变量都设置为 `"zh_CN.UTF-8"` 的原因是不同软件读取本地化变量的优先级存在差异，此配置策略可避免因软件读取优先级差异导致语言设置不一致。
