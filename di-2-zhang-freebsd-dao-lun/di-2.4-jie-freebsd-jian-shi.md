...
在贝尔实验室退出 Multics 项目前，Dennis Ritchie 和 Ken Thompson 已经意识到了 Multics 的潜力。1969 年，Ken Thompson 在一台闲置的 PDP-7 计算机上着手开发一款新程序 Unics（Uniplexed Information and Computing Service，**单路复用** 信息和计算服务）。随后在 1970 年初，他们以开发文本处理（text processing）的名义提出购买申请，在研究部门主管 Lee McMahon 和 Doug McIlroy 的支持下获得资金，于同年 5 月下达订单，夏末收到了 PDP-11/20 计算机，将 Unics 移植到了这台性能更强的机器上。
...
1974 年，加州大学伯克利分校的 Bob Fabry 教授从 AT&T 获得了 UNIX 的源代码许可证。Bob Fabry 此前在 1973 年的国际计算机学会（Association for Computing Machinery，ACM）操作系统原理研讨会上见过 UNIX 第 4 版，并有意引入加州大学伯克利分校。1979 年，Bob Fabry 在伯克利成立了计算机系统研究小组（CSRG）。1980 年 4 月，美国国防高级研究计划局（DARPA）与伯克利签订合同后，CSRG 开始基于 AT&T Research Unix 进行系统性修改和改进，并将修改后的版本称为“BSD Unix”或“BSD”。

- 1978 年 3 月 9 日：1BSD 发布

基于 UNIX 创建的伯克利软件发行版（1BSD）是 UNIX 第六版的一种附加组件，而非独立完整的操作系统。此版本发行了大约 30 份。

- 1979 年 5 月 10 日：2BSD 发布（最终版本）

第二款伯克利软件发行版（2BSD）初版发布于 1978 年，经过增补后完整发布于 1979 年 5 月。涉及 1BSD 的软件更新，以及由 Bill Joy 新开发的两个至今仍在 UNIX 系统上使用的程序：vi 文本编辑器（ex 的可视化版本）和 Csh。2BSD 是 Bill Joy 参与 PDP-11 相关工作的最后一个 BSD 版本，发行了约 75 份。
...
ipfirewall（IPFW）随 FreeBSD 2.0-RELEASE 引入，这种采用“首次匹配（First Match）”规则的防火墙自此成为系统的重要组成部分。ipfw 曾作为 Mac OS X（10.0 至 10.6 版本）的内置数据包过滤防火墙（自 Mac OS X 10.7“Lion”起，Apple 将默认防火墙替换为 PF）。
...
kqueue(2) 是旨在替代 select/poll 的创新解决方案，于 2000 年 7 月 27 日随着 FreeBSD 4.1-RELEASE 引入。这一可扩展的事件通知接口与后来 Linux 内核中引入的 epoll 机制在目标和设计思路上具有相似性。
...
- 2004 年 3 月：首届 AsiaBSDCon；5 月首届 BSDCan
...
- 2005 年 11 月 1 日：新的 FreeBSD Logo
...
- 2012 年 7 月：Ports 开始从 CVS 向 Subversion 迁移，2013 年 2 月完成

Ports 自 2012 年 7 月起进入 CVS 与 SVN 双轨运行期，项目于 2013 年 2 月 28 日正式关闭 CVS 访问，彻底完成了向 Subversion 的过渡。
...
- 2012 年 9—11 月：黑客入侵 FreeBSD 集群（9 月 19 日起可能已被入侵，11 月 11 日被发现）

FreeBSD 项目集群检测到黑客入侵，攻击通过窃取开发者 SSH 密钥实现，影响第三方软件包构建系统，未涉及基础系统源代码。项目花费数月进行审计与还原。

- 2013 年 9 月 17 日：OpenZFS 项目启动

OpenZFS 项目衍生于 OpenSolaris。2013 年 9 月 17 日，ZFS 开源项目宣布 OpenZFS 成为 ZFS 的社区主导继任项目，并创建了一个正式的社区来维持开发和支持。但此时 FreeBSD 依旧使用的是最早的 OpenSolaris ZFS。
...
在 13.0-RELEASE 中，FreeBSD 将 ZFS 实现从基于 illumos 的代码库切换到了统一的 OpenZFS 2.0 代码库，使 FreeBSD 能够更快地获取 ZFS 上游改进。该迁移计划最早于 2018 年提出。

- 2024 年 8 月：德国主权技术基金赞助 FreeBSD 项目实施基础设施现代化

该项目的主要目标是改进基本系统、Ports 和软件包的安全工具，更新项目基础设施以加快开发速度、增强构建安全性，并降低新开发者的参与门槛。项目于 2024 年 8 月启动，至 2025 年 12 月完成。

- 2024 年 11—12 月：笔记本和桌面工作组 LDWG 成立
...
- 2026 年 7 月 14 日：真正自由的操作系统：基本系统完全移除 GPL 代码
