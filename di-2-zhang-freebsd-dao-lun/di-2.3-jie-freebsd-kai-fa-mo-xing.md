...
> **注意**
>
> `freebsd-update` 命令仅适用于 ALPHA、BETA、RC 和 RELEASE 版本；其余版本（如 CURRENT、STABLE 的非发布快照）需通过源代码编译或使用二进制的 PkgBase 更新。
...
CURRENT 分支中的代码在经过充分测试后（通常需满足 MFC 最短三天的要求，MFC 指 `Merge From CURRENT`，类似于 `backporting` 即向后移植）会推送到 STABLE 分支，但这并不保证两个分支都没有重大缺陷。尽管 FreeBSD-STABLE 分支应该始终能够编译并运行，但这并无保证。
...
2. 安装新的 FreeBSD-STABLE 系统，可以从 FreeBSD 镜像站获取基于 FreeBSD-STABLE 的每周快照进行安装。
