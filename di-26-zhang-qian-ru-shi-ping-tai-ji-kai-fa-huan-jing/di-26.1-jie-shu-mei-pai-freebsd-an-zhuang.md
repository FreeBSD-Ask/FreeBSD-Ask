...
| 接口类型 | 说明 |
| -------- | ---- |
| 视频输出 | HDMI × 2（micro-HDMI） |
| 显示接口 | DSI（MIPI 显示串行接口） |
| 相机接口 | CSI（MIPI 相机串行接口） |
| 音频接口 | I²S（集成电路内置音频总线） |
| USB 接口 | USB 2.0 / USB 3.0 |
| GPIO 引脚 | 通用输入输出（含 UART、I²C、SPI、PWM） |
| PCIe 接口 | PCIe 2.0/3.0 ×1（需 FPC 转接板） |
| 网络供电 | POE（以太网供电，需 POE HAT） |
| 实时时钟 | RTC（树莓派 5 内置，需外接电池） |
...
FreeBSD 对不同硬件体系结构的支持采用等级划分机制。目前，AArch64（ARM64）架构属于 [一级架构](https://www.freebsd.org/platforms/)，该架构获得了 FreeBSD 项目的核心支持。但在软件生态的完整性方面，AArch64 架构仍略逊于 AMD64（x86-64）架构，仍有部分软件包目前无法通过 FreeBSD Ports 以源代码形式成功构建。

FreeBSD 硬件架构支持等级：

| 架构等级 | 架构 | 说明 |
| -------- | ---- | ---- |
| 一级架构 | AMD64（x86-64）、AArch64（ARM64） | 核心支持，安全/勘误公告，完整 Ports 构建 |
| 二级架构 | armv7、i386、powerpc64le 等 | 提供构建支持，但无完整保障 |
| 三级架构 | mips、sparc64 等 | 实验性支持，社区维护 |
