...
|ports|Gentoo 的包管理器 Portage（命令为 `emerge`）即是源于此。用于帮助用户从源代码编译安装软件|不需要可以不配置。换言之，等同于 Gentoo 的 Distfiles 源|
|portsnap|拉取 Ports 的源代码模板（本身不含源代码，只是一些描述文件和补丁集），同 Gentoo 的源一样|已在 FreeBSD 14 中废弃，无需配置。后续版本亦不再使用，改用 `git` 或压缩包等方式获取。换言之，这个源类似 Gentoo 的 ebuild 数据库|
