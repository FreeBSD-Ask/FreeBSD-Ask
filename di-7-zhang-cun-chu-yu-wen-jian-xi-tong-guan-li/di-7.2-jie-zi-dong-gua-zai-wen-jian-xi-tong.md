...
可移动存储介质的自动挂载是操作系统的重要功能之一。

FreeBSD 提供了多种自动挂载技术方案供用户选择。
...
DSBMD（Desktop Scriptable Block Device Manager Daemon）是 FreeBSD 的介质和文件系统类型检测守护进程，采用客户端-服务器架构设计，允许客户端以受控方式挂载存储设备。其配置为开箱即用状态，默认即可实现基本功能。
