...
IPFirewall（IPFW，IP Firewall）是由 FreeBSD 项目开发和维护的防火墙软件，也是 FreeBSD 中最早的包过滤软件。IPFW 作为可加载模块存在于基本系统内核（GENERIC）中。

作为 FreeBSD 原生防火墙，IPFW 基于规则编号的优先级机制，为网络访问控制提供了灵活而精确的管理框架。规则编号越小，优先级越高，可覆盖编号较大的规则。
...
2. 开发一款 IPFW 图形化管理工具，分别支持 TUI 和 GUI。
