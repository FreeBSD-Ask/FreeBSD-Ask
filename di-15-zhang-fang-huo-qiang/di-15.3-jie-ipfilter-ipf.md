...
说明：IPF 默认允许所有流量通过，需要明确写规则以阻止访问。
...
1. **接收 traceroute 的 ICMP 超时消息**

```ini
pass in quick proto icmp from any to 192.168.1.184 icmp-type 11
```

允许接收来自中间路由器的 ICMP“时间超时”报文，用于 traceroute。
...
```ini
rdr em0 192.168.1.184/32 port 8080 -> 192.168.1.184 port 80 tcp
```

将本机 em0 网卡的 8080 端口流量转发到本机 80 端口。此规则应写入 `/etc/ipnat.rules` 文件，而非 `ipf.rules`。
