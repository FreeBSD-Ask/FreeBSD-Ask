...
> **警告**
>
> 即使已加入 `wheel` 组，也应再加入 `video` 组，视频硬件解码功能可能出现异常，且 Wayland 下普通用户将无权限调用显卡。
...
显卡驱动成功后，系统中将出现 `card0` 设备（默认编号为 `0`，如有第二块显卡则为 `card1`），同时可能还出现 `backlight0` 设备（HDMI 输出下通常不存在该设备）。
...
4. 模拟驱动故障场景：故意加载错误版本的 `drm` 内核模块，观察 `KLD depends on kernel - not available or version mismatch` 报错，然后使用 Ports 重新编译安装驱动解决问题。
