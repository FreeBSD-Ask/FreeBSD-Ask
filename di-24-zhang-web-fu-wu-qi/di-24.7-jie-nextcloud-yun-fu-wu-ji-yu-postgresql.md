...
Nextcloud 前端需经 PHP 访问，须安装 PHP 的 Apache 模块。可通过 `php -v` 命令检查当前 PHP 版本，确保与系统安装的版本一致：
...
如果在 poudriere(8) 或 Synth 中构建启用了 ZTS 的 PHP 相关 Ports，请将 WITH_MPM=event 添加到 /etc/make.conf，以防止构建失败。
...
自 Nextcloud 27.1.0 起，推荐使用 PHP 8.2 版本。
```
...
安装完成后会重定向到插件推荐，可忽略该推荐页面，重新打开 `http://ip/nextcloud` 即可正常使用。
...
Nextcloud 还有许多常用插件可安装使用。其他常用插件可通过命令 `pkg search -x nextcloud | grep php83` 查找，然后通过 pkg 安装。
