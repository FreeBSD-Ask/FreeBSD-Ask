...
> **注意**
>
> 在完成兼容层构建并执行 chroot 进入环境后，终端界面可能看似无响应；Bash 进程虽未产生输出，但仍可正常接收输入并执行命令。即使将默认 shell 切换至 Zsh，该现象仍然存在，且 TTY 终端会报告错误：`linux: jid 0 pid 1154 (bash): linux_ioctl_fallback fd=0, cmd=0x802c542a ('T',42) is not implemented`。根据 FreeBSD 源代码仓库中的相关提交记录 [linux: support termios2 ioctls](https://github.com/freebsd/freebsd-src/pull/1949)，该 bug 已在 FreeBSD 15.0-STABLE 版本中得到修复。
