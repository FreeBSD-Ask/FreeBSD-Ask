...
chroot 进入的 Linux 兼容层内部若无声音输出，通常需要在 FreeBSD 中直接运行 Linux 程序，而非使用 chroot。
...
Linux 兼容层中的代理软件会影响 FreeBSD 的网络状态，因此可直接在兼容层安装 Clash for Windows。若在兼容层中设置代理，则 FreeBSD 同样可借助该代理访问网站，按常规方式安装配置即可。

## Linux 兼容层网络（Linux QQ 无网络）

如果本机同时有以太网卡 em0 和无线网卡 wlan0，但实际使用的是 wlan0，则 Linux 兼容层可能无法正常联网。FreeBSD 的 em0 对应兼容层的 eth0，wlan0 对应 eth1，依此类推。Linux 兼容层无法自动检测哪个接口有网络，且默认使用 eth0。

若使用 wlan0 而未使用 em0，需要为 em0 分配一个未使用的 IP：
...
以上 `192.168.11.1` 为未分配的假 IP，可自行设置为其他值。
...
如果遇到类似“sh: warning: setlocale: LC_ALL: cannot change locale (zh_CN.UTF-8)”的错误，以 openSUSE 为例，安装 `glibc-lang` 和 `glibc-locale` 这两个包，退出兼容层后重新进入即可。其他系统可以搜索相似的包，例如 CentOS 兼容层，应该安装 `glibc-locale-source`、`glibc-langpack-zh`。
...
国内某些镜像站可能出于流量限制等原因，只允许通过包管理器获取软件包，禁止通过浏览器或其他方式获取软件包。当使用非指定包管理器拉取软件时，就会报错“403 Forbidden”（禁止访问）。

这通常是通过限制包管理器的 UA（User-Agent）实现的。

因此最简便的方法是更换无限制的镜像站，如 TUNA/USTC Mirror。
