# 24.3 Ubuntu/Debian 兼容层

本节基于 debootstrap 在 FreeBSD 上构建 Ubuntu 22.04 LTS 兼容层，并附 Debian 12 构建脚本，含 X11 应用运行、Windows 11 字体安装及与 systemd、Wine 相关的故障排除。

> **注意**
>
> 由于 FreeBSD Ports 中的 [sysutils/debootstrap](https://www.freshports.org/sysutils/debootstrap/) 长期未进行版本更新，经实际测试，该工具目前尚不支持 Ubuntu 24.04 和 Debian 13 发行版的基本系统引导。
...
可采用类似方法构建 Debian 兼容层。其他系统的支持情况请参见 **/usr/local/share/debootstrap/scripts/** 目录。
...
需要设定一个合适的 Linux 内核版本，否则 chroot 时可能会提示 `FATAL: kernel too old`。
...
完成文件系统挂载和内核版本设定后，进入 Ubuntu 兼容层进行后续配置。
...
- 通过软件图标定位，进入 **/usr/share/applications** 目录，根据软件包名找到对应文件，并使用文本编辑器（如 `ee`、`nano`）打开（**.desktop 文件**本质上是文本文件，而非软链接或图片），找到其中的程序启动命令并复制到终端运行即可。
...
曾尝试使用 [https://github.com/zq1997/deepin-wine](https://github.com/zq1997/deepin-wine) 源以安装 deepin-qq、deepin-wechat 等软件，但均提示“段错误”。所有 Wine 程序均无法正常运行。如果读者能解决这个问题，请提出 issue 或者 PR。
...
1. 修改 Ubuntu 兼容层脚本，使其同时支持 Ubuntu 24.04 和 Debian 13，并分析当前 debootstrap 不支持这些版本的原因，贡献上游。

2. 在 Ubuntu 兼容层中移除 **/etc/fstab** 文件中的 **/home** 挂载，或启用它，对比两种情况下用户对文件系统的访问权限差异，分析 nullfs 绑定挂载机制。

3. 复现文中提到的 Wine 段错误问题，使用系统参数 `compat.linux.debug` 开启调试日志，分析错误发生的位置，并解决。
