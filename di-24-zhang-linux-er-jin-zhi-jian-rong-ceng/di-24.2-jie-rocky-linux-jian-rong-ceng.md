...
该版本号仅具有参考意义，而非绝对标识。其原因在于：文件 **/etc/redhat-release** 是由 Rocky Linux 软件包 `rocky-release` 提供的静态文本文件。由于 Linux 发行版质上是多个独立软件包的集合体，单个软件包的版本信息无法决定发行版中所有软件包均归属于某一特定版本，Linux 发行版的版本号是一个相对概念。
