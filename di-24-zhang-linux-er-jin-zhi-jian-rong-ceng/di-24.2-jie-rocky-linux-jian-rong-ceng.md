# 24.2 Rocky Linux 兼容层

FreeBSD 通过 emulators/linux-rl9 Port（内置 linux_base-rl9）提供 Rocky Linux 9.7 兼容层。本节涵盖安装步骤、版本验证及基础配置。
...
3. Rocky Linux 作为 CentOS 终止后的 RHEL 下游替代品，其生态的前提是 RHEL 源码的可用性。分析在 IBM/Red Hat 收紧源码分发政策后，Rocky Linux 的合规风险对 FreeBSD 企业 Linux 兼容层策略的间接影响。
