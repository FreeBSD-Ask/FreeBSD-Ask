...
1. 在 FreeBSD 中，实现在 Gentoo 兼容层中移除 `FEATURES` 中的 `-sandbox` 相关选项。
2. 尝试将 Gentoo 的 Portage 的部分功能移植到 FreeBSD Ports 框架。
3. Gentoo Portage 与 FreeBSD Ports 同为源码编译包管理系统，但 Portage 以 USE 标记实现编译时功能选择，FreeBSD Ports 则以 OPTIONS 对话框实现类似功能。比较两种编译时定制机制在依赖解析与仓库维护两方面的架构差异，并讨论二者是否具备融合为一套统一元构建系统的可能。
