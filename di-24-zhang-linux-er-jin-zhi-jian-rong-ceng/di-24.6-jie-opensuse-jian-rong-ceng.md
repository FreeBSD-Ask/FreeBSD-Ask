...
本节所提供的自动化脚本采用 openSUSE Leap dnf 容器镜像作为基本系统。尽管该镜像基于 dnf 包管理器构建，但 openSUSE 中的 opi 工具会自动安装 zypper 包管理器，从而实现双包管理器的共存机制。不过，用户仍需按照 [清华大学镜像站 openSUSE 使用帮助](https://mirrors.tuna.tsinghua.edu.cn/help/opensuse/) 的指引，手动完成 zypper 软件源的配置工作。
...
同样地，当使用 openSUSE Tumbleweed 时，dnf 和 zypper 均不可用。该问题不影响 Leap 对应的脚本，但影响 Tumbleweed 对应的脚本，导致无法正常使用。已提交 [Bug 报告](https://bugzilla.opensuse.org/show_bug.cgi?id=1213158)。

例如，当尝试安装 Vim 时，会遇到一个错误信息。
...
在 LXC Leap 中，用 rpm 替换 rpm-ndb 可以解决该问题，dnf 不会出现类似问题。该问题仅存在于 zypper 中。然而，在 openSUSE Tumbleweed LXC 中，dnf 和 zypper 都存在该问题，替换 rpm-ndb 无法解决。
...
3. openSUSE 同时维护 Leap（固定发行）和 Tumbleweed（滚动发行）两个分支，其兼容层版本策略面临两难。比较固定发行版与滚动发行版在 FreeBSD 兼容层中的维护负担差异，分析滚动发行版的高频更新是否与二进制兼容层的语义冻结需求构成根本冲突。
