...
C/C++ 是两门具有悠久历史的经典编程语言，至今仍在系统编程、高性能计算、嵌入式开发等领域占据核心地位。
...
考虑到 C 与 C++ 在语法、工具链及编译流程上具有高度的相似性，本节将以 C 语言为例，详细介绍开发环境的配置方法。
...
需要注意的是，以这种方式安装的 VS Code 实际上是 [Code-OSS](https://github.com/microsoft/vscode)。Code-OSS 与 VS Code 的区别主要在于许可证不同以及可用的闭源资源不同，类似 Chromium 与 Chrome 的关系。可以阅读 [原文](https://github.com/microsoft/vscode/wiki/Differences-between-the-repository-and-Visual-Studio-Code) 以深入了解二者的区别。
...
鉴于 C/C++ 生态中可选的工具链与调试方案极为丰富，本节仅选取三种典型组合作为示例，分别对应调试适配器 `lldb-mi`，以及构建系统 `CMake + Ninja` 和 `Meson + Ninja`。本节将以 Visual Studio Code 作为前端开发环境，演示如何将这些底层工具集成于编辑器之中。

需要说明的是，C/C++ 项目的配置并无“万能模板”，实际开发中需根据项目结构和构建需求灵活调整。因此，本节将以一个极简实例为切入点：项目名称为 `test`，仅包含一个源文件 `main.c`。通过此实例，可直观理解配置文件的编写逻辑，并以此为起点应对更复杂的项目场景。
