...
Rsync（Remote Sync，远程同步）是由 Andrew Tridgell 和 Paul Mackerras 共同开发的高效文件同步工具。Rsync 采用增量传输算法，仅传输文件差异部分，显著减少网络带宽消耗与同步时间，是系统备份与数据镜像的常用工具。
...
设计需求：把服务器 B 的数据同步到服务器 A，即 B（服务端）→ A（客户端），完成数据的完整镜像。
...
2. 加固 rsync 服务安全，使之适用于生产环境。总结并提交 PR 至本节。
3. rsync 通过滚动校验和算法实现增量同步，ZFS 则通过快照差异实现块级复制。比较两种同步策略在网络带宽效率、内存开销与一致性保障方面的差异，并分析在何种场景下文件级增量同步优于块级快照传输。
