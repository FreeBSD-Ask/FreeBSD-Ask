...
FTP（File Transfer Protocol，文件传输协议）是 TCP/IP 协议栈中最早的应用层协议之一，用于在网络上传输文件。

在 FreeBSD 系统中搭建 FTP 服务器，可实现局域网或互联网环境下的文件快速传输与共享。
...
| ProFTPD | GPLv2 许可证授权、支持多种操作系统、单一主配置文件、可扩展性强 | 适合对 Apache HTTP Server 了解的用户（语法类似） |
| vsftpd | GPLv2 许可证授权、支持 IPv6 和 SSL 加密、支持虚拟用户、虚拟 IP、细粒度用户与 IP 控制、带宽限制 | 面向生产环境 |

### FTP 协议的安全性

在配置 FTP 服务器时，安全性是需重点考量的因素。

标准 FTP 协议在传输过程中并不安全：密码与数据均以明文形式发送，在标准部署中未加密。
...
Pure-FTPd 已移除 RFC 2640 支持，该标准定义了 FTP 协议的国际化框架。因此，在 Windows 系统下使用 FTP 命令行访问包含非英文字符的文件时可能出现乱码。具体变更可参见 Pure-FTPd 1.0.48 版本发布公告。
...
通过 pkg 安装的 Pure-FTPd 包不包含数据库支持功能，因此需要通过 Ports 安装以启用 MySQL 集成。
...
> **技巧**
>
> 其设计思路为：数据库中的虚拟用户将继承 `pw useradd` 所创建用户的权限及 GID/UID，并通过数据库中的用户信息进行 FTP 操作。
...
> **技巧**
>
> ProFTPD 在 Windows 自带的 FTP 连接下不会出现乱码问题。

### 安装 ProFTPD

> **注意**
>
> 本节基于 MySQL 8.0。关于 MySQL 的安装和基本设置，请参见其他章节。
...
> **警告**
>
> 如果不手动创建目录（即使系统已自动创建，也建议再次手动创建），会提示 `proftpd[3435]: warning: handling possibly truncated configuration data at line 65 of '/usr/local/etc/proftpd.conf'`。
...
> **警告**
>
> 上面的 UID GID 必须与 proftpd 用户相同，否则仅有读取权限而无写入权限。
>
...
vsftpd（Very Secure FTP Daemon，即“非常安全的 FTP 守护进程”）注重安全性设计，在 Linux 系统中使用较为广泛。经测试，在 Windows 环境下的 FTP 客户端中不会出现乱码问题。
...
| pam_pwdfile | 一个 PAM 模块，允许使用独立于系统账户的密码文件（类似 **/etc/passwd** 格式）进行用户身份验证 |
| apache24 | 提供 `htpasswd` 命令 |

> **技巧**
>
> 在 FreeBSD 系统中，**/etc/passwd** 文件仅用于兼容性目的，实际认证使用的是相应的数据库文件。
...
vsftpd 的主配置文件位于 **/usr/local/etc/vsftpd.conf**，可直接进行修改；同时提供了备用模板文件 **/usr/local/etc/vsftpd.conf.sample**。
...
vsftpd 日志在 **/var/log/vsftpd.log** 文件中。
...
1. 为 Pure-FTPd 重新实现 RFC 2640。

2. 将文中涉及的数据库都修改为 PostgreSQL。
