...
> **警告**
>
> 如果不手动创建该目录，将提示 `proftpd[3435]: warning: handling possibly truncated configuration data at line 65 of '/usr/local/etc/proftpd.conf'`。
...
vsftpd（Very Secure FTP Daemon，即"非常安全的 FTP 守护进程"）注重安全性设计，在 Linux 系统中应用广泛。经测试，在 Windows 环境下的 FTP 客户端中不会出现乱码问题。
