...
通过 Webmin 界面，可将控制台设置为中文显示。在 Webmin 中依次进入 Change Language and Theme，在 `Webmin UI language` 字段中选择 `Personal choice`，然后选择 `Simplified Chinese (ZH_CN.UTF8)`，点击 `Make Changes` 按钮。返回菜单 → Dashboard 后，控制台界面将刷新为中文。
