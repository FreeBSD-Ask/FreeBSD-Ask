...
“Samba”一词源自 Andrew Tridgell 以 Unix 的 `grep` 命令从系统词典中搜索所得，选取包含 S、M、B（Server Message Block 缩写）字母的单词。该词在葡萄牙语和英语中也指“桑巴舞”。
...
> **技巧**
>
> 必须将 FreeBSD 用户账户映射到 Samba 账号数据库，才能使 Windows 客户端访问共享。使用 pdbedit(8) 将已有的 FreeBSD 用户账户 `ykla` 映射到 Samba：
>
> ```sh
> # pdbedit -a -u ykla
> ```
