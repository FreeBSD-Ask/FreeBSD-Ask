...
Samba 是对服务器消息块/通用 Internet 文件系统（Server Message Block/Common Internet File System，SMB/CIFS）协议的自由软件重新实现。其核心目标是实现 UNIX 系统与 Windows 网络环境的互操作性。
...
Samba 需要在防火墙上开放以下端口：TCP 端口 139（NetBIOS 会话服务）与 445（SMB over TCP），以及 UDP 端口 137（NetBIOS 名称服务）与 138（NetBIOS 数据报服务）。这些端口是 SMB/CIFS 协议正常工作所必需的通信端口。
...
> **技巧**
>
> 必须将 FreeBSD 用户账户映射到 SambaSAMAccount 数据库，才能让 Windows 客户端访问共享。使用 pdbedit(8) 将现有的 FreeBSD 用户账户 `ykla` 映射到 Samba：
>
> ```sh
> # pdbedit -a -u ykla
> ```
...
Samba 由三个独立的守护进程组成。nmbd 和 smbd 守护进程都由 samba_enable 启动。

- 在 Windows 下使用如下命令访问共享文件夹（以实际 IP 为准，请勿直接照搬）。

在 Windows 系统中访问共享文件夹的操作说明：按下 Windows 键 + **R**（小写 r 也可）打开“运行”对话框。

在对话框中输入以下路径（请将示例 IP 地址和共享文件夹名替换为实际值），访问网络共享资源：
