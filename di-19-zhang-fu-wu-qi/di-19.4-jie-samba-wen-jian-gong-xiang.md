...
从技术架构看，Samba 由 smbd（文件与打印服务守护进程）与 nmbd（NetBIOS 名称服务守护进程）两个核心组件协同工作，以此融入 Windows 网络环境：smbd 负责处理文件共享与打印服务请求，nmbd 提供 NetBIOS 名称解析与网络浏览功能。

Samba 须在防火墙中放行以下端口：TCP 139（NetBIOS 会话服务）与 445（SMB over TCP），以及 UDP 137（NetBIOS 名称服务）与 138（NetBIOS 数据报服务）。

“Samba”一词源自 Andrew Tridgell 以 Unix 的 `grep` 命令从系统词典中搜索所得，选取包含 S、M、B（Server Message Block 缩写）字母的单词。该词在葡萄牙语和英语中亦指“桑巴舞”。
