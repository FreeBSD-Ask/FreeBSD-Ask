...
NFS（Network File System，网络文件系统）最初由 Sun Microsystems 于 1984 年开发，是一种分布式文件系统协议，支持跨操作系统文件共享。

NFS 采用远程过程调用（Remote Procedure Call，RPC）机制实现透明文件访问，客户端可如访问本地文件系统一般访问远程服务器上的文件。
...
- 在客户端上执行以下命令，将远程服务器 `server` 的 **/usr/home/logs** 目录挂载到本地默认挂载点 **/mnt**：
