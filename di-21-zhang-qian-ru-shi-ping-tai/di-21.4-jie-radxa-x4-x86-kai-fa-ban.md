...
关于 FreeBSD 旧版本的兼容性说明：FreeBSD 14.3-RELEASE 存在潜在问题，如果使用 eMMC 存储版本且将 FreeBSD 系统安装在 eMMC 上，则固态硬盘的分区数量不得超过 5 个，否则 ZFS 文件系统将无法正常启动。
...
完成无线网卡驱动安装后，还需配置以优化无线连接的稳定性与性能。需要说明的是，当前驱动尚不支持 Wi-Fi 5/6 标准。
...
过高的工作温度可能导致系统不稳定、性能降频甚至硬件永久性损坏。
