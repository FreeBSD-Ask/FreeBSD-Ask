...
配置文件按层级结构构建，**/boot/device.hints** 是系统启动时由 loader(8) 读取的静态配置文件。
...
# syscons(4) 传统控制台驱动
# 注意：syscons(4) 自 FreeBSD 9.x 起已被 vt(4)（Newcons）逐步取代。
# vt(4) 提供了更好的 Unicode 支持、KMS 集成和现代显卡兼容性，已成为默认控制台驱动。
# 以下 hint.sc 条目仅为历史参考，现代 FreeBSD 默认使用 vt(4)。
hint.sc.0.at="isa"
