...
VirtualBox 是甲骨文（Oracle）公司开发的跨平台第二类虚拟机管理程序（Type-2 hypervisor），适用于包括 Windows、macOS、Linux 和 FreeBSD 在内的大多数操作系统。它同样可运行 Windows 或类 UNIX 的虚拟机系统。
...
使用 pkg 安装的预编译内核模块可能与当前运行的内核版本不匹配，并且缺少虚拟机所需的增强工具，而通过 Ports 编译安装可确保内核模块与当前系统内核完全匹配。
...
因扩展包在 FreeBSD 上暂不可用，OSE 版本的宿主机控制器仅能模拟 USB 1.1 设备。
