# 25.1 使用 bhyve 及 vm-bhyve 工具安装 Windows 11
...
出现 `Press any key to boot from a CD/DVD` 提示后，快速按几次回车键。

> **技巧**
>
> 如忘记按键而退回至 UEFI Shell，停止虚拟机，重新执行上述安装步骤。
>
...
将第二项改为"Chinese (Simplified, Mainland China)"。
...
在 tigervnc-viewer 中输入 `localhost`，点击连接，随后按任意键继续安装过程。

> **技巧**
>
> 终止虚拟机：若虚拟机无响应导致命令无效，使用 `kill -9` 强制终止，以免影响宿主机关机；若虚拟机阻碍宿主机关机，可在 tty 中按 `Ctrl`+`C` 跳过等待并强制关机。
>
> ```sh
> # vm list
...
隐私设置。可根据需要调整，随后点击“Accept”。

![隐私设置](../.gitbook/assets/win14.png)

安装过程中系统会执行更新，可尝试跳过此步骤（经测试 `oobe\bypassnro` 无效），根据实际情况决定。
...
如遇问题，先重启宿主机；若问题仍存在，可借助 `ifconfig` 检查并删除多余网卡。

若虚拟机长时间保持 stopped 状态，检查网络配置。
...
- Microsoft. Minimum System Requirements - Windows IoT Enterprise[EB/OL]. [2026-04-17]. <https://learn.microsoft.com/en-us/windows/iot/iot-enterprise/hardware/system_requirements>. Microsoft Learn. Windows 11 IoT Enterprise LTSC 将 TPM 2.0 列为可选要求。
- FreeBSD Project. usbhid(4)[EB/OL]. [2026-04-17]. <https://man.freebsd.org/cgi/man.cgi?query=usbhid&sektion=4&manpath=FreeBSD+16.0>. FreeBSD USB HID 子系统手册页。
