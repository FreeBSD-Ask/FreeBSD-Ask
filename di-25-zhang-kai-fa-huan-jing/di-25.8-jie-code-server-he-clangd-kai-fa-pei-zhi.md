# 25.8 code-server 和 clangd 开发配置

本节目前在 13.2-RELEASE 和 14.0-RELEASE 上测试正常，其他版本请谨慎参考。

code-server 是 VS Code 的开源服务器版，可通过 Arch Linux 兼容层运行（因上游 FreeBSD 版存在问题）。本节配置 code-server + clangd，利用 FreeBSD 内核路径劫持实现兼容层内调用 FreeBSD 原生开发工具。
