...
本节介绍 LLVM/Clang 工具链版本管理、gdb 与 lldb 调试器使用，以及相关 C/C++ 标准库的链接与安装方法。
...
鉴于 C/C++ 生态中可选的工具链与调试方案极为丰富，本节仅选取三种典型组合作为示例，分别对应调试适配器 `lldb-mi`，以及构建系统 `CMake + Ninja` 和 `Meson + Ninja`。本节以 Visual Studio Code 作为前端开发环境，说明如何将这些底层工具在编辑器中集成。
