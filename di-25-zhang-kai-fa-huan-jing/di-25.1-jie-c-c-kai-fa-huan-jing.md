...
因此需要安装 LLVM，可以使用各版本的 LLVM，但版本至少不应低于系统自带的 Clang。在 FreeBSD 15.0-RELEASE 中，基本系统中的 Clang 版本为 19。
...
针对 C++ 的开发需求，可参照 C 语言的配置步骤，在编译器选择、项目设置等方面做相应调整即可。
...
由于 CLion 使用 CMake，因此使用方法较 VSCode 更为简单，只需正确配置 `CMakeLists.txt` 文件即可（可参照上文 VSCode 的配置），不再赘述。
