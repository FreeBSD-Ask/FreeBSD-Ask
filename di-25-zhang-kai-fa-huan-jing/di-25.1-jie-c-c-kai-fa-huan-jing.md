...
针对 C++ 的开发需求，可参照 C 语言的配置步骤，在编译程序选择、项目设置等方面做相应调整即可。
...
鉴于 C/C++ 生态中可选的工具链与调试方案极为丰富，本节仅选取三种典型组合作为示例，分别对应调试适配器 `lldb-mi`，以及构建系统 `CMake + Ninja` 和 `Meson + Ninja`。本节以 Visual Studio Code 作为前端开发环境，演示如何将这些底层工具在编辑器中集成。
