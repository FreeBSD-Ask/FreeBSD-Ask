...
C/C++ 是两门具有悠久历史的经典编程语言，至今仍在系统编程、高性能计算、嵌入式开发等领域广泛应用。

FreeBSD 内核及大量用户态工具均以 C 语言编写，充分体现了 C 语言在系统级开发中的独特优势。

本节涵盖 LLVM/Clang 工具链版本管理、gdb 与 lldb 调试器使用，以及相关 C/C++ 标准库的链接与安装方法。
...
因此需要安装 LLVM，此处可以使用各个版本的 LLVM，但其版本至少不应低于系统自带的 Clang。在 FreeBSD 15.0-RELEASE 中，基本系统中的 Clang 版本为 19。
...
if(NOT DEFINED LLVM_VERSION_MAJOR)
  set(LLVM_VERSION_MAJOR 23) # 注意此处，代表写作本节时，主线版本是 LLVM 23
...
FreeBSD 将 LLVM 导入了基本系统源代码中，但并非直接导入，而是经过了一定的处理，撰写本节时 FreeBSD src 内置的 LLVM 仍为 LLVM 19，故而，注明版本号的文件路径位于 [lib/clang/include/clang/Basic/Version.inc](https://github.com/freebsd/freebsd-src/blob/main/lib%2Fclang%2Finclude%2Fclang%2FBasic%2FVersion.inc)：
...
- Jianping-Duan. algcl[EB/OL]. [2026-03-26]. <https://github.com/Jianping-Duan/algcl>. 提供了可直接运行于 FreeBSD 的 C 语言算法与数据结构编程实例。
- Zed Industries. Zed[EB/OL]. [2026-04-17]. <https://zed.dev/>. Zed 编辑器官方网站，介绍了其由 Atom 和 Tree-sitter 创建者开发的背景。

## FreeBSD 部分 C/C++ 开发工具概览

| 工具 | 简介 | 主要开发语言 | 编辑器类型 |
| ---- | ---- | ------------ | ---------- |
| [Code-OSS](https://github.com/microsoft/vscode) | Visual Studio Code 的开源版本，去除了微软闭源组件和遥测功能，是一款功能强大的可扩展代码编辑器 | TypeScript / JavaScript（Electron 框架） | 轻度至中度（通过扩展可接近 IDE 功能） |
...
考虑到 C 与 C++ 在语法、工具链及编译流程上具有高度的相似性，本节以 C 语言为例，详细介绍开发环境的配置方法。
对于 C++ 的开发需求，可参考 C 语言的配置步骤，在编译器选择、项目设置等方面进行相应调整即可。
...
需要注意的是，以这种方式安装的 VS Code 实际上是 [Code-OSS](https://github.com/microsoft/vscode)。Code-OSS 与 VS Code 的区别主要在于许可证不同以及可用的闭源资源不同，类似 Chromium 与 Chrome 的关系。可以阅读 [原文](https://github.com/microsoft/vscode/wiki/Differences-between-the-repository-and-Visual-Studio-Code) 以了解二者的区别。
...
由于 FreeBSD Ports 中的 VSCode 实际为 Visual Studio Code 的开源版本 Code-OSS，该版本移除了微软的闭源组件及遥测功能，因此无法直接访问微软官方的扩展市场，也无法安装或使用依赖微软专有库的官方 C/C++ 扩展。为了在 FreeBSD 上获得完整的 C/C++ 开发支持，需选用一组开源替代插件，以提供语言服务、调试适配及构建任务集成等核心功能。
...
鉴于 C/C++ 生态中可选的工具链与调试方案极为丰富，本节仅选取三种典型组合作为示例，分别对应调试适配器 `lldb-mi`，以及构建系统 `CMake + Ninja` 和 `Meson + Ninja`。本节以 Visual Studio Code 作为前端开发环境，演示如何将这些底层工具集成于编辑器之中。

需要说明的是，C/C++ 项目的配置并无“万能模板”，实际开发中需根据项目结构和构建需求灵活调整。因此，本节以一个极简实例为切入点：项目名称为 `test`，仅包含一个源文件 `main.c`。通过此实例，可直观理解配置文件的编写逻辑，并以此为起点应对更复杂的项目场景。
...
然后同理，在 `.vscode` 路径下创建 `launch.json` 文件和 `tasks.json` 文件。
...
由于 CLion 使用 CMake，所以使用方法相较于 VSCode 更简单，只需正确配置 `CMakeLists.txt` 文件即可（可参照上文 VSCode 的配置），不再赘述。

> **注意**
>
> 在 FreeBSD 上使用 CLion 进行 C/C++ 的开发，如果并非特殊情况，尽量使用 gcc 和 gdb 套件以避免一些错误。
...
要修复此问题，需编辑 CLion 的 **/usr/local/share/jetbrains/clion/plugins/plugin-classpath.txt** 文件：

然后将其中的乱码删除（删至 `<idea-plugin`），重启 CLion 后可正常进入软件。参见：FreeBSD Bugzilla. Bug 290663 - devel/jetbrains-clion: fails to start[EB/OL]. [2026-03-26]. <https://bugs.freebsd.org/bugzilla/show_bug.cgi?id=290663>. 记录 CLion 在 FreeBSD 上的启动问题及解决方案。
...
Zed 的调试功能主要基于调试适配器协议（Debug Adapter Protocol，DAP）实现，并默认依赖特定的调试适配器，例如 CodeLLDB（用于 LLDB 后端）或 GDB，以支持包括 Rust、C/C++ 在内的多种语言的调试。CodeLLDB 作为主要的 LLDB DAP 适配器，其官方支持平台仅限于 Linux、macOS 和 Windows，未包含 FreeBSD 或其他 BSD 变体。这导致在 FreeBSD 环境下，CodeLLDB 调试路径默认不可用，从而使 Zed 的内置调试功能受到限制。

Zed 本身支持 LLDB 作为调试后端（例如在调试 Zed 自身源代码时可通过 rust-lldb 进行），但其通用调试系统严格依赖 DAP 协议，而非 LLDB 的 Machine Interface（MI）接口（如 LLDB-MI）。因此，直接集成 LLDB-MI 并非 Zed 的标准支持方式。若需使用 LLDB，必须通过兼容的 DAP 适配器（如 CodeLLDB 或官方 lldb-dap）实现，而这些适配器在 FreeBSD 上要么缺乏官方支持，要么存在兼容性问题。
...
> **思考题**
>
> 本节介绍的开发方法主要围绕编译器、调试器以及基础的构建工具展开，并未将 NeoVim、Emacs 等高度可定制化的编辑器作为核心推荐。
>
...
1. 使用 CMake + Ninja 构建一个简单的 C 语言项目，配置 Clang 调试环境，并在 VSCode 中实现断点调试。
2. 对比 Clang 与 GCC 在 FreeBSD 上的默认编译参数，分析两者对系统性能优化的不同策略及其影响。
3. 修改 Zed 编辑器的调试配置，尝试集成 LLDB-MI 并测试调试一个简单的 C++ 程序。
