...
Wine（"Wine Is Not an Emulator"）是兼容层软件，可在类 UNIX 操作系统上运行 Windows 应用程序。本节介绍 pkg 安装及基础配置方法，并说明字体与 DRM 驱动的依赖关系。以下内容仅在物理机上测试通过。Wine 依赖 DRM 驱动提供图形硬件加速，虚拟机环境下可能无法正常运行，通常会提示不支持 3D 加速。此外，虚拟机可能无法完整支持某些 CPU 指令集，同样会导致程序启动失败。
