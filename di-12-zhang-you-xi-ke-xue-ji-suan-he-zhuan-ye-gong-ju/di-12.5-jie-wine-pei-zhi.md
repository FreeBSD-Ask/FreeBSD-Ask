...
Wine（“Wine Is Not an Emulator”）是兼容层软件，可在类 UNIX 操作系统上运行 Windows 应用程序。本节介绍 pkg 安装及基础配置方法，并说明字体与 DRM 驱动的依赖关系。以下内容仅在物理机上测试通过。Wine 依赖 DRM 驱动提供图形硬件加速，虚拟机环境下可能无法正常运行，通常会提示不支持 3D 加速。此外，虚拟机对某些 CPU 指令集的支持可能不完善，同样会导致程序启动失败。
...
Winetricks 是一款脚本（约两万行代码），封装了许多与 Wine 相关的实用功能，例如安装和卸载软件、安装字体等，可用于规避许多常见的 Wine 问题。
