...
1. 在 FreeBSD 上安装 HMCL 启动器并运行 Minecraft Java 版，记录安装过程及遇到的问题。

2. 在 FreeBSD 上搭建 Minecraft Java 版服务器，配置服务器参数并测试客户端连接。

3. 比较同一硬件上 FreeBSD 与 Linux 运行 Minecraft 服务端时的 CPU 与内存占用差异。
