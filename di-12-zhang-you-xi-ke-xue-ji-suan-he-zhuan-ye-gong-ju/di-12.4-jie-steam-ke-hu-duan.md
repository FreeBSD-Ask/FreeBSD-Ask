...
加载该内核模块是运行 Steam 的前提条件，该模块使 FreeBSD 系统能够执行 Linux 二进制文件。
...
将 sysctl 系统控制变量 `security.bsd.unprivileged_chroot` 与 `vfs.usermount` 设置为 `1`。这两个参数分别允许非特权用户使用 chroot 和挂载文件系统。Steam 需要这些权限来创建隔离环境和挂载必要的文件系统。
...
1. 在 FreeBSD 上通过 linux-steam-utils 安装 Steam 客户端，测试一款 Linux 游戏的运行效果。
