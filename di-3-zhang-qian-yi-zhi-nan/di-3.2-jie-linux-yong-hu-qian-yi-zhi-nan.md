...
Linux 与 FreeBSD 同属类 UNIX 操作系统，但二者在内核架构、包管理哲学及系统设计理念等方面存在根本差异。深入理解 BSD 的构建方式与构建初衷并非易事，由此产生的失望在所难免。
...
- BSD 是最早的开源理念实践者之一，BSD 许可证也是历史上最早出现的自由软件许可证之一（早于 1989 年的 GPL，但晚于 1985 年的 GNU Emacs 许可证）。
...
```text
FreeBSD 操作系统 = 基本系统 + 第三方软件

┌─────────────────────────────────────────────┐
│              FreeBSD 操作系统               │
├─────────────────────┬───────────────────────┤
│     基本系统        │     第三方软件        │
│  (freebsd-src)      │  (freebsd-ports)      │
├─────────────────────┼───────────────────────┤
│  · 内核             │  · pkg 二进制包       │
│  · 用户空间（世界） │  · Ports 源代码编译   │
│  · 系统库 libc      │  · 37,000+ 软件       │
│  · 编译器 LLVM/Clang│  · /usr/local/ 安装   │
│  · Shell (sh)       │                       │
│  · 基础工具         │                       │
├─────────────────────┴───────────────────────┤
│  两者独立维护，互不干扰                      │
...
以用户进程身份运行 init，可模拟 AT&T System V UNIX 的行为：超级用户可在命令行中指定所需的运行级别，该 init 进程向原始（PID 为 1 的）init 进程发送特定信号，以执行相应操作。
...
| ------------------- | ------------- | -------- | ---- |
| `lsusb` | `sysutils/usbutils` | 显示 USB 信息 | 也可粗略使用 `cat /var/run/dmesg.boot` |
| `lspci` | `sysutils/pciutils` | 显示 PCI 信息 | 也可粗略使用 `cat /var/run/dmesg.boot` |
...
文件系统层次标准（FHS）最初由 Linux 基金会 Linux 标准基础工作组（Linux Standard Base, LSB）维护，2025 年起由 FreeDesktop.org 托管和管理，定义了类 UNIX 操作系统中目录结构和目录内容的规范，当前版本为 FHS 3.0（最初发布于 2015 年，最新修订发布于 2026 年 4 月 8 日）。
...
| **/proc** | Linux procfs（§6.1.5） | procfs(4) 默认不使用，仅出于兼容目的保留 |
...
trixie/sid # trixie 即 Debian 13，于 2025 年 8 月 9 日发布，为当前稳定版本。Debian 12 bookworm 为旧稳定版（oldstable）。
...
openSUSE 因其 Logo 形象，在社区中昵称为“大蜥蜴”。
...
openSUSE 有时会在稳定版的软件包中引入实验性功能，且可能没有明确的提示。这可能导致用户视其为软件缺陷（Bug）反馈。用户可能需要提交问题报告后，才能从维护者处得知该行为是实验性功能所致。
...
openSUSE 原生的包管理器是 `zypper`。有用户与 Fedora 的 `dnf` 对比，认为 `zypper` 在交互响应速度或某些场景下的性能表现有待优化 ~~例如你数数这有几个字母？~~，`zypper` 相比 `dnf` 存在明显的卡顿和延迟。且这两个包管理器在某些情况下存在竞争关系，参见：Bug 1213158 - Packages install failed with ndb backend[EB/OL]. [2026-03-26]. <https://bugzilla.opensuse.org/show_bug.cgi?id=1213158>。
...
- BleepingComputer. Malware Found in Arch Linux AUR Package Repository[EB/OL]. (2018-07-10)[2026-04-18]. <https://www.bleepingcomputer.com/news/security/malware-found-in-arch-linux-aur-package-repository/>. 报道了 2018 年 AUR 中 acroread 等至少三个软件包被植入恶意代码。
- LWN.net. Malware found in the Arch Linux AUR repository[EB/OL]. (2018-07-10)[2026-04-18]. <https://lwn.net/Articles/759461/>. LWN 对同一事件的独立报道，交叉验证了 AUR 恶意软件包事实。
