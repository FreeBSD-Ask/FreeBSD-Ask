...
**RELEASE** 版本适用于生产环境，通常被视为 **稳定版** 或长期支持（LTS）版本。而 ***STABLE*** 和 ***CURRENT*** 则属于 **开发分支**，**通常不建议用于生产环境**（“稳定”与“不稳定”是相对概念，例如 [Netflix 几乎所有节点都运行着 **current**](https://freebsdfoundation.org/netflix-case-study/) ）。
...
>
> ——参见：FreeBSD Glossary STABLE[EB/OL]. [2026-03-26]. <https://wiki.freebsd.org/Glossary#STABLE>
...
>
> ——参见：FreeBSD Manual Pages freebsd-update[EB/OL]. [2026-03-26]. <https://man.freebsd.org/cgi/man.cgi?freebsd-update>
...
FreeBSD 日为 6 月 19 日。FreeBSD 基金会和社区在这天庆祝 FreeBSD 的生日。——FreeBSD Foundation. Join us to celebrate FreeBSD Day![EB/OL]. [2026-03-26]. <https://freebsdfoundation.org/freebsd-day/>.
...
- The New Stack. Apple’s Open Source Roots: The BSD Heritage Behind macOS and iOS[EB/OL]. [2026-03-26]. <https://thenewstack.io/apples-open-source-roots-the-bsd-heritage-behind-macos-and-ios/>. 苹果（Apple）的操作系统如 macOS、iOS 等大量复用了 BSD（不止是 FreeBSD）的技术栈。BSD 可以称得上是 macOS 的开源基石。
...
- QNX. Search Results[EB/OL]. [2026-03-26]. <https://www.qnx.com/developers/docs/8.0/search.html?searchQuery=freebsd>. QNX 操作系统。QNX 是一款实时操作系统。QNX 以前是黑莓手机的操作系统。QNX 现在被广泛应用为汽车安全操作系统（即车机， 安卓是运行在 QNX 之上的操作系统，与车机系统分离 ）。国内新能源车辆普遍采用了 QNX 操作系统（关键领域占比超过九成）。QNX 大量复用了 FreeBSD 的技术栈。
...
- OpenHarmony. kernel_liteos_a[EB/OL]. [2026-03-26]. <https://gitee.com/openharmony/kernel_liteos_a/tree/master>. OpenHarmony LiteOS 内核引入了一些 FreeBSD 代码用作驱动等。
...
- FreeBSD 的代码风格是 Kernighan & Ritchie 经典著作 *The C Programming Language*（中译本：Kernighan, B. W., Ritchie, D. M. C 程序设计语言 (原书第 2 版)[M]. 徐宝文, 李志, 译. 尤晋元, 审校. 北京: 机械工业出版社, 2019. ISBN 978-7-111-61794-5.）中使用的风格。
...
- 可以避免在产品和架构中出现共同故障点。
- 安全事件审计。
- RobertWatson, StaceySon. TrustedBSD - FreeBSD Wiki[EB/OL]. (2022-09-15)[2026-03-26]. <https://wiki.freebsd.org/TrustedBSD/>. FreeBSD 集成了标准 UNIX DAC、ACL、TrustedBSD MAC 安全框架（基于 POSIX®.1e 草案的安全扩展）
...
目前，大多数将 Linux 用作专用设备操作系统，或基于其他 GPL 软件构建商业产品的公司，并未严格遵守 GPL 协议发布其修改后的代码。而对于国内公司来说，他们甚至不知 GPL 为何物，只认为免费就可以了，而那些为逃避 GPL 强制开源规定而采取规避措施的企业产品，是不值得信任和使用的。抢注开源软件商标的事情亦时有发生。比较起来，那些使用 FreeBSD 的公司至少更为诚实、可靠和值得信赖。也真正使得 BSD 的代码为众人所用——就算有人认为 FreeBSD 已日薄西山——事实上，这些人可能一直生活在 FreeBSD 技术的光照之下。

##### 参考文献

- 王波. FreeBSD 在中国的未来[M]//FreeBSD 使用大全. 第 2 版. 北京: 机械工业出版社, 2002: 前言. ISBN: 978-7-111-10286-1. 探讨 FreeBSD 在中国的发展前景与应用前景。
