...
CURRENT 分支中的代码在经过充分测试后（需满足 MFC 最短三天的要求，MFC 指 `Merge From CURRENT`，类似于 `backporting` 即向后移植）会推送到 STABLE 分支，但这并不保证两个分支都没有重大缺陷。参见 [FreeBSD Release Engineering](https://docs.freebsd.org/en/articles/freebsd-releng/)  。
...
- [谁在使用 FreeBSD](https://book.bsdcn.org/fan-yi-wen-zhang-cun-dang/2025-nian-1-yue/di-1.5-jie-shui-zai-shi-yong-freebsd)  ，有一份 FreeBSD 基金会官方版本。汇集了 FreeBSD 在各领域的典型应用案例。
...
- [Submitting GitHub Pull Requests to FreeBSD](https://freebsdfoundation.org/our-work/journal/browser-based-edition/configuration-management-2/submitting-github-pull-requests-to-freebsd/)  ，翻译在 [在 GitHub 上向 FreeBSD 提交 PR](https://github.com/taophilosophy/freebsd-journal-cn/blob/main/2024-0506/zai-github-shang-xiang-freebsd-ti-jiao-pr.md) 。详解 FreeBSD 通过 GitHub 接受贡献的流程与规范。
- [Contribution Guidelines for GitHub](https://github.com/freebsd/freebsd-src/blob/main/CONTRIBUTING.md) ，应该以此为准。是 FreeBSD 源代码贡献的官方指导与要求。
- Linux Kernel 由 Linus 一人裁决：“[Linus Torvalds 是决定改动能否进入 Linux 内核的最终裁决者。](https://www.kernel.org/doc/html/latest/translations/zh_CN/process/submitting-patches.html) ”。展示 Linux 内核开发的集中式决策模式。
- [Linux 内核编码风格](https://www.kernel.org/doc/html/latest/process/coding-style.html) 。规范 Linux 内核代码风格与格式要求。
- Linux 内核开发是个 [较为封闭的过程](https://www.kernel.org/doc/html/latest/process/submitting-patches.html) ，只有少数人能够参与直接提交代码。说明 Linux 内核开发的参与门槛与流程。
- [Sandbox Your Program Using FreeBSD's Capsicum](https://cdaemon.com/posts/capsicum) ，翻译在 [利用 FreeBSD Capsicum 框架实现程序沙箱化](https://book.bsdcn.org/fan-yi-wen-zhang-cun-dang/2025-nian/capsicum) 。这是篇有关 Capsicum 框架的简介。是 FreeBSD 安全沙箱框架的基本原理与使用方法。
