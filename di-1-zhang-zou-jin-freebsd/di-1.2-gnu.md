...
GNU 操作系统和自由软件运动作为现代开源软件生态系统的奠基性力量，对软件产业和技术文化产生了深远且持久的影响。让我们先从 Linux 内核文档中的相关内容开始了解。根据 Linux 内核文档 [1.5. Licensing](https://www.kernel.org/doc/html/latest/process/1.Intro.html)（另见 [Linux kernel licensing rules](https://www.kernel.org/doc/html/latest/process/license-rules.html)）：
...
由于 Unix 在后期发展中逐渐呈现封闭化趋势，许可证价格高昂且使用权受到商业公司的严格限制等多重原因，理查德·马修·斯托曼（Richard Matthew Stallman，RMS）希望创造一款自由且与 Unix 兼容的操作系统。这一愿景不仅是技术层面的追求，更是对软件自由理念的深刻实践与制度性探索。
...
在项目初期，RMS 为 Unix 开发了大量实用程序（用户空间组件），然而 GNU 的目标操作系统始终未能形成一个稳定的内核（即使到 2025 年，[GNU Hurd](https://hurd.gnu.org/) [备份](https://web.archive.org/web/20260116222211/https://www.gnu.org/software/hurd/) 项目仍处于未完成状态）。Linux 内核的诞生打破了这一技术僵局。
...
为了坚持开源定义（Open Source Definition，OSD），并防范开源运动原则的滥用而成立的开放源代码促进会（OSI）对开源协议是这么定义的（基于 [Debian Free Software Guidelines](https://www.debian.org/social_contract#guidelines) [备份](https://web.archive.org/web/20260114070814/https://www.debian.org/social_contract#guidelines), DFSG），为了避免歧义，我们 [全文引用](https://opensource.org/osd) 如下：
...
OSI 基于开放软件定义（OSD）的开放认证许可证，已成为全球公认的开放软件判定标准，并通过国际组织、产业界与政府政策的采纳而确立其权威性。参见 [International Authority & Recognition](https://opensource.org/about/authority) 。
...
- 开源软件：使用上述开源协议授权的软件。所有自由软件都是开源软件，但是所有开源软件不一定是自由软件。二者的定义和理念都有一定的差异。自由软件是开源软件的充分但非必要条件。
