...
自托管云存储与协作平台在数据主权、隐私保护和定制化方面具有不可替代的价值，为用户提供了完全控制数据的能力。Nextcloud 作为此类平台中的佼佼者，提供了完整的企业级协作功能集，包括文件管理、日历、联系人、在线办公等丰富应用。本节将介绍基于 PostgreSQL 关系型数据库在 FreeBSD 上部署 Nextcloud 的系统化方法，PostgreSQL 以其 ACID 合规性和事务支持为 Nextcloud 提供了可靠的数据持久化保障，确保数据的一致性和完整性。

Nextcloud 是一个开源的云存储和协作平台，提供文件同步、分享、日历、联系人等功能，其模块化架构允许用户根据需求选择和安装不同的应用组件。本节将介绍基于 PostgreSQL 数据库在 FreeBSD 上安装和配置 Nextcloud 的方法。
...
Nextcloud 需要一个数据库来存储数据，本节使用 PostgreSQL 作为数据库。安装 PostgreSQL（请注意与上面通过 Ports 默认安装的 `postgresql-client` 版本保持一致）。
...
如果使用 Apache 作为 Web 服务器，需要进行相应的配置，使 Apache 能够正确处理 Nextcloud 的 PHP 文件。请读者参考其他章节内容完成 Apache 的安装和服务自启。
...
- [System requirements](https://docs.nextcloud.com/server/latest/admin_manual/installation/system_requirements.html) [备份](https://web.archive.org/web/20260115044800/https://docs.nextcloud.com/server/latest/admin_manual/installation/system_requirements.html)，各种软件的版本需求
- [PHP Modules & Configuration](https://docs.nextcloud.com/server/latest/admin_manual/installation/php_configuration.html) [备份](https://web.archive.org/web/20260116023053/https://docs.nextcloud.com/server/latest/admin_manual/installation/php_configuration.html)，需要和可选的 PHP 模块及配置
