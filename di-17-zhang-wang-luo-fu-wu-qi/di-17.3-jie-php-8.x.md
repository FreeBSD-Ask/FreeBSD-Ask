...
动态网页技术是现代 Web 应用的核心支撑，服务器端脚本语言在实现业务逻辑、数据处理和用户交互方面发挥着关键作用。PHP 作为最主流的服务器端脚本语言之一，其 8.x 版本在性能优化、类型系统和安全特性方面都有显著改进。本章将详细介绍在 FreeBSD 上配置 PHP 8.x 运行环境的方法。

## 安装 PHP

PHP（PHP: Hypertext Preprocessor，超文本预处理器）是一种广泛使用的开源脚本语言，特别适合 Web 开发，支持嵌入式 HTML 编程和多种数据库接口。在 FreeBSD 上可以通过 pkg 或 Ports 两种方式进行安装。

使用 pkg 包管理器安装 PHP 是最便捷的方式，可以同时安装 PHP 本体、扩展模块以及 Apache 模块：
...
或者使用 Ports 方式安装 PHP，需要分别进入对应的目录进行编译安装：
...
安装完成后，需要对 PHP 进行基本配置，包括复制配置文件和启动 PHP-FPM（FastCGI 进程管理器）服务。
...
设置 PHP-FPM 服务开机自启，可以确保系统重启后 PHP-FPM 服务自动运行：
...
- [Apache and PHP](https://forums.freebsd.org/threads/apache-and-php.80625/) [备份](https://web.archive.org/web/20260115141813/https://forums.freebsd.org/threads/apache-and-php.80625/)，此处提示要安装 `mod_php84`
...
- [How to Install PHP and PHP-FPM on FreeBSD 14.0](https://docs.vultr.com/how-to-install-php-and-php-fpm-on-freebsd-14-0) [备份](https://web.archive.org/web/20260115141744/https://docs.vultr.com/how-to-install-php-and-php-fpm-on-freebsd-14-0)，本文过去曾主要参考此文
