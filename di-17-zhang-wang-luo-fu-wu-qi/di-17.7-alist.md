...
可以直接使用 OpenList 官方提供的二进制包进行安装。请以普通用户身份执行以下操作：
...
>**技巧**
>
>根据 [手动安装](https://docs.oplist.org/zh/guide/install/manual.html) 文档及实际测试，无需额外配置后端 OpenList-Frontend，该文档提供完整的手动安装指导。
...
安装完依赖后，可以开始编译构建 OpenList。请以普通用户执行。
...
- [alist 指定 --data 运行不起作用](https://github.com/AlistGo/alist/issues/2580) [备份](https://web.archive.org/web/20260115142159/https://github.com/AlistGo/alist/issues/2580)，记录了 --data 参数使用时的问题及解决方案。
- 另外 [start 等静默启动](https://alist.nn.ci/zh/guide/install/manual.html#%E5%AE%88%E6%8A%A4%E8%BF%9B%E7%A8%8B) [备份](https://web.archive.org/web/20260115142208/https://alistgo.com/zh/guide/install/manual.html#_6-%E5%8D%B8%E8%BD%BD%E8%AF%B4%E6%98%8E) 会强制 `--force-bin-dir`。对于 `start` 指定任何 `data` 均会在后面附带  `--force-bin-dir`。但是仍然是无效的，该文档详述了静默启动的行为特性。
...
- 具体用法参考 [bindfs](https://man.freebsd.org/cgi/man.cgi?query=bindfs)，该手册页详细介绍了文件系统挂载与权限映射的完整参数集。
...
可以使用 inotify-tools 监控目录变化，从而触发影视刮削操作。inotify-tools 为 shell 脚本提供了用于文件系统事件的简单接口。
...
- inotifywait 用法参见 [inotifywait](https://man.freebsd.org/cgi/man.cgi?query=inotifywait&apropos=0)，提供文件系统事件监控的完整命令参数说明。

### 写入刮削的信息

监控脚本触发后，可以将刮削的信息写入视频所在目录下的 `top.md` 等文件。

TMDb 在国内可以正常使用，但可能受到 DNS 污染，解决 DNS 污染后即可正常访问。具体刮削脚本参见 [https://gist.github.com/ykla/91e27db14e68ba60903e97fe1c437246](https://gist.github.com/ykla/91e27db14e68ba60903e97fe1c437246) [备份](https://web.archive.org/web/20260118011652/https://gist.github.com/ykla/91e27db14e68ba60903e97fe1c437246)。
...
2. 解决 bindfs 的多线程模型存在的缺陷，贡献上游。

3. 将其制作成 Port 贡献到 FreeBSD。
