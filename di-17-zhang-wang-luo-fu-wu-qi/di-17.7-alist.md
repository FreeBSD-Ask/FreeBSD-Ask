...
为了方便管理 OpenList 服务，可以编写一个 FreeBSD RC 脚本。创建并编辑 `/usr/local/etc/rc.d/openlist`：
...
RC 脚本编写完成后，可以使用它来管理 OpenList 服务。设置 OpenList 服务开机自启
...
服务配置完成后，可以启动 OpenList 服务并进行初始化。启动 OpenList 服务实例
