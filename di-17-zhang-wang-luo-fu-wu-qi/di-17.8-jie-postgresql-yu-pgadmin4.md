...
`postgresql_initdb_flags` 参数,默认是`"--encoding=utf-8 --lc-collate=C"`,意义一目了然，不作解释
...
`-A scram-sha-256` 用于指定默认的认证方法，不使用的话 `pg_hba.conf` 中默认认证方法为trust，即无密码登录

`-W` 用于在初始化时要求设置 postgres 用户密码，这样不用登录后在设置密码。

这样可以简化一些初始化工作。同样也可以利用 `postgresql_initdb_flags` 参数灵活控制初始化工作。

### 管理多个数据库实例(集簇)

PostgreSql 允许在一台机器上运行多个实例，这在测试不同配置，隔离环境时非常有用。

管理多个数据库实例的工具在 Debian/Ubuntu 系统上常用的是 `pg_ctlcluster`,这是一个 `pg_ctl` 的封装。
...
```
sysrc postgresql_profiles+=main  # 增加一个 main 实例
sysrc postgresql_profiles+=dev  # 增加一个 dev 实例
# 当然可以直接 sysrc postgresql_profiles="main dev"
sysrc postgresql_main_data=/var/db/postgres/main  # 对应 main 实例数据目录
sysrc postgresql_dev_data=/var/db/postgres/dev  # 对应 dev 实例数据目录
# 必须为每个实例设置数据目录
# 以上为必须
sysrc postgresql_main_initdb_flags="-E utf-8 -A scram-sha-256 --lc-collate C -W"
# 为 main 实例设置初始化参数，dev 实例默认初始化，作一对比
...
1. 把实例名放入 rc 参数名中，就是对应实例的参数名。
2. `postgresql_data`,`postgresql_enable` 等参数还是可以使用，这时这些参数作为实例对应参数的默认值使用，在找不到实例参数时使用。
3. 管理多个实例时，建议只使用实例名相关的参数，不去使用没有实例名的参数。好处是清晰明确，控制上也灵活，不同实例可以有不同设置。
...
**注意:** 两个不同的实例必须运行在不同的端口上，所以需要修改对应实例的端口号

这里 main 实例使用默认的5432端口，不作修改；dev 实例使用5433端口，修改对应配置文件
