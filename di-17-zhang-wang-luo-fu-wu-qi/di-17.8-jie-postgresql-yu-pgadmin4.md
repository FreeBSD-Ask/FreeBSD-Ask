...
`postgresql_initdb_flags` 参数,默认是`"--encoding=utf-8 --lc-collate=C"`。意义一目了然：指定编码为 UTF-8，语言为 C，即实质上是英语 ASCII 编码。
...
- `postgresql_data`,`postgresql_enable` 等参数还是可以使用，这时这些参数作为实例对应参数的默认值使用，在找不到实例参数时使用。
- 管理多个实例时，建议只使用实例名相关的参数，不去使用没有实例名的参数。好处是清晰明确，控制上也灵活，不同实例可以有不同设置。
...
这里 `main` 实例使用默认的 `5432` 端口，不作修改；dev 实例使用5433端口，修改对应配置文件
