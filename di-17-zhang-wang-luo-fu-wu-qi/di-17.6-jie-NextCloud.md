...
编辑 `/usr/local/etc/nginx/nginx.conf`，修改如下：
...
http {
    ...
    server {
        ...
        # 如果请求 /nextcloud 没有尾部斜杠，重定向到 /nextcloud/
        location = /nextcloud {
            return 301 /nextcloud/;
        }

        # 使用 alias 映射 /nextcloud/ 到实际目录 /usr/local/www/nextcloud/
        location /nextcloud/ {
            alias /usr/local/www/nextcloud/;
            index index.php index.html;
            # 尝试直接访问文件或目录，找不到时交由 @nextcloud 处理
            try_files $uri $uri/ @nextcloud;
        }

        # fallback location，用于将所有请求转发到 index.php
        location @nextcloud {
        ...
        location ~ ^/nextcloud/(.+\.php)$ {
            alias /usr/local/www/nextcloud/$1;
            fastcgi_split_path_info ^(.+\.php)(/.*)$;
            include fastcgi_params;
            fastcgi_param SCRIPT_FILENAME /usr/local/www/nextcloud/$1;
            fastcgi_param PATH_INFO $fastcgi_path_info;
            fastcgi_pass 127.0.0.1:9000;
        }

        # PHPS 文件处理，直接以纯文本显示源代码
        location ~ ^/nextcloud/(.+\.phps)$ {
            alias /usr/local/www/nextcloud/$1;
