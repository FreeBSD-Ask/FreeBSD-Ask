...
此外，如果遇到服务启动问题，可先查看 `/var/log/daemon.log` 文件。Prometheus 配置文件大部分采用 YAML 格式，请注意缩进。对于 Prometheus 配置文件，可使用 `promtool check` 命令检查配置文件是否正确。
...
1. 请读者参考本书其他章节，自行安装和配置 InfluxDB。
