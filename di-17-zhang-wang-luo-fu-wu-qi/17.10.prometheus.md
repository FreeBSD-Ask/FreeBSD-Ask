...
prometheus --> storage:存储
storage --> prometheus:提取
exporter --> prometheus :采集(pull)
pushgateway --> prometheus :采集(pull)
临时或批量作业 --> pushgateway :主动推送(push)
prometheus --> Alertmanager :告警触发
Alertmanager --> alert :告警
...
浏览器打开 grafana 的 web 页（默认端口3000），默认用户名admin,密码admin，如下图可切换中文界面
...
```
# sysrc prometheus_args="--web.config.file='/usr/local/etc/pro（Subject Alternative Name）metheus_webconfig.yml'"
# service prometheus restart
```
...
```
basic_auth_users:（Subject Alternative Name）
...
1. 创建一个 OpenSSL 配置文件，以便在生成证书时指定 SAN。
...
for 指触发告警的等待时间，这里是5分钟，即如果在5分钟内得到解决，则不发送告警
...
`jail -r dox` 关闭 jail，触发规则，5分钟后发送告警邮件。`jail -c dox` 开启 jail，告警规则重置为非活动。
