...
# service postgresql enable      # 设置 PostgreSQL 数据库服务开机自启
# service nginx enable           # 设置 Nginx 服务开机自启
...
```sql
# service postgresql initdb                  # 初始化 PostgreSQL 数据库
# su - postgres                             # 切换到 PostgreSQL 用户
$ /usr/local/bin/pg_ctl -D /var/db/postgres/data16 -l logfile start   # 启动 PostgreSQL 服务并指定数据目录和日志文件
...
postgres=# grant all on database zabbix to zabbix;  # 冗余授权，确认 zabbix 用户对数据库所有权限
GRANT
