...
Zabbix 是一个开源的企业级监控解决方案，用于监控网络服务、服务器硬件及应用程序。本节将介绍基于 PostgreSQL 数据库在 FreeBSD 上安装和配置 Zabbix 7 的方法。

## 安装 zabbix7-server

zabbix7-server 是服务器端（Zabbix Server）。它负责接收和处理监控数据，应安装在监控服务器上。

使用 pkg 安装会额外安装 MySQL，并可能导致绑定冲突，因此推荐使用 Ports 安装。首先进入对应的 Ports 目录并进行配置：
...
按如图配置，选中 `PGSQL` 后按回车键，然后执行编译安装命令：
...
Zabbix 需要一个数据库来存储监控数据，本节使用 PostgreSQL 作为数据库。请读者自行完成 PostgreSQL 16 的安装、初始化及服务自启。

## 安装 Nginx

Zabbix 前端需要通过 Web 服务器访问，本节使用 Nginx 作为 Web 服务器。请读者自行完成 Nginx 的安装和服务自启。

## 安装 zabbix7-frontend

zabbix7-frontend 是 Web 控制前端（Zabbix Frontend）。

使用 pkg 安装是最快捷的方式，执行以下命令即可：

```sh
# pkg ins zabbix7-frontend-php83
```

上述安装过程会自动安装 PHP，本文中安装的版本是 PHP 8.3（写作本文时 PHP 8.4 会报错），具体可用版本可以到 [zabbix7-frontend](https://www.freshports.org/net-mgmt/zabbix7-frontend) 看一下。

或者使用 Ports 方式安装，需要先进入对应的目录再执行编译安装命令：
...
Zabbix 前端需要通过 PHP 访问 PostgreSQL 数据库，因此需要安装相应的 PHP 数据库扩展。注意所使用的 PHP 版本要与 Zabbix 前端兼容。
...
zabbix7-agent 是 Zabbix 的代理程序，负责在被监控主机上收集监控数据并发送给 Zabbix Server。应安装在需要被监控的服务器上。
...
为了确保 Zabbix 相关服务在系统启动时自动运行，需要将相关服务配置为开机自启。
...
需要在 PostgreSQL 中创建专门的数据库和用户供 Zabbix 使用，并导入 Zabbix 的初始数据。请读者自行初始化 PostgreSQL 数据库后再执行以下命令：
...
以下是与 PostgreSQL 配置相关的参考文献，供读者进一步查阅。

- [PostgreSQL15 Public Schema 没有权限问题解决](https://showme.codes/zh-cn/2024-01-01-postgresql15-public-schema-permission/) [备份](https://web.archive.org/web/20260115044335/https://showme.codes/zh-cn/2024-01-01-postgresql15-public-schema-permission/)

## 设置 Zabbix Server

需要配置 Zabbix Server 的主要配置文件，使其能够连接到 PostgreSQL 数据库。Zabbix Server 的主要配置文件位于 `/usr/local/etc/zabbix7/zabbix_server.conf`。
...
- [Monitor Your Hosts with Zabbix](https://freebsdfoundation.org/our-work/journal/browser-based-edition/networking-10th-anniversary/practical-ports-monitor-your-hosts-with-zabbix/) [备份](https://web.archive.org/web/20260115044243/https://freebsdfoundation.org/our-work/journal/browser-based-edition/networking-10th-anniversary/practical-ports-monitor-your-hosts-with-zabbix/)
