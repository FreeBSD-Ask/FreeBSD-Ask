...
Caddy 是一个现代化的 Web 服务器，具有自动 HTTPS 配置等特性。本节将介绍如何在 FreeBSD 上安装和配置 Caddy。

## 安装 Caddy

可以通过 pkg 或 Ports 两种方式安装 Caddy。

使用 pkg 包管理器安装 Caddy 是最快捷的方式，执行以下命令即可完成安装。

```sh
# pkg install caddy
```

或使用 Ports 方式安装 Caddy，需要先进入对应的目录再执行编译安装命令。
...
安装完成后，可以查看 Caddy 安装包的说明文档，了解必要的配置和注意事项：
...
编辑 `/usr/local/www/caddy/index.html`，写入测试内容：
...
在本机打开 `https://localhost/`，可以看到测试页面：

![Caddy FreeBSD](../.gitbook/assets/caddy1.png)

### 参考文献

- [Caddy 安装与使用教程](https://blog.ovvv.top/posts/f3ac7ef6/) [备份](https://web.archive.org/web/20260115044043/https://blog.ovvv.top/posts/f3ac7ef6/)，本文测试页面的 HTML 来自此处，提供 FreeBSD 上 Caddy 的配置实践示例

## 课后习题
1. 查找 Caddy Ports 目录中的 Makefile，分析其编译配置。修改构建选项以禁用自动 HTTPS 功能，然后重新编译安装 Caddy。配置 Caddy 仅提供 HTTP 服务，并对比完整配置与 HTTP-only 配置在内存占用和证书管理复杂度上的差异。
2. 重构 Caddy 最小化配置文件，仅保留静态文件服务功能，移除所有自动 HTTPS 和反向代理相关配置，对比完整配置的启动时间和功能灵活性。

3. 修改 Caddy 默认的运行用户配置，从 www 改为使用独立的 caddy 用户，同时调整文件权限使 caddy 用户仅能访问网站根目录，验证配置变更效果。
