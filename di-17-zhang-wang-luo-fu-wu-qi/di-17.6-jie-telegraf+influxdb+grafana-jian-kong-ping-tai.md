...
读者可以自行修改 influxd 的配置文件。

- 配置文件路径为 `/usr/local/etc/influxd.conf`。


通过编辑 `/usr/local/etc/influxd.conf` 文件可进行自定义配置。
...
这里使用的是 InfluxDB 1.8 版本，需要在配置文件中进行如下配置：
...
详情请看 [官方文档](https://docs.influxdata.com/telegraf/v1.26/configuration/)。
...
此处导入一个 id 为 [928](https://grafana.com/grafana/dashboards/928-telegraf-system-dashboard/) 的模版。
