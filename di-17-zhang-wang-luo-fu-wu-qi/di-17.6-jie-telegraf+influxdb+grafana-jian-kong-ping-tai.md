...
系统监控是运维管理和性能优化的基础，通过实时采集、存储和可视化系统指标，可以实现故障预警、性能分析和容量规划等关键功能。Telegraf、InfluxDB 与 Grafana 构成的 TIG 堆栈是业界广泛采用的开源监控解决方案，遵循数据采集、存储与可视化分离的设计原则。其中 InfluxDB 作为时序数据库专门用于高效存储时间序列数据，Telegraf 作为插件化的采集代理负责指标收集，Grafana 则提供强大的可视化能力。

Telegraf、InfluxDB 与 Grafana 组成了一套完整的开源监控解决方案。其中 InfluxDB 用于存储时间序列数据，Telegraf 用于采集数据，Grafana 用于数据可视化。
...
详情参见 [官方文档](https://docs.influxdata.com/telegraf/v1.26/configuration/) [备份](https://web.archive.org/web/20260114210354/https://docs.influxdata.com/telegraf/v1/configuration/)。
...
数据源配置完成后，可以配置 Dashboard 来可视化展示监控数据。选择用于展示数据的 Dashboard，可以自行开发，也可以使用 [官方模版库](https://grafana.com/grafana/dashboards/) [备份](https://web.archive.org/web/20260115142046/https://grafana.com/grafana/dashboards/) 中其他人开发的模板。

此处导入 id 为 [928](https://grafana.com/grafana/dashboards/928-telegraf-system-dashboard/) [备份](https://web.archive.org/web/20260115142101/https://grafana.com/grafana/dashboards/928-telegraf-system-dashboard/) 的模版。
