...
服务初始化完成后，可以通过浏览器访问 OpenList 的 Web 界面。访问 `http://ip:5244`，请将 `ip` 替换为你的局域网地址，用户名为 `admin`
...
新建 `userapp-xxx.desktop` 文件（你只可改动字段 `xxx` 为你想要的名称，其他字段不可改变。如 `userapp-abc.desktop`）文件，以便 xdg-open 调用上面的脚本，在本文中为 `~/.local/share/applications/userapp-vlc.desktop` 文件：
...
在使用 OpenList 过程中如遇到问题，可以查看相关日志文件进行排查。本节还列出了一些待完善的事项。
