...
Wine 的工作机制是拦截系统调用，即软件对操作系统的请求，并将这些调用从 Windows 的形式转换为 FreeBSD 可理解的形式。如有需要，它还会将系统响应翻译为 Windows 软件所预期的格式。Wine 提供了 Windows 应用程序所期望的许多资源，在某些方面可以说，它模拟了一个 Windows 环境。
