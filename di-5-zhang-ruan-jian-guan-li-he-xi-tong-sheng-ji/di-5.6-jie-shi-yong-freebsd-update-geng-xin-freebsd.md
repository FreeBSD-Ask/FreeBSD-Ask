...
FreeBSD 提供了实用工具 `freebsd-update`，用于安装系统更新，包括升级到新的大版本。`freebsd-update` 最早在 FreeBSD 6.2-RELEASE 中被纳入基本系统，用于安全更新和勘误补丁；在 FreeBSD 6.3-RELEASE 中引入了 upgrade 命令，至 7.0-RELEASE 时被正式作为官方支持的升级途径，标志着 FreeBSD 系统更新机制进入了标准化发展阶段。
...
这里列出的是发生变动的文件，仅需要在确认后输入字母 q（代表“quit”，退出）并按回车键即可。
