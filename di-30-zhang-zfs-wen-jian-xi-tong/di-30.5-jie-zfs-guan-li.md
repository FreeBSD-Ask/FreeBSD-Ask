...
两个常用且实用的数据集属性是 NFS 和 SMB 共享选项。设置这些属性可以定义 ZFS 是否通过网络共享数据集以及如何共享。FreeBSD 同时支持 NFS（`sharenfs`）和 SMB（`sharesmb`）共享属性，其中 SMB 共享需安装 Samba。以下以 NFS 为例，要获取共享的当前状态，可输入：
...
> **警告**
>
> `-maproot=root` 将 NFS 客户端的 root 用户映射为服务器端的 root，远程 root 用户将对共享数据集拥有完全读写权限。NFS 默认启用 `root_squash`（将客户端 root 映射为 `nobody`），使用 `-maproot=root` 相当于关闭此安全机制。建议仅在受信任的内部网络中使用，或改为 `-maproot=nobody` 以限制权限，也可映射到指定的非特权用户（如 `-maproot=1000`）。
...
ZFS 克隆创建与提升流程：
  ...
  阶段一：创建克隆
  ─────────────────
  原数据集（zroot/home/ykla）
     │
     │  zfs snapshot（创建快照）
     ▼
  快照 zroot/home/ykla@test
     │
     │  zfs clone（创建可写克隆）
     ▼
  克隆 zroot/home/ykla2
  （origin 属性指向 @test，依赖快照存在）

  阶段二：提升克隆（zfs promote）
  ─────────────────
  提升前：
     原数据集 ──► 快照 @test ◄── 克隆（origin）

  提升后：
     克隆（已提升为独立数据集）──► 快照 @test ◄── 原数据集
     （origin 属性清除，父子关系反转）
...
ZFS 复制（send/receive）数据流：
  ...
  zfs receive backup/mypool
     ...
     │  还原为文件系统
     ▼
  备份池 backup/mypool（含 @replica1 快照）

  增量复制（后续备份）：
  ──────────────────────
  源池 mypool（已有 @replica1，新增 @replica2）
     │
     │  zfs send -i @replica1 @replica2
     │  （仅发送两个快照间的差异）
     ▼
  增量数据流
     │
...
通过网络发送流数据是维持远程备份的有效手段，但存在一个缺点：经网络链路发送的数据未加密，任何人都可拦截流数据并将其还原为原始数据，而无需发送方的授权。这在通过互联网向远程主机发送数据时不可接受。为确保数据安全传输，可使用 SSH 加密发送的数据。由于 ZFS 要求将流从标准输出重定向，可通过 SSH 轻松管道传输。为确保文件系统内容在远程系统上静态存储时保持加密，推荐使用 ZFS 原生加密。在接收方创建加密数据集：

```sh
# zfs create -o encryption=aes-256-gcm -o keyformat=passphrase recvpool/backup
```

接收的流数据将自动以加密形式写入磁盘。ZFS 原生加密自 OpenZFS 0.8.0 起已成熟可用，与 ZFS 快照、发送/接收无缝集成，且无需额外安装内核模块。
...
`refreservation` 属性用于设置引用预留（refreservation），适用同样的原则，格式为 `refreservation=大小`。
...
并非所有数据都适合去重。如果池中的数据没有冗余，去重未必能带来节省。ZFS 可通过模拟去重来预估潜在的空间节省（建议先导出池以确保结果准确）：
