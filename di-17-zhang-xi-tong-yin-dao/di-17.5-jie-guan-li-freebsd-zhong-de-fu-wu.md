...
> **注意**
>
> 关键字 `enable`、`disable`、`delete` 参见：FreeBSD Project. Add new rc keywords: enable, disable, delete[EB/OL]. [2026-03-26]. <https://reviews.freebsd.org/D17113>.
...
> ```
>
> 上述命令中 `"YES"` 的双引号可省略，系统会自动添加。`"NO"` 同理。
...
precious_machine="NO"  # 设置为 YES 可防止意外执行 shutdown(8) 命令
...
- 关机并断电推荐使用 `shutdown -p now`，该命令通过向 init(8) 发送信号来运行 `/etc/rc.shutdown` 脚本（源代码位于 `libexec/rc/rc.shutdown`）有序关闭服务后断电。也可使用 `halt -p`，该命令直接刷新文件系统缓存并向所有进程发送 SIGTERM 及 SIGKILL 信号后断电，不执行 `/etc/rc.shutdown`。

重启：

- 重启命令与 Linux 相同，均为 `reboot`，但参数不通用。
- FreeBSD 下 `reboot` 直接刷新文件系统缓存，向所有进程发送 SIGTERM 及 SIGKILL 信号后重启系统，不执行 `/etc/rc.shutdown`。若需有序关闭服务后重启，应使用 `shutdown -r now`，该命令通过 init(8) 运行 `/etc/rc.shutdown`。若需快速重启（跳过向进程发送信号，可能导致数据丢失），可使用 `reboot -q` 命令，该选项仅刷新文件系统缓存后立即重启。`fastboot` 是 `reboot` 的别名，行为与 `reboot` 相同。

> **注意**
>
> FreeBSD 下，`shutdown` 命令允许 root 用户和 operator 组成员执行；`reboot` 与 `halt` 命令仅限 root 用户执行。
