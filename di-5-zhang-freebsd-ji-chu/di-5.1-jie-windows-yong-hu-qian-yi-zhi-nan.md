...
盘符是一种抽象映射，本身并无固定不变的物理意义。文件系统中并不存在硬编码的 `C` 盘标识，这也解释了为何在其他操作系统中（包括 Windows 双系统环境）均看不到 `C` 盘。只有实际启动系统后，Windows 才能确定哪个分区对应 `C` 盘，并写入注册表。其他盘符的分配则存在不确定性，`D` 盘变为 `E` 盘的情形也并不罕见，例如某虚拟光驱可能在开机时被自动加载。
...
- FreeBSD Project. login.conf(5)[EB/OL]. [2026-04-18]. <https://man.freebsd.org/cgi/man.cgi?query=login.conf&sektion=5&manpath=freebsd-16.0>. FreeBSD 登录类能力数据库，源文件位于 **usr.bin/login/login.conf**，编译后路径为 **/etc/login.conf**，用于设置字符编码等用户环境。
