...
系统账户由源代码中的 [main/etc/master.passwd](https://github.com/freebsd/freebsd-src/blob/main/etc/master.passwd) 文件定义，截至写作时共计 26 个（25 个 UID < 1000 的账户加上 nobody(65534)，该数量随版本迭代可能变化），`_dhcp`、`ntpd` 便属于此类系统账户。系统账户是具有受限权限的专用账户，通常用于运行系统服务和守护进程。
...
Shell (sh csh tcsh nologin) [sh]: sh  # 除非手动设置默认 Shell，否则默认 Shell 为 sh
...
> **警告**
>
> `operator` 组可能授予意外的类似超级用户的访问权限，包括但不限于关机（`shutdown`）和访问 **/dev** 中的部分设备（如磁盘设备），因此使用该组时请务必小心。注意 `reboot` 和 `halt` 仅限 root 用户，operator 组成员无法执行。
...
> **警告**
>
> `pw userdel -r` 将永久删除用户及其主目录和所有相关信息，操作不可逆。如果主目录为 ZFS 数据集且已清空，pw(8) 会尝试自动销毁该数据集（但不会处理子数据集和快照）。请确认用户名正确无误。
...
常用参数：`-r` 删除用户，同时删除用户主目录及所有相关信息；如果不使用该参数则信息保留，仅删除用户。如果主目录为 ZFS 数据集且已清空，pw(8) 会尝试自动销毁该数据集，但不会处理数据集内的子数据集和快照。
