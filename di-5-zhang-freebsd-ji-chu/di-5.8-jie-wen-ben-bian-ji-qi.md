...
无模态编辑器的典型代表是 ee 和 Emacs，其按键操作在所有状态下含义一致，通常直接输入字符即插入文本。模态编辑的设计源于 Bill Joy 于 1976 年为 ex 编辑器开发的 visual mode（可视模式），该模式在 1979 年 ex 2.0 版本中独立为 vi 命令。其设计动机是通过模式切换，在 ADM-3A 终端的有限键盘上实现高效编辑，无需同时按下控制键即可执行编辑命令。
