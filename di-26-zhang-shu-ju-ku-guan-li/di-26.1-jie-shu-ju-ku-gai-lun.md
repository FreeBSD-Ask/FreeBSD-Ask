...
> **注意**
>
> SQL 标准中不存在 `RENAME DATABASE` 语句。MySQL 早期版本曾提供该语法，但出于数据安全考虑已于 5.1.23 版本移除。现代 MySQL 需要重命名数据库时，应先导出数据，创建新数据库后再导入数据。
...
- `char` 是字符类型，如果需要存储字符串，需要指定字符数，即 `char(字符数)`
- 通过 `unsigned` 关键字可将数据类型设置为无符号类型
...
| **NO ACTION** | 类似 RESTRICT。在 MySQL 中，二者完全等价，均立即拒绝违反参照完整性的操作。在 SQL 标准中，RESTRICT 不可延迟；若约束为 DEFERRABLE，则 NO ACTION 可延迟到事务结束时检查完整性 |
