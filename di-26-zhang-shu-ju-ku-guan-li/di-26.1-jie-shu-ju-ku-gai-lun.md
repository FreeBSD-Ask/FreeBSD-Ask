...
本节介绍关系型数据库的基本操作与 SQL 语法。本节中的 SQL 示例均基于 MySQL 语法，部分命令（如 `SHOW DATABASES`、`DESC`、`AUTO_INCREMENT`、`MODIFY COLUMN`、反引号引用等）为 MySQL 专有，不兼容标准 SQL 或其他数据库系统（如 PostgreSQL）。PostgreSQL 的对应语法参见 26.2 节。
...
```sql
use db_name; -- MySQL 专有命令，PostgreSQL 中使用 \c db_name
```
...
```sql
show tables; -- MySQL 专有命令，PostgreSQL 中使用 \dt 或查询 INFORMATION_SCHEMA.TABLES
```

## 展示表结构（MySQL 专有）

```sql
desc table_name; -- MySQL 专有命令（DESCRIBE 的缩写），PostgreSQL 中使用 \d table_name 或查询 INFORMATION_SCHEMA.COLUMNS
```
...
(
  ...
  列名 数据类型 not null,
  -- 自动编号
  列名 数据类型 auto_increment -- MySQL 专有，PostgreSQL 使用 SERIAL 或 GENERATED ... AS IDENTITY
);
```
...
```sql
alter table table_name modify column col_name new_data_type; -- MySQL 专有语法，PostgreSQL 使用 ALTER TABLE table_name ALTER COLUMN col_name TYPE new_data_type
```
...
```sql
alter table table_name drop primary key; -- MySQL 专有语法，PostgreSQL 使用 ALTER TABLE table_name DROP CONSTRAINT 约束名
```
...
```sql
alter table table_name
drop index constraint_name; -- MySQL 专有语法，PostgreSQL 使用 ALTER TABLE table_name DROP CONSTRAINT constraint_name
```
...
```sql
alter table table_name
drop foreign key fk_name; -- MySQL 专有语法，PostgreSQL 使用 ALTER TABLE table_name DROP CONSTRAINT fk_name
```
...
```sql
ALTER TABLE 表名
MODIFY COLUMN 列名 新数据类型; -- MySQL 专有语法，PostgreSQL 使用 ALTER COLUMN ... TYPE
...
`LIMIT a, b` 用于限制查询结果的数量，其中 a 表示起始位置（从 0 开始），b 表示返回的记录条数。此语法为 MySQL 专有，PostgreSQL 使用 `LIMIT b OFFSET a`。

```sql
SELECT * from `学生表` -- 反引号为 MySQL 专有的标识符引用方式，标准 SQL 使用双引号
ORDER BY `学号` desc
LIMIT 0,3; -- MySQL 专有语法，PostgreSQL 写法为 LIMIT 3 OFFSET 0
```
