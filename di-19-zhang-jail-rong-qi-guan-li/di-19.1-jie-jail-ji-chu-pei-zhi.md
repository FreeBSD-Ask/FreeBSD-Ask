...
使用 `jls` 命令可查看当前运行中的 Jail 信息。默认输出包含四列：Jail 标识符（jid）、IP 地址（ip4.addr）、主机名（host.hostname）和根目录路径（path）。`jls -N` 可将第一列替换为 Jail 名称（若无名称则回退为数字 ID），`jls -v` 则提供每 Jail 多行的详细摘要，额外显示名称、dying 状态、cpuset ID 及 IPv6 地址等。
...
也可以使用 jail 命令直接操作。`-r` 会终止指定 Jail 内的所有进程及其子 Jail，`-rc` 将先移除再重新创建，实现重启。此外 `jail -cm` 在 Jail 不存在时创建、已存在时修改：
