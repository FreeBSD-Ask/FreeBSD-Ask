...
部分教程中提到使用 `qjail config -k jailname` 打开 raw_sockets 功能来实现外网访问，这是一种误解。raw_sockets 仅为 ping 等工具所需，并非网络访问的必要条件。在 Jail 中打开 raw_sockets 存在安全风险，这是 Jail 环境默认的安全设计。因此除非确实需要在 Jail 中使用 ping 等工具，否则不建议打开 raw_sockets 功能。
