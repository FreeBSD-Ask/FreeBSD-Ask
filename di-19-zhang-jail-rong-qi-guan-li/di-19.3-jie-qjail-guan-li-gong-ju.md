...
建立 jail2 后，自动建立 **/usr/jails/jail2/usr/local/etc/pkg/repos/FreeBSD.conf** 文件，即修改了此后所有 Jail 的默认 `pkg` 镜像。但 jail1 未生成该文件，因生成 jail1 时尚未在 flavors 目录中写入相应文件。
...
选项 `-P`（大写）使用宿主机的 Ports 更新 Jail 的 Ports 树。如果主机已有 Ports，则可使用 `-P`（大写），避免重复下载 Ports：
...
Qjail 提供 `qjail config` 命令，用于定制每个 Jail 的配置。由于 Jail 的核心配置参数在其运行状态下无法安全修改，运行该命令前须先停止目标 Jail。
...
部分教程中提到使用 `qjail config -k jailname` 打开 `raw_sockets` 功能来实现外网访问，这是一种误解。`raw_sockets` 仅为 `ping` 等工具所需，并非网络访问的必要条件。在 Jail 中打开 `raw_sockets` 存在安全风险，这是 Jail 环境默认的安全设计。自 FreeBSD 8.0 起可通过 `sysctl security.jail.allow_raw_sockets` 在内核层面控制此行为，例如使用 `sysctl security.jail.param` 或 `sysctl -d security.jail.param.*` 浏览各个 Jail 相关的 sysctl 项，其中 `-d` 选项用于显示指定 OID 的描述。因此除非确实需要在 Jail 中使用 `ping` 等工具，否则不建议打开 `raw_sockets` 功能。
