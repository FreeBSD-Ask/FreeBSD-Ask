...
部分教程中提到使用 `qjail config -k jailname` 打开 `raw_sockets` 功能来实现外网访问，这纯属误解。`raw_sockets` 仅为 `ping` 等工具所需，并非网络访问的必要条件。在 Jail 中打开 `raw_sockets` 存在安全风险，这是 Jail 环境默认的安全设计。除非确实需要在 Jail 中使用 `ping` 等工具，不建议开启 `raw_sockets` 功能。

Jail 尚无法连接网络，它绑定在 `lo1` 网络接口上，`lo1` 并不能直接访问外网，需借助 PF 设定网络，其中 `em0` 为外网接口。使用 `ifconfig` 命令可查找系统中的外网接口名称。
