...
Qjail 是用于部署 Jail 环境的工具，源自 ezjail 3.1，并进行了功能扩展。
...
ifconfig_lo1_alias0="inet 192.168.1.1-9" # 创建独立网段，共 9 个 IP 可供分配。需要根据实际网络环境调整地址范围
```
...
- **sharedfs** 包含一份只读的操作系统可执行文件库，通过 nullfs 在各 Jail 之间共享以节省存储空间。nullfs 是一种特殊的文件系统，用于在同一主机的不同位置挂载同一文件系统。
...
建立 jail2 后，自动建立 **/usr/jails/jail2/usr/local/etc/pkg/repos/FreeBSD.conf** 文件，即修改了之后所有 Jail 的默认 pkg 镜像。但 jail1 未生成该文件，因生成 jail1 时尚未在 flavors 目录中写入相应文件。
...
有 `-p`（小写）、`-P`（大写）两个选项，`-p`（小写）使用 portsnap 更新 Jail 的 Ports 树。`-P`（大写）使用宿主机的 ports 更新 Jail 的 Ports 树。如果主机已有 Ports，则可使用 `-P`（大写），避免重复下载 ports：
...
Qjail 提供 `qjail config` 命令以对每个 Jail 进行定制化配置。由于 Jail 的核心配置参数在其运行状态下无法安全地修改，运行该命令前须先停止目标 Jail。
...
部分教程中提及使用 `qjail config -k jailname` 打开 raw_sockets 功能来实现外网访问，这是一种误解。raw_sockets 仅为 ping 等工具所需，并非网络访问的必要条件。在 Jail 中打开 raw_sockets 存在安全风险，这是 Jail 环境默认的安全设计。因此除非确实需要在 Jail 中使用 ping 等工具，否则不建议打开 raw_sockets 功能。

Jail 尚无法连接网络，因其绑定在 lo1 网络接口上，lo1 并不能直接访问外网，需要通过 PF 设定网络，其中 `em0` 为外网接口。使用 `ifconfig` 命令可查找系统中的外网接口名称。
...
以下命令均在 Jail 控制台下运行，pkg 安装时可根据需要使用镜像。如果使用镜像，可在 Jail 控制台中以与宿主机相同的方式进行设置，详见相关章节。
...
2. 修改 Qjail 的 flavor 机制，自定义一个新 flavor 并应用于新 Jail。

3. 对比 Qjail、ezjail 和 iocage 三种 Jail 管理工具的功能差异，编写对比报告。
