...
布尔参数（如 `allow.raw_sockets`、`mount.devfs`）除赋值 `"true"`/`"false"` 外，还可使用无值快捷写法（如 `allow.raw_sockets` 等价于 `allow.raw_sockets = "true"`），或以 `no` 前缀表否定（如 `allow.nomount` 等价于 `allow.mount = "false"`）。
