...
指定 Windows iso 文件开始正常安装。当在安装模式下运行时，`vm-bhyve` 将等待，直到 VNC 客户端连接后再启动客户机。这能让你抓住 Windows 可能显示的提示“Press any key to boot from a CD/DVD“。你能在 `vm list` 中看到，在此时，虚拟机将显示为锁定：
