...
Rsync 即远程同步（Remote Sync，Rsync），是由 Andrew Tridgell 开发的高效文件同步工具。Rsync 采用增量传输算法（delta-transfer algorithm），仅传输文件中发生变化的部分，显著减少网络带宽消耗与同步时间，是系统备份与数据镜像的常用工具。其算法通过计算文件块的校验和来确定差异，实现高效数据同步。

## 环境概述

在开始配置之前，先了解一下本节将使用的环境。首先介绍本节配置中涉及的两台服务器。
