...
GitLab EE 是 GitLab 的企业版本，提供额外的企业级功能。GitLab CE（社区版）是开源版本，功能相对简化。

## 安装 GitLab EE

本节介绍如何安装 GitLab 企业版（Enterprise Edition，EE），有两种安装方式可供选择。
...
GitLab 需要使用 PostgreSQL 数据库。当前 GitLab 支持的 PostgreSQL 版本为 16 及以上版本。

请读者参考本书其他相关章节完成 PostgreSQL 数据库的部署。
...
编辑 `/usr/local/www/gitlab/config/puma.rb` 文件，将 `workers 3` 改为上面输出的值，即改为 `workers 16`。

### 数据库连接配置

编辑 `/usr/local/www/gitlab/config/database.yml` 文件，将 `password: "secure password"` 修改为 `password: "password"`（`password` 是上面设置的数据库密码）。
...
……此处省略大概需要几十分钟，若提示内存溢出，请增加 swap，一般来说需要执行两次，若内存溢出需要再重复执行一次……
...
- 孟古一. GitLab EE 16 安装破解教程[EB/OL]. (2023-07-05)[2026-03-26]. <https://blog.mengguyi.com/articles/GitLab-Install.html>
...
- 李俊才. CI/CD 笔记 .Gitlab 系列：2024 更新后 - 设置 GitLab 导入源[EB/OL]. (2024-03-11)[2026-03-26]. <https://bbs.huaweicloud.com/blogs/423539>
...
1. 在 FreeBSD 上安装 GitLab CE（社区版）而非 EE 版，完成完整配置流程并验证代码仓库创建、提交与克隆功能，提交 PR 至本文。

2. 对 GitLab 进行安全加固，使之适用于生产环境。总结并提交 PR 至本文。
