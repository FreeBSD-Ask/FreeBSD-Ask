...
在分布式系统环境中，准确的系统时间是确保日志一致性、证书有效性、分布式事务协调以及网络协议正常工作的基础。本节详细介绍 FreeBSD 系统的时区配置与网络时间协议（Network Time Protocol，NTP）时间同步服务的部署与管理。时间同步的准确性直接关系到系统安全与数据一致性。
...
ntpd 是 FreeBSD 系统的默认 NTP 守护进程，用于持续同步系统时间。ntpdate 是一个独立工具，用于一次性同步系统时间。
...
1. 在 FreeBSD 上配置 OpenNTPD 替代默认 ntpd，设置多个本地 NTP 服务器源，验证时间同步精度并比较与 ntpd 的差异。

2. 分析 FreeBSD 的 adjkerntz 机制，查看其如何处理 RTC 与系统时间的关系，讨论该设计是如何实现跨平台兼容性的。

3. 修改 ntpd 的默认配置，禁用本地时钟作为备用时间源，验证系统行为变化并分析该修改对系统时间稳定性的影响。
