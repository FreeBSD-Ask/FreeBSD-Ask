...
Webmin 是一款基于 Web 的系统管理工具，为 Unix-like 操作系统提供了图形化管理界面，可用于管理用户账户、磁盘配额、服务配置、网络设置等系统管理任务。其设计理念是通过直观的 Web 界面降低系统管理的复杂度。Webmin 使用内置的 Web 服务器提供服务，默认监听 10000 端口。

## 安装 Webmin

本节介绍如何安装 Webmin，有两种主要安装方式可供选择，可根据实际情况选择合适的安装方法。
...
安装完成后，需要通过配置向导进行初始设置。启动 Webmin 安装向导，并配置启用 SSL。
...
服务启动后，可以将 Webmin 界面设置为中文显示。在 Webmin 中依次进入 Change Language and Theme，在 `Webmin UI language` 字段中先选择 `Personal choice`，然后选择 `Simplified Chinese (ZH_CN.UTF8)`，点击 `Make Changes` 按钮。之后返回菜单 -> Dashboard，控制台界面将刷新为中文。
...
3. 修改 Webmin 的默认 SSL 配置，禁用旧版本 TLS 协议。贡献至本文。
