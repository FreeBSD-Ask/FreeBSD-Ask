...
FTP 即文件传输协议（File Transfer Protocol）。作为 TCP/IP 协议栈中最早的应用层协议之一，FTP 采用客户端-服务器（Client-Server）架构，通过两个独立的 TCP 连接（控制连接与数据连接）实现文件的双向传输。FTP 存在主动模式（PORT）和被动模式（PASV）两种工作方式：主动模式由服务器主动向客户端发起数据连接，被动模式由客户端向服务器发起数据连接。在 FreeBSD 系统中，通过搭建 FTP 服务器可以实现局域网或互联网环境下的文件快速传输与共享。
...
Pure-FTPd 已移除对 [RFC 2640](https://www.rfc-editor.org/rfc/rfc2640) [备份](https://web.archive.org/web/20260115141032/https://www.rfc-editor.org/rfc/rfc2640)（Internationalization of the File Transfer Protocol，FTP 协议的国际化）的支持，该 RFC 定义了 FTP 的国际化框架。因此在 Windows 系统下使用 ftp 命令行访问包含非英文字符的文件可能出现乱码，且无法修改编码。参见 [Version 1.0.48 released](https://www.pureftpd.org/project/pure-ftpd/news) [备份](https://web.archive.org/web/20260117042926/https://www.pureftpd.org/project/pure-ftpd/news/)，该版本发布公告说明了此变更。
...
>**技巧**
>
>基本思路是数据库中的虚拟用户将继承 `pw useradd` 创建的用户的权限及 GID/UID，然后通过数据库中的用户进行 FTP 操作。
...
- [Linux 环境下 FTP 权限设置详解与操作步骤全攻略](https://my.oschina.net/emacs_8748786/blog/17171107) [备份](https://web.archive.org/web/20260129115210/https://my.oschina.net/emacs_8748786/blog/17171107)，775 权限设置来自此处
...
- [Virtual Hosting With PureFTPd And MySQL](http://km.npru.ac.th/userfiles/R013/km_articles_files/20120227104346_Virtual%20Hosting%20With%20PureFTPd%20And%20MySQL.pdf)，本文框架基于此，该文档介绍了 Pure-FTPd 与 MySQL 集成的虚拟主机配置方案。
...
vsftpd 即 Very Secure FTP Daemon（非常安全的 FTP 守护进程），注重安全性设计，在 Linux 系统中较为流行。经过测试，在 Windows 上的 FTP 客户端中也不会出现乱码。
...
官网文档在此 [VSFTPD.CONF](https://security.appspot.com/vsftpd/vsftpd_conf.html) [备份](https://web.archive.org/web/20260115141030/https://security.appspot.com/vsftpd/vsftpd_conf.html)，中文翻译在金步国的 [vsftpd.conf 中文版](https://www.jinbuguo.com/vsftpd/vsftpd.conf.html) [备份](https://web.archive.org/web/20260115043719/https://www.jinbuguo.com/vsftpd/vsftpd.conf.html)。
...
- [How to set up vsftpd on FreeBSD 12?](https://www.osradar.com/how-to-set-up-vsftpd-on-freebsd-12/) [备份](https://web.archive.org/web/20260118011502/https://www.osradar.com/how-to-set-up-vsftpd-on-freebsd-12/)，其修改自 [Installing and configuring vsftpd on FreeBSD](https://dnaeon.github.io/installing-and-configuring-vsftpd-on-freebsd/) [备份](https://web.archive.org/web/20260115043448/https://dnaeon.github.io/installing-and-configuring-vsftpd-on-freebsd/)，本文框架基于此，该文档提供了 FreeBSD 上 vsftpd 的完整安装配置指南。
...
1. 在 FreeBSD 上通过 ports 编译安装 Pure-FTPd 并启用 PostgreSQL 支持，创建测试数据库表，验证虚拟用户可以正常登录并上传下载文件。

2. 选取 vsftpd 的 chroot 机制，分析其如何在用户隔离与系统复杂性之间做权衡，尝试修改默认配置使 chroot 目录可写并验证其安全风险。

3. 修改 ProFTPD 的默认端口配置，从 21 改为 2121，同时调整被动端口范围，验证服务可用性并分析该修改对系统安全性的影响。
