...
vsftpd 即 Very Secure FTP Daemon（非常安全的 FTP 守护进程）。在 Linux 上比较流行。
...
- 创建密码数据库（我上面添加了用户 `virtual`，密码是 `z` ）

```sh
# htpasswd -c -b /usr/local/etc/vsftpd_login.db virtual z
...
FreeBSD Journal 2015.11-12.pdf
哲学史期末.pdf
226 Transfer complete
ftp: 收到 56 字节，用时 0.00秒 56.00千字节/秒。
ftp> bye
221 Goodbye.
ftp>
```
