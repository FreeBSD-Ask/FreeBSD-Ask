...
FTP 即文件传输协议（File Transfer Protocol）。作为 TCP/IP 协议栈中最早的应用层协议之一，FTP 采用客户端-服务器（Client-Server）架构，通过两个独立的 TCP 连接（控制连接与数据连接）实现文件的双向传输。在 FreeBSD 系统中，通过搭建 FTP 服务器可以实现局域网或互联网环境下的文件快速传输与共享。
...
vsftpd 即 Very Secure FTP Daemon（非常安全的 FTP 守护进程）。在 Linux 上比较流行。经过测试，在 Windows 上的 FTP 客户端中也不会出现乱码。
