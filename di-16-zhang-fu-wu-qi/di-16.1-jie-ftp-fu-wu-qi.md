...
FTP（File Transfer Protocol，文件传输协议）是 TCP/IP 协议栈中最早的应用层协议之一。该协议采用客户端-服务器（Client-Server）架构，通过两个独立的 TCP 连接（控制连接与数据连接）实现文件的双向传输。
在 FreeBSD 系统中搭建 FTP 服务器，可实现局域网或互联网环境下的文件快速传输与共享。

FTP 支持主动模式（PORT）和被动模式（PASV）两种工作方式：主动模式下服务器主动向客户端发起数据连接，被动模式下则由客户端向服务器发起数据连接。

本章将介绍三种常用的 FTP 服务器软件：

| 软件 | 特点 | 适用场景和人群 |
| ---- | ---- | -------------- |
| Pure-FTPd | ISC 许可证授权、安全优先、支持多种操作系统、FTP 协议规范实现较为完整 | 为 ISP 和托管服务提供了高度灵活性、适用于新手 |
| ProFTPD | GPLv2 许可证授权、支持多种操作系统、单一主配置文件、可扩展性强 | 适合对 Apache HTTP Server 了解的用户 |
...
Pure-FTPd 已移除 RFC 2640[EB/OL]. [2026-03-26]. <https://www.rfc-editor.org/rfc/rfc2640>（Internationalization of the File Transfer Protocol，FTP 协议的国际化）的支持，该 RFC 定义了 FTP 的国际化框架。因此在 Windows 系统下使用 ftp 命令行访问包含非英文字符的文件时可能出现乱码，且无法修改编码。具体变更可参见 Pure-FTPd. Version 1.0.48 released[EB/OL]. [2026-03-26]. <https://www.pureftpd.org/project/pure-ftpd/news> 版本发布公告。
...
- falko. Virtual Hosting With PureFTPd And MySQL (Incl. Quota And Bandwidth Management)[EB/OL]. (2007-11-07)[2026-03-25]. <https://archive.org/download/pdfbackup_18_aout-2014/Virtual_Hosting_With_PureFTPd_And_MySQL__Incl._Quota_And_Ban.pdf>.
...
请自行安装配置 MySQL 8.x，需要确保与 `proftpd-mod_sql_mysql` 安装的 `databases/mysql8X-client` 版本一致。
...
vsftpd（Very Secure FTP Daemon，非常安全的 FTP 守护进程）注重安全性设计，在 Linux 系统中较为流行。经过测试，在 Windows 下的 FTP 客户端中也不会出现乱码问题。
