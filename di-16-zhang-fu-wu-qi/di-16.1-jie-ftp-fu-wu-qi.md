...
若不创建，会提示 `proftpd[3435]: warning: handling possibly truncated configuration data at line 65 of '/usr/local/etc/proftpd.conf'`。
...
append [本地文件] [远程文件] 以 本地文件 为文件名向服务器上传本地文件本地文件

ascii 将 FTP 文件传送类型设置为 ASCII 模式
