...
多操作系统共存（Multi-boot）是计算机系统中通过引导管理器在多个操作系统之间选择启动的技术方案。其实现原理是在磁盘的不同分区上安装不同的操作系统，由引导管理器（Boot Manager）在系统启动时提供选择界面。FreeBSD 的引导管理器 `boot0`（又称 Boot Easy）支持多操作系统启动，boot0cfg 用于管理引导管理器。
