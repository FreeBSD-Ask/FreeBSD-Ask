...
UTM（逆构词为 Universal Turing Machine）是开源、基于 QEMU、面向 Apple 设备的虚拟机软件，支持 ARM 和 x86 架构虚拟机。建议下载 aarch64（arm64）架构版本，不建议使用 amd64（x86-64）版本，除非有特殊需求。运行相同架构的虚拟机可获得更高性能和更流畅的系统体验（M1 为 aarch64 架构）。
