...
在典型的 Linux 系统中，initrd 是一个打包为内存盘的精简根文件系统，内含驱动程序、挂载工具以及启动初始化程序所必需的数据。开机时，引导加载程序（Bootloader）加载内核与 initrd，由 initrd 中的脚本执行启动准备，随后将控制权移交给硬盘上的初始化程序。
...
2. 对比文中列出的六种失败方案，分析每种方案失败的根本技术原因，尝试改进其中一种方案并验证其可行性。
