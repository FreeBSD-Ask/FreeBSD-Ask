...
```sh
# 将外部 6667 端口重定向到内部 IRC 服务器
redirect_port tcp 192.168.0.2:6667 6667
# 将外部 80 端口重定向到内部 Web 服务器
redirect_port tcp 192.168.0.3:80 80
```

将这些参数添加到上面规则集中 NAT 实例 1 的配置中后，TCP 端口将转发至运行 IRC 和 HTTP 服务的局域网客户机。
...
地址重定向在可用多个 IP 地址时非常有用。每台局域网客户机可通过 ipfw(8) 分配其专有的外部 IP 地址，之后将重写来自局域网客户机的出站数据包，使用正确的外部 IP 地址，并将所有传入该 IP 地址的流量重定向回特定的局域网客户机。这也称为静态 NAT。例如，若有 IP 地址 `128.1.1.1`、`128.1.1.2` 和 `128.1.1.3` 可用，`128.1.1.1` 可作为 ipfw(8) 机器的外部 IP 地址，而 `128.1.1.2` 和 `128.1.1.3` 则分别转发到局域网客户机 `A` 和 `B`。
