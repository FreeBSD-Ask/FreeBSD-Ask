...
> **警告**
>
> 只有包含 `log` 选项的防火墙规则才会记录日志。默认规则不包括此选项，必须手动添加。因此，建议编辑默认规则集以启用日志记录。另外，如果日志存储在单独的文件中，可能需要启用日志轮替。没有 **/etc/rc.conf** 变量来设置日志限制。要限制每次连接尝试记录规则的次数，请在 **/etc/sysctl.conf** 中指定数字：
