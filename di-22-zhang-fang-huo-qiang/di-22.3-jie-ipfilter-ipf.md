...
若这是唯一的 Web 服务器，此规则也适用，因其将所有外部 HTTP 请求重定向至 `10.0.10.25`：
...
若无 FTP 代理，则需使用以下防火墙规则。需要注意，若无代理，则需要允许所有高于 `1024` 的端口：
...
每次编辑包含 NAT 规则的文件时，运行 `ipnat` 需要使用 `-CF` 选项以删除当前的 NAT 规则并刷新动态转换表的内容。并使用 `-f` 指定要加载的 NAT 规则文件名：
