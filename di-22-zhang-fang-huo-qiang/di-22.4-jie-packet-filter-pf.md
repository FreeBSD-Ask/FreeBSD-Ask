...
本节演示如何配置运行 PF 的 FreeBSD 系统，使其作为至少一台其他计算机的网关。网关需要至少两个网络接口，每个接口连接到不同的网络。在此例中，**em0** 连接到互联网，**em1** 连接到内部网络。
...
3. 接下来，在 **/usr/local/etc/spamd/spamd.conf** 中配置 spamd，并添加一些 **rc.conf** 参数。
   安装 mail/spamd 后，会附带一个示例配置文件（**/usr/local/etc/spamd/spamd.conf.sample**）和 **spamd.conf** 的手册页。参阅这些文件以获取更多配置选项。
...
> **技巧**
>
> 使用示例 **spamd.conf** 中的所有黑名单将阻塞大量的互联网地址。管理员需要编辑该文件，创建最佳配置，使用适当的数据源，并在必要时添加自定义列表。

   接下来，将以下条目添加到 **/etc/rc.conf**。有关其他标志的说明，参见手册页：

   ```sh
   obspamd_flags="-v" # 使用 "" 并查看 spamd-setup(8) 获取标志
   ```
...
2. 要以灰名单模式运行 spamd，需要在 **/etc/rc.conf** 中添加 `-b` 标志至 `obspamd_flags`，使 spamd 启用灰名单模式：

   ```sh
   obspamd_flags="-b"  # 启用灰名单模式
   ```
...
一些服务，如 NFS，需要特定的分片处理选项。更多信息，参阅 [https://home.nuug.no/~peter/pf/en/scrub.html](https://home.nuug.no/~peter/pf/en/scrub.html)。
...
ALTQ 不是可加载的内核模块，也不包含在 GENERIC 内核中。若系统的接口支持 ALTQ，需要创建定制内核。以下是可用的内核选项。第一个选项是启用 ALTQ 所必需的。还需要至少一个选项以指定排队调度算法：
