...
| 算法 | 全称 | 说明 |
| ---- | ---- | ---- |
| CBQ | 基于类别的排队 | 将连接的带宽划分为不同的类别或队列，根据过滤规则优先处理流量。 |
| RED | 随机早期检测 | 通过测量队列长度并与最小值和最大值进行比较来避免网络拥塞。当队列长度超过最大值时，所有新数据包会被随机丢弃。 |
| RIO | 随机早期检测输入/输出 | RED 维护多个平均队列长度和多个阈值，每个 QOS 级别有一个阈值。 |
| HFSC | 层次化公平服务曲线分组调度器 | 详见 [https://www.cs.cmu.edu/~hzhang/HFSC/main.html](https://www.cs.cmu.edu/~hzhang/HFSC/main.html)。 |
| PRIQ | 优先级排队 | 始终优先通过较高队列中的流量。 |
| CoDel | 受控延迟 | 用于缓解缓冲区膨胀的调度算法，通过监控数据包在队列中的停留时间来决定是否丢弃数据包。 |
| FAIRQ | 公平排队 | 为每个流量流分配独立的队列，确保各流公平分享带宽。 |
