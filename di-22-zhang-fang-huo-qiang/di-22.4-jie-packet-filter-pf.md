...
- 若 PF 内核模块未加载，运行 `pfctl` 时会提示 `pfctl: /dev/pf: No such file or directory`。此时应先执行 `kldload pf` 加载模块，或重启系统后再执行 `service pf start`。
...
FreeBSD 14 起，`scrub` 推荐作为 `match` 规则的选项使用，而非独立语句。最简单的形式适用于大多数配置：
