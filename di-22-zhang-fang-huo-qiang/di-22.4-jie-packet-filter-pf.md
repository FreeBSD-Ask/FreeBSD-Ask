...
还可以在启动 PF 时传递其他选项，这些选项在 pfctl(8) 中有详细说明。将此条目添加或更改到 **/etc/rc.conf** 中，并在引号（`""`）之间指定任何需要的标志：
...
为了监控通过 PF 防火墙的流量，可以考虑安装包或 Port [sysutils/pftop](https://cgit.freebsd.org/ports/tree/sysutils/pftop/) 。安装后，可以运行 pftop 查看流量的实时快照，其格式类似于 top(1)。
...
1. 安装 [mail/spamd](https://cgit.freebsd.org/ports/tree/mail/spamd/) 软件包和 Ports 。要使用 spamd 的灰名单功能，必须将 fdescfs(5) 挂载到 **/dev/fd**。将以下行添加到 **/etc/fstab**：
