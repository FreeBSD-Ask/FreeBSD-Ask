...
在 PF 中，`scrub` 是一个启用网络数据包规范化的关键字。该过程会丢弃具有无效标志组合的 TCP 数据包。启用 `scrub` 提供了一定的保护，防止某些基于不正确处理数据包碎片的攻击。虽然有多个选项可供选择，但最简单的形式适用于大多数配置：

```sh
scrub in all
```

对于碎片重组，FreeBSD 14 起推荐在规则集的 `options` 部分使用 `set reassemble yes`，而非在 scrub 规则中使用 `fragment reassemble` 选项：

```sh
set reassemble yes no-df
```

`no-df` 选项表示同时重组设置了"禁止分片"标志的碎片，重组后的数据包将清除该标志。

一些服务，如 NFS，需要特定的碎片处理选项。有关更多信息，请参考 [https://home.nuug.no/~peter/pf/en/scrub.html](https://home.nuug.no/~peter/pf/en/scrub.html)。

以下示例设置了碎片重组、清除 DF 标志，并通过 scrub 规则将最大分段大小设置为 1440 字节：
