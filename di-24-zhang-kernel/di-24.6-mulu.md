...
本章系统阐述 FreeBSD 操作系统源代码的目录组织结构，从整体架构到模块细分，帮助读者建立对 FreeBSD 代码库的系统性认知。理解源代码目录结构是深入系统开发、定制和优化的基础，它揭示了操作系统各组件之间的逻辑关系和依赖层次，是系统工程实践重要的知识框架。
...
图片由 [treesize_free](https://www.jam-software.com/treesize_free) [备份](https://web.archive.org/web/20260118170515/https://www.jam-software.com/treesize) 生成。
...
```sh
/usr/src/
├── .arcconfig Phabricator 是 FreeBSD 的代码审查工具，该文件为其配置文件
├── .arclint Lint 检查用
├── .cirrus-ci Cirrus CI 指定安装 pkg 的脚本，供 CI 使用
├── .cirrus.yml Cirrus CI 配置文件
├── .clang-format  ClangFormat 工具的配置文件，设置了代码风格
├── .git-blame-ignore-revs Git 配置文件，用于忽略特定提交（如代码格式化）的影响
├── .gitattributes 指定 Git 在 git diff 时使用特定的差异比较方式
├── .github GitHub 相关配置，包括提交模板、工作流和代码维护者信息
...
├── bin 系统命令和用户命令
├── cddl 各种在通用开发与发行许可（Common Development and Distribution License，CDDL）下的命令和库，例如 ZFS
├── contrib 第三方软件
...
├── kerberos5 Heimdal Kerberos 5 在 FreeBSD 中的实现
├── lib 系统库 
├── libexec 系统守护进程（daemon）
...
├── sys 内核源码
├── targets 支持实验性的 DIRDEPS_BUILD 机制
...
├── compat Linux 兼容层以及 FreeBSD 32 位兼容性支持
├── conf 内核构建配置层
├── contrib 第三方软件，如 OpenZFS、zstd(1)
│   └── dev 包含部分无线网卡驱动，如 iwlwifi(4)、rtw88(4)、rtw89(4)、ath10k、ath11k、ath12k、rtwn(4) 等
├── crypto 加密驱动程序
├── ddb 交互式内核调试器 ddb(4)
├── dev 设备驱动程序和其他架构无关代码，如 iscsi(4)、iwx(4)、iwm(4)、xz(1)
├── dts Device Tree Source（DTS）设备树源码
├── fs 除了 UFS、NFS、ZFS 以外的文件系统
├── gdb 内核远程 GDB 存根 gdb(4) 实现
├── geom GEOM 框架 geom(4)
...
├── powerpc PowerPC/POWER（32 位、64 位）架构支持
...
├── xdr 外部数据表示（External Data Representation，XDR）相关，供 RPC 使用
