...
virtio 条目提供了一种通用总线供设备驱动程序使用。该总线必须与和主机通信的接口结合使用。VirtIO 规范定义了多种此类接口，包括 PCI 和 MMIO。
