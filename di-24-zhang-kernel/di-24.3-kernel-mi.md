...
此 PCI 以太网卡基于通用 MII 总线控制器代码。`mwl` 依赖 `mwlfw`。
...
这两个选项分别用于调整 mbuf 集群大小和 mbuf 大小。在将它们的默认值更改为其他值时，需要非常小心网卡驱动模块，因为这可能导致内核假定的 mbuf 大小与模块假定的 mbuf 大小不匹配。目前，唯一能够检测这种不匹配的驱动是 [ti(4)](https://man.freebsd.org/cgi/man.cgi?query=ti&sektion=4)。
