...
未指定值则假定为 NULL。通用选项在 `sys/conf/options`，架构特定选项在 `sys/conf/options.<架构>`。

### 文件

- `sys/compile/NAME` — 根据内核配置创建的编译目录  
- `sys/conf/Makefile.arch` — 架构的 Makefile 片段  
- `sys/conf/files` — 所有架构通用设备  
- `sys/conf/files.arch` — 架构的设备  
- `sys/conf/options` — 所有架构通用选项  
- `sys/conf/options.arch` — 架构的选项  
