...
理论上所有 GENERIC 内核选项都能在机器无关和 amd64/i368 机器相关选项中找到说明。详细选项列表及设备行说明可参考对应的 `NOTES` 文件。
...
IPv4 网络协议簇。IP 协议族是一组协议，分层于互联网协议（IP）传输层之上，并使用互联网地址格式。该协议族为 `SOCK_STREAM`、`SOCK_DGRAM` 和 `SOCK_RAW` 套接字类型提供协议支持；其中 `SOCK_RAW` 接口提供了对 IP 协议的访问。
...
在内核中包含文件 `INCLUDE_CONFIG_FILE`。可以方便的引入更多存在于其他文本中的选项。
...
smbios，System Management BIOS SYNOPSIS 系统管理 BIOS（System Management BIOS）概要支持。
