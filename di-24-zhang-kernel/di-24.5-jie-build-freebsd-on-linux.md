...
本章系统阐述在 Linux 系统上交叉构建 FreeBSD 操作系统的完整方法论。交叉构建是嵌入式系统开发和跨平台软件分发的关键技术，通过在主机构建系统（build host）上生成目标架构（target architecture）的可执行代码，实现了开发环境与运行环境的解耦，是跨平台开发实践中的核心技术手段。以下是操作环境和步骤说明：
...
在长时间构建过程中，避免系统自动熄屏是很重要的。可依次进入“设置”→“电源”→“节电选项”，然后将“熄屏”选项设置为“从不”。

## 禁用 systemd-oomd

自 Ubuntu 22.04 LTS（Desktop）以降，系统默认启用了 systemd-oomd 这一服务。该服务会在系统达到某阈值时强制结束当前高占用的进程，可能导致构建失败，且这一操作不会向用户提供任何提示信息。
...
| **WITHOUT_LIB32=yes**                 | 禁用 32 位兼容库及相关组件，因为测试未通过 |
...
- [Jammy Jellyfish Release Notes](https://discourse.ubuntu.com/t/jammy-jellyfish-release-notes/24668) [备份](https://web.archive.org/web/20260118015226/https://discourse.ubuntu.com/t/jammy-jellyfish-release-notes/24668)，Ubuntu 22.04 LTS 发行说明，参见 systemd v249.11 这一小节。系统介绍 Ubuntu 22.04 的新特性，包括 systemd-oomd 服务。
- [FreeBSD 手册](https://docs.freebsd.org/en/books/handbook/cutting-edge/#building-on-non-freebsd-hosts) [备份](https://web.archive.org/web/20260116210848/https://docs.freebsd.org/en/books/handbook/cutting-edge/#building-on-non-freebsd-hosts)，26.9. Building on non-FreeBSD Hosts，只提供了基本思路，参考价值有限。官方非 FreeBSD 主机构建 FreeBSD 的权威指南。
- [Building on non-FreeBSD hosts](https://wiki.freebsd.org/BuildingOnNonFreeBSD) [备份](https://web.archive.org/web/20260117180234/https://wiki.freebsd.org/BuildingOnNonFreeBSD)，仅提供基本思路，参考价值有限。社区维护的跨平台构建实践经验汇总。
- [man src.conf](https://man.freebsd.org/cgi/man.cgi?src.conf)，构建参数参考此处。FreeBSD 构建系统配置参数的完整官方手册。
- [cross-bootstrap-tools.yml](https://github.com/freebsd/freebsd-src/blob/main/.github/workflows/cross-bootstrap-tools.yml)，FreeBSD 项目自身使用的交叉编译 CI。项目官方的持续集成工作流程示例。
