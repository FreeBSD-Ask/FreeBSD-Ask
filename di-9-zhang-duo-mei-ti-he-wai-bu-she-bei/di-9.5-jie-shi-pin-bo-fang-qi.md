...
点击 `Skin`（皮肤）选项，然后点击界面左下角的设置级别按钮，将当前的 `Basic`（简单）级别改为 `Expert`（专家）或 `Standard`（标准）级别，否则不可见 `Fonts`（字体）等高级设置选项。随后将 `Fonts`（字体）设置为 `Arial based`，否则中文可能出现乱码显示问题。
...
1. 在物理机（非虚拟机）上使用 mpv 在 TTY 环境中播放视频，验证其能否正常工作。

2. 安装 Kodi 并配置中文界面，测试中文字幕的显示效果。

3. 修改 SMPlayer 的默认视频输出驱动（如从 xv 切换到 vdpau），对比不同输出驱动的播放性能。
