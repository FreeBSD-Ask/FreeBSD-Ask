...
> **技巧**
>
> 本节介绍将 FreeBSD 配置为打印服务器。如果 FreeBSD 仅作为打印客户端，通过 USB 本地连接打印机打印，而不需要共享打印服务，则 avahi-app 和 dbus 并非必需组件。
...
截至本章撰稿时，FreeBSD 端的测试页打印功能尚未完成验证。

## 课后习题

1. 配置 CUPS 打印服务器并打印 FreeBSD 测试页，分析测试页内容如何反映打印系统的配置状态，并复现从局域网不同设备打印的完整流程。
2. 完善 CUPS 界面的中文支持。
3. CUPS 以 IPP 协议替代传统的 LPD 与 SMB 打印协议，将打印任务建模为带有属性的结构化文档。比较 IPP 与 LPD 在错误恢复、作业状态查询与安全性方面的协议差异，分析 IPP 的复杂性是否为家庭打印场景引入了不必要的开销。
