...
上文中的 `pcm6: <Realtek ALC892 (Rear Digital)> (play)` 为数字输出接口。一般集成声卡的模拟接口采样率通常为 48 kHz，部分 HDA 编解码器可支持 96 kHz 或更高；数字接口则绕过了板载 DAC 的采样率限制，可使用支持更高采样率的外部 DAC。标准 S/PDIF 可支持至 96 kHz，在特定配置下可达 192 kHz。
