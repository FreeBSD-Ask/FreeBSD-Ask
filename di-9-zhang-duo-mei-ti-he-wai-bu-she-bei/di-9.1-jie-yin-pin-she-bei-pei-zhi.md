...
`snd_hda` 是 FreeBSD 内核中用于支持 Intel High Definition Audio（HDA）规范声卡的驱动模块，适用于绝大多数现代集成声卡。该驱动通常会在系统启动时自动加载；若默认内核未包含或未自动加载该驱动，则需手动加载对应内核模块以启用音频功能。使用 `kldstat` 命令检查驱动是否已加载。
