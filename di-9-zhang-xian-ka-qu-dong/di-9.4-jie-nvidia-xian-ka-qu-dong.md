...
不具备显卡直通能力的笔记本设备，必须先按照其他章节安装并配置英特尔核芯显卡驱动（相关 DRM 模块），随后再参照下文完成 NVIDIA 独立显卡驱动的配置。

```sh
是否笔记本？
├── 是（笔记本）→ Intel/AMD 核显 → NVIDIA（混合显卡，如有）                 　　　　　　　　　　　　　　　　　　　　　　　　　
└── 否（台式机）      　　　　
    ├── 是否有核显？      　　　　
    │   ├── 否（Intel F/KF）→ NVIDIA（直连）       　　　　　　　　　　　　　　　　　　　　　　　　　　
    │   └── 是（核显）     　　　　　　　　
    │       ├── 是否使用独显输出？         　　　　　　　　　　　　
    │       │   ├── 是 → NVIDIA（直连为主）       　　　　　　　　　　　　　　　　　　　　　　　　　
    │       │   └── 否 → Intel + NVIDIA（可选）     　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　
...
> **警告**
>
> X11 用户请勿尝试加载 `nvidia-drm.ko` 内核模块，该操作可能导致系统崩溃。Wayland 用户按需加载。
