...
对于大多数用户而言，掌握 `sudo su` 这一基础命令即可满足日常需求。doas 是 OpenBSD 原生工具，设计目标是替代 sudo 的最常用功能子集，配置语法远比 sudoers 简洁。
...
- `/usr/local/etc/doas.conf.sample` 内容如下：
...
安装 sudo-rs 前必须先卸载 sudo，在该安装方式下二者不共存。
