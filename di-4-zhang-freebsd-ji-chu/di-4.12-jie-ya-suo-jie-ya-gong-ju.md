...
压缩和解压工具是计算机文件管理中不可或缺的基础工具。从计算机科学的角度看，数据压缩的本质是利用信息的统计冗余性，通过特定的编码算法减少数据表示所需的存储空间。根据是否允许信息丢失，压缩算法可分为无损压缩（lossless compression）和有损压缩（lossy compression）两大类。本节所涉及的工具均采用无损压缩算法，即解压后的数据与原始数据完全一致。
...
GNU tar 和 bsdtar 两者在处理某些扩展属性和 ACL 时行为不同；bsdtar 的 `-l` 选项遵循 ISO/IEC 9945-1:1996（“POSIX.1”）的定义（检查链接），而 GNU tar 1.15 之前的版本将 `-l` 作为 `--one-file-system` 的同义词。使用 `@-` 约定在不同归档格式之间转换时，可能导致硬链接信息丢失，这是不同归档格式存储硬链接信息的方式不兼容所致。
...
rar 格式是专有格式，但 unrar 工具在大多数类 Unix 系统上可用；rar 压缩工具在某些发行版上可能受限（版权原因）。
...
```sh
$ tar --zstd -cvf test.tar.zst /home/ykla/test/ # 打包并压缩成 zstd 格式
```
