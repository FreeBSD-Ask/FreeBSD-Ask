...
- **功能分离原则**：将不同功能的文件分配到不同的目录中，使得各文件系统可以独立管理、挂载和备份。例如，将系统二进制文件（`/bin`、`/sbin`）与用户应用程序（`/usr/local`）分离，将配置文件（`/etc`）与日志数据（`/var/log`）分离。将 `/var` 与 `/` 分离是有利的，因为 `/var` 包含日志目录、假脱机目录和各种临时文件，可能会被填满，而填满根文件系统是不当的做法。
