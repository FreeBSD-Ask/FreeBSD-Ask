...
openSUSE 有时会在稳定版的软件包中引入实验性功能，且可能没有明确的提示（参见：Bugzilla – Bug 1213157 repo <http://download.opensuse.org/update/leap/15.5/oss>. metadata expired[EB/OL]. [2026-03-26]. <https://bugzilla.opensuse.org/show_bug.cgi?id=1213157> .）。这可能导致用户将其视为软件缺陷（Bug）进行反馈。用户可能需要提交问题报告后，才能从维护者处得知该行为是实验性功能所致。
