...
以用户进程身份运行 init，可模拟 AT&T System V UNIX 的行为：超级用户可在命令行中指定所需的运行级别，该 init 进程向原始（PID 为 1 的）init 进程发送特定信号，以执行相应操作。例如，在 FreeBSD 中执行 `init 0`（发送 SIGUSR1 实现停机）、`init 6`（发送 SIGINT 实现重启）、`init 1`（发送 SIGHUP 重新加载 /etc/ttys）。

PID 为 1 的 init 进程还响应以下信号（需通过 `kill` 命令手动发送）：SIGHUP（重新加载 /etc/ttys 并重启 getty）、SIGINT（重启）、SIGUSR1（停机）、SIGUSR2（停机并关闭电源，需硬件支持）。
