...
许多 Linux 体系的核心概念与技术实践，最早由 BSD 系统提出并实践，包括：
...
以用户进程身份运行 init，可模拟 AT&T System V UNIX 的行为：超级用户可在命令行中指定所需的运行级别，该 init 进程向原始（PID 为 1 的）init 进程发送特定信号，以执行相应操作。例如，在 FreeBSD 中执行 `init 0` 仍然表示关机。
