...
PID 为 1 的 init 进程还响应以下信号（需通过 `kill` 命令手动发送）：SIGUSR1（仅停止运行，不断电）、SIGWINCH（停止运行、关电源并重启，需硬件支持）。
...
本附录对主流 GNU/Linux 发行版进行对比分析，帮助读者更全面地了解各 GNU/Linux 发行版。
...
Debian Stable 发行版的软件包策略以稳定为主，大部分软件在发布后，其主版本号在生命周期内通常不会升级，软件版本会被锁定，因此应将 Stable 同时理解为“稳定”与“固定”。如果需要获取更新的软件版本，则需切换到 Testing 或 Unstable（Sid）分支，但这会牺牲系统的整体稳定性。
