...
除了基本配置外，还可以通过主题美化 Shell 界面。Powerlevel10k 作为一款广泛使用的 Zsh 主题，以下为其安装和配置方法：
...
1. 配置 Zsh 环境，安装 Powerlevel10k 主题，并编写一个自定义 Shell 函数用于自动化 FreeBSD 系统更新（`freebsd-update` + `pkg upgrade`），记录配置流程与函数实现。
2. 对比 csh/tcsh 与 sh 在 FreeBSD 基本系统中的实现差异，从历史兼容性、脚本语法和交互体验三个维度分析 csh 曾作为默认 root Shell 的原因。
3. 查阅 csh/tcsh 的源代码，分析其缺乏现代功能（如语法高亮、自动补全框架）的架构限制。
