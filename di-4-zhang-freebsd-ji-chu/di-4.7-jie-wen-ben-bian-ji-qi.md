...
就编辑范式而言，文本编辑器可分为两大类：**模态编辑器**（modal editor）和**无模态编辑器**（modeless editor）。
...
LazyVim 是一款开箱即用的 NeoVim 配置发行版（distribution），基于 lazy.nvim 插件管理器，集成了代码补全、LSP、文件浏览器、Git 集成等 IDE 特性，非常适合需要快速获得强大编辑体验的用户。
