...
理解权限的工作机制对于确保用户能够访问所需文件、同时防止不当访问操作系统或其他用户拥有的文件至关重要。
...
- **Capsicum 沙箱框架**：通过能力（capability）机制限制进程可访问的系统资源范围，实现最小权限原则。
...
[chmod(1)](https://man.freebsd.org/cgi/man.cgi?query=chmod&sektion=1) 常用选项如下：

| 选项 | 说明 | 备注 |
| ---- | ---- | ---- |
| `-R` | 递归修改文件层次结构的权限 | 注意使用通配符 `.*` 时可能意外匹配 `..` 硬链接 |
| `-h` | 修改符号链接本身的权限而非其指向的文件 | 默认情况下对符号链接执行 chmod 修改的是目标文件的权限 |
| `-v` | 显示详细修改过程 | 指定两次时同时显示旧模式和新模式（八进制和符号表示） |

> **注意**
>
> - 
> - `-v` 选项为非标准扩展，在脚本中使用不被推荐。
...
这些值与 chmod(1) 一起使用，但使用字母而非数字。例如，以下命令将阻止与 FILE 关联的组成员和所有其他用户访问 FILE：

```sh
% chmod go= FILE
```

~~这里的 go 并不是代表“前进”，而是“组所有者”和“其他”。~~

当需要对文件进行多组更改时，可以提供逗号分隔的列表。例如，以下命令移除 FILE 的组和“world”写权限，并为所有人添加执行权限：
...
> - umask 中的执行位（`0111`）对文件创建的影响取决于请求模式：若请求模式中不包含执行位（如 `0666`），则 umask 中的执行位屏蔽无效；若请求模式包含执行位（如 `0777`，如 `mkdir`），则执行位屏蔽有效。
> - 在 Jail 容器中，umask 继承自主机系统的进程环境，不受 Jail 配置的直接影响。

## `chown` 命令

[chown(8)](https://man.freebsd.org/cgi/man.cgi?query=chown&sektion=8) 常用选项如下：

| 选项 | 说明 | 备注 |
|------|------|------|
| `-R` | 递归修改文件层次结构的属主 | 注意使用通配符 `.*` 时可能意外匹配 `..` 硬链接 |
| `-h` | 修改符号链接本身的属主而非其指向的文件 | 默认情况下对符号链接执行 chown 修改的是目标文件 |
| `-v` | 显示详细修改过程 | 指定两次时同时显示旧属主/属组和新属主/属组 |

用于修改文件的属主，包括所属用户和所属组。只有文件所有者或超级用户（root）才能修改文件的属主。
...
除基本的读、写、执行权限外，UNIX 系统还定义了三种特殊权限位，它们在系统安全和功能实现中扮演着关键角色。这些设置对于某些 UNIX 操作至关重要，因为它们提供了通常不授予普通用户的功能。要理解它们，必须注意实际用户 ID（real user ID）与有效用户 ID（effective user ID）之间的区别：实际用户 ID 是拥有或启动进程的 UID，而有效 UID 是进程运行时所采用的 UID。
...
chflags(1) 首次出现于 4.4BSD，是 BSD 系统特有的功能，Linux 不原生支持（需通过 `chattr` 命令和 `lsattr` 命令实现类似功能）。

[chflags(1)](https://man.freebsd.org/cgi/man.cgi?query=chflags&sektion=1) 常用选项如下：

| 选项 | 说明 | 备注 |
|------|------|------|
| `-R` | 递归修改文件层次结构的标志 | 注意使用通配符 `.*` 时可能意外匹配 `..` 硬链接 |
| `-h` | 修改符号链接本身的标志而非其指向的文件 | 默认情况下对符号链接执行 chflags 成功但无效果 |
| `-v` | 显示详细修改过程 | 指定两次时同时显示旧标志和新标志（八进制表示） |
...
> - 已知 Bug：仅有限数量的工具支持 chflags 标志，包括 ls(1)、cp(1)、find(1)、install(1)、dump(8)、restore(8)；目前 pax(1) 不支持 chflags。

## FreeBSD 文件标志

除了文件权限外，FreeBSD 还支持使用“文件标志”。这些标志增加了额外的安全和控制层级，适用于文件和目录。通过文件标志，甚至可以阻止 root 删除或修改文件。

文件标志使用 chflags(1) 修改。例如，要在文件 `file1` 上启用系统不可删除标志，发出以下命令：
...
1. 修改 FreeBSD 中某个系统目录的默认权限配置，验证其行为变化。
2. 查看 FreeBSD 内核中权限检查的核心源代码，尝试使其更加细粒度。
