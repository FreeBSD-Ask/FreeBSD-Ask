...
读、写、执行权限除了用 `rwx` 表示外，亦可分别对应数字 4、2、1，无权限即 0。每三位权限对应的数字相加后组合，即形成 3 位数字的表示方式（记忆口诀：“读 4 写 2 执行 1”）。
...
系统是如何控制设备权限的呢？传统的 UNIX 哲学是“一切皆文件”，将大多数硬件设备视为一个文件，程序可以打开它、读写它。这些特殊的设备文件存储在 `/dev/` 目录中。
...
目录也被视作文件进行相同的处理。它们也有读、写和执行权限。目录的可执行位与文件略有不同。当一个目录被设置为可执行时，表示可以使用 `cd` 命令切换进入该目录。
...
还有一些额外的权限位，主要用于特殊场景，例如 setuid 可执行文件和 sticky 目录。

## 符号权限

符号权限使用字符而非八进制值来为文件或目录分配权限。符号权限使用语法 `(对象)(操作)(权限)`，其中可用的值如下：
...
当需要对文件进行多组更改时，可以提供逗号分隔的列表。例如，以下命令移除 test 的组和“world”写权限，并为所有人添加执行权限：
...
事实上，权限字符是八进制，而不是十进制。将八进制 755 转为二进制则为：111 101 101。具体计算过程：
...
例如，若进程请求以模式 `0666`（所有用户读写）创建文件，而当前 umask 为 `0022`。
...
除了文件权限外，FreeBSD 还支持使用“文件标志”。这些标志增加了额外的安全和控制层级，适用于文件和目录。通过文件标志，甚至可以阻止 root 删除或修改文件。
