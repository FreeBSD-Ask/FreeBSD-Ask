...
C/C++ 是两门具有悠久历史的经典编程语言，至今仍在系统编程、高性能计算、嵌入式开发等领域占据核心地位。FreeBSD 作为一个以稳定性、高性能和完整内核源代码开放著称的操作系统，其内核及大量用户态工具均以 C 语言编写，充分体现了 C 语言在系统级开发中的独特优势：可移植的底层内存操作能力、极小的运行时开销、与操作系统接口的高度契合性，以及长期积累的庞大生态基础。
...
需要注意的是，以这种方式安装的 VS Code 实际上是 [Code-OSS](https://github.com/microsoft/vscode)。Code-OSS 与 VS Code 的区别主要在于许可证不同以及可用的闭源资源不同，类似 Chromium 与 Chrome 的关系。可以阅读 [阅读原文](https://github.com/microsoft/vscode/wiki/Differences-between-the-repository-and-Visual-Studio-Code) 深入了解二者的区别。
