...
Minecraft 是 Java 开发的沙盒游戏，不同版本对 JDK 版本有特定要求（1.20.5+ 需 Java 21）。本节覆盖 FreeBSD 上 OpenJDK 安装、启动器配置及服务器端部署。

## 桌面端

### 安装 OpenJDK

Minecraft 对 JDK 版本有特定要求，不同游戏版本所需的 Java 版本不同：1.17 需要 Java 16，1.18 至 1.20.4 需要 Java 17，1.20.5 及更高版本需要 Java 21。本节测试使用 JDK 21 可正常运行最新版 Minecraft，该版本在 FreeBSD 上提供了良好的兼容性和稳定性。
...
FreeBSD 中常见有两种启动器，分别是 HMCL 和 Prism Launcher。其中 Prism Launcher 可通过 FreeBSD Ports 获得，而 HMCL 只能手动下载 `.jar` 文件运行。
...
Prism Launcher 是使用 C++ 和 Qt 框架开发的开源启动器。该启动器默认禁止离线登录，并对[绕过方法](https://github.com/antunnitraj/Prism-Launcher-PolyMC-Offline-Bypass)加以限制，仅推荐正版用户使用。
...
若使用 KDE 桌面环境，需更新第三方软件包到最新版本。KDE 桌面环境对 Qt 版本有特定要求，Prism Launcher 依赖的 Qt 库需与系统库保持一致。
...
过旧的 JDK 版本不受服务器端支持，本节测试使用 JDK 21 可正常运行，该版本在服务器环境中表现稳定。
...
当前默认配置下，服务器启用了正版验证机制。该机制与 Mojang 会话服务器和 Microsoft 身份验证服务通信，以验证玩家身份，非正版用户和未登录用户无法连接。如需在局域网或测试环境中使用，可关闭正版验证功能：在 **/usr/local/etc/minecraft-server/server.properties** 文件中，将 `online-mode=true` 修改为 `online-mode=false` 即可。该操作会降低服务器安全性，请在受控环境中使用。
