...
安装完成后新建项目并进入编辑器，可能会发现界面响应延迟且 CPU 占用率较高。这是因为在 FreeBSD 默认配置下，Godot 的渲染由 CPU 进行软件光栅化处理，并且仅使用单个线程，对系统性能构成显著负担。
...
如此在打开项目并进入 Godot 编辑器后，通过系统监视器观察，缩放 Godot 窗口时 CPU 占用率没有明显变化，表明渲染工作已由 GPU 处理。
