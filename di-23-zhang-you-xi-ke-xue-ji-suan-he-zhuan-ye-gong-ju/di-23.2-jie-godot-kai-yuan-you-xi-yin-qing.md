...
Godot 是开源 2D/3D 游戏引擎。FreeBSD 通过 Ports 提供 Godot-tools（编辑器）和 Godot（导出模板）两个软件包。本节介绍安装与基础项目配置。
...
安装完成后新建项目并进入编辑器，可能会发现界面响应延迟且 CPU 占用率较高。这是因为在 FreeBSD 默认配置下，Godot 的渲染由 CPU 完成软件光栅化处理，并且仅使用单个线程，对系统性能构成显著负担。
