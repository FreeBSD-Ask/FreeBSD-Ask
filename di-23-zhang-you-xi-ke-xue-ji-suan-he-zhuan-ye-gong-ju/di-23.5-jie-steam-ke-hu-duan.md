...
Steam 没有原生 FreeBSD 客户端，但其 Linux 版本可通过 FreeBSD Linux 兼容层运行。本节介绍 games/linux-steam-utils Port 的安装与配置方法，使 FreeBSD 用户能够接入 Linux 游戏生态。
...
本小节介绍常见问题的解决方法，遇到问题时可按以下内容进行排查。
...
该问题可通过安装中文字体来解决，推荐安装 `wqy-fonts` 或 `noto-fonts-cjk` 等字体包。
