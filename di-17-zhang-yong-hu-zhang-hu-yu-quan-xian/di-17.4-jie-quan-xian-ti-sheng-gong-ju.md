...
| ---- | ---- |
| `nopass` | 免密码执行 |
| `keepenv` / `setenv` | 环境变量控制（继承或设置） |
| `args` | 命令参数限制 |
| `nolog` | 不记录日志 |
| `persist` | 密码缓存（仅 Port **security/opendoas** 支持） |
...
sudo 的初始作者是 Bob Coggeshall 和 Cliff Spencer（原始版本，约 1980 年），面向 4.1BSD。sudo 主要重写和维护者是 Todd C. Miller。
...
- Sudo. A Brief History of Sudo[EB/OL]. [2026-03-26]. <https://www.sudo.ws/about/history>. sudo 官方简史。
...
## 示例：保留 Qt/KDE 的安装路径变量（旧版 Qt3/KDE3 时代变量，现代系统已不再使用）。
...
```sh
$ sudo su
[sudo: authenticate] Password: # 此处不显示输入内容，无掩码提示
#
```
