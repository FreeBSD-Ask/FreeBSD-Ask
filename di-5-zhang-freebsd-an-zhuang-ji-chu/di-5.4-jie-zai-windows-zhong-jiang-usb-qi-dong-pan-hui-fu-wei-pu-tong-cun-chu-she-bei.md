# 5.4 在 Windows 中将 USB 启动盘恢复为普通存储设备
...
使用 Rufus、Win32DiskImager 等软件制作 U 盘启动盘用于安装系统后，用户可能会发现 U 盘的可见容量仅有约 30–100 MB（即 EFI 系统分区，具体大小因镜像而异）。
...
判断哪个是目标 U 盘，通常可根据 U 盘容量判断。如果不确定 U 盘容量，可查询购买记录或取下 U 盘查看其外壳上标注的容量。

- 通过容量判断：64 GB 的 U 盘在 Windows/Linux 中通常显示为约 57–59.6 GB（因操作系统使用二进制单位 GiB 但标注为 GB，64×10⁹÷1024³≈59.6，实际显示还受文件系统开销影响），在 macOS 中显示为 64 GB（macOS 使用十进制单位）；
...
确定目标 U 盘后，开始还原操作：首先删除所有分区，随后创建新的分区。
...
| 适用场景 | 老式计算机 | 现代计算机 |
| 最大磁盘容量 | 2 TB | 9.4 ZB |
| 最大分区数 | 4 个主分区 | 128 个（Windows） |
| 引导方式 | BIOS/Legacy | UEFI |

两种分区表在磁盘上的存储结构对比如下：

![分区表结构比较](../.gitbook/assets/gpt-mbr-diff.png)
...
DISKPART> ass letter=E # 挂载到 E 盘，也可以将 U 盘移除后再插入
