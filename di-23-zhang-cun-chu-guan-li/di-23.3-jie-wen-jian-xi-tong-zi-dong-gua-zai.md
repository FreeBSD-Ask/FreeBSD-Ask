...
automount 已内置于 FreeBSD 基本系统，配合 devd(8) 监控设备插入事件并自动执行挂载操作，默认挂载点位于 **/media** 目录。
...
| `-media` | 查询还未挂载但包含有效文件系统的设备。通常用于访问可移动媒体上的文件 |
...
该配置以后台方式启动 `dsbmc-cli` 并启用自动挂载。`-a` 参数用于启用自动挂载模式，客户端持续监控设备事件并自动执行相应操作，此方式不提供图形界面提示。
...
# 使用 ntfs-3g 挂载 NTFS，设置文件权限掩码 fmask=137、目录权限掩码 dmask=027，并指定属主 UID 与 GID
ntfs_mount_cmd = "/usr/local/bin/ntfs-3g -o \"uid=${DSBMD_UID},gid=${DSBMD_GID},fmask=137,dmask=027\" ${DSBMD_DEVICE} \"${DSBMD_MNTPT}\""

# …………中间配置内容省略…………

# 以普通用户方式挂载 NTFS，设置文件权限掩码 fmask=137、目录权限掩码 dmask=027
...
- FreeBSD Project. FreeBSD Handbook, Chapter 20: Storage[EB/OL]. [2026-04-14]. <https://docs.freebsd.org/en/books/handbook/disks/>. FreeBSD 手册中关于存储与文件系统的配置指南。
