...
已向 Ports 维护者提交了合并请求（[Bug 291295 - net/mihomo: Add rc.conf and some Post-installation](https://bugs.freebsd.org/bugzilla/show_bug.cgi?id=291295)），该请求是为 Mihomo 添加系统服务管理支持，截至撰稿时尚未收到回应。在官方集成完成之前，可使用下文提供的自定义 RC 脚本以实现服务化管理。
...
RC 脚本提供了多个命令行参数和配置选项，以下列举常用参数。这些命令会直接将配置写入 **/etc/rc.conf** 文件，如果配置有误可直接修改对应行。
