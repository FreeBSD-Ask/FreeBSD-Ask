...
2. 对比 Mihomo 与 Sing-Box 在规则集（rule-set）匹配引擎上的数据结构差异，分析不同规则匹配算法（线性扫描、Radix Tree、AC 自动机）在万级节点规则集合下的性能特征。
3. 代理工具的规则匹配模块本质上是一种内容过滤机制。分析网络代理领域的内容过滤逻辑与防火墙领域的内容过滤逻辑在“知情同意”伦理原则上的差异，讨论代理工具开发者为何必须同时实现两种功能。
