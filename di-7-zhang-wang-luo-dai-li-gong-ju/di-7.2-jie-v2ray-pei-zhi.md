...
安装完成后，需要启动代理软件。若已在其他平台（如 Windows）上使用 V2RayN 等客户端配置了代理节点，可将配置导出为 config.json 文件并复制到 FreeBSD 系统中。
...
在使用 Xray 时可能遇到资源文件加载失败的问题。FreeBSD 系统中，xray-core 的 pkg 安装的资源文件位于 **/usr/local/share/xray-core/**，但程序默认会在可执行文件同级目录 **/usr/local/bin/** 查找。若启动时报 `open /usr/local/bin/geosite.dat: no such file or directory` 错误，可采用以下两种方案：
...
若 **/usr/local/etc/xray-core/** 目录下存在其他 `.json` 样例文件，应将其移除，避免配置冲突，因 Xray 可能扫描目录下的所有 JSON 文件。
