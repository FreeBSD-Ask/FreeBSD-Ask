...
部分教程中提到使用 `qjail config -k jailname` 启用 `raw_sockets` 功能以实现外网访问，此系常见误解。`raw_sockets` 仅为 `ping` 等工具所需，并非网络访问的必要条件。在 Jail 中启用 `raw_sockets` 存在安全风险，此为 Jail 环境默认的安全设计。除非确实需要在 Jail 中使用 `ping` 等工具，否则不宜启用 `raw_sockets` 功能。
