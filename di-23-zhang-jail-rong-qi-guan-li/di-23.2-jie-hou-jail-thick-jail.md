...
`exec.clean` 用于在容器启动时，让 `exec.start` 等命令在干净的环境中运行，而非沿用启动 Jail 时的环境变量；仅保留 `HOME`、`SHELL`、`TERM`、`USER`，其中 `HOME` 和 `SHELL` 会被设为目标登录用户的默认值，`USER` 被设为目标登录用户名，`TERM` 则从当前环境中导入。
