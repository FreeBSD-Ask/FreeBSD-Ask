...
`jail_parallel_start="YES"` 启用 Jail 并行启动；默认情况下 Jail 会串行启动，设置此选项后所有容器将并行启动。
...
`allow.raw_sockets = 1;` 允许容器创建原始套接字 (Raw Socket) 。FreeBSD Jail 默认的安全策略禁止容器直接创建原始套接字，而 `ping` 命令需要利用 Raw Socket 来发送 ICMP 报文。如果不添加此配置，执行 `ping` 将提示 `Operation not permitted`。
