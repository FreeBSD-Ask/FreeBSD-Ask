...
运行以下命令为 Jail 安装基本系统，建议使用 `base.txz` 解压安装，省去编译的时间，注意 Jail 的 **用户空间要和宿主机一致**，在 FreeBSD 15 的宿主机上，只能稳定运行 FreeBSD 15 的用户空间 Jail ， FreeBSD 的内核和用户空间是一个整体（基本系统，又称世界），用户空间不一致会导致兼容性问题。  
...
想象一个虚拟的交换机上，插着一根无形的网线。水晶头插在交换机上。网线网桥侧端口叫做 `epair0a`，容器侧叫做 `epair0b`。

`epair` 是由内核指定的固定前缀，后面接着代表第几对虚拟连接的数字。

如果要更改网卡名称，可以执行命令 `ifconfig epair0a name newname` 更改。为了避免名称混淆和混乱，不建议如此。
...
FreeBSD Jail 默认的安全策略禁止容器直接创建原始套接字 (Raw Socket) ，而 `ping` 命令恰恰需要利用 Raw Socket 来发送 ICMP 报文，如果执行 `ping` 命令，将提示 `Operation not permitted`，可以在 `myjail {...}` 块中添加配置 `allow.raw_sockets;` 即可使用 `ping`。
...
启动以后使用 `jls` 命令查看，如果容器创建成功，且正在运行，该容器将出现在列表中。
...
在 VNET 模式下，IP 地址由容器内部管理， Address 字段看不到 IP 是正常的。
...
安装成功以后可以执行 `pkg update -f` 强制重新获取软件仓库目录，然后执行 `pkg ins nginx`，执行 `service nginx onestart` 单次启动 `nginx`，成功后会得到如下输出：
...
使用 `ifconfig` 命令确认 IP 地址，如果完全按照本节步骤进行，地址应为 `192.168.1.120`。
...
按 **Ctrl**+**D** 或者输入 `exit` 可以退出 Jail 容器的命令行。

回到宿主机环境，执行 `fetch -o - http://192.168.1.120 | grep Welcome` 可以查看 NGINX 是否正常工作。

如果容器网络通畅，且 Nginx 正常工作，将得到如下输出：
