...
FreeBSD Jail 默认的安全策略禁止容器直接创建原始套接字 (Raw Socket) ，而 `ping` 命令恰恰需要利用 Raw Socket 来发送 ICMP 报文，如果执行 `ping` 命令，将提示 `Operation not permitted`，在 `myjail {...}` 块中添加配置 `allow.raw_sockets;` 即可使用 `ping`。
