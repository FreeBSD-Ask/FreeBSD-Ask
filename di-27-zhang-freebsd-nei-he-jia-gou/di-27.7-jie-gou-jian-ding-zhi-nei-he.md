...
- 降低内存占用。定制内核通过省略不使用的功能和设备驱动来减少内存占用。内核代码始终驻留在物理内存中，无法被应用程序占用，因此定制内核对于内存较小的系统尤为重要。
...
内核配置文件的格式简单，详见 config(5)。

> **警告**
>
> 删除设备或选项的支持可能导致内核损坏。例如，如果从内核配置文件中删除 nvme 驱动程序，使用 `nvme` 磁盘驱动程序的系统可能无法启动。如有疑问，最好将支持保留在内核中。
...
实际上，上面的例子出自 16.0-CURRENT 的 GENERIC 内核配置文件，-CURRENT 与 -STABLE、-RELEASE 的重要差异就在于 -CURRENT 引入了大量的调试功能。该功能即是通过 `include "std.debug"` 引入的。因此部分用户会体验到 -CURRENT 系统性能明显不佳。
