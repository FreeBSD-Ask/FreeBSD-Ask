...
不要直接编辑 GENERIC 文件。而应将文件复制并编辑其副本。配置文件名称必须 **全部使用大写字母**。在管理多台具有不同硬件的 FreeBSD 机器时，建议将其命名为机器的主机名。
...
待保存了自定义配置文件的编辑内容，即可按照以下步骤编译内核源代码。
...
如果内核版本与系统工具的构建版本不同，例如，在 -RELEASE 系统上安装了从 -CURRENT 源代码构建的内核，那么许多系统状态命令，如 ps 和 vmstat 将无法工作。为了解决这个问题，请重新编译并安装与内核版本相同的用户空间（世界）。使用与操作系统其他部分版本不同的内核版本从来不是一个好主意。
