...
`KTRACE` 启用系统调用跟踪功能 [ktrace(2)](https://man.freebsd.org/cgi/man.cgi?query=ktrace&sektion=2)。为了更好地支持 SMP，KTRACE 使用一个工作线程来异步处理大多数跟踪事件，而非由生成事件的线程直接处理。这需要预先分配一个用于存储跟踪事件对象的池。选项 `KTRACE_REQUEST_POOL` 指定该池的初始大小。
