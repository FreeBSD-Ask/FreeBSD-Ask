...
可以在编译时指定根设备和文件系统类型；如果引导程序无法正确识别根设备，此处提供了备用方案；或在启动内核时指定 `RB_DFLTROOT` 标志（`-r`）来覆盖选项。
...
STACK 将启用 [stack(9)](https://man.freebsd.org/cgi/man.cgi?query=stack&sektion=9) 功能，可捕获内核栈。若内核编译中包含 [DDB(4)](https://man.freebsd.org/cgi/man.cgi?query=ddb&sektion=4)，则 [stack(9)](https://man.freebsd.org/cgi/man.cgi?query=stack&sektion=9) 也会自动编译进内核。
