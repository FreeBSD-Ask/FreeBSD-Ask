...
实际上，上面的例子出自 16.0-CURRENT 的 GENERIC 内核配置文件，-CURRENT 与 -STABLE、-RELEASE 的重要差异就在于 -CURRENT 引入了大量的调试功能。该功能即由 `include "std.debug"` 引入，部分用户因而会体验到 -CURRENT 系统性能显著下降。
...
由于环境变量 `SYSDIR` 决定了源代码路径（可覆盖 **/usr/src/sys**），编译内核需在 **/usr/src** 目录下执行。切换到此目录：
