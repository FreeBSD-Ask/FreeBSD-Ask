...
> **警告**
>
> 非默认配置的测试不如 GENERIC 配置充分。虽然定制内核可以带来特定的好处，但它也增加了遇到构建或运行时问题的风险。仅建议有明确理由作出更改，且愿意在必要时调试的专业用户使用定制内核配置。
...
如果内核版本与系统工具的构建版本不同，例如，对 -RELEASE 系统安装了从 -CURRENT 源代码构建的内核，那么许多系统状态命令，如 ps 和 vmstat 将无法工作。为了解决这个问题，请重新编译并安装与内核版本相同的用户空间（世界）。不建议使用与操作系统其他部分版本不同的内核。
