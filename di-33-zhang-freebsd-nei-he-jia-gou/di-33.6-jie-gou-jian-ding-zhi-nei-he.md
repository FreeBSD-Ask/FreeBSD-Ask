...
目前，FreeBSD 内核中的大部分功能都包含在可以根据需要动态加载和卸载的模块中。这使得运行时内核能够立即适应新硬件，因此将这种内核称为模块化内核。
...
- 降低内存占用。定制内核通过省略未使用的功能和设备驱动来减少内存占用。内核代码始终驻留在物理内存中，应用程序无法使用，因此定制内核对于内存较小的系统尤为重要。
