...
```ini
makeoptions	CONF_CFLAGS=-fno-builtin  # ①② 禁用 memcmp 等 gcc 内建函数
#makeoptions	DEBUG=-g		# ③ 为内核构建包含 gdb(1) 调试符号的版本
#makeoptions	KERNEL=foo		# ④ 构建内核 "foo" 并安装到 "/foo"
#makeoptions	MODULES_OVERRIDE="ext2fs sound/sound sound/driver/maestro3" # ⑤ 仅构建 ext2fs 模块及所需的声音系统部分
makeoptions	DESTDIR=/tmp # 将目标安装目录临时设为 /tmp
```
...
options 	MAXPHYS=(128*1024) # MAXPHYS 表示原始（raw）I/O 块设备访问的最大大小，默认值在 ILP32 平台上为 128K，LP64 平台上为 1M
```
...
`EARLY_PRINTF` 可在内核非常早期（在调用 `cn_init()` 之前）使用一种特殊的 `printf`（`eprintf`）。这通常仅用于启动早期调试。由于该功能通常不可用，且所需的 `eputc()` 也未定义，因此一般情况下它未被定义，此处也将其注释掉。
...
options 	ROUTETABLES=2		# 分配的 FIB 最多可达 65536。默认值为 1，但此非推荐设置，因为每个 FIB 表项均占用较大内存
...
使用这些功能需启用 [ktr(4)](https://man.freebsd.org/cgi/man.cgi?query=ktr&sektion=4)，随后通过 sysctl 设置打开或关闭各种日志位。使用 [ktrdump(8)](https://man.freebsd.org/cgi/man.cgi?query=ktrdump&sektion=8) 提取日志，再通过显示程序处理，就能生成图表等可视化内容。
...
options 	NETGRAPH_DEBUG		# 启用 netgraph(4) 及其节点的额外调试
```
...
```ini
options 	VIMAGE
options 	VNET_DEBUG	# VIMAGE 调试支持
```
...
options 	IPDIVERT		# 转发套接字。启用 divert IP 套接字，用于 ipfw divert；编译进内核则依赖 IPFIREWALL
...
options 	IPSTEALTH		# 支持隐形转发（转发数据包但不修改 TTL），用于隐藏防火墙免受 traceroute 等工具探测
...
基于 [gjournal(8)](https://man.freebsd.org/cgi/man.cgi?query=gjournal&sektion=8) 的 UFS 日志支持。
...
```ini
options 	SCSI_DELAY=5000	# 对常规 SCSI 设备采取保守延迟
```
...
options 	KBD_DELAY2=15		# 定义按键重复延迟
```
...
makeoptions	KBDMUX_DFLT_KEYMAP=it.iso  # 设置默认键盘布局为意大利语 ISO
```
...
启用 [ahd(4)](https://man.freebsd.org/cgi/man.cgi?query=ahd&sektion=4) 的 target mode 操作单元位图。
...
还可通过 `atacore` 驱动，此后再按厂商判断各个总线和芯片组驱动以选择单独的驱动。例如，要构建一个仅支持 VIA 芯片组的系统，可以省略 `ata` 这行，仅包含驱动 `atacore`、`atapci` 和 `atavia`。
...
[uart(4)](https://man.freebsd.org/cgi/man.cgi?query=uart&sektion=4) 串行接口通用驱动程序。
...
[uart(4)](https://man.freebsd.org/cgi/man.cgi?query=uart&sektion=4) 驱动选项。
...
`flags` 用于支持控制台的串行驱动，例如 [uart(4)](https://man.freebsd.org/cgi/man.cgi?query=uart&sektion=4)：
...
如果网卡驱动需要特定 PHY 的支持，可以通过添加 `device mii`、`device mii_bitbang`，随后再添加相应的 PHY 驱动以实现。
...
```ini
device		lio		# Marvell 23XX 以太网适配器支持
```
...
这两个选项分别用于调整 mbuf 集群大小和 mbuf 大小。由于更改默认值可能导致内核假定的 mbuf 大小与模块假定的 mbuf 大小不匹配，因此在将它们的默认值更改为其他值时，必须格外谨慎处理网卡驱动模块。目前，唯一能够检测这种不匹配的驱动是 [ti(4)](https://man.freebsd.org/cgi/man.cgi?query=ti&sektion=4)。
...
> **注意**
>
> FireWire（IEEE 1394）驱动已在 FreeBSD 15.0 中被标记为废弃（deprecated），并计划在 FreeBSD 16 中移除。FireWire 硬件已基本退出市场，新系统无需启用此选项。
