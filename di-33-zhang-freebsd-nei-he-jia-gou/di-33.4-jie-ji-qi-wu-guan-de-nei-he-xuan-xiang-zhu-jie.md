...
> **注意**
>
> **必须同时定义 INET 和 INET6**。不必启用 IPv6，但 SCTP 是双栈协议，因为一个关联（association）可以同时跨越 IPv6 和 IPv4 地址，目前还没有拆分 IPv6 和 IPv4。
...
`SCTP_DEBUG` 可开启各种详细打印功能。它由一个位掩码控制（可通过 socket 选项和 sysctl 设置）。包含该选项本身不会立即产生日志，必须先设置相应的位。但打印信息可能非常冗长。因此，如果没有启用该选项，代码就不会做位检测和打印，这样可提高运行速度。如果并非调试用途，请不要使用。
