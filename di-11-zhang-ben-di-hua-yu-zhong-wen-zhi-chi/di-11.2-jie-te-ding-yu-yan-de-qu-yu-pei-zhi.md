...
例如，之前提到的简体中文示例可以在 **/etc/login.conf** 中的配置如下：
...
使用 `vipw` 添加新用户时，可以指定 **chinese** 分级来设置区域设置简体中文（UTF-8 编码）：

```ini
用户名:密码:1111:11:chinese:0:0:User Name:/home/user:/bin/sh
```

例如要为用户 ykla 分配 **chinese** 分级，目前待分配：
...
如果所有新用户使用相同的语言（假设为简体中文），可以在 **/etc/adduser.conf** 中设置 `defaultclass=chinese`。
