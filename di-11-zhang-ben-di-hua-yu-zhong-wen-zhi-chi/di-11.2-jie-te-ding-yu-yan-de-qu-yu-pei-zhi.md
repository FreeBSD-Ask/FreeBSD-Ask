...
以下是一个用户的 **~/.login_conf** 示例，为简体中文 UTF-8 编码设置变量。由于一些应用程序没有正确处理中文、日文和韩文的区域设置变量，需要更多的变量来确保应用程序正确工作：

```ini
# 不使用中国大陆的货币单位和时间格式的用户可以手动更改每个变量
me:
	:lang=zh_CN.UTF-8:
	:setenv=LC_ALL=zh_CN.UTF-8,LC_COLLATE=zh_CN.UTF-8,LC_CTYPE=zh_CN.UTF-8,LC_MESSAGES=zh_CN.UTF-8,LC_MONETARY=zh_CN.UTF-8,LC_NUMERIC=zh_CN.UTF-8,LC_TIME=zh_CN.UTF-8:
	:charset=UTF-8: # 注意此行
	:xmodifiers="@im=fcitx": # 设置 fcitx5 为 XIM 输入服务器
```
