...
- vt(4)：`ls /usr/share/vt/fonts`

对于 vt(4)，可在 **/boot/loader.conf** 中设置字体：

```ini
screen.font="font_name"
```

对于 vt(4)，键盘映射文件位于 `/usr/share/vt/keymaps/`，可通过 kbdcontrol(1) 加载。
...
接下来，设置正确的控制台终端类型，在 **/etc/ttys** 中为所有虚拟终端条目配置。以下是各字符集已定义的终端类型（vt(4) 使用 `xterm` 作为默认终端类型）：
