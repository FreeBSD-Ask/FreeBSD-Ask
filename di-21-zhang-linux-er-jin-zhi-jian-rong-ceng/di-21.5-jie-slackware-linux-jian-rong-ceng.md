# 21.5 Slackware Linux 兼容层

本节将系统地介绍在 FreeBSD 操作系统上构建 Slackware Linux 兼容层的技术方法与实践步骤。

需要特别说明的是，Slackware Linux 官方并未提供预构建的基本系统镜像供直接下载。因此，需要在 Slackware Linux 或 FreeBSD 环境下，通过特定脚本自行生成基本系统（在 FreeBSD 环境下执行该脚本，需预先安装 `bash` 和 `gmake` 软件包）。
...
1. 查找 Slackware-WSL 项目的源码，分析其生成 Slackware 基本系统的脚本，尝试在 FreeBSD 环境下运行该脚本并生成可用的基本镜像

2. 尝试使用 GitHub CI 自动生成 Slackware 基本系统。
