...
PF（Packet Filter，包过滤器）是一款源自 OpenBSD 的防火墙，提供了丰富功能，包括 ALTQ（Alternate Queuing，交替队列）等。作为一种状态防火墙（stateful firewall），PF 通过跟踪网络连接状态实现精细化的访问控制，是现代操作系统安全架构的核心组件。PF 设计遵循最小权限原则（principle of least privilege），提供了高效的包过滤机制。
