...
PF（Packet Filter，包过滤器）是一款源自 OpenBSD 的防火墙，提供了丰富功能，包括 ALTQ（Alternate Queuing，交替队列）等。

Packet Filter（PF）作为一种状态防火墙（stateful firewall），通过跟踪网络连接状态实现精细化的访问控制，是现代操作系统安全架构的核心组件。PF 设计遵循最小权限原则（principle of least privilege），提供了高效的包过滤机制。

## 启用 PF

启用 PF 防火墙需要完成一系列准备工作，具体步骤如下。
...
以下是一些常用的 PF 规则示例，可根据实际需求进行选择和组合。
...
以下是一个完整的规则集文件示例，可直接复制到 `/etc/pf.conf` 中使用。
