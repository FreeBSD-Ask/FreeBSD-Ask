...
IPFIREWALL（IPFW，IP 防火墙）是由 FreeBSD 项目开发和维护的防火墙软件。作为 FreeBSD 原生防火墙，IPFW 体现了 BSD 系统在网络安全架构上的设计哲学。其基于规则编号的优先级机制，为网络访问控制提供了灵活而精确的管理框架。IPFW 作为可加载模块存在于基本系统内核（GENERIC）中。
...
如需自定义 IPFW 规则，可编辑规则文件，以下是一个完整的示例。
