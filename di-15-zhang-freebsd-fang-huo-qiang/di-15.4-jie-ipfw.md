...
>**警告**
>
>IPFW 默认会有一条规则，规则号为 `65535`，是不可以删除的：这条规则会将所有流量都切断。故在未配置好防火墙前，请勿启动 IPFW，否则就会面临被阻挡在防火墙之外的麻烦。
...
# sysrc firewall_logif="YES"  # 将日志打印到 `ipfw0` 这个设备里
```
...
- 我们可以将默认拒绝的规则 `65535 deny ip from any to any` 改成默认允许
