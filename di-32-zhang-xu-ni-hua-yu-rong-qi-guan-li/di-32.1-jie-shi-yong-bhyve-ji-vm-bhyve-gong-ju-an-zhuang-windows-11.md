# 32.1 使用 bhyve 及 vm-bhyve 工具安装 Windows 11
...
![bhyve 虚拟化架构层次](../.gitbook/assets/bhyve-class.png)
...
> **技巧**
>
> 如忘记按键而退回至 UEFI Shell，请停止虚拟机，重新执行上面的安装步骤。
>
...
> ```
>
> 等待约 10-30 秒后确认进程已退出，再运行 `bhyvectl --destroy --vm=winguest` 清理。仅在 SIGTERM 多次尝试仍无效时，方可使用 `kill -9` 作为最后手段（**警告：可能导致虚拟机磁盘数据损坏**）。
